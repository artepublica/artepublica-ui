...
function evaluateOuterDrawerListItems(
  ...
  routes.forEach((route, index) => {
    ...
    if (base_route_name.length) {
      if (Object.hasOwn(drawerItems, base_route_name)) {
...
function DrawerContent(props: DrawerContentComponentProps): JSX.Element {
  ...
  const drawerItems = evaluateOuterDrawerListItems(
    state.routes,
    restProps.descriptors,
  );
    ...
    return (
      <DrawerContentScrollView {...props}>
        {Object.keys(drawerItems).map((item) => {
          ...
          const focusedRouteName = focusedRoute.name.substr(
            0,
            focusedRoute.name.indexOf('_'),
          );
          return (
            <DrawerItem
              key={item}
  ...
  const descriptors = Object.keys(restProps.descriptors).reduce<
  ...
  return (
    <DrawerContentScrollView {...props}>
      ...
      {Object.keys(descriptors).map((key) => {
        ...
        const label = descriptor.options.title!.substr(
          descriptor.options.title!.indexOf('-') + 1,
          descriptor.options.title!.length - 1,
        );
