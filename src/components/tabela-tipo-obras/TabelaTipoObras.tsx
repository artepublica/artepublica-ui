...
function headers({
  ...
  tipologia,
  zona,
}: TabelaTipoObrasProps): string[] {
  const defaultHeaders = [tipo, `Total: ${tipos.length}`];
  ...
  if (Platform.OS !== 'web') {
    ...
    if (tipologia === true && zona === true) {
      ...
      return [...defaultHeaders, 'Tipologias/Obras', 'Bairros/Obras'];
    } else {
      return [...defaultHeaders, 'Zonas/Bairros', 'Bairros/Obras'];
    }
  }
}

function TabelaTipoObras({
  tipo,
  tipos,
  tipologia,
  zona,
}: TabelaTipoObrasProps): JSX.Element {
  const rows = tipos.map((tipo) => {
    const rows = [tipo.nome, tipo.obras.length.toString()];
    ...
    if (Platform.OS === 'web') {
      if (tipologia === true) {
        const tipologias = tipo.obras
          ...
          .map((tipologia) => {
            const obras = tipo.obras.filter(
              (obra) =>
                obra.Tipologia === tipologia ||
                (tipologia === 'Desconhecida' && obra.Tipologia == null),
            );
            ...
            .map(
              (tipologia) =>
                `${tipologia.nome} (${tipologia.obras.length}): ${tipologia.obras
                  .map(
                    (obra) =>
                      `${obra.Titulo ?? 'Desconhecida'} [${getYear(obra.DataInauguracao) ?? 'S.D.'}]`,
                  )
                  .join(', ')}`,
            )
            .join('\n\n'),
        ...
        const zonas = tipo.obras
          ...
          .map((zona) => {
            const obras = tipo.obras.filter(
              (obra) =>
                obra.Zona === zona ||
                (zona === 'Desconhecida' && obra.Zona == null),
            );
        ...
        const bairros = tipo.obras
          ...
          .map((bairro) => {
            const obras = tipo.obras.filter(
              (obra) =>
                obra.Bairro === bairro ||
                (bairro === 'Desconhecida' && obra.Bairro == null),
            );
  ...
  return (
    <Table
      ...
      rows={rows}
      widthArr={[tamanhoColunaTipologia, tamanhoColunaTotal, undefined]}
    />
  );
}
