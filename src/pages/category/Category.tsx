    ...
    .filter(
      (key) =>
        (typed_obra_artepublica[key].Autores != null &&
          typed_obra_artepublica[key].Autores?.find(
            (artista) => artista.Pessoa?.Nome === autor,
          ) != null) ||
        (autor === 'Desconhecida' &&
          typed_obra_artepublica[key].Autores == null),
    )
    ...
    tipologias
      .map((tipologia) => {
        const titulos_tipologia = Object.keys(typed_obra_artepublica)
          ...
          .filter(
            (key) =>
              (typed_obra_artepublica[key].Tipologia != null &&
                typed_obra_artepublica[key].Tipologia === tipologia.id) ||
              (tipologia.id === 'Desconhecida' &&
                typed_obra_artepublica[key].Tipologia == null),
          )
...
function Category(): JSX.Element {
  ...
  >(function (r, a) {
    const r_top = r.find((top) => top.nome === a);
    if (!r_top) {
      const obras: { nome: string; inauguracao: string }[] = Object.keys(
        typed_obra_artepublica,
      )
        .filter((key) => typed_obra_artepublica[key].Categoria === category)
        .filter(
          (key) =>
        ...
        .map((key) => ({
          ...
          inauguracao:
            typed_obra_artepublica[key].DataInauguracao ?? 'Desconhecida',
        }));
  ...
  artistas_total_sort_total = artistas_total_sort_total.sort((a, b) =>
    a.total < b.total ? 1 : -1,
  );
  ...
  return (
    <View style={style.container}>
      <ScrollView style={{ width: '100%' }}>
        ...
        <Table
          headers={['Tipologia', `Total: ${tipologias_total.length}`]}
          rows={tipologias_total.map((top) => [top.nome, top.total.toString()])}
        />
        <View style={{ height: 12 }} />

        <Table
          headers={['Natureza', `Total: ${naturezas_total.length}`]}
          rows={naturezas_total.map((top) => [top.nome, top.total.toString()])}
        />
        <View style={{ height: 12 }} />

        <Table
          headers={['Natureza', `Total: ${artistas_total.length}`, 'Obras']}
          rows={artistas_total.map((top) => [
            top.nome,
            top.total.toString(),
            top.obras
              .map((obra) => `${obra.nome} (${obra.inauguracao})`)
              .join(', '),
          ])}
        />
        <View style={{ height: 12 }} />
