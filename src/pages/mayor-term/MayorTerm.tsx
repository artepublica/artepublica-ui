...
import { getYear, magenta, reduceListOfList, TypologyTheme, useTheme } from '@utils';

function Block({ heritages, years }: { heritages: Heritage[]; years: number[] }): JSX.Element {
      ...
      if (year != null && years.includes(year)) {
        ...
        if (y == null) {
          result.push({
            year,
            heritages: heritages
              .filter((heritageInt) => getYear(heritageInt.OpeningDate) === year)
  ...
  const typologies = heritagePerYear
    .map((year) => year.heritages.map((heritage) => heritage.Typology ?? 'Desconhecida'))
    .reduce<string[]>((r, l) => {
  ...
  const typologiesTotal = typologies.reduce<{ type: string; name: string; data: (number | null)[] }[]>(
    (series, typology) => {
      const typologyTotal = heritagePerYear.map((year) => {
        const total = year.heritages.filter((heritage) => heritage.Typology === typology).length;
...
function Network({ heritages, years, mayor }: { heritages: Heritage[]; mayor: string; years: number[] }): JSX.Element {
  const { theme } = useTheme();
  const { width, height } = useWindowDimensions();
  const headerHeight = useHeaderHeight();
  const insets = useSafeAreaInsets();
  ...
  const authors = termHeritages
    ...
    .reduce<{ id: string; marker: { radius: number }; color: string }[]>((r, author) => {
          ...
          .filter(
            (heritage) =>
              (heritage.Authors != null &&
                heritage.Authors?.find((_author) => _author.Person?.Name === author.id) != null) ||
...
function Sankey({ heritages, years, mayor }: { heritages: Heritage[]; mayor: string; years: number[] }): JSX.Element {
  const { theme } = useTheme();
  const { width, height } = useWindowDimensions();
  const headerHeight = useHeaderHeight();
  const insets = useSafeAreaInsets();
  ...
  const authors = termHeritages
    .map((heritage) => heritage.Authors ?? [{ Person: { Nome: 'Desconhecida' } } as Author])
    .reduce<string[]>((r, l) => {
      Array.prototype.push.apply(
        r,
        l.map<string>((author) => author.Person?.Name ?? 'Desconhecida'),
...
function MayorTerm({ heritages }: { heritages: Heritage[] }): JSX.Element {
  ...
  const [dropdownValue, setDropdown] = useState('Cesar Epitácio Maia (1993 - 1996)');
  ...
  return (
    <ScrollView style={{ width: '100%', paddingTop: 12, paddingHorizontal: 12 }}>
      ...
      <View style={{ paddingTop: 12 }}>
        <Network heritages={heritages} years={years.years} mayor={years.mayor} />
      </View>
      <View style={{ paddingTop: 12 }}>
