...
function getTrocasArtigosJornais(
...
function DependencyWheelRefactor({
  ...
  const dependencyWheelsPoliticaPublica = trocasPoliticaPublica.map((troca) =>
    converterTrocaParaDependencyWheel(troca),
  );
  const dependencyWheelsArtigosJornais = trocasArtigosJornais.map((troca) =>
    converterTrocaParaDependencyWheel(troca),
  );
  const dependencyWheelsLivros = trocasLivros.map((troca) =>
    converterTrocaParaDependencyWheel(troca),
  );
  const dependencyWheelsExposicoes = trocasExposicoes.map((troca) =>
    converterTrocaParaDependencyWheel(troca),
  );

  const dep1 = juntarDependencyWheel(
    dependencyWheelsPoliticaPublica,
    dependencyWheelsArtigosJornais,
  );
  const dep2 = juntarDependencyWheel(dep1, dependencyWheelsLivros);
  const dependencyWheels = juntarDependencyWheel(
    dep2,
    dependencyWheelsExposicoes,
  );
      ...
      dataLabels: {
        enabled:
          showLabel === 0
            ...
            : showLabel === 1
              ? true
              : agenteDaPolitica(politicaPublica, no.node) ||
                autorObraDaPolitica(politicaPublica, no.node),
      },
  ...
  const dataFiltrada = dependencyWheels.filter(
    (wheel) =>
      nosFiltrados.find((no) => no.node === wheel.from) &&
      nosFiltrados.find((no) => no.node === wheel.to),
  );
  nosFiltrados
    .sort((a, b) => b.node.localeCompare(a.node))
    .sort((a, b) => (a.weight < b.weight ? 1 : -1));

  const [shuffleDados, setarShuffleDados] = useState(dataFiltrada);

  useEffect(() => {
    setarShuffleDados(dataFiltrada);
  }, [politicaPublica, peso, height, labelEmCima]);
  ...
  const table = nosFiltrados.filter(
    (no) => !agenteDaPolitica(politicaPublica, no.node),
  );
  ...
  return (
    <>
      ...
      <View style={{ flex: 1 }}>
        <Table
          headers={[`pessoa`, 'Capital Simbólico']}
          rows={table.map((no) => [no.node, no.weight.toString()])}
          widthArr={[undefined, 50]}
        />
      </View>
    </>
  );
