...
function Obras(): JSX.Element {
  ...
  const obrasComImagem = Object.keys(typed_obra_artepublica)
    .sort((keyA, keyB) => {
      ...
      return (obraA.Titulo ?? 'Desconhecida').localeCompare(
        obraB.Titulo ?? 'Desconhecida',
      );
    })
    .sort((keyA, keyB) => {
      const obraA: Obra = typed_obra_artepublica[keyA];
      ...
      return (getYear(obraA.DataInauguracao) ?? 0) <
        (getYear(obraB.DataInauguracao) ?? 0)
        ? -1
        : 1;
    })
    .sort((keyA, keyB) => {
      ...
      return (obraA.Tipologia ?? 'Desconhecida').localeCompare(
        obraB.Tipologia ?? 'Desconhecida',
      );
    });
  ...
  const data = obrasComImagem
    .reduce<string[]>((resultado, key, index) => {
      ...
      if (index === 0) {
        return [obra.Tipologia ?? 'Desconhecida', key];
      } else {
        ...
        if (
          ...
          return [...resultado, obra.Tipologia ?? 'Desconhecida', key];
        }
      }
                          ...
                          <View style={{ padding: 8 }}>
                            ...
                            >
                              {`${obra.Titulo ?? 'Desconhecida'}${
                                obra.DataInauguracao
                                  ? `, ${getYear(obra.DataInauguracao)}`
                                  : ', s.d.'
                              }${Platform.OS === 'web' ? '\n\n' : '\n'}`}
                            </Text>
                            <Text
