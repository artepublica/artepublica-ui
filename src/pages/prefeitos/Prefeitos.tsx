...
function Block({
  ...
  const prefeitos = Object.keys(typed_prefeitos).map(
    (key) => typed_prefeitos[key],
  );
  const prefeitosSelecionados = prefeitos.filter((prefeito) => {
    const mandatosIntern = prefeito.Mandatos?.filter((mandato) =>
      mandatos.includes(
        `${prefeito.Pessoa?.Nome ?? 'Desconhecida'} (${getYear(mandato.DataInicio)} - ${getYear(mandato.DataFim)})`,
      ),
    );
  ...
  const anosSelecionados = anosMandatosSelecionados
    ...
    .reduce<number[]>((resultado, anoSelecionado, index) => {
      if (
        index === 0 ||
        index === anosMandatosSelecionados.length - 1 ||
        anosMandatosSelecionados[index + 1] === anoSelecionado + 1
      ) {
        resultado.push(anoSelecionado);
      } else {
        for (
          let i = anoSelecionado;
          i < anosMandatosSelecionados[index + 1];
          i++
        ) {
          resultado.push(i);
        }
  ...
  const total_prefeitos = prefeitosSelecionados.reduce<
