...
function Tipo_Decada({ tipo }: Tipo_DecadaProps): JSX.Element {
  ...
  const tipos = Object.keys(all)
    ...
    .map((key) =>
      tipo === 'Autor'
        ? MapAutor(key)
        : all[key].map((obra) => obra[tipo as keyof Obra] ?? 'Desconhecida'),
    )
    .reduce<string[]>((r, l) => {
      Array.prototype.push.apply(r, l);
      return r;
  ...
  const total_tipos = tipos.reduce<
    ...
    const total_tipo = Object.keys(all)
      ...
      .map((key) => {
        const total = all[key].filter(
          (obra) =>
            obra[tipo as keyof Obra] === _tipo ||
            (obra[tipo as keyof Obra] == null && _tipo === 'Desconhecida'),
        ).length;
  ...
  const streamgraphSeries = tipos.reduce<
    { type: string; name: string; data: (number | null)[] }[]
  >((series, _tipo) => {
    const total_tipo = Object.keys(all)
      .filter((key) => key !== 'null' && all[key].length > 0)
