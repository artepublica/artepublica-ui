import { Data, Fact, Obra } from '@domain';

import * as authors from './authors';
import * as persons from './persons';

export const CoronelAssuncao_RuffoFanucchi: Obra = {
  ID: 1,
  Imagem: 'CoronelAssuncao_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: 'Coronel Assunção',
  Material: 'Bronze',
  DataInauguracao: '05/1959',
  DatasImportantes: [new Data('05/1959', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Gamboa',
  Endereco: 'Praça Coronel Assunção- Praça da Harmonia',
  Latitude: '-22.895707',
  Longitude: '-43.1912978',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido pela Prefeitura da Cidade ao oficial da policia militar do Distrito Federal, com o apoio de uma comissão composta pelo Tenente Coronel João Pereira Cunha, o Major Jos´Pinto Lemos e o capitão Cordeiro FRnaça.
    O busto do Coronel tinha, até 2007, de cada lado do pedestal um canhão e na frente um cão em posição de guarda também em bronze. O cão era o símbolo da antiga polícia militar, o animal imortalizado era conhecido como Brutus que pertenceu ao Coronel`,
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FonteDaCrianca_HenriFredericIselin: Obra = {
  ID: 2,
  Imagem: 'FonteDaCrianca_HenriFredericIselin.jpeg',
  Authors: [authors.HenriFredericIselin],
  Titulo: 'Fonte da Criança',
  Material: 'ferro fundido',
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Santo Cristo',
  Endereco: 'Largo do Santo Cristo',
  Latitude: '-22.899398',
  Longitude: '-43.201561',
  Status: 'Permanece em espaço público',
  Descricao: `Fundição Val D'Osne`,
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const NossaSenhoradaImaculadaConceicao_Desconhecido: Obra = {
  ID: 4,
  Imagem: 'NossaSenhoradaImaculadaConceicao_Desconhecido.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: 'Nossa Senhora da Imaculada Conceição',
  Material: 'argamassa',
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Saúde',
  Endereco: 'Praça Major Valô',
  Latitude: '-22.898852',
  Longitude: '-43.183123',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EsculturaParaORio_WaltercioCaldas: Obra = {
  ID: 5,
  Imagem: 'EsculturaParaORio_WaltercioCaldas.jpeg',
  Authors: [authors.WaltercioCaldas],
  Titulo: 'Escultura para o Rio',
  Material: 'peça em concreto revestida com pedra portuguesa',
  Altura: '8m',
  Largura: '15m',
  Profundidade: '7,7m',
  ValorObra: 'R$ 60.000,00',
  DataInauguracao: '05/02/1997',
  DatasImportantes: [new Data('05/02/1997', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Beira Mar com Avenida Antonio Carlos, Praça Itália',
  Latitude: `-22.9111895`,
  Longitude: `-43.1713323`,
  SuporteEspacial: 'Trevo',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A escultura é formando duas peças. Uma faixa parte do chão, contava a obra, dando uma sensação do caminho percorrido.
    O local escolhido inicialmente para instalar a escultura foi a Rua da Candelária. A peça levou em consideração um dos traços
    mais característico da via pública no Rio, a pedra portuguesa, que ele colocou em situação vertical, ao contrárico da
    horizontalidade costumeira. A peça recupera a sinuosidade dos desenhos das calçadas cariocas e é, assim uma homenagem a tradição
    urbanista da Cidade. Acolocação das pedras foi feita manualmente pelo artesão José de Lino.`,
  Fatos: [
    new Fact(
      '1994',
      '1997',
      'Política Pública',
      'Esculturas Urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Promotor',
      'Secretaria Municipal de Cultura',
      [persons.HelenaMariaPortoSeveroDaCosta],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Patrocínio',
      'O artista precisou buscar patrocínio extra para a conclusão da peça',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1997',
      '1997',
      'Vandalismo',
      'Obra foi danificada e artista solicita reparo da peça à prefeitura.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '28/12/1997',
      '28/12/1997',
      'Crítica',
      'Essa peça é uma felicidade absoluta. O único porém é que se dilui em alguns angulos pela falta de contraste com os prédios. Mas até esses argumentos são a favor da escultura. A ideia é pegar o chão da cidade e levantá-lo é uma manobra simples e bonita',
      [persons.ReynaldoRoels],
      'Rio de Janeiro',
    ),
    new Fact(
      '28/12/1997',
      '28/12/1997',
      'Crítica',
      'Não é a obra que mais gosto do Waltércio Caldas. Conceitualmente é interessante porque parece emergir do chão, mas acho que ele não conseguiu desenvolver bem essa ideia. Não é uma obra feliz.',
      [persons.FredericoMoraes],
      'Rio de Janeiro',
    ),
    new Fact(
      '17/05/2015',
      '7/05/2015',
      'Demolição',
      'A obra foi demolida para construção do VLT',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Reconstrução', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarechalMachadoBittencourt_JoseOtavioCorreaLima: Obra = {
  ID: 6,
  Imagem: 'MarechalMachadoBittencourt_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: 'Marechal Machado Bittencourt',
  Material: 'Bronze',
  DataInauguracao: '1898',
  DatasImportantes: [
    new Data('1898', 'arte ambiente'),
    new Data('11/1988', 'inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida General Justo',
  Latitude: '-22.9053928',
  Longitude: '-43.1690508',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: 'O busto esta instalado no local onde foi assassinado.',
  Fatos: [new Fact('', '', 'Homenageado', 'Marechal Bittencourt', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const TeixeiraDeFreitas_RodolfoBernardelli: Obra = {
  ID: 7,
  Imagem: 'TeixeiraDeFreitas_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Teixeira de Freitas',
  Material: 'Bronze',
  DataInauguracao: '07/08/1905',
  DatasImportantes: [
    new Data('1904', 'data da obra-jornal do commercio'),
    new Data('07/08/1905', 'data inauguração -inventário dos monumentos'),
    new Data(
      '20/03/1910',
      'data de trasnferência para Avenida Marechal Camara',
    ),
    new Data('1983', 'data de transferência para entrada da Praça Paris'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: '-22.9172682',
  Longitude: '-43.1762654',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao:
    'No local foi removido um Chafariz ornamental. Segundo reportagem no Jornal do Commercio a obra é de 1904.',
  Fatos: [
    new Fact(
      '',
      '',
      'Homenageado',
      'Teixeira de Freitas, jurtista e professor que escreveu sobre a Consolidação das Leis Civis por iniciativa do Instituto da Ordem dos Advogados',
      [],
      '',
    ),
    new Fact(
      '1983',
      '1983',
      'Transferencia',
      'Foi transferida para a localização atual',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BuarquedeMacedo_JoseOtavioCorreaLima: Obra = {
  ID: 8,
  Imagem: 'BuarquedeMacedo_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: 'Buarque de Macedo',
  Material: 'Bronze',
  DataInauguracao: '05/1938',
  DatasImportantes: [new Data('05/1938', 'inauguração')],
  Financiamento: 'subscrição pública',
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Marechal Camara',
  Latitude: '-22.910434',
  Longitude: ' -43.169182',
  SuporteEspacial: 'canteiro',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'A estátua representa o Conselheiro de pé, trajando casaca, tendo nas mãos papéis, ao seu lado um globo terrestre, juntos a uma pasta na qual está escrita a palavra agricultura.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Buarque de Macedo', [], ''),
    new Fact(
      '',
      '',
      'Subscrição pública',
      'Buarque de Macedo',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ColunaMediaemCantoneirasDiagonais_FranzWeissmann: Obra = {
  ID: 9,
  Imagem: 'ColunaMediaemCantoneirasDiagonais_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: 'Coluna Media em Cantoneiras Diagonais',
  Material: 'aço pintado',
  Altura: '400 cm',
  Largura: '120 cm',
  Profundidade: '120 cm',
  DataInauguracao: '01/1977',
  DatasImportantes: [new Data('01/1977', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Marechal Camara, 171',
  Latitude: '-22.9092439',
  Longitude: '-43.1696687',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Escultura em chapas e cantoneiras dividindo o espaço entre preenchido e vazado.',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const BernardoPereiradeVasconcelos_FloryGama: Obra = {
  ID: 10,
  Imagem: 'BernardoPereiradeVasconcelos_FloryGama.jpeg',
  Authors: [authors.FloryGama],
  Titulo: 'Bernardo Pereira de Vasconcelos',
  Material: 'Bronze',
  DataInauguracao: '12/1966',
  DatasImportantes: [new Data('01/1977', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Passos com Avenida Marechal Floriano',
  Latitude: '-22.901980',
  Longitude: '-43.184362',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Homenageado voltado para o Colegio por ele criado, num reconhecimento público de seu trabalho. Para a homenagem foi criada uma comissão de professores presidida pelo Sr. Waldemiro Potsch com a participação de Carlos Potsch, George Samner, Tito Urbano Silveira, Pereira Reis Jr. Pedro Couto Jr, Isaac Volhan, Hugo Segadas Viana, Jose Carvalha e Libano Guedes .',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Tiradentes_FranciscoAndrade: Obra = {
  ID: 11,
  Imagem: 'Tiradentes_FranciscoAndrade.jpeg',
  Authors: [authors.FranciscoAndrade],
  Titulo: 'Tiradentes',
  Material: 'Bronze',
  DataInauguracao: '05/1926',
  DatasImportantes: [new Data('05/1926', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Antonio Carlos',
  Latitude: '-22.9042531',
  Longitude: '-43.1744892',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'A escultura mostra um mártir com pulsos algemados, olhos voltados para o céu e semblante tranquilo, como que altivo perante a condenação à morte.// Reprodução Fotográfica Cesar Barreto',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Tiradentes', [], ''),
    new Fact(
      '',
      '',
      'Iniciativa',
      'Câmara dos Deputados do Estado do Rio de Janeiro, por ocasião da construção da sede do Congresso Nacional',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Concurso Público',
      'Para escolher o artista',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Bem tombado Municipal', '', [], 'Rio de Janeiro'),
    new Fact('', '', 'Bem tombado Federal', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FranciscoPereiraPassos_RodolfoBernardelli: Obra = {
  ID: 12,
  Imagem: 'FranciscoPereiraPassos_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Francisco Pereira Passos',
  Material: 'bronze e pedestal em granito',
  DataInauguracao: '1914',
  DatasImportantes: [
    new Data('11/1914', 'inauguração - inventário dos Monumentos'),
    new Data('1913', 'inauguração - arte Ambiente'),
    new Data('01/04/1994', 'transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Vargas com Avenida Rio Branco',
  Latitude: '-22.901197',
  Longitude: '-43.178895',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento em bronze do ex-prefeito do Rio de Janeiro, Pereira Passos (1836-1913), foi uma iniciativa dos funcionários da Prefeitura como forma de gratidão ao grande reformador da cidade. Na estátua, ele é representado por meio corpo em bronze sob pedestal de granito, tendo nas mãos o projeto da Avenida Central.// 
    O monumento ao Prefeito Pereira Passos foi uma inicativa dos funcionários da Municipalidade e erguido na Praça da
    República, próximo ao antigo prédio do Palácio da Prefeitura. Em 1 de abril de 1994 foi retirado para a demolição do antigo
    prédio da prefeitua. Posteriormente foi para o entroncamento da Avenida Rio Branco e Presidente Vargas, atual localização.// Francisco Franco Pereira Passos foi engenheiro, político brasileiro e prefeito do Rio de janeiro de 1902 a 1906. Ficou conhecido pela mais importante reforma urbana feita na cidade. Com a sua transformação, o Rio de Janeiro passou a ser chamado mundialmente de Cidade Maravilhosa.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MachadoDeAssis_HumbertoCozzo: Obra = {
  ID: 13,
  Imagem: 'MachadoDeAssis_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: 'Machado de Assis',
  Material: 'Bronze',
  DataInauguracao: '06/1929',
  DatasImportantes: [new Data('06/1929', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Wilson, 203-ABL',
  Latitude: '-22.910646',
  Longitude: '-43.1728122',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Na época foi instalado na frente à Academia Brasileira de Letras, mas hoje se encontra entre os dois predios da Academia, contudo é uma obra pública.// Localizada na Avenida Presidente Wilson, em frente a sede da Academia Brasileira de Letras, a qual fundou e foi presidente, a estátua em bronze de Machado de Assis foi construída após pedido popular em comemoração ao 90º aniversário deste que foi um dos maiores escritores da língua portuguesa de todos os tempos.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Machado de Assis', [], ''),
    new Fact('', '', 'Subscrição popular', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Homenagem',
      'Comemoração ao 90º aniversário de nascimento do escritor',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Ritmo_BrunoGiorgi: Obra = {
  ID: 14,
  Imagem: 'Ritmo_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: 'Ritmo',
  Material: 'Bronze',
  ValorObra: '16 milhões de cruzeiros',
  DataInauguracao: '06/1985',
  DatasImportantes: [new Data('06/1985', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Antonio Carlos, 104',
  Latitude: '-22.909945',
  Longitude: '-43.172403',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Trata-se de uma peça abstrata, ultima obra publica de Bruna Giorgi.',
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doada pela ECISA para incorporar a fachada da Academia Brasileira de Letras- ABL, na comemoração do seu 88º aniversário',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1983',
      '1983',
      'Promotor',
      'A peça foi adquirida do artista em 1983, pela ECISA e ficou guardada por 1 ano e 3 meses até ir para o espáco público',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '08/1984',
      '08/1984',
      'Autoreização',
      'Autorizado pelo Departamento de Parques e Jardins para ser instalada em espaço público',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ManuelBandeira_OttoDumovich: Obra = {
  ID: 15,
  Imagem: 'ManuelBandeira_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: 'Manuel Bandeira',
  Material: 'Bronze',
  Altura: '120 cm',
  Peso: '500 kg',
  DataInauguracao: '04/2007',
  DatasImportantes: [new Data('04/2007', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Antonio Carlos, 104',
  Latitude: '-22.909993',
  Longitude: '-43.172435',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Manuel Bandeira esta sentado próxima a uma escrivaninha, onde estão dispostos vários objetos pessoais, com toda
    as peças em bronze, sobre um pedestal de concreto de forma piramidal. A neo-barroca do artista tem a fisionomia bem marcante
    do poeta.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Manuel Bandeira', [], ''),
    new Fact(
      '',
      '',
      'Inauguração',
      'A inauguração da obra integra a comemoração dos 110 anos da Associação Brasileira de Letras (ABL)',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Solicitação',
      'A obra foi uma solicitação da Academia Brasileira de Letras, através do presidente Marcos Vilaça em comemoração ao centeário de Manoel Bandeira',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const GrandeQuadradoVermelho_FranzWeissmann: Obra = {
  ID: 16,
  Imagem: 'GrandeQuadradoVermelho_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: 'Grande Quadrado Vermelho',
  Material: 'aço pintado',
  Altura: '590 cm',
  Largura: '430 cm',
  Profundidade: '500 cm',
  DataInauguracao: '09/1996',
  DatasImportantes: [new Data('09/1996', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida República do Paraguai',
  Latitude: `-22.907512`,
  Longitude: `-43.180963`,
  SuporteEspacial: 'trevo',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça geométrica com espaço vazado como um elemento definidor. O plano, as articulações e o elemento vazado, cria a
    tridimensionalidade aberta, caracteristica do artista. Weissmann é o artista do corte e da solda, cuja força e ritmo é propiciado
    com as interseçoes dos planos. A cor é planejada, viva.`,
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const OPassante_JoseResende: Obra = {
  ID: 18,
  Imagem: 'OPassante_JoseResende.jpeg',
  Authors: [authors.JoseResende],
  Titulo: 'O Passante',
  Material: 'aço corten',
  Altura: '540 cm',
  Largura: '300 cm',
  Profundidade: '46 cm',
  ValorObra: 'R$ 60.000,00',
  DataInauguracao: '09/02/1996',
  DatasImportantes: [new Data('09/02/1996', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Largo da Carioca',
  Latitude: '-22.908135',
  Longitude: '-43.177795',
  SuporteEspacial: 'Calçadao',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Os locais da implantação do projeto ficou previamente designados pela SMC (Secretaria Municipal de Cultura) e em
    seguida foram selecionados artistas em função de sua adequação aos espaços. Os critérios adotados pela comissão foram: a
    importância do artista para o desenvolvimento da arte brasileira recente; a adequação do trabalho do artista às condições
    espaço público, resistência aos elementos da natureza e ao vandalismo e escala monumental. A seleção da obra ficou a critério
    do artista. A obra foi executada a partir de um protótipo e todo o processo de construção foi contratado ao agente do artista,
    Galeria Paulo Fernando, que cuidou os patrocínios e da instalação da peça em seu lugar definitivo. O crítico Ronaldo, [o associa às
    figuras delgadas e dramáticas de um dos semi-deuses da arte moderna, Alberto Giacometti... O grande passante substitui uma desenvoltura
    casual, que nao deixa, no entranto de sofrer a ansiedade de seu proprio equilibrio físico. Comparado ao heroísmo espiritual das personagens
    de Giacometti, o Grande PAssante celebra simplesmente o novo herói anonimo que circula pelas ruas... O que qualifica estéticamente,
    todavia, é o calculo sutil de uma estrutura que ginga].`,
  Fatos: [
    new Fact(
      '1994',
      '1997',
      'Política Pública',
      'Esculturas Urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Promotor',
      'Secretaria Municipal de Cultura',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Patrocínio',
      'O artista precisou buscar patrocínio extra para finalização do trabalho',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '12/12/1995',
      '12/12/1995',
      'Notícia',
      'Inauguração prevista para o dia 20/12/1995',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const JoseBonifacio_LouisRochet: Obra = {
  ID: 19,
  Imagem: 'JoseBonifacio_LouisRochet.jpeg',
  Authors: [authors.LouisRochet],
  Titulo: 'José Bonifácio',
  Material: 'Bronze',
  DataInauguracao: '09/1872',
  DatasImportantes: [new Data('09/1872', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Largo de São Francisco',
  Latitude: '-22.905109',
  Longitude: '-43.180847',
  SuporteEspacial: 'largo',
  Base: 'Com pedestal',
  MaterialBase: 'marmore',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido para a comemoração docinquentenárioda Independência do Brasil,
    por iniciativa do Instituto Histórico e Geográfico Brasileiro. A estátua de José Bonifácio esta
    assentado sobre um pedestal octogonal de mármore de Jura, extraído a jazida da cadeia de montanhas
    na França. José Bonifácio esta representando em corpo inteiro e de pé. Tem mão direita a pena apoiada
    em livros, como a simbolizar o instrumento com que escreveu o glorioso Manifesto as Nações. Destacando-se
    nas faces as figuras alegóricas da Justiça, Ciência, Integridade e Poesia.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const TresVelas_IvanPaulPaccini: Obra = {
  ID: 20,
  Imagem: 'TresVelas_IvanPaulPaccini.jpeg',
  Authors: [authors.IvanPaulPaccini],
  Titulo: 'Tres Velas',
  Material: 'concreto',
  DataInauguracao: '1992',
  DatasImportantes: [new Data('1992', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Cidade Nova',
  Endereco: 'Praça Noronha Santos',
  Latitude: '-22.906273',
  Longitude: '-43.196862',
  SuporteEspacial: 'praça',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FontedoMenino_MestreValentim: Obra = {
  ID: 21,
  Imagem: 'FontedoMenino_MestreValentim.jpeg',
  Authors: [authors.MestreValentim],
  Titulo: 'Fonte do Menino',
  Material: 'granito e chumbo',
  DataInauguracao: '1783',
  DatasImportantes: [
    new Data('1783', 'inauguração - arte ambiente'),
    new Data('01/1841', 'inauguração - inventário dos monunentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Lapa',
  Endereco: 'Passeio Público',
  Latitude: '-22.913821',
  Longitude: '-43.175929',
  SuporteEspacial: 'praça',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Originalmente, a fonte era composta por um menino de mármore segurando um cágado,
    que lançava água num barril de gnaisse. Foi substituído em 1841 por um anjo de chumbo, que
    segura à cartela com a legenda Sou útil inda brincando Em 2004 a estátua de chumbo, depois de
    diversas e graves mutilações, foi substituída por uma cópia em bronze.`,
  Fatos: [new Fact('', '', 'Bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FontedodosAmores_MestreValentim: Obra = {
  ID: 22,
  Imagem: 'FontedodosAmores_MestreValentim.jpeg',
  Authors: [authors.MestreValentim],
  Titulo: 'Fonte dos Amores',
  Material: 'granito e bronze',
  DataInauguracao: '01/1783',
  DatasImportantes: [new Data('01/1783', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Lapa',
  Endereco: 'Passeio Público',
  Latitude: '-22.913794',
  Longitude: '-43.175953',
  SuporteEspacial: 'praça',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `São dois Jacares entrelaçados fundidos na Casa do Trem, atual Museu personalidades Nacional
    e estão fixados sobre um pedra artificial. Trata-se de uma fonte por que da boca de cada jacaré
    escorre água abastecendo uma bacia de gnaisse. Originariamente, havia um coqueiro e três garças, 
    de cujos bicos a água caia. O coqueiro foi retirado em 1906, devido a ferrugem, por determinação
    do Conde dos Arcos e as garças não se conhece o destino. É representatiivo porque é a primeira
    estilização de animais da fauna nacional. Desativado desde o ano de 2016.`,
  Fatos: [new Fact('', '', 'Bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const CarlosGomes_RodolfoBernardelli: Obra = {
  ID: 23,
  Imagem: 'CarlosGomes_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Carlos Gomes',
  Material: 'Bronze',
  DataInauguracao: '1902',
  DatasImportantes: [
    new Data('1902', 'inauguração - arte ambiente'),
    new Data('1960', 'inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Treze de Maio',
  Latitude: '-22.909463',
  Longitude: '-43.176623',
  SuporteEspacial: 'calçada',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'A estátua esteve durante 49 anos voltada para o Teatro Municipal do Rio de Janeiro.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Carlos Gomes', [], ''),
    new Fact(
      '',
      '',
      'Réplica',
      'A estátua é uma réplica da obra de Rofolfo Bernardelli ',
      [],
      'Campinas',
    ),
    new Fact(
      '',
      '',
      'Financiamento',
      'A obra foi financiada pela Secretaria de Viação e Obras da Prefeitura do Distrito Federal',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Localização',
      'Por sugestão do barítono Paulo Fortes, a estátua foi instalada no lugar da estátua de Chopin, que retornou para a Praia Vermelha',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Inauguração',
      'O monumento marcou o início da temporada líricada nacional de 1960',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2009',
      '2009',
      'transferência',
      'Por solicitação da direção do teatro à Prefeitura, e aprovada pela Comissão de Obras de Artes da Secretaria Municipal de Cultura, a estátua foi transferida para lateral do teatro.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const VicenteCelestino_TitoBernucci: Obra = {
  ID: 24,
  Imagem: 'VicenteCelestino_TitoBernucci.jpeg',
  Authors: [authors.TitoBernucci],
  Titulo: 'Vicente Celestino',
  Material: 'Bronze',
  DataInauguracao: '04/08/1970',
  DatasImportantes: [new Data('04/08/1970', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9064864',
  Longitude: '-43.1898684',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao:
    'E a representação da fisionomiade Vicente Celestino, uma gratidão do povo carioca ao cantor.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Vicente Celestino', [], ''),
    new Fact('', '', 'Iniciativa', 'Proposta por Frederico Trota', [], ''),
    new Fact('', '', 'Bem Tombado Estadual', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const Sinho_TitoBernucci: Obra = {
  ID: 25,
  Imagem: 'Sinho_TitoBernucci.jpeg',
  Authors: [authors.TitoBernucci],
  Titulo: 'Sinho',
  Material: 'Bronze',
  DataInauguracao: '08/1970',
  DatasImportantes: [new Data('08/1970', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9059339',
  Longitude: '-43.1901802',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'É a representação da fisionomia do compositor, um agradecimento o povo carioca.',
  Fatos: [
    new Fact('', '', 'Bem Tombado Estadual', '', [], ''),
    new Fact(
      '18/09/1888',
      '1930',
      'Homenageado',
      'José Barbosa da Silva, o Sinhô',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const BenjaminConstant_DecioVillares_EduardoDeSa_VicenteOrnelas: Obra = {
  ID: 26,
  Imagem: 'BenjaminConstant_DecioVillares_EduardoDeSa_VicenteOrnelas.jpeg',
  Authors: [authors.DecioVilares, authors.EduardoDeSa, authors.VicenteOrnelas],
  Titulo: 'Benjamin Constant',
  Material: 'Bronze',
  DataInauguracao: `07/1926`,
  DatasImportantes: [
    new Data(`07/1926`, 'inauguração'),
    new Data('06/1949', 'transferida para o Campo de Santana'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9066323',
  Longitude: '-43.1896404',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao: `O monumento tem como figura principal a escultura da Humanidade, simbolizada em uma mulher, tendo seu filho nos braços. A imagem da esposa de Benjamim, traz a bandeira república sendo bordada por
    sua filha e que Lea ofereceu a Escola Militar do Rio de Janeiro e a Escola Superior da Guerra. E o monumento que apresenta o maior número de símbolos exprimindo fatos históricos ligados a Pátria e a Humanidade.`,
  Fatos: [
    new Fact('', '', 'Bem tombado Estadual', '', [], 'Rio de Janeiro'),
    new Fact('', '', 'Homenageado', 'Benjamim Constant', [], ''),
    new Fact(
      '',
      '',
      'Oferecimento',
      'A obra foi oferecida à cidade por Amaro da Silveira.',
      [],
      '',
    ),
    new Fact(
      '2013',
      '2013',
      'Vandalismo',
      'O livro em bronze da obra foi danificado e removido para o depósito do Parque Noronha Santos',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const PauloDeFrontin_UgoTaddey: Obra = {
  ID: 27,
  Imagem: 'PauloDeFrontin_UgoTaddey.jpeg',
  Authors: [authors.UgoTaddey],
  Titulo: 'Paulo de Frontin',
  Material: 'Bronze',
  DataInauguracao: '09/1925',
  DatasImportantes: [new Data('09/1925', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Cinelândia',
  Latitude: '-22.9114573',
  Longitude: '-43.1757271',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Abaixo de sua figura feminina segurando dois ramos de louro. como engenheiro da formação da cidade, no dia de seu 65º aniversário.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Engenheiro Paulo de Frontin', [], ''),
    new Fact(
      '',
      '',
      'Promotor',
      'Iniciativa de amigos e admiradores',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '21/10/2008',
      '21/10/2008',
      'Furto',
      'O furto do florão de bronze foi registrado na 5ª DP.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JuscelinoKubitschek_Zanucchi: Obra = {
  ID: 28,
  Imagem: 'JuscelinoKubitschek_Zanucchi.jpeg',
  Authors: [authors.Zanucchi],
  Titulo: 'Juscelino Kubitschek',
  Material: 'Bronze',
  DataInauguracao: '1961',
  DatasImportantes: [
    new Data('1961', 'inauguração'),
    new Data('1983', 'inauguração google arts'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Floriano',
  Latitude: '-22.9114573',
  Longitude: '-43.1757271',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Constituído pela cabeça em bronze do ex-presidente e base de concreto. Erguido originalmente por seus admiradores em 1961. Foi destruído por um grupo de oficiais militares saídos do Clube Militar logo após a vitória do golpe de estado, em 1º de abril de 1964. Reconstituído após a abertura política em 1983.// Com o lema "crescer 50 anos em 5", Juscelino Kubitschek foi um dos mais emblemáticos Presidentes do Brasil. Governante entre 1951​ e 1961, ele acelerou a economia brasileira aumentando as indústrias e transformando a sociedade do país em mais urbana que rural. Em sua gestão, trocou a capital do país do Rio de Janeiro para a recém construída cidade de Brasília, na região Centro-Oeste, feita no tempo recorde de 41 dias.// ',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const FranciscoSerrador_ACoutinho: Obra = {
  ID: 29,
  Imagem: 'FranciscoSerrador_ACoutinho.jpeg',
  Authors: [authors.ACoutinho],
  Titulo: 'Francisco Serrador',
  Material: 'Bronze',
  DataInauguracao: '1928',
  DatasImportantes: [
    new Data('1928', 'inauguração - arte ambiente'),
    new Data('08/1944', 'inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Cinelândia',
  Latitude: '-22.9113639',
  Longitude: '-43.1757647',
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: 'ao incentivador do cinema do Brasil',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Francisco Serrador', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Homenagem',
      'Homenagem promovida na Quinzena Serrador, idealizada pelas companhias de teatro de Eva Tudor, Dulcina e Jayme Costa',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GetulioVargas_LuizSerri: Obra = {
  ID: 30,
  Imagem: 'GetulioVargas_LuizSerri.jpeg',
  Authors: [authors.LuizSerri],
  Titulo: 'Getulio Vargas',
  Material: 'Bronze',
  DataInauguracao: `1954`,
  DatasImportantes: [
    new Data(`1954`, 'data de inauguração'),
    new Data(`1962`, 'data de inauguração- sefcretaria de conservação'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Floriano',
  Latitude: `-22.9109921`,
  Longitude: `-43.1759181`,
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Nessa ocasião, foi instalada no pedestal A Carta Testamento de Getúlio, como o principal documento relacionado a sua morte.// Getúlio Vargas é o presidente do Brasil com maior tempo de governo na história. Primeiro, de 1930 a 1945. Depois, foi eleito democraticamente de 1951 até seu suicídio em 1954, totalizando 18 anos no poder. Dentre os destaques do seu governo estão a consolidação de leis trabalhistas como carteira de trabalho, salário mínimo, férias remuneradas e licença maternidade.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Getulio Vargas', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'A obra foi implantada por admiradores e partidários',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1954',
      '1954',
      'Inauguração',
      'A primeira homenagem da cidade do Rio de Janeiro a Getúlio Vargas aconteceu logo após sua morte, em 1954, quando foi instalado um busto sobre um vaso de mármore carrara.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '24/08/1962',
      '24/08/1962',
      'Reinauguração',
      'O busto foi reinaugurado em seu devido pedestal com uma placa contendo A Carta Testamento de Getúlio,',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '08/018',
      '08/2018',
      'Furto',
      'A placa com a Carta Testamento foi furtada',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PescadorNapolitano_FrancoisRude: Obra = {
  ID: 31,
  Imagem: 'PescadorNapolitano_FrancoisRude.jpeg',
  Authors: [authors.FrancoisRude],
  Titulo: 'Pescador Napolitano',
  Material: 'marmore de carrara',
  DataInauguracao: '01/1906',
  DatasImportantes: [new Data('01/1906', '')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9064598',
  Longitude: '-43.1899272',
  SuporteEspacial: 'praça',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Escultura da figura de um menino em mármore sentado brincando com uma tartaruga. Essa peça foi apresentada em gesso no Salão de Paris em 1831 e em mármore em 1833.Está é uma cópia da escultura em mármore.É uma reprodução da existente do Museu do Louvre.',
  Fatos: [new Fact('', '', 'Bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FonteJovemEuropa_MathurinMoreau: Obra = {
  ID: 32,
  Imagem: 'FonteJovemEuropa_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'Fonte Jovem Europa',
  Material: 'ferro fundido',
  DataInauguracao: '01/1906',
  DatasImportantes: [new Data('01/1906', '')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9063904',
  Longitude: '-43.1881627',
  SuporteEspacial: 'praça',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Fonte tipo estela, com uma escultura de estilo neoiclassico, representando de uma jovem figura feminina, a Jovem Europa, que através de uma bica que jorra a água, fundidas na França em 1885.Essa fonte forma um grupo de quatro conjuntos, expostos no centro do parque. Na sua origem eram oito conjuntos. quando o parque ocupava até a Avenida Presidente Vargas. Três desapareceram em 1945 e uma quarta no ano 2000. Essas peças foram instaladas posteriormente a inauguração, após inúmeras queixas dos frequentadores, da ausencia de água no parque.',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ASereia_ProvinSerres: Obra = {
  ID: 33,
  Imagem: 'ASereia_ProvinSerres.jpeg',
  Authors: [authors.ProvinSerres],
  Titulo: 'A Sereia',
  Material: 'ferro fundido',
  DataInauguracao: '01/1906',
  DatasImportantes: [new Data('01/1906', '')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao:
    'Escultura instalada no Campo de Santana foi transferida para o depósito em 2005, devido a acentuada corrosão, impedindo de sua permanecia em funcionamento com água.',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
    new Fact('', '2005', 'removida para o depósito', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Primavera_GustaveFredericMichel: Obra = {
  ID: 34,
  Imagem: 'Primavera_GustaveFredericMichel.jpeg',
  Authors: [authors.GustaveFredericMichel],
  Titulo: 'Primavera',
  Material: 'mármore de Carrara ',
  DataInauguracao: '1906',
  DatasImportantes: [
    new Data('1906', 'inauguração - arte ambiente'),
    new Data('01/1914', 'inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9066996',
  Longitude: '-43.1890224',
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao:
    'Escultura representando uma mulher, de grande sensualidade, com movimentos leves e um discreto sorriso. cabelos livres e soltos, segurando uns arranjos de flores, típico da primavera. Apoiada em uma espécie de pedra onde na parte posterior encontramos um arranjo com três flores e respectiva ramagem, na sua mão esquerda casal de pombos enamorados e seus pés estão descalços.',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Outono_GustaveFredericMichel: Obra = {
  ID: 35,
  Imagem: 'Outono_GustaveFredericMichel.jpeg',
  Authors: [authors.GustaveFredericMichel],
  Titulo: 'Outono',
  Material: 'mármore de Carrara',
  DataInauguracao: '1906',
  DatasImportantes: [
    new Data('1906', 'inauguração - arte ambiente'),
    new Data('01/1914', 'inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9066858',
  Longitude: '-43.1880704',
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao:
    'Escultura de uma mulher envolta em um pano,olhando em direção a uma cesta de vime cheia de frutas, especialmente maçã e uva, cabelos presos pelo arranjo de uma flor. A vesta esta cobrindo somente as pernas.',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Inverno_PaulJeanBaptisteGasg: Obra = {
  ID: 36,
  Imagem: 'Inverno_PaulJeanBaptistaGasg.jpeg',
  Authors: [authors.PaulJeanBaptisteGasg],
  Titulo: 'Inverno',
  Material: 'mármore de carrara',
  DataInauguracao: '1906',
  DatasImportantes: [new Data('1906', 'inauguração - arte ambiente')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Campo de Santana',
  Latitude: '-22.9066858',
  Longitude: '-43.1880704',
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Verao_PaulJeanBaptisteGasg: Obra = {
  ID: 37,
  Imagem: 'Verao_PaulJeanBaptisteGasg.jpeg',
  Authors: [authors.PaulJeanBaptisteGasg],
  Titulo: 'Verao',
  Material: 'mármore de carrara',
  DataInauguracao: '1906',
  DatasImportantes: [new Data('1906', 'inauguração - arte ambiente')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Campo de Santana',
  Latitude: '-22.9066858',
  Longitude: '-43.1880704',
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'Bem tombado estadual', '', [], ''),
    new Fact('', '', 'Bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const HeitorVillaLobos_BrunoGiorgi: Obra = {
  ID: 38,
  Imagem: 'HeitorVillaLobos_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: 'Heitor Villa Lobos',
  Material: 'Bronze',
  DataInauguracao: '04/1960',
  DatasImportantes: [
    new Data('04/1960', 'inauguração'),
    new Data('1959', 'criação'),
    new Data('17/11/1998', 'transferência para Avenida Rio Branco, 200'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Av. Avenida Rio Branco,200',
  Latitude: '-22.908963',
  Longitude: '-43.1763671',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'A homenagem ao maior compositor brasileiro está instalada ao lado do Theatro Municipal do Rio de Janeiro. Furtada na década de 1990, foi refundida em bronze a partir do molde original e recolocada no seu pedestal.// sua contribuição na música brasileira.// Heitor Villa-Lobos (1887-1959) foi um dos maiores maestros brasileiros de todos os tempos e se tornou o mais conhecido e significante compositor latino americano até hoje. Suas composições se tornaram únicas devido a presença da influência da tradição clássica européia e da música popular brasileira e indígena. Ele costumava dizer "Eu não uso o folclore, eu sou o folclore" para mostrar sua posição única entre outros compositores clássicos. "Trenzinho do Caipira” é uma de suas obras mais famosas mundialmente.',
  Fatos: [
    new Fact('', '', 'Homenageado', '', [persons.VillaLobos], 'Rio de Janeiro'),
    new Fact('17/11/1998', '17/11/1998', 'Réplica', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Fundição/ atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1998',
      '1998',
      'Réplica',
      'Custou R$ 13.000,00',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const GeneralSantander_FranciscoVictorPalma: Obra = {
  ID: 39,
  Imagem: 'GeneralSantander_FranciscoVictorPalma.jpeg',
  Authors: [authors.FranciscoVictorPalma],
  Titulo: 'General Santander',
  Material: 'Bronze',
  DataInauguracao: '12/1941',
  DatasImportantes: [new Data('12/1941', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Rio Branco com Avenida Presidente Wilson',
  Latitude: '-22.912566',
  Longitude: '-43.174678',
  Base: 'Com pedestal',
  MaterialBase: 'marmore',
  Status: 'Permanece em espaço público',
  Descricao:
    'A estátua do heroí nacional foi uma oferta do governo colombiano ao Brasil.',
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doação do governo colombiano ao Brasil',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'General Santander', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EvandroLinseSilva_AlfredoCeschiatti: Obra = {
  ID: 40,
  Imagem: 'EvandroLinseSilva_AlfredoCeschiatti.jpeg',
  Authors: [authors.AlfredoCeschiatti],
  Titulo: 'Evandro Lins e Silva',
  Material: 'Bronze',
  DataInauguracao: '10/2007',
  DatasImportantes: [new Data('10/2007', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Rio Branco com Avenida Presidente Wilson',
  Latitude: '-22.9124803',
  Longitude: '-43.1742838',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O busto foi uma homengamem da IBMEC ao jurista, na comemoração da adoção da Praça IV de julho e do novo espaço revitalizado.',
  Fatos: [
    new Fact(
      '',
      '',
      'Homenagem',
      'Homenagem da IBMEC em comemoração da revitalização da Praça IV de Julho',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'Jurista Evandro Lins e Silva', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const LutaDesigual_LDespresLaupedeCluny: Obra = {
  ID: 41,
  Imagem: 'LutaDesigual_LeonDespresLaupedeCluny.jpeg',
  Authors: [authors.LDespresLaupedeCluny],
  Titulo: 'Luta Desigual',
  Material: 'Peça em argamassa',
  DataInauguracao: '1880',
  DatasImportantes: [
    new Data('1872', 'data da obra'),
    new Data('1880', 'data de inauguração'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da República',
  Latitude: '-22.9075507',
  Longitude: '-43.1892197',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'grupo escultórico que representa o combate entre um homem e um felino,sobre um dos elementos criados por AugusteFrancoisMarieGlaziou, uma pedra redonda, a cerca de 2,5 m de altura. A peça é datada de 1872',
  Fatos: [new Fact('', '', 'Bem tombado estadual', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const ZumbidosPalmares_RomeuAlves: Obra = {
  ID: 43,
  Imagem: 'ZumbidosPalmares_RomeuAlves.jpeg',
  Authors: [authors.RomeuAlves],
  Titulo: 'Zumbi dos Palmares',
  Material: 'Bronze',
  DataInauguracao: '11/1986',
  DatasImportantes: [new Data('11/1986', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Vargas, 4',
  Latitude: '-22.9070093',
  Longitude: '-43.196244',
  Base: 'Com pedestal',
  MaterialBase: 'alvenaria revestida em mármore branco',
  Status: 'Permanece em espaço público',
  Descricao: `Zumbi dos Palmares não teve sua fisionomia identificada pela história, portanto,
    para representa-lo utilizou-se como molde a cabeça de um Rei Nigeriano, cujo original encontra-se
    no Museu de Londres. Idealizado pelo antropólogo e escritor Darcy Ribeiro, enquanto cumpria o
    mandato de vice-governador do estado do Rio de Janeiro, o monumento em homenagem a Zumbi dos
    Palmares foi elaborado pelo arquiteto João Filgueiras Lima, Authors: da disposição arquitetônica
    que remete ao Egito, com a base de mármore na forma do tronco de um pirâmide. Instalada na
    movimentada Avenida Presidente Vargas, no Rio de Janeiro (RJ), a obra tem o objetivo de
    fortalecer a consciência negra. Como o rosto do principal líder quilombola da história do
    Brasil não foi registrado, o molde utilizado foi uma peça em bronze que representa um rei do
    Benim, do século XII. A ideia de apropriação da forma de uma escultura do acervo do Museu
    Britânico, em Londres, partiu de Darcy Ribeiro, que mandou ampliar a peça original de 36
    centímetros para três metros, fundindo-a em 800 quilos de bronze.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Zumbi dos Palmares', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Idealização',
      'A obra foi idealizada pelo Senador Darcy Ribeiro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const AUniao_MathurinMoreau: Obra = {
  ID: 44,
  Imagem: 'AUniao_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'União',
  Material: 'ferro fundido',
  DataInauguracao: '1880',
  DatasImportantes: [new Data('01/1863', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.906880',
  Longitude: ' -43.182298',
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao: `Peça neoclássica que personifica os ideais valorizados pela filosofia Positivista
    de Auguste Comte. Um atraso na confecão e entrega fez com que as estátuas fossem instaladas em
    1863, um ano após a inauguração da praça. Na reforma de 1949/50 as estátuas foram transferida
    para o Campo de São Cristóvão permanecendo até os anos de 1980, quando do ínicio das obras da
    abertura da Linha Vermelha foram recolhidas no depósito da Fundação Parques e Jardins,
    permanecendo até 1992 quando foram novamente transferidas para o recém criado Parque Noronha
    Santos. No ano de 1999, para valorizar o acervo das peças das fundições francesas no Rio de
    Janeiro, foi montada uma exposição na Praça Nossa Senhora da Paz, quando foram transferida
    temporariamente. Finalmente em 2005, com o Projeto Monumenta Praça Tiradentes, a escultura
    retornou ao seu lugar de origem. Fundição Val D'Osne`,
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ALiberdade_MathurinMoreau: Obra = {
  ID: 45,
  Imagem: 'ALiberdade_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'Liberdade',
  Material: 'ferro fundido',
  DataInauguracao: '1863',
  DatasImportantes: [new Data('1863', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.907154',
  Longitude: '-43.183231',
  Base: 'Com pedestal',
  MaterialBase: 'ferrofundido',
  Status: 'Permanece em espaço público',
  Descricao:
    'Peça neoclássica que personifica os ideais valorizados pela filosofia Positivista de Auguste Comte. Um atraso na confecão e entrega fez com que as estatuas fossem instaladas em 1863, um ano após a inauguração da praça. Na reforma de 1949/50 as estátuas foram transferida para o Campo de São Cristóvão permanecendo até os anos de 1980, quando do início das obras da abertura da Linha Vermelha foram recolhidas no depósito da Fundação Parques e Jardins, permanecendo até 1992 quando foram novamente transferidas para o recém criado Parque Noronha Santos. No ano de 1999, para valorizar o acervo das peças das fundições francesas no Rio de Janeiro, foi montada uma exposição na Praça Nossa Senhora da Paz, quando foram transferida temporariamente. Finalmente em 2005, com o Projeto Monumenta Praça Tiradentes, a escultura retornou ao seu lugar de origem.',
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AJustica_MathurinMoreau: Obra = {
  ID: 46,
  Imagem: 'AJustica_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'Justiça',
  Material: 'ferro fundido',
  DataInauguracao: '1863',
  DatasImportantes: [new Data('1863', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.906740',
  Longitude: '-43.182365',
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao:
    'Peça neoclássica que personifica os ideais valorizados pela filosofia Positivista de Auguste Comte. Um atraso na confeção e entrega fez com que as estatuas fossem instaladas em 1863, um ano após a inauguração da praça. Na reforma de 1949/50 as estatuas foram transferida para o Campo de São Cristóvão permanecendo até os anos de 1980, quando do inicio das obras da abertura da Linha Vermelha foram recolhidas no depósito da Fundação Parques e Jardins, permanecendo até1992 quando foram novamente transferidas para o recém criado Parque Noronha Santos. No ano de 1999, para valorizar o acervo das peças das fundições francesas no Rio de Janeiro, foi montada uma exposição na Praça Nossa Senhora da Paz, quando foram transferida temporariamente. Finalmente em 2005, com o Projeto Monumenta Praça Tiradentes, a escultura retornou ao seu lugar de origem.',
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AFidelidade_MathurinMoreau: Obra = {
  ID: 47,
  Imagem: 'AFidelidade_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'Fidelidade',
  Material: 'ferro fundido',
  DataInauguracao: '1863',
  DatasImportantes: [new Data('1863', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.906920',
  Longitude: '-43.183229',
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao:
    'Peça neoclássica que personifica os ideais, valorizados pela filosofia Positivista, de Auguste Comte. Um atraso na confecão e entrega fez com que as estatuas fossem instaladas em 1863, um ano após a inauguração da praça. Na reforma de 1949/50 as estatuas foram transferida para o Campo de São Cristóvão, permanecendo até os anos de 1980. Quando do inicio das obras da abertura da Linha Vermelha, foram recolhidas no depósito da Fundação Parques e Jardins, permanecendo até1992, quando foram novamente transferidas para o recém criado Parque Noronha Santos. No ano de 1999, para valorizar o acervo das peças das fundições francesas no Rio de Janeiro, foi montada uma exposição na Praça Nossa Senhora da Paz, quando foram transferida temporariamente. Finalmente em 2005, com o Projeto Monumenta Praça Tiradentes, a escultura retornou ao seu lugar de origem.',
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JoaoCaetano_FranciscoManuelChavesPinheiro: Obra = {
  ID: 48,
  Imagem: 'JoaoCaetano_FranciscoManuelChavesPinheiro.jpeg',
  Authors: [authors.FranciscoManuelChavesPinheiro],
  Titulo: 'João Caetano',
  Material: 'Bronze',
  DataInauguracao: '03/05/1891',
  DatasImportantes: [
    new Data(
      '03/05/1891',
      'data de inauguração - Em frente a Academia Imperial de Belas Artes',
    ),
    new Data('1909', 'TRansferida para o Campo de Santana'),
    new Data('24/08/1916', 'Transferida para frente do Teatro João Caetano'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.906740',
  Longitude: '-43.182365',
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Transferida',
  Descricao: `Monumento executado em Roma e instalado inicialmente em frente a antiga Academia
    de Belas Artes, na Travessa das Belas Artes, em 3 de maio de 1891, por iniciativa do ator
    Francisco Correa Vasques. Em 1909 o Monumento foi transferido para o Campo de Santana.
    Em 24 de agosto de 1916 , no 43º da morte do homenageado foi transferido para esse local,
    em frente ao teatro que hoje leva o seu nome, na gestão Julio Furtado da Inspetoria de Caça,
    Matas e Florestas. O homenageado esta representado comos trajes do personagem principal da
    peça "Oscar, filho de Ossion", um das suas melhores representações.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const DomPedroI_LouisRochet: Obra = {
  ID: 49,
  Imagem: 'DomPedroI_LouisRochet.jpeg',
  Authors: [authors.LouisRochet],
  Titulo: 'Dom Pedro I',
  Material: 'bronze e ferro fundido',
  Peso: '55000 kg',
  DataInauguracao: '03/1862',
  DatasImportantes: [new Data('03/1862', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Tiradentes',
  Latitude: '-22.9065327',
  Longitude: '-43.182986',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Esse é o primeiro monumento construído no Brasil, idealizado para as comemorações os 50 anos da 1º Constituição
    Brasileira. Trata-se de um monumento de gratidãodo povo brasileiro ao Imperador D. Pedro. O pedestal é de 3,30 metros até o
    alto da cantaria; 6,40 até o alto da cornija, e 6 metros a estatua e seu plinto. O peso total do bronze é de 55 toneladas,
    sendo 28 todo o pesdestal e 12 t. a estatua equestre e 10 tonedas os dois grupos grandes e 5 os dois grupos pequenos. O
    Monumento está cercado por um gradil de ferro fundido em forma ortogonal que entre círculos se encontra a coroa imperial com
    a legenda D. Pedro I. Em cada ângulo do gradil, se encontra uma coluna um lampião. O gradil não existia quando o monumento foi
    inaugurado, pois só em 1865 foi colocado. A pavimentação entre o gradil e o monumento é de mármore, sendo o pedestal em granito
    cinza não polido, também na forma ortogonal, e o monumento é de bronze. Nas suas faces principais, abaixo da estátua de D. Pedro I, 
    encontram-se quatro alegorias que simbolizamos rios. Amazonas, Paraná, Madeira e São Francisco. A alegoria do São Francisco é
    representado por um índio sentado perto de um tamanduá bandeira e uma capivara. O rio Madeira mostra um índio com arco flecha em
    atitude de disparar, próximo a uma tartaruga, uma ave e peixe.O rio Amazonas é representado por uma índia com uma criança ás
    costas dominado, e uma índio com os pés sobre um jacaré, e uma arara. O rio Paraná tem como alegoria um índio segurando uma
    flecha e uma índia tocando. Na parte superior da face principal estão as armas do Império e a seguinte inscrição: A D. Pedro I,
    Gratidão dos Brasileiros. Nas faces laterais, as armas bragantinas vigiadas por dragões. A estátua mostra D. Pedro I montado a
    cavalo, apresentando na sua mão direito o auto de Independência do Brasil.`,
  Fatos: [
    new Fact('', '', 'Bem tombado federal', '', [], 'Rio de Janeiro'),
    new Fact('', '', 'Bem tombado estadual', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'cívica',
  Natureza: 'Referencial',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SolarWindsurfer_JesperNeergaard: Obra = {
  ID: 50,
  Imagem: 'SolarWindsurferALevezadaPedra_JesperNeergaard.jpeg',
  Authors: [authors.JesperNeergaard],
  Titulo: 'Solar Windsurfer',
  Material: 'mármore de Carrara',
  DataInauguracao: '10/2001',
  DatasImportantes: [new Data('1863', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Salgado Filho, Aeroporto Santos Dumont',
  Latitude: '-22.9103746',
  Longitude: '-43.1673523',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Peça esculpida no bloco em mármore de Carrara, cortado por linhas verticais
    profundas que se direcionam para ocentro da peça.`,
  Fatos: [
    new Fact(
      '03/1997',
      '03/1997',
      'fato',
      'Empresa Brasileira de Infra Estrutura Aeroportuária do Aeroporto Santos Dumont',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '02/1999',
      '02/1999',
      'Aprovação',
      'Comissão Carioca de Promoção Cultural da Prefeitura aprovou o projeto de construção da peça',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Patrocínio',
      'Rio Service Ltda, Servenco Construtora S.A, Integral Transporte e Agenciamento Maritimo Ltda e a Sata patrocinaram os custos da confecção da obra na cidade de CArrara',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const SantosDumont_AmadeuZani: Obra = {
  ID: 51,
  Imagem: 'SantosDumont_AmadeuZani.jpeg',
  Authors: [authors.AmadeuZani],
  Titulo: 'Santos Dumont',
  Material: 'Bronze',
  DataInauguracao: '10/1942',
  DatasImportantes: [new Data('10/1942', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Senador Salgado Filho',
  Latitude: '-22.911628',
  Longitude: '-43.167748',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `grupo escultórico de placas de granito encaixadas onde foram inseridas as esculturas. O maior destaque a figura de Santos Dumont pensativo e acima deste, a figura do Ícaro prestes a voar.
    Além desses elementos tem-se peças de avião e figuras humanas caídas.`,
  Fatos: [new Fact('', '', 'Homenageado', 'Santos Dumont', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const MulhercomAnfora_HumbertoCozzo: Obra = {
  ID: 52,
  Imagem: 'MulhercomAnfora_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: 'Mulher com Ânfora',
  Material: 'Bronze',
  DataInauguracao: '01/1934',
  DatasImportantes: [new Data('01/1934', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Praça Pio XI',
  Latitude: '-22.887475',
  Longitude: '-43.7738661',
  Status: 'Permanece em espaço público',
  Descricao: `Foi uma encomendado da Prefeitura em 1934 a Humberto Cozzo para esconder o ar
    condicionado do Teatro Municipal foi transferida e inaugurada na Praça Pio XI. Escultura
    chamada pelo artista de Oferenda está<no centro do lago, que tem a forma de rosa, com um jato
    de água saindo a ânfora. O Chafariz e uma obra de arte em bronze esculpida em 1934 por Humberto
    Cozzo, a pedido do Prefeito Pedro Ernesto. Essa escultura já esteve na Praça Floriano, no Tearo
    Municipal para esconder o ar condicionado foi transferida para o Largo da Glória em 1950, para
    embelezar após a retirada da Fonte Remos Pinto. Foi novamente transferia devido a obra do Metro
    para o depósito da Fundação Parques e Jardins, e posteriormente transferida para a parca Pio XI 
    Chamada pelo artista de "Oferenda" está<no centro do lago, que tem a forma de rosa, com um jato
    de água saindo a ânfora. Desativado desde maio de 2017.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MahatmaGandhi_SankhoChaudhuri: Obra = {
  ID: 53,
  Imagem: 'MahatmaGandhi_SankhoChaudhuri.jpeg',
  Authors: [authors.SankhoChaudhuri],
  Titulo: 'Mahatma Gandhi',
  Material: 'Bronze',
  DataInauguracao: '1949',
  DatasImportantes: [
    new Data('1949', 'data de inauguração - arte ambiente'),
    new Data('02/1965', 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: '-22.9121617',
  Longitude: '-43.1762046',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('02/1965', '02/1965', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Presente',
      'Presente do governo Indiano ao antigo Estado da Guanabara',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'Mahatma Gandhi', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Girardet_JoseOtavioCorreaLima: Obra = {
  ID: 54,
  Imagem: 'Girardet_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: 'Girardet',
  Material: 'Bronze',
  DataInauguracao: '12/1945',
  DatasImportantes: [
    new Data('12/1945', 'data de inauguração'),
    new Data('2016', 'Data furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: '-22.9119411',
  Longitude: '-43.1751653',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `Girardet foi homenageado nessa praça por sua atuação como numismático e professor
    da Escola Nacional de Balas Artes e de gravura na Casa da Moeda, no momento da comemoração do
    99º aniversario. O busto foi furtado em agosto de 2016.`,
  Fatos: [
    new Fact('12/1945', '12/1945', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Homenageado',
      'Girardet, professor da Escola Nacional de Belas Artes e de gravura na Casa do Moeda, foi homenageado em comemoração ao seu 99º aniversário',
      [],
      'Rio de Janeiro',
    ),
    new Fact('08/2016', '08/2016', 'Furto', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CatullodaPaixaoCearense_HonorioPecanha: Obra = {
  ID: 55,
  Imagem: 'CatullodaPaixaoCearense_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: 'Catulo da Paixão Cearense',
  Material: 'granito petropolis',
  DataInauguracao: '01/1940',
  DatasImportantes: [new Data('01/1940', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: '-22.9119411',
  Longitude: '-43.1751653',
  Base: 'Com pedestal',
  MaterialBase: 'granito petropolis',
  Status: 'Permanece em espaço público',
  Descricao:
    'Subscrição pública. A obra apresenta somente a sua fisionomia do homenageado sobre um pedestal de granito aparelhado, com uma pequena dedicatória esculpida na pedra.',
  Fatos: [
    new Fact(
      '01/1940',
      '01/1940',
      'Inauguração',
      'O busto de Adolfo Varnhagem foi colocado celebração ao centenário da fundação do Instituto Histórico e Geográfico Brasileiro',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'Catulo', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Subscrição pública',
      'Iniciativa promovida pelo jornal A noite',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const AdolfoBergamini_HildegardoLeaoVelloso: Obra = {
  ID: 56,
  Imagem: 'AdolfoBergamini_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: 'Adolfo Bergamini',
  Material: 'Bronze',
  DataInauguracao: '10/1946',
  DatasImportantes: [new Data('10/1946', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: '-22.9119411',
  Longitude: '-43.1751653',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Apesar do prédio ter sido demolido nos anos 70, a homenagem permaneceu na mesma posição.',
  Fatos: [
    new Fact('10/1946', '10/1946', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '10/1946',
      '10/1946',
      "Promotor'",
      'Amigos e admiradores',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Homenageado',
      'Ex-prefeito Adolfo Bergamini',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Doação',
      'Dr. Josino de Araujo Medeiros',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ChafarizdoMonroe_MathurinMoreau: Obra = {
  ID: 57,
  Imagem: 'ChafarizdoMonroe_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: 'Chafariz do Monroe',
  Material: 'ferro fundido',
  DataInauguracao: '01/1880',
  DatasImportantes: [new Data('01/1880', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: '-22.911848',
  Longitude: '-43.1753428',
  Status: 'Permanece em espaço público',
  Descricao: `Maior Chafariz artístico em ferro fundido do Brasil. Foi adquirido em 1878 pelo
    imperador Pedro II, na Áustria. Executado na França pelas fundições da região do Val DOsne, que
    naquela época forneciam inúmeras peças artísticas para o mundo, foi instalado no Largo do Paço,
    para o embelezamento da praça. Durante do governo de Carlos Lacerda em decorrência das obras para
    construção da Av. Perimental (1962) foi transferido para a Praça da Bandeira, ali permanecendo
    ate 1978. Em 1976 devido à demolição do Palácio do Monroe que fora construído para ser o Pavilhão
    dos Estados Unidos na Exposição Internacional de 1922, ficou no final da Av Rio Branco com um
    enorme espaço aberto, o qual foi preenchido de forma harmoniosa, com a implantação deste Chafariz.
    A obra com dez metros de altura, ergue-se sobre embasamento de granito, dividido em conjuntos. O
    primeiro conjunto ( plintos) forma a bacia circular onde em quatro lados diametralmente opostos
    estão inseridas quatro carrancas entre folhagens . Da boca de cada carranca jorra a água
    recolhida em uma bacia de todas as águas do Chafariz. Acima desta peca tem um frontal decorativo 
    curvo em forma de concha sobre volutas. Acima temos um grupo escultórico composto por duas
    figuras de Atlantis e duas Cariatides com louros, vestes tipicamente gregas com os corpos em
    movimento sustentado nos braços uma grande bacia superior. Cada peca tem cerca de 2,5 metros de
    altura e têm ao seu redor figuras de anjos. Entre cada figura existe uma pequena bacia
    semicircular de onde escorre a água de pequenos bicos. Esta desativado no ano desde 2005.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const AshbelGreenSimentoneHelen_JoasPereiraDosPassos: Obra = {
  ID: 58,
  Imagem: 'AshbelGreenSimentoneHelen_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: 'Ashbel Green Simenton e Helen',
  Material: 'Bronze',
  DataInauguracao: '08/2009',
  DatasImportantes: [
    new Data('08/2009', 'data de inauguração'),
    new Data('2011', 'transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Silva Jardim, 87',
  Latitude: '-22.907639',
  Longitude: '-43.1814216',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: 'Estátua do casal Ashbel e Helen. ',
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Fundição e Atelier Messias',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2011',
      '2011',
      'Transferencia',
      'A obra foi transferida devido à obras urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'A obra foi pela igreja presbiteriana à cidade do Rio de Janeiro.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenagem',
      'A homenagem realizada no sesquicentenário da chegada do casal missionário, Ashbel e Helen, ao Brasil',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AlmirantePedroMaxdeFrontin_HildegardoLeaoVelloso: Obra = {
  ID: 59,
  Imagem: 'AlmirantePedroMaxdeFrontin_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: 'Almirante Pedro Max Frontin',
  Material: 'Bronze',
  DataInauguracao: '1954',
  DatasImportantes: [
    new Data('1954', 'data de inauguração'),
    new Data('1998', 'transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mauá',
  Latitude: '-22.896188',
  Longitude: '-43.180461',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto foi realizado por Leão Veloso e inaugurado na Praia do Flamengo em 1954.
    Foi transferido 1998 onde recebeu um novo pedestal econstruídoum painel no seu entorno,
    atribuido a J.Segura.`,
  Fatos: [
    new Fact(
      '1954',
      '1954',
      'Inauguração',
      'Estátua foi inaugurada na Praia do Flamengo',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1998',
      '1998',
      'transferência',
      'A peça foi transferida para a Praça Mauá e ganhou além de um novo pedestal, um mural ao entorno atribuido a J. Segura.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1998',
      '1998',
      'Reinauguração',
      'A reinauguração da peça foi realizada pelo Ministro da Marinha Almirante Esquadra Mauro Cesar Rodrigues Pereira e pela Sra. Gloria Maria Frontim Pereira Souza, sobrinha neta do homenageado, ao som da canção DNOG, tocada pela banda dos Fuzileiros Navais do Comando do 1º Distrito Naval',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BaraodeMaua_RodolfoBernardelli: Obra = {
  ID: 60,
  Imagem: 'BaraodeMaua_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Barão de Mauá',
  Material: 'Bronze',
  DataInauguracao: '04/1910',
  DatasImportantes: [new Data('04/1910', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mauá',
  Latitude: '-22.8960957',
  Longitude: '-43.180201',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O Monumento a Mauá teve origem no Clube de Engenharia, para ocinqüentenário das estradas de ferro, que apresentou um parecer naquele sentido, merecendo aprovação.A pedra fundamental do monumento foi lançada no dia 30 de abril de 1904. Composto por uma coluna dórica talhada em granito Irajá, assente em uma base de cantaria em no seu topo está a estátua de bronze em tamanho natural, de Visconde de Mauá.  A estátua apresenta na mão um chapéu alto e a bengala. Na base dois baixo; relevos em bronze, simbolizando, uma indústria e representando o outro a Baronesa, a primeira locomotiva no Brasil.',
  Fatos: [
    new Fact('10/1946', '10/1946', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '10/1946',
      '10/1946',
      "Promotor'",
      'Amigos e admiradores',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Homenageado',
      'Ex-prefeito Adolfo Bergamini',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Doação',
      'Dr. Josino de Araujo Medeiros',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PauloDeOliveiraSampaio_Desconhecida: Obra = {
  ID: 61,
  Imagem: 'PaulodeOliveiraSampaio_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: 'Paulo de Oliveira Sampaio',
  Material: 'Bronze',
  DataInauguracao: '1980',
  DatasImportantes: [new Data('1980', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Ancora',
  Latitude: '-22.903738',
  Longitude: '-43.171320',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AtilioCorreaLima_CPincy: Obra = {
  ID: 62,
  Imagem: 'AtilioCorreaLima_CPincy.jpeg',
  Authors: [authors.CPincy],
  Titulo: 'Atílio Correia Lima',
  Material: 'Bronze',
  DataInauguracao: '01/1994',
  DatasImportantes: [new Data('01/1994', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Ancora',
  Latitude: '-22.9056345',
  Longitude: '-43.1683567',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Esse monumento homenageia ao arquiteto e urbanista Atílio Correia Lima que projetou a Estação de Hidroavião',
  Fatos: [
    new Fact('01/1994', '01/1994', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Homenageado',
      'O arquiteto Atílio Correia Lima que projetou a Estação de Hidroavião',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AmizadeEntreOsPovos_CharlesKeck_BenevenutoBerna: Obra = {
  ID: 63,
  Imagem: 'AmizadeEntreOsPovos_CharlesKeck_BenevenutoBerna.jpeg',
  Authors: [authors.CharlesKeck, authors.BenevenutoBerna],
  Titulo: 'Amizade Entre Os Povos',
  Material: 'Bronze',
  DataInauguracao: '07/1931',
  DatasImportantes: [new Data('07/1931', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça IV de Julho',
  Latitude: '-22.9122058',
  Longitude: '-43.1732543',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento comemorativo do centenário da nossa Independencia, objetivando fortalecer
    a amizade Brasil-Estados Unidos. A escultura femininade pé,tem na mão direita uma palma de louros,
    e, na esquerda, os pavilhões americano e brasileiro. Na base frontal do pedestal, veem-se os
    medalhões entrelaçados de George Washington e José Bonifácio de Andrada e Silva. A estátua foi
    obtida por subscrição entre o povo dos Estados Unidos da América do Norte, sob os aspirios da
    América Chamber of Comner for Brazil. Oartista americano Charles Keck a execução da escultura.
    Em 1922, perante Authors:idades brasileiras e americanas, a escultura foi doada ao Brasil pelo
    Sr. Charles Hughes, A obra ficou guardada na Companhia Expresso Federal até 1931, quando o
    Centro Carioca, promoveu sua inauguração proxima a Igreja de Santa Luiza. Em4 de julho de 1942
    o monumento foi reinaugurado sobre outro pedestal de granito, elaborado pelo escultor Bernevenuto
    Berna, na Praça IV de Julho, em frente ao Consulado dos Estados Unidos.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const TeresaCristinaBragancadeBourbon_JoseLuizRibeiro: Obra = {
  ID: 64,
  Imagem: 'TeresaCristinaBragancadeBourbon_JoseLuizRibeiro.jpeg',
  Authors: [authors.JoseLuizRibeiro],
  Titulo: 'Imperatriz Teresa Cristina Bragança de Bourbon',
  Material: 'Bronze',
  DataInauguracao: '11/2008',
  DatasImportantes: [
    new Data('11/2008', 'data de inauguração'),
    new Data('31/01/2019', 'Furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Itália',
  Latitude: '-22.9108196',
  Longitude: '-43.1714926',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Imperatriz Teresa Cristina', [], ''),
    new Fact(
      '',
      '',
      'Homenagem',
      'A homenagem faz parte das comemorações dos 200 anos da vinda da Família Real Portuguesa para o Brasil',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Apoio', 'Consulado da Itália', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AmoraLira_EugeneLouisLequesne: Obra = {
  ID: 65,
  Imagem: 'AmoraLira_EugeneLouisLequesne.jpeg',
  Authors: [authors.EugeneLouisLequesne],
  Titulo: 'Amor à Lira',
  Material: 'ferro fundido',
  DataInauguracao: '06/1968',
  DatasImportantes: [new Data('17/06/1968', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Estado da Guanabara',
  Latitude: '-22.906610',
  Longitude: '-43.177912',
  Status: 'Permanece em espaço público',
  Descricao: `A primeira localização foi no Reservatório do Pedregulho sendo tranferido para a
    caixa da água do Morro do Cantagalo em 1960. Em 1968 foi doado ao espaço publcio da cidade e
    instalado na Praça Floriano e era conhecido como "bebedouro para pombos". Foi retirada em 1972
    devido a obra do Metro da Cidade. Em 14 de novembro de 1974 foi inaugurada na Praça Estado da
    Guanabara onde permanece até hoje. O conjunto tem a escultura de uma criança com a asa, como um
    anjo dedilhando uma lira, sobreum pedestal quadrado de onde verte água de cada lado sobre uma
    pequena bacia também de ferro. Esta desativado desde o ano de 2010.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const DuquedeCaxias_RodolfoBernardelli: Obra = {
  ID: 66,
  Imagem: 'DuquedeCaxias_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Duque de Caxias',
  Material: 'Bronze',
  DataInauguracao: '15/08/1899',
  DatasImportantes: [new Data('10/1946', 'data de inauguração arte ambiente')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Duque de Caxias',
  Latitude: '-22.9042759',
  Longitude: '-43.1891264',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao: `O monumento foi erguido, inicialmente na antiga praça Duque de Caxias,
    atualmente Largo do Machado, e inaugurado em 15 de agosto de 1899. Quando se construiu o
    prédio do Ministério do Exército na Avenida Presidente Vargas providenciou-se um
    concurso público para a construção de um monumento à Duque de Caxias, Patrono do Exército,
    contudo a solução encontrada foi à transferência do monumento existente no Largo do Machado.
    Foi reinaugurado em um panteon em 30 de agosto de 1949. A estátua foi fundida nas oficinas
    Thiebot, em Paris. A obra apresenta Caxias, montando um cavalo, em uniforme de Marechal do
    Exército. tendo na mão direita um binóculo e está em atitude de observação. O Pantheon para
    onde foi deslocado a estátua de Caxias foi destinada a conservar os restos mortais do o
    Patrono do Exército Brasileiro. Foi construído numa área de cerca de 5.300m2, externamente
    vestido de mármore. Na face posterior, fronteira ao jardim da praça, foi aplicado o Brasão
    de Duque de Caxias, em bronze. Na face anterior bem em frente ao portão do Ministério da
    Guerra, fica a porta de entrada em bronze com as armas da República em bronze, assim como
    a palavra Pantheon. Por esta entrada tem-se acesso à cripta. em forma circular, tem logo
    à entrada, lateralmente, dois painéis, uma homenagem ao Exército e outro a Marinha sendo
    toda revestida de mármore em tons de rosa claro e verde.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BaraodoRioBranco_HildegardoLeaoVelloso: Obra = {
  ID: 67,
  Imagem: 'BaraodoRioBranco_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: 'Barão do Rio Branco',
  Material: 'Bronze',
  DataInauguracao: '09/1943',
  DatasImportantes: [new Data('09/1943', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça dos Expedicionários',
  Latitude: '-22.9068559',
  Longitude: '-43.1729258',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A grande estátua em bronze tem atrás de si um marco de fronteira, que simboliza a demarcação e caracterização dos limites do Brasil. Nas laterais foram fundidos,
    em bronze, dois altos-relevos representando os dois maiores trabalhos de Rio Branco: a conquista pacífica das Missões e do Amapá. Na parte posterior o escultor Leão Veloso fez
    a representação dos rios Amazonas e Uruguai.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Barão do Rio Branco', [], ''),
    new Fact(
      '',
      '',
      'Projeto',
      'Felix Charpentier, baixo relevo em bronze com pedestal de granito',
      [],
      '',
    ),
    new Fact(
      '',
      '',
      'Reformulação da obra',
      'Após a morte de Charpentier, a obra foi reformulada.',
      [],
      '',
    ),
    new Fact(
      '',
      '',
      'Comissão de reformulação da obra',
      'Leão Veloso e o arquiteto Alcides Rocha Miranda',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Subscrição Pública',
      'Aberta por iniciativa do Jornal do Comércio',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Execução',
      'O artista Hildegardo Leao Veloso iniciou os trabalhos em Paris',
      [],
      '',
    ),
    new Fact(
      '',
      '',
      'Cópia',
      'Há uma cópia em mármore no Palácio Itamaraty',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EvaristodeMoraes_LaurindoRamos: Obra = {
  ID: 68,
  Imagem: 'EvaristodeMoraes_LaurindoRamos.jpeg',
  Authors: [authors.LaurindoRamos],
  Titulo: 'Evaristo de Moraes',
  Material: 'Bronze',
  DataInauguracao: '07/1942',
  DatasImportantes: [
    new Data('07/1942', 'data de inauguração - inventário dos monumentos'),
    new Data('1992', 'Furto'),
    new Data('1992', 'data de reinauguração - arte ambiente'),
    new Data('2016', 'Furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma GAndhi',
  Latitude: '-22.9119411',
  Longitude: '-43.1751653',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao:
    'Como houve saldo o Dr. Mello Vianna e Salles Malheiros resolveram fundir outro busto.',
  Fatos: [
    new Fact('07/1942', '07/1942', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Homenageado',
      'Evaristo de Morais, criador da Justiça do trabalho em 1930.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Promotor',
      'O Clube dos Advogados foi responsável pela iniciativa e formou uma comissão dirigida pelo Dr. Fernando Mello Vianna, então presidente da Ordem dos Advogados.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Patrocínio',
      'A comissão dirigida pelo Dr. Fernando Mello Vianna angariou a quantia necessária para execução da peça',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Cópia',
      'Um novo busto foi fundido',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Patrocínio',
      'Custeado pela Sociedade de Advogados Criminalistas do Estado do Rio de Janeiro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MarechalDeodorodaFonseca_ModestinoKanto: Obra = {
  ID: 69,
  Imagem: 'MarechalDeodorodaFonseca_ModestinoKanto.jpeg',
  Authors: [authors.ModestinoKanto],
  Titulo: 'Marechal Deodoro da Fonseca',
  Material: 'Bronze',
  Altura: '23 m',
  Peso: '850.000 kg',
  DataInauguracao: '11/1937',
  DatasImportantes: [new Data('11/1937', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Deodoro da Fonseca',
  Latitude: '-22.9140107',
  Longitude: '-43.1747018',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é o mausoléu onde estão os restos mortais do Marechal Deodoro e de sua esposa. O monumento é composto por  ao alto com o fundador da República, a cavalo, em atitude de comando.
    Nas laterais diversos vultos da época da Proclamação Câmara e Floriano Peixoto. Na frente do embasamento, destaca-se a imagem da República, simbolizada por uma mulher. Em outra face aparece a figura de
    D. Rosa Paulina da Fonseca, mãe de Deodoro.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Marechal Deodoro da Fonseca', [], ''),
    new Fact('', '', 'bem tombado estadual', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AnaNeri_LuizFerrer: Obra = {
  ID: 70,
  Imagem: 'AnaNeri_LuizFerrer.jpeg',
  Authors: [authors.LuizFerrer],
  Titulo: 'Ana Néri',
  Material: 'Bronze',
  DataInauguracao: '12/1956',
  DatasImportantes: [new Data('12/1956', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da Cruz Vermelha',
  Latitude: '-22.9116486',
  Longitude: '-43.1881644',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O monumento foi erguido depois de numa resolução formada na Terceira Conferência Interamericana, realizada no Rio de Janeiro em 1935, onde contribuíram as Sociedades Nacionais da Cruz Vermelha dos seguintes países: Argentina, Bolívia, Brasil, Canadá, Chile Colômbia, Costa Rica, Cuba, Equador, Estados Unidos, Guatemala, Haiti, Honduras, México, Nicarágua, Paraná, República Dominicana, São Salvador, Uruguai e Venezuela.',
  Fatos: [
    new Fact('10/1946', '10/1946', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '10/1946',
      '10/1946',
      "Promotor'",
      'Amigos e admiradores',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Homenageado',
      'Ex-prefeito Adolfo Bergamini',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1946',
      '10/1946',
      'Doação',
      'Dr. Josino de Araujo Medeiros',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const OlavoBilac_HumbertoCozzo: Obra = {
  ID: 71,
  Imagem: 'OlavoBilac_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: 'Olavo Bilac',
  Material: 'Bronze',
  DataInauguracao: '12/1935',
  DatasImportantes: [new Data('12/1956', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: '-22.9129431',
  Longitude: '-43.1766633',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Imponente busto em bronze sobre pedestal de granito, homenageando o poeta e jornalista carioca, o nome mais popular e admirado da poesia parnasiana no Brasil. //O destaque no monumento é painel em alto relevo de bronze onde esta representado seu poema épico "O caçador de esmeraldas". Foi promovido pela Academia de Letras para a recuperação do poeta instalado no Passeio Público para criar naquele espaço "O Pantheon dos Poetas e dos Artistas". O busto foi entregue pelo escultor dentro do prazo previsto.// Olavo Bilac (1865 - 1918) foi um célebre escritor e jornalista brasileiro. Poeta parnasiano, fundou a Academia Brasileira de Letras ao lado de Machado de Assis e outros grandes pensadores de seu tempo. Republicano e nacionalista muito ativo politicamente, Bilac fez campanhas cívicas a favor da alfabetização e foi autor do “Hino à Bandeira”, em 1889.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Olavo Bilac', [], 'Rio de Janeiro'),
    new Fact('', '', 'Promotor', 'Academia de Letras', [], 'Rio de Janeiro'),
    new Fact(
      '02/07/1935',
      '02/07/1935',
      'Contrato Execução',
      'O contrato de execução da obra foi assinado pelo Conde Afonso Celso, por parte da Academia de Letras',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const RodolfoBernardelli_JoseOtavioCorreaLima: Obra = {
  ID: 103,
  Imagem: 'RodolfoBernardelli_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: `Rodolfo Bernardelli`,
  Material: 'Bronze',
  DataInauguracao: `04/1934`,
  DatasImportantes: [new Data(`04/1934`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9125871`,
  Longitude: `-43.176523`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto em homenagem a Bernardelli foi realizado por seu discípulo Correa Lima, que gravou no próprio bronze a sua intenção: Ao Prof. Rodolfo Bernardelli.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Homenageado',
      'Rodolfo Bernardelli',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PedroAmerico_PauloMazzucchelli: Obra = {
  ID: 73,
  Imagem: 'PedroAmerico_PauloMazzucchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: 'Pedro Américo',
  Material: 'Bronze',
  DataInauguracao: '01/1925',
  DatasImportantes: [new Data('01/1925', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da Cruz Vermelha',
  Latitude: '-22.9140638',
  Longitude: '-43.1762933',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O monumento homenagea o artista entre os icones brasileiros. Foi uma iniciativa da Sociedade Brasileira de Belas Artes.',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], 'Rio de Janeiro'),
    new Fact('', '', 'Homenageado', 'Pedro Américo', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Sociedade Brasileira de Belas Artes',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlbertoNepomuceno_RodolfoBernardelli: Obra = {
  ID: 74,
  Imagem: 'AlbertoNepomuceno_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Alberto Nepomuceno',
  Material: 'Bronze',
  DataInauguracao: '04/1935',
  DatasImportantes: [new Data('04/1935', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: '-22.9137136',
  Longitude: '-43.1767548',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O monumento ao "Pai do Nacionalismo da musica erudita" foi realizado por seu amigo Rodolfo Bernardelli. Em 1885, Nepomuceno mudou-se para o Rio de Janeiro e foi morar na residencia dos Bernardelli, onde firmou amizade, o que pemitiu que tivesse continuidade seus estudos musicas. O monumento foi uma iniciativa do Diretorio Academico do Instituto Nacional de Musica. A inauguração foi realizada pela maestrina Joanidia Sodre. Estivram presentes o maestro Francisco Braga, professores do Instituto Nacional de Msica, os membros da diretoria, promotores da homenagem.',
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const APaz_SironFranco: Obra = {
  ID: 75,
  Imagem: 'APazAmpulheta_SironFranco.jpeg',
  Authors: [authors.SironFranco],
  Titulo: 'A Paz',
  Material: 'concreto',
  DataInauguracao: '06/1992',
  DatasImportantes: [new Data('06/1992', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça 22 de Abril',
  Latitude: '-22.9106088',
  Longitude: '-43.1685749',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em concreto armado em forma estilizada de ampulheta. Ao centro tem uma 4 caixas de vidro para a colocação
    de areia de vários paises. Na face superior existe uma abertura para colocar de terra de várias partes do mundo. Nas paredes do
    Monumento está gravado em baixo relevo no concreto as palavras “Paz Mundial” em 28 idiomas. Em 31 de março de 1992, iniciaram os
    entendimentos para a construção do Monumento a Paz, pela comunidade Bahia. O  projeto arquitetonica, concebido pelo artista
    plástico Siron Franco, o   projeto de Arquitetura Paisagismo Venus S.Pezeshr e o projeto de estrutura foi da empresa Metodo
    Engenharia   Ltda e a construtora foi a Mendes Junior S.A. Assim o Monumento foi construído por iniciativa da Comunidade Bahia
    no Brasil, que custeou toda a obra, para o Forum Global da Conferência da ONU-ECO 92, uma recordação de sua importância mundial
    de uma declaração e de seu propósito. Paizes que depositaram amostra do solo até o ano de 1997; Antígua; França; Guatemala;
    Luxemburgo; Malta; Managua; Autralia; Austria; Bahamas; Benin; Bermudas; Bielorrussia; Bolívia; Bostswana; Brasil; Bulgária;
    Butã; Cambodja; Canadá; China; Chipre; Colômbia; Ilhas Cook; Costa Rica; Rep.da Coréia; Republica Democrática da Coréia;
    Croácia; Dinamarca; Dominica; Republica Dominicana; El Salvador; Equador; Republica Eslovaca; Espanha; Estados Unidos; da
    América; Etiópia; Fiji; Filipinas; Granada; Grécia; Quijana; Guiné Bissau, Holanda; Hungria; Índia; Irlanda; Islândia; Israel;
    Itália; Jamaica; Japão; Jordânia; Kiribate; Laos; Latvia Lesoto; Libéria; Liechtenstein; Malásia; Ilhas Marshall; Maurities;
    México; Miamar; Moldavia; Mongólia; Nepal; Nicarágua; Nigéria; Noruega; Nova Caledonia; Nova Zelândia; Nova Guiné; Paraguai;
    Peru; Polônia; Portugal; Quênia; Romênia; San Marino; São Vicente; e As Granadenas; SenegalSerra Leoa; Seychelles; Síria;
    Slovênia; Suazilândia; Suécia; Suriname; Tchecoslováquia, Tanzânia; Trinidad e Tobago; Turisia; Turquia; Tuvalu; Uguada;
    Ukrania; Vanatu; Zaire; Zâmbia; Zimbábue.`,
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const EdsonLuis_CristinaPozzobon: Obra = {
  ID: 76,
  Imagem: 'EdsonLuis_CristinaPozzobon.jpeg',
  Authors: [authors.CristinaPozzobon],
  Titulo: 'Edson Luis',
  Material: 'aço pintado',
  DataInauguracao: '03/2008',
  DatasImportantes: [new Data('12/1956', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Ana Amélia',
  Latitude: '-22.9090811',
  Longitude: '-43.1716851',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura composta por uma base de 3 metros pintada de vermelho e um mastro com uma bandeira em movimento
    rasgada vermelho ao meio com uma mancha vermelha.`,
  Fatos: [],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarcilioDias_LuisPaesLeme: Obra = {
  ID: 77,
  Imagem: 'MarcilioDias_LuisPaesLeme.jpeg',
  Authors: [authors.LuisPaesLeme],
  Titulo: 'Marcílio Dias',
  Material: 'Bronze',
  DataInauguracao: '06/1948',
  DatasImportantes: [
    new Data('06/1948', 'data de inauguração'),
    new Data('1988', 'data transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Barão Ladário',
  Latitude: '-22.8988874',
  Longitude: '-43.179053',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao:
    'O busto esta sob a guarda do Arsenal devido a obra da praça Barão de Ladário.',
  Fatos: [
    new Fact(
      '2011',
      '2011',
      'Transferencia',
      'A obra foi transferida devido à obras urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'A obra foi doada pelo Clube Naval à cidade.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenagem',
      'Homenagem ao 882º aniversário da Batalha do Riachuelo e da morte de Marcílio Dias',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BaraodeLadario_Joceil: Obra = {
  ID: 78,
  Imagem: 'BaraodeLadario_Joceil.jpeg',
  Authors: [authors.Joceil],
  Titulo: 'Barão de Ladário',
  Material: 'Bronze',
  DataInauguracao: '1980',
  DatasImportantes: [
    new Data('1980', 'data de inauguração - arte ambiente'),
    new Data('12/1989', 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Barão de Ladário',
  Latitude: '-22.8990329',
  Longitude: '-43.1767519',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Efigie do homenageado foi realizada pela Marinha de Guerra para o reconhecimento público. O busto esta sob a guarda do Arsenal devido a obra da praça Barão de Ladário, até 20 de março de 2017 quando foi reinaugurada a praça.',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const CristianoOttoni_RodolfoBernardelli: Obra = {
  ID: 79,
  Imagem: 'CristianoOttoni_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: 'Cristiano Ottoni',
  Material: 'Bronze',
  DataInauguracao: '03/1908',
  DatasImportantes: [new Data('03/1908', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Cristiano Ottoni',
  Latitude: '-22.9041532',
  Longitude: '-43.1907208',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'O monumento foi erguido em praça pública em frente a antiga estaçãoferroviária. Quando dasreformas na áreaa estátua foi transferida para a fachada do prédio da Central do Brasil. O homenageado esta de pé tendo uma das mãos sobre o peito. Originalmente o pedestal tinha cinco (5) metros de altura, e em cada lado do pedestal simbolizam chaminés de locomotivas e semi-círculos imitando rodas. Erguido por subscrição popular promovida pela Comissão do Jubileuda Estrada de Ferro Central do Brasil',
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SaturninoSoaresMeirelles_Desconhecida: Obra = {
  ID: 80,
  Imagem: 'SaturninoSoaresMeirelles_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: 'Saturnino Soares Meirelles',
  Material: 'Bronze',
  DatasImportantes: [new Data('', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça da Cruz Vermelha',
  Latitude: '-22.9116486',
  Longitude: '-43.1881644',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Este busto foi erguido por iniciativa do Instituto Hahnemanniano do Brasil e doado á Cidade.',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ChiquinhaGonzaga_HonorioPecanha: Obra = {
  ID: 81,
  Imagem: 'ChiquinhaGonzaga_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: 'Chiquinha Gonzaga',
  Material: 'Bronze',
  DataInauguracao: '10/1942',
  DatasImportantes: [new Data('10/1942', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: '-22.9134168',
  Longitude: '-43.1774497',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'A peça apresenta a homenageada elegante com vestimenta de época.// Busto em bronze foi uma iniciativa da Sociedade Cultural Chiquinha Gonzaga. A maestrina e compositora foi a primeira mulher brasileira a assinar partituras chegando a realizar dezenas de peças teatrais e mais de 2 mil composições musicais.// Compositora e maestrina carioca, Chiquinha Gonzaga (1847-1935) destaca-se na história e cultura brasileira pela coragem com que enfrentou a sociedade patriarcal criando uma profissão inédita para a mulher. Primeira pianista de choro, gênero popular da música brasileira, ela também é autora da primeira marcha carnavalesca com letra ("Ó Abre Alas”, 1899) e também a primeira mulher a reger uma orquestra no Brasil.',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Chiquinha Gonzaga', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Sociedade Cultural de Chiquinha Gonzaga, liderada por Meriza Lyra.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const IrineuMarinho_BenevenutoBerna: Obra = {
  ID: 82,
  Imagem: 'IrineuMarinho_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Irineu Marinho`,
  Material: 'Bronze',
  DataInauguracao: `03/1937`,
  DatasImportantes: [new Data(`03/1937`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9131599`,
  Longitude: `-43.1772132`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem a Irineu Marinho esta na entrada do parque, voltado para o Lapa. É composto pelo busto do homenageado e por uma engrenagem tipica de um jornal, identificando sua história.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact(
      '',
      '',
      'Inauguração',
      'Em comemoração à imprensa',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'Irineu Marinho', [], 'Rio de Janeiro'),
    new Fact('', '', 'Promotor', 'Centro Carioca', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PlacidoDeCastro_ArmandoSocratesSchnoor: Obra = {
  ID: 83,
  Imagem: 'PlacidoDeCastro_ArmandoSocratesSchnoor.jpeg',
  Authors: [authors.ArmandoSocratesSchnoor],
  Titulo: `Plácido de Castro`,
  Material: 'Bronze',
  DataInauguracao: `01/1973`,
  DatasImportantes: [new Data(`01/1973`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9126679`,
  Longitude: `-43.176283`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao:
    'Este busto foi erguido por iniciativa do Instituto Hahnemanniano do Brasil e doado á Cidade.',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact(
      '',
      '',
      'Homenageado',
      'Major Plácido de Castro é o único militar homenageado no Passeio Público',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FontedoTritao_NicolinaVazDeAssisPintodoCouto: Obra = {
  ID: 84,
  Imagem: 'FontedoTritao_NicolinaVazDeAssisPintodoCouto.jpeg',
  Authors: [authors.NicolinaVazDeAssisPintodoCouto],
  Titulo: `Fonte do Tritão`,
  Material: 'Bronze',
  DatasImportantes: [new Data(`12/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9137649`,
  Longitude: `-43.1767648`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A escultura do tritão é uma reprodução da obra de Nicolina Vaz de Assis, que realizou
    a escultura original. A figura de um tritão segurando um cantil de onde escorre a água.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const JosePauloSilva_CarlosDelNegro: Obra = {
  ID: 85,
  Imagem: 'JosePauloSilva_CarlosDelNigro.jpeg',
  Authors: [authors.CarlosDelNigro],
  Titulo: 'Jose Paulo Silva',
  Material: 'Bronze',
  DataInauguracao: `07/1969`,
  DatasImportantes: [
    new Data(`07/1969`, 'data de inauguração'),
    new Data(`10/2017`, 'Furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9137193`,
  Longitude: `-43.1774099`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `Por sua vida dedicada a música ele se encontra voltado para a Escola Nacional de Música. Peça foi furtada em outubro de 2017.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Paulo Silva', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Sociedade de Amigos do Professor Paulo Silva (SAPPS)',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Inauguração',
      'A obra foi inaugurada pelo presidente da SAPPS, professor Modesto de Abreu',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlmiranteJuliodeNoronha_ABraga: Obra = {
  ID: 85,
  Imagem: 'AlmiranteJuliodeNoronha_ABraga.jpeg',
  Authors: [authors.ABraga],
  Titulo: 'Almirante Julio d eNoronha',
  Material: 'Bronze',
  DataInauguracao: `06/1980`,
  DatasImportantes: [new Data(`06/1980`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Praça Julio de Noronha',
  Latitude: `-22.9622217`,
  Longitude: `-43.1675421`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao Almirante e o reconhecimento e por sua participação no Conselho Naval. O monumento foi inaugurado com a presença do Prefeito Julio Coutinho e de Israel Klabin, que ali compareceu como um "homem do mar" não como uma autoridade.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'Julio de Noronha', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const RaymundoCorrea_HonorioPecanha: Obra = {
  ID: 86,
  Imagem: 'RaymundoCorrea_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Raymundo Corrêa`,
  Material: 'Bronze',
  DataInauguracao: `11/1944`,
  DatasImportantes: [
    new Data(`11/1944`, 'data de inauguração'),
    new Data(`01/2018`, 'furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9132256`,
  Longitude: `-43.1764889`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `O monumento o imortaliza pelos carater popular de seus sonetos.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Raymundo Corrêa', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Academia de Letras com colaboração do Itamarati',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FranciscoBraga_PauloMazzucchelli: Obra = {
  ID: 87,
  Imagem: 'FranciscoBraga_PauloMazzucchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: `Francisco Braga`,
  Material: 'Bronze',
  DataInauguracao: `11/1952`,
  DatasImportantes: [new Data(`11/1952`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.913711`,
  Longitude: `-43.1769561`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `O monumento a Francisco Braga foi inaugurado na esquina das ruas Siqueira Campos
    com Henrique Oswaldo . Foi uma iniciativa do Departamento de Educação e com as palavras do
    prefeito João Vital e do escritor Alvaro Moreyra, em nome dos amigos do homenageado.
    Em 9 de dezembro de 1952, a peça foi furtada e mais tarde reposta no Passeio Paublico, voltado
    para a Escola Nacional de Música, na Rua do Passeio. A peça foi furtada em abril de 2016.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const VictorMeirelles_EduardoDeSa: Obra = {
  ID: 88,
  Imagem: 'VictorMeirelles_EduardoDeSa.jpeg',
  Authors: [authors.EduardoDeSa],
  Titulo: `Victor Meirelles`,
  Material: 'Bronze',
  DataInauguracao: `11/1925`,
  DatasImportantes: [new Data(`11/1925`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9137417`,
  Longitude: `-43.1772696`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `no dia que comemorava o 69º aniversario da instituição, por ter sido o pintor catarinense um benemerito da instituição.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Victor Meirelles', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Doação',
      'Sociedade Propagadora das Belas Artes, mantenedora do Liceu de Artes e Ofícios do RIo de Janeiro.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MestreValentim_RomeuAlves: Obra = {
  ID: 89,
  Imagem: 'MestreValentim_RomeuAlves.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Mestre Valentim`,
  Material: 'Bronze',
  DataInauguracao: '2004',
  DatasImportantes: [new Data('2004', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9131342`,
  Longitude: `-43.1770958`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi realizado nas comemorações centenário de sua morte.O monumento foi
    realizado nas comemorações do centenário de sua morte. Não existe um retrato de Valentim,
    assim o escultor Joaquim Rodrigues Moreira Junior reproduziu da obra do Lucílio de Albuquerque.
    O busto original inaugurado 01/03/1913. Essa peça foi furtada no ano de 8 de janeiro de
    2004 e refeita pelo artista Romeu Alves. A idéia de se fazer o Mestre Valentim, surgiu em 1899
    quando Araújo Vianna, professor de Historia da Arquitetura da Escola de Belas Artes, escreveu
    um artigo no jornal  A Noticia, ressaltando a necessidade de se homenagear  o artista.// Mestre Valentim foi um dos principais artistas do Brasil Colonial. Considerado um artista completo por suas produções escultóricas, arquitetônicas e urbanísticas, sua expressão foi grandiosa nos estilos Barroco, Rococó e Clássico. Influenciado pela missão francesa, medida que trouxe artistas e urbanistas apoiados pelo rei D. João VI para produzir arte no Rio de Janeiro, ele criou, projetou e executou importantes espaços sociais urbanos, como o Chafariz da Pirâmide, localizado na Praça XV, e o Passeio Público, primeiro jardim aberto à população da cidade.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CastroAlves_EduardoDeSa: Obra = {
  ID: 90,
  Imagem: 'CastroAlves_EduardoDeSa.jpeg',
  Authors: [authors.EduardoDeSa],
  Titulo: `Castro Alves`,
  Material: 'Bronze',
  DataInauguracao: `09/1913`,
  DatasImportantes: [new Data(`09/1913`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9132919`,
  Longitude: `-43.1763128`,
  Base: 'Com pedestal',
  MaterialBase: 'pedra sabão',
  Status: 'Permanece em espaço público',
  Descricao: `Busto em bronze de poeta brasileiro conhecido por combater a escravidão (1847-1871)// O monumento se destaca com a peça de meio corpo com um instrumento musical nas mãos. O pedestal com representação marajoara, com pedras de diferentes tonalidades, forma uma composição original.// Nascido no interior da Bahia, Castro Alves (1847-1871) foi um dos mais icônicos poetas do Romantismo Brasileiro no século 19. Inspirado pela temática social, como o combate à escravidão e opressão, ficou conhecido como "Poeta dos Escravos". Seu trabalho mais marcante é "Navio Negreiro", um poema épico-dramático que denuncia a escravidão dos negros africanos e seu transporte até chegar ao Brasil.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Castro Alves', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FerreiraDeAraujo_RodolfoBernardelli: Obra = {
  ID: 91,
  Imagem: 'FerreiraDeAraujo_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Ferreira de Araújo`,
  Material: 'Bronze',
  DataInauguracao: `08/1912`,
  DatasImportantes: [new Data(`09/1913`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.913711`,
  Longitude: `-43.1769561`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Na inauguração estava localizado a esquerda do portão central.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact(
      '08/1912',
      '08/1912',
      'Inauguração',
      'A obra foi inaugurada pelo General Bento Ribeiro, então prefeito do Distrito Federal e pelo Conde Afonso Celso, presidente do Instituto personalidades e Geografico.',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Homenageado', 'Ferreira de Araújo', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Diretoria da Gazeta de Notícias',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GoncalvesDias_RodolfoBernardelli: Obra = {
  ID: 92,
  Imagem: 'GoncalvesDias_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Gonçalves Dias`,
  Material: 'Bronze',
  DataInauguracao: `06/1901`,
  DatasImportantes: [new Data(`06/1901`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9137136`,
  Longitude: `-43.1767548`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem a Gonçalvez Dias foi a primeira a ser instalada no parque e um dos
    primeiros bustos executados no Pais para área pública. A peça em bronze apresenta o rosto
    do homeangeado com expressão firme,
    cabelos encaracolados, bigodes e barba sem vestimenta. O pedestal de granito tem linhas
    retas, deixando a busto expressivo destacam-se no espaço.// Antonio Gonçalves Dias foi advogado, jornalista e poeta brasileiro, no século 19. Como um dos grandes nomes do Romantismo Brasileiro da primeira geração, seu trabalho mais memorável é "Canção do Exílio", poema em que mostra a aversão aos valores portugueses e exalta os valores naturais do Brasil, devido ao rompimento recente de Portugal e sua colônia.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Gonçalvez Dias', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Promotor',
      'Olavo Bilac e Rodolfo Bernardelli',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Primavera_MathurinMoreau: Obra = {
  ID: 93,
  Imagem: 'Primavera_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Primavera`,
  Material: 'ferro fundido',
  DataInauguracao: `1860`,
  DatasImportantes: [
    new Data(`01/1860`, 'data de inauguração - arte ambiente'),
    new Data(`01/1862`, 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.913654`,
  Longitude: `-43.1766988`,
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua neoclássica da figura de uma mulher, segurando uma guirlanda de flores, trajando uma veste vaporosa. Foirealizada nas Fundições de Val dOsne, França.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Inverno_GustaveFredericMichel: Obra = {
  ID: 94,
  Imagem: 'Inverno_GustaveFredericMichel.jpeg',
  Authors: [authors.GustaveFredericMichel],
  Titulo: `Inverno`,
  Material: 'ferro fundido',
  DataInauguracao: `1860`,
  DatasImportantes: [
    new Data(`01/1860`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9133127`,
  Longitude: `-43.1763977`,
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de uma figura feminina envolta a um manto que a encoberta até sua cabeça. Tem uma das mais suas mãos segurando a coberta e a mão se dirigindo para um fogareiro que aquece as suas mãos em gestos suaves Seu corpo está em leve movimento como que procura afastar sua veste do fogo, apesar de procurar o calor.
    O pedestal também em ferro fundido assemelha-se a todos o do conjunto. Peça circular decorada com ramalhete de flores com trigo e mascara.
    A estátua do Inverno do Passeio possuía originalmente uma lamparina. No Passeio Publico nos anos de 1990 existiam tres figuras que representavam as estações do ano ( Verão, Outono e Inverno). O conjunto somente ficou completo com a instalação dessa escultura no ano de 2000.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Outono_MathurinMoreau: Obra = {
  ID: 95,
  Imagem: 'Outono_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Outono`,
  Material: 'ferro fundido',
  DataInauguracao: `01/1860`,
  DatasImportantes: [
    new Data(`01/1860`, 'data de inauguração - arte ambiente'),
    new Data(`01/1862`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Removida do espaço público',
  Descricao: `Estátua neoclássica da figura de homem com um grande cacho de uva, sobre um pedestal também em ferro fundido. A peça foi transferida para o depósito em janeiro de 2016, devido ao dano no pedestal. `,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Verao_GustaveFredericMichel: Obra = {
  ID: 96,
  Imagem: 'Verao_GustaveFredericMichel.jpeg',
  Authors: [authors.GustaveFredericMichel],
  Titulo: `Verão`,
  Material: 'ferro fundido',
  DataInauguracao: `01/1860`,
  DatasImportantes: [
    new Data(`01/1860`, 'data de inauguração - arte ambiente'),
    new Data(`01/1862`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9129741`,
  Longitude: `-43.1770187`,
  Base: 'Com pedestal',
  MaterialBase: 'ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua neoclássica da figura de homem segurando uma foice e um machado, sobre um pedestal em ferro fundido.<Provavelmente trazida para o Rio de Janeiro na reforma realizada pelo paisagista AugusteFrancoisMarieGlaziou por determinação de D.Pedro II.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Castro Alves', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const NelsonMege_HonorioPecanha: Obra = {
  ID: 99,
  Imagem: 'NelsonMege_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Nelson Megé`,
  Material: 'metalon',
  DataInauguracao: `01/1956`,
  DatasImportantes: [
    new Data(`01/1956`, 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praça Comendador Mage',
  Latitude: `-22.8073476`,
  Longitude: `-43.1792296`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Busto em bronze erigido por iniciativa dos moradores da Ilha do Governador.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const OMascate_HonorioPecanha: Obra = {
  ID: 100,
  Imagem: 'OMascate_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `O Mascate`,
  Material: 'Bronze',
  DataInauguracao: `11/1991`,
  DatasImportantes: [new Data(`11/1991`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Buenos Aires e Regente Feijó, no SAARA',
  Latitude: `-22.9053822`,
  Longitude: `-43.1849111`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Criado em 1964 pela Confederação Nacional do Comércio e pelo Departamento Nacional do Senac, o troféu tinha como
    finalidade homenagear empresários que se destacam por sua contribuição ao desenvolvimento do país. Esculpido em bronze pelo
    artista plástico Honório Pecanha, o troféu representa a figura de um mascate, comerciante que se dedicava a vender os mais
    variados produtos de uso pessoal e doméstico, em distantes lugares do Brasil. A Prefeitura do Rio, por meio da Secretaria de
    Conservação e Serviços Públicos, entrega recuperado nesta sexta-feira, dia 18 de maio, a escultura em bronze “O Mascate”, instalada
    entre as ruas Buenos Aires e Regente Feijó, na Saara, Centro do Rio.`,
  Fatos: [
    new Fact(
      '1991',
      '1991',
      'Doação',
      'Oferecido pela Confederação Nacional do Comercio ao SAARA Sociedade de Amigos das Adjacências da Rua da Alfândega',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenagem',
      'Aos 50 anos da Federação do Comercio atacadista no Estado do Rio de Jaaneiro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1964',
      '1964',
      'Original',
      'A escultura é uma cópia ampliada do troféu O Mascate, esculpido por Honório Peçanha',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Atelier Mestre Liboredo',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Manutenção',
      'A Secretaria de Conservação e Serviços Públicos, representado pela Gerência de Monumentos e Chafarizes, recuperou o pedestal da obra, realizou sua limpeza e aplicação de paralóide',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const LuisdeCamoes_AntonioTaunay: Obra = {
  ID: 101,
  Imagem: 'LuisDeCamoes_AntonioTaunay.jpeg',
  Authors: [authors.AntonioTaunay],
  Titulo: `Luis de Camões`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, '')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Luis de Camões',
  Latitude: `-22.9055189`,
  Longitude: `-43.1821352`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Esse busto é uma réplica do existente no Instituto Histórico e Geografico Brasileiro - IHGB, Rio de Janeiro ,
    executado pelo escultor francês Antonio Taunay. Homenagemcom a figura clássica de Camões com o rosto de lado com coroa de
    louros e vestimenta da época. A peça esta sobre um pequeno pedestal de granito polido, de lado para Real Gabinete de Leitura
    com o olhar dirigido para aquela instituição.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const RetanguloVazado_FranzWeissmann: Obra = {
  ID: 102,
  Imagem: 'RetanguloVazado_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Retângulo Vazado`,
  Material: 'aço pintado',
  Altura: '320 cm',
  Largura: '280 cm',
  Profundidade: '250 cm',
  Peso: '5000 kg',
  ValorObra: 'R$60.000,00',
  DataInauguracao: `27/09/1996`,
  DatasImportantes: [new Data(`27/09/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Luís de Camões, 30',
  Latitude: `-22.9060513`,
  Longitude: `-43.181959`,
  SuporteEspacial: 'Calçadão',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça geométrica com espaço vazado como um elemento definidor. O plano, as articulações e o elemento vazado, cria
    a tridimensionalidade aberta, caracteristica do artista.`,
  Fatos: [
    new Fact(
      '1994',
      '1997',
      'Política Pública',
      'Esculturas Urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Promotor',
      'Secretaria Municipal de Cultura',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '27/09/1996',
      '27/09/1996',
      'Inauguração',
      'Franz Weissmann esteve presente na inauguração ao lado da Secretária Municipal de Cultura, Helena Severo',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const SiqueiraCampos_Desconhecida: Obra = {
  ID: 103,
  Imagem: 'SiqueiraCampos_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Siqueira Campos`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Copacabana',
  Endereco: 'Praça General Eugênio Franco',
  Latitude: `-22.9863366`,
  Longitude: `-43.1902023`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlegoriadaMusica_RodolfoBernardelli: Obra = {
  ID: 104,
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Alegoria da Musica`,
  Material: 'Bronze',
  DataInauguracao: `1909`,
  DatasImportantes: [new Data(`1909`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Heitor de Melo',
  Latitude: `-22.908402`,
  Longitude: `-43.175969`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const RuiBarbosa_DeoclecianoMartinsDeOliveiraFilho: Obra = {
  ID: 105,
  Imagem: 'RuiBarbosa_DeoclecianoMartinsDeOliveiraFilho.jpeg',
  Authors: [authors.DeoclecianoMartinsDeOliveiraFilho],
  Titulo: `Rui Barbosa`,
  Material: 'Bronze',
  DataInauguracao: `01/1966`,
  DatasImportantes: [new Data(`01/1966`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Dom Manoel',
  Latitude: `-22.9069145`,
  Longitude: `-43.1715492`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é uma propriedade do Tribunal de Justiça do Rio de Janeiro e ocupa um espaço público, por cessão, para o reconhecimento a um dos principais jurista brasileiro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ElevandoSe_JoseResende: Obra = {
  ID: 106,
  Imagem: 'ElevandoSe_JoseResende.jpeg',
  Authors: [authors.JoseResende],
  Titulo: `Elevando-se`,
  Material: `Aço Corten`,
  DataInauguracao: `01/2005`,
  DatasImportantes: [new Data(`01/2005`, 'data de inauguração')],
  Propriedade: 'Privada',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Rosário',
  Latitude: `-22.9013886`,
  Longitude: `-43.175858`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Peça com cortes retos em dobras.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const VenusReclinada_JoseResende: Obra = {
  ID: 107,
  Imagem: 'VenusReclinada_JoseResende.jpeg',
  Authors: [authors.JoseResende],
  Titulo: 'Venus Reclinada',
  Material: `Peça em aço corten`,
  DataInauguracao: `01/1991`,
  DatasImportantes: [new Data(`01/1991`, 'data de inauguração')],
  Propriedade: 'Privada',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Rosário',
  Latitude: `-22.9013886`,
  Longitude: `-43.175858`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Peça em aço expressando um corpo. É uma peça privada em cessão de uso em área pública. A obra, segundo o artista ganhou o apelido de Negona. A obra nunca foi pichada e é preservada por todos os guardadores de carro que atuan na região. Ela não é pública, o proprietário é PAulo Fernandes`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FranciscoMignone_BrunoGiorgi: Obra = {
  ID: 108,
  Imagem: 'FranciscoMignone_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: `Francisco Mignone`,
  Material: 'Bronze',
  DataInauguracao: `1978`,
  DatasImportantes: [new Data(`1978`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Passeio, 98',
  Latitude: `-22.9135478`,
  Longitude: `-43.1803599`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Encontro_FranzWeissmann: Obra = {
  ID: 109,
  Imagem: 'Encontro_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Encontro`,
  Material: 'aço pintado',
  Altura: '450 cm',
  Largura: '400 cm',
  Profundidade: '200 cm',
  DataInauguracao: `01/1985`,
  DatasImportantes: [new Data(`01/1985`, 'data de inauguração')],
  Propriedade: 'Privada',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Carmo, 34',
  Latitude: `-22.9040276`,
  Longitude: `-43.1756872`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Peça em aço corten pintada, se referindo ao encontro de duas tiras, ou duas partes de um cubo. Escultura do Conjunto
    Cândido Mendes, e exposta em área publica desde 1985.`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'Homenageado', 'Castro Alves', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const DSebastiaoLeme_HeitorUsai: Obra = {
  ID: 110,
  Imagem: 'DSebastiaoLeme_HeitorUsai.jpeg',
  Authors: [authors.HeitorUsai],
  Titulo: `D. Sebastiao Leme`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua de Santana',
  Latitude: `-22.907748`,
  Longitude: `-43.194313`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MonumentoaJuventude_BrunoGiorgi: Obra = {
  ID: 111,
  Imagem: 'MonumentoaJuventude_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: `Monumento à Juventude`,
  Material: 'Granito',
  Altura: '4,45m',
  DataInauguracao: `01/1947`,
  DatasImportantes: [new Data(`01/1947`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Graça Aranha',
  Latitude: `-22.9097733`,
  Longitude: `-43.1740579`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de um casal de jovens caminhando. Foi executada especialmente para os jardins do antigo edifício do Ministério da Educação e Saúde, atual Palácio Capanema, obra prima da arquitetura modernista no Brasil. Esse monumento é um Patrimônio da União. Obra teria 7 metros de altura`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AssisChateaubriand_BrunoGiorgi: Obra = {
  ID: 112,
  Imagem: 'AssisChateaubriand_BrunoGiorgi.jpeg',
  Authors: [authors.HeitorUsai],
  Titulo: `Assis Chateaubriand`,
  Material: 'Bronze',
  DataInauguracao: `03/1933`,
  DatasImportantes: [new Data(`03/1933`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  Status: 'Removida do espaço público',
  Descricao: `A homenagem partiu do Jornal do Comércio, que solicitou Authors:ização a Prefeitura para instalar o monuemtno. Originariamente instalado na Rua da Candelária foi transferido para o depósito em2013.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const Terra_FranzWeissmann: Obra = {
  ID: 113,
  Imagem: 'Terra_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Terra`,
  Material: 'aço corten',
  DataInauguracao: `01/1984`,
  DatasImportantes: [new Data(`01/1984`, 'data de inauguração')],
  Propriedade: 'Privada',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Conjunto Universitário Cândido Mendes',
  Latitude: `-22.90412`,
  Longitude: `-43.1747159`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Escultura do Conjunto Cândido Mendes`,
  Fatos: [
    new Fact(
      '1958',
      '1958',
      'Produção',
      'Ano de produção da obra',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ModestinoKanto_HonorioPecanha: Obra = {
  ID: 114,
  Imagem: 'ModestinoKanto_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Modestino Kanto`,
  Material: 'Bronze',
  DataInauguracao: `09/1969`,
  DatasImportantes: [new Data(`09/1969`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Liceu de Artes e Ofícios',
  Latitude: `-22.9071588`,
  Longitude: `-43.193359`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact(
      '',
      '',
      'Homenageado',
      'Modestino Kanto foi aluno, professor e diretor do Liceu',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenagem',
      'A obra é uma homenagem do antigo Estado da Guanabara e dos colegas de magistério do Liceu, por iniciativa de Artur da Mota Pereira.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Comissão Patrocinadora',
      'Artur da Mota Pereira, Luis Cardosos Palmeira, Alvaro Paes de Barros Filho, Américo da Mota Pereira e Silvio Viana Freire',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const JoaoCandido_ValterBrito: Obra = {
  ID: 115,
  Imagem: 'JoaoCandido_ValterBrito.jpeg',
  Authors: [authors.ValterBrito],
  Titulo: `João Candido- Almirante Negro`,
  Material: 'Bronze',
  DataInauguracao: '2006',
  DatasImportantes: [
    new Data(`2006`, 'data de inauguração - arte ambiente'),
    new Data(`12/11/2007`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça XV de novembro',
  Latitude: `-22.9032446`,
  Longitude: `-43.1745771`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `e somente em 2008 foi instalado definitivamente na Praça XV, local que marca a Revolta das Chibatas.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'João Candido, o Almirante Negro', [], ''),
    new Fact(
      '',
      '',
      'Doação',
      'A obra foi doada à cidade pela Secretaria Especial de Igualdade Racial em homenagem ao Almirante Negro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2008',
      '2008',
      'transferência',
      'A obra foi instalada definitivamente na Praça XV',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const DomJoaoVI_BarataFeijo: Obra = {
  ID: 116,
  Imagem: 'DomJoaoVI_BarataFeijo.jpeg',
  Authors: [authors.BarataFeijo],
  Titulo: `Dom João VI`,
  Material: 'Bronze',
  Altura: '3 m',
  DataInauguracao: `06/1965`,
  DatasImportantes: [new Data(`06/1965`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça XV',
  Latitude: `-22.9023544`,
  Longitude: `-43.1727947`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua equestre tendo a figura de D. João VI montado, tendo a mão direita o globo terrestre, símbolo do poder. A estátua esta sobre um pedestal de concreto.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'João VI', [], ''),
    new Fact(
      '',
      '',
      'Homenagem',
      'A obra foi oferecida no 4 Centenário da Fundação da cidade do Rio de Janeiro, como homenagem ao Rei de Portugal',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Doação', 'Governo Português', [], 'Rio de Janeiro'),
    new Fact('', '', 'Execução', 'Barata Feio', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const GeneralOsorio_RodolfoBernardelli: Obra = {
  ID: 117,
  Imagem: 'GeneralOsorio_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `General Osório`,
  Material: 'Bronze',
  DataInauguracao: `11/1894`,
  DatasImportantes: [new Data(`11/1894`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça XV de Novembro',
  Latitude: `-22.9030346`,
  Longitude: `-43.1744503`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Foi construido como o mousoleu do General. É composto de um grande pedestal revestido de granito de Baveno, onde estão fixados dois baixos-relevos e a estátua de Osorio montado a cavalo,
    com espada em punho. Foi Erguido como um tributo ao seu excelente desempenho como um militar na "guerra da Tríplice Aliança" que se desenvolveu entre 1864 e 1870.`,
  Fatos: [
    new Fact('', '', 'Homenageado', 'General Osório', [], ''),
    new Fact('', '', 'Subscrição popular', '', [], 'Rio de Janeiro'),
    new Fact('2014', '2014', 'Vandalismo', '', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Depósito',
      'O florão de bronze e o gesso dos painéis laterais estão guardados no depósito da Prefeitura',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PequenoJornaleiro_AnisioOscarMotta: Obra = {
  ID: 118,
  Imagem: 'PequenoJornaleiro_AnisioOscarMotta.jpeg',
  Authors: [authors.AnisioOscarMotta],
  Titulo: `Pequeno Jornaleiro`,
  Material: 'Bronze',
  DataInauguracao: `06/1933`,
  DatasImportantes: [new Data(`06/1933`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Removida do espaço público',
  Descricao: `Obra do caricaturista Fritz, pseudônimo de Anísio Oscar Mota homenageando os garotos
    que vendiam jornais pelas ruas do Rio. Iniciativa do jornal A Noite, foi inaugurada na 
    confluência das ruas Ouvidor e Miguel Couto junto à Av.Rio Branco. Em 1996escultura foi
    transferida durante o Projeto Rio Cidade, para a Rua Sete de Setembro.
    Consta que a estátua representa o menino José Bento de Carvalho, que aos dez anos, percorria
    as ruas vendendo jornais. Cantava o dia inteiro, e era famoso pela facilidade com que subia nos 
    bondes e atravessava as ruas da cidade, a apregoar, aos gritos, as manchetes do dia.
    Em 11 de janeiro de 2016 a escultura do Pequeno Jornaleiro foi retirada da Rua Sete de Setembro
    e transferida para o Parque Noronha Santos, por causa das obras do VLT. O historiador Luciano
    Magno, lidera uma campanha para que a escultura do artista Fritz (1895-1969) volte ao seu lugar
    de origem, na calçada da Av. Rio Branco esquina com a Rua do Ouvidor.Ontem, Eduardo Paes, que
    tem consciência da importância do monumento, foi lacônico: Depois da obra voltamos com ela.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const JoaoCalvino_JoasPereiraDosPassos: Obra = {
  ID: 119,
  Imagem: 'JoaoCalvino_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `João Calvino`,
  Material: 'Bronze',
  DataInauguracao: `07/2009`,
  DatasImportantes: [
    new Data(`07/2009`, 'data de inauguração'),
    new Data(`10/2016`, 'Furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Silva Jardim',
  Latitude: `-22.9075`,
  Longitude: `-43.1813563`,
  Base: 'Sem pedestal',
  Status: 'Furtada',
  Descricao: `Obra em homenagem ao fundador do Calvinismo. O monumento foi uma solicitação da Igreja Presbiteriano a Prefeitura
    do Rio de Janeiro, pelas comemorações da reforma religiosa.No dia 10 de julho de 2009, foi inaugurada a Praça João Calvino,
    substituindo o nome anterior Espaço Guinard. A alteração foi decorrente da parceria público-privada entre a prefeitura da
    cidade e a Igreja Presbiteriana do Rio de Janeiro, que idealizou o novo espaço público.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Fundição e Atelier Messias',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ReverendoMathathiasGomes_JoasPereiraDosPassos: Obra = {
  ID: 120,
  Imagem: 'ReverendoMathathiasGomes_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Reverando Mathathias Gomes`,
  Material: 'Bronze',
  DataInauguracao: `07/2002`,
  DatasImportantes: [new Data(`07/2002`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Silva Jardim, 87',
  Latitude: `-22.9075127`,
  Longitude: `-43.1816077`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Esse monumento foi uma solicitação do Reverendo Guilhermino da Igreja Presbiteriana a Prefeitura da cidade.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Artística RioArte',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SemTitulo_IvensMachado: Obra = {
  ID: 121,
  Imagem: 'SemTitulo_IvensMachado.jpeg',
  Authors: [authors.IvensMachado],
  Titulo: `Sem Titulo`,
  Material: `vergalhão de aço, forrada com fibra de vidro e revestida em argamassa`,
  Altura: '500 cm/ 425 cm',
  Largura: '300 cm/ 250 cm',
  Profundidade: '820 cm/ 860 cm',
  ValorObra: 'R$ 60.000,00',
  DataInauguracao: `01/1997`,
  DatasImportantes: [new Data(`01/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `Escultura em concreto armado em forma retrocida . O conjunto e pintado na cor concreto com um circulo no seu
    centro pintado de vermelho. Foi transferida em 2013 para o parque devido as obras urbanas que atingiram a Rua Uruguaiana.`,
  Fatos: [
    new Fact(
      '1994',
      '1997',
      'Política Pública',
      'Esculturas Urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Promotor',
      'Secretaria Municipal de Cultura',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '01/1997',
      '01/1997',
      'Inauguração',
      'A obra foi inaugurada na Rua Uruguaiana',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const PrincesaIsabel_Desconhecida: Obra = {
  ID: 122,
  Imagem: 'PrincesaIsabel_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Princesa Isabel`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Treze de Maio',
  Latitude: `-22.908892`,
  Longitude: `-43.177235`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const SemTitulo_AmilcarDeCastro: Obra = {
  ID: 123,
  Imagem: 'SemTitulo_AmilcardeCastro.jpeg',
  Authors: [authors.AmilcarDeCastro],
  Titulo: `Sem Título`,
  Material: 'aço corten',
  Altura: '200 cm',
  Largura: '300 cm',
  Peso: '2000 kg',
  ValorObra: 'R$60.000,00',
  DataInauguracao: `10/06/1996`,
  DatasImportantes: [new Data(`10/06/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Canteiro Central Avenida Delfim Moreira, Posto 12',
  Latitude: '-22.988471',
  Longitude: '-43.2277304',
  SuporteEspacial: 'Canteiro Central',
  Base: 'Com pedestal',
  Status: 'Transferida',
  Descricao: `Também conhecida como Estrela. Escultura que utiliza a técnica corte e dobra, criando formas no espaço ao ser vista de vários ângulos.
    A escultura apresentada pela primeira vez na bienal de São Paulo. Trata-se do resultado de um projeto pré-concebido. CAda corte e dobra
    no aço é planejada com sensibilidade cativante. Apesar de ser realizada em materiais e técnicas industriais, seu trabalho é puro gesto.
    O trabalho também dialoga com o tempo ao incorpoarar a oxidação do material à condiçao natural do trabalho.`,
  Fatos: [
    new Fact(
      '1953',
      '1953',
      'Projeto',
      'A peça foi um projeto para a 2 Bienal de São Paulo. É a peça precurssora do seu trabalho com dobras.',
      [],
      'São Paulo',
    ),
    new Fact(
      '',
      '',
      'Características obra',
      'Obra vale-se do uso de corte, dobra, luz e sombra. A forma surge a partir do corte e da dobra do metal',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Política Pública',
      'Esculturas Urbanas',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1997',
      'Promotor',
      'Secretaria Municipal de Cultura',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/06/1996',
      '10/06/1996',
      'Inauguração',
      'Segunda peça do E.U. a ser inaugurada. Amilcar de Casto esteve presente na inauguração ao lado da Secretária Municipal de Cultura',
      [persons.HelenaMariaPortoSeveroDaCosta],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Pixinguinha_OttoDumovich: Obra = {
  ID: 124,
  Imagem: 'Pixinguinha_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Pixinguinha`,
  Material: 'Bronze',
  Altura: '170 cm',
  DataInauguracao: `23/04/1996`,
  DatasImportantes: [new Data(`23/04/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Travessa do Ouvidor',
  Latitude: `-22.9038942`,
  Longitude: `-43.1777031`,
  SuporteEspacial: 'Calçada',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A peça foi baseada na foto de um ensaio realizado por Pixinguinha em Nova York em 1963. Anos depois Alfredo Viana
    Neto, filho de Pixinguinha, ficou impressionado com a réplica de seu pai e na oportunidade de homenagear o compositor sugeriu
    a obra de Dumovich. A estatua de tamanho natural representa Pixinguinha tocando saxofone, de gravata borboleta, chapéu de palha
    e sapato bicolor. No pedestal está fixado uma placa em bronze composto com os Oito Batutas, grupo que acompanhava Pixinguinha`,
  Fatos: [
    new Fact(
      '23/04/1996',
      '23/04/1996',
      'Inauguração',
      'Inauguração da estátua foi feita em comemoração ao 99 aniversário do artista e marca o início das comemorações de seu centenário. Com a Travessa do Ouvidor lotada, Braguinha- parceiro de Pixinguinha- e Alfredo da Rocha Vianna Neto, filho do compositor, retiraram a capa que cobria a estátua de bronze.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Braguinha_OttoDumovich: Obra = {
  ID: 124,
  Imagem: 'Braguinha_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Braguinha`,
  Material: 'Bronze',
  DataInauguracao: `02/2004`,
  DatasImportantes: [new Data(`02/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça Demétrio Ribeiro',
  Latitude: `-22.9620366`,
  Longitude: `-43.1780575`,
  SuporteEspacial: 'Calçada',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Foi doado à cidade pela empresa Unisys Brasil, para homenagear o ídolo do carnaval carioca e morador do bairro de Copacabana.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SergioCardoso_Marlota: Obra = {
  ID: 125,
  Imagem: 'SergioCardoso_Marlota.jpeg',
  Authors: [authors.Marlota],
  Titulo: `Sérgio Cardoso`,
  Material: 'Bronze',
  DataInauguracao: `01/1973`,
  DatasImportantes: [new Data(`01/1973`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Santa Teresa',
  Endereco: 'Rua Dias de Barros',
  Latitude: `-22.9188802`,
  Longitude: `-43.183083`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Furtada',
  Descricao: `Pequena homenagem doado à cidade do Rio de Janeiro. Na placa que identifica o monumento tem a seguinte frase: Tudo começou aqui em Santa Tereza".
    A efigie em bronze do artista foi furtada em 30 de maio de 2017`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const CarlosGomes_UgoTaddey: Obra = {
  ID: 126,
  Imagem: 'CarlosGomes_UgoTaddey.jpeg',
  Authors: [authors.UgoTaddey],
  Titulo: `Carlos Gomes`,
  Material: 'Bronze',
  DataInauguracao: `05/1938`,
  DatasImportantes: [new Data(`05/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Parque dos Tamoios',
  Latitude: `-22.7622363`,
  Longitude: `-43.1071716`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `É uma homenagem idealizada por Pedro Bruno ao grande maestro brasileiro. O monumento foi uma iniciativa do Prof. Manoel Antonio Lima de magalhães inaugurado na Festa dos Passaros e Arvores.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AndreThevet_CyniraNDantas: Obra = {
  ID: 127,
  Imagem: 'AndreThevet_CyniraNDantas.jpeg',
  Authors: [authors.CyniraNDantas],
  Titulo: `Andre Thevet`,
  Material: 'Bronze',
  DataInauguracao: `12/1994`,
  DatasImportantes: [new Data(`12/1994`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praia do Imbuca',
  Latitude: `-22.7622363`,
  Longitude: `-43.1071716`,
  SuporteEspacial: 'Praça',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é a referência ao descobridor da Ilha de Paquetá.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BebedouroDosPassarosI_PedroBruno: Obra = {
  ID: 128,
  Imagem: 'BebedouroDosPassarosI_PedroBruno.jpeg',
  Authors: [authors.PedroBruno],
  Titulo: `Bebedouro dos Passaros I`,
  Material: 'ferro fundido e concreto',
  DataInauguracao: '1940',
  DatasImportantes: [new Data(`1940`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praça Bom Jesus',
  Latitude: `-22.76128`,
  Longitude: `-43.1099875`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `grupo escultórico formado por um par de  peixes estilizados em concreto pintado de verde, com uma ânfora ao centro.
    Desativado em data desconhecida.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const BebedouroDosPassarosII_PedroBruno: Obra = {
  ID: 129,
  Imagem: 'BebedouroDosPassarosII_PedroBruno.jpeg',
  Authors: [authors.PedroBruno],
  Titulo: `Bebedouro dos Pássaros II`,
  Material: 'ferro fundido e concreto',
  DataInauguracao: '1940',
  DatasImportantes: [new Data(`1940`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praça São Roque',
  Latitude: `-22.7532936`,
  Longitude: `-43.1151116`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `grupo escultórico formado por um par de peixes estilizados em concreto pintado de verde, com uma ânfora ao centro.
    Desativado em data desconhecida.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const BebedouroDosPassarosIII_PedroBruno: Obra = {
  ID: 130,
  Imagem: 'BebedouroDosPassarosIII_PedroBruno.jpeg',
  Authors: [authors.PedroBruno],
  Titulo: `Bebedouro dos Pássaros III`,
  Material: 'ferro fundido e concreto',
  DataInauguracao: '1940',
  DatasImportantes: [new Data(`1940`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praiaa José Bonifácio',
  Latitude: `-22.7623976`,
  Longitude: `-43.1124363`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `grupo escultórico formado por um par de peixes estilizados em concreto pintado de verde, com uma ânfora ao centro.
    Desativado em data desconhecida.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const PedroBruno_PauloMazzucchelli: Obra = {
  ID: 131,
  Imagem: 'PedroBruno_PauloMazzucchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: `Pedro Bruno`,
  Material: 'Bronze',
  DataInauguracao: `11/1948`,
  DatasImportantes: [new Data(`11/1948`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praça Pedro Bruno',
  Latitude: `-22.7622363`,
  Longitude: `-43.1071716`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Esse monumento foi inaugurado pelo seu próprio Pintor Pedro Bruno, responsável pelo seu projeto e estruturação artística da Ilha de Paquetá, como homenagem dos moradores.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GetulioVargas_Desconhecida: Obra = {
  ID: 132,
  Imagem: 'GetulioVargas_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Getulio Vargas`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Praça São Roque',
  Latitude: `-22.7532936`,
  Longitude: `-43.1151116`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Golfinho_AugustoSilva: Obra = {
  ID: 133,
  Imagem: 'Golfinho_AugustoSilva.jpeg',
  Authors: [authors.AugustoSilva],
  Titulo: `Golfinho`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Paquetá',
  Endereco: 'Rua Grossa',
  Latitude: `-22.7622363`,
  Longitude: `-43.1071716`,
  Base: 'Com pedestal',
  MaterialBase: 'pedra',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de golfinho, apoiado sobre blocos de pedra voltado para o mar.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Modulo65_AscanioMMM: Obra = {
  ID: 134,
  Imagem: 'Modulo65_AscanioMMM.jpeg',
  Authors: [authors.AscanioMMM],
  Titulo: `Modulo 6.5`,
  Material: 'alumínio pintado',
  Altura: '300 cm',
  Largura: '900 cm',
  Profundidade: '450 cm',
  DataInauguracao: '1982',
  DatasImportantes: [
    new Data(`1982`, 'data de inauguração - arte ambiente'),
    new Data(`05/1997`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Cidade Nova',
  Endereco: 'Rua Afonso Cavalcanti',
  Latitude: `-22.910466`,
  Longitude: `-43.205079`,
  SuporteEspacial: 'praça',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça em alumínio dobrada, e pintada com tinta automotiva na cor branca. Inicialmente encomendada para marcar o
    encerramento do encontro da Câmara Interamericana de Arbitragem Comercial e da Associação Ibero-Americana de Câmaras de Comércio
    do Rio de Janeiro. Por haver no local originalmente escolhido uma estátua do Papa João Paulo lI,`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Obra doada à cidade pela Associação Comercial do Rio de Janeiro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const UlyssesGuimaraes_SironFranco: Obra = {
  ID: 135,
  Imagem: 'UlyssesGuimaraes_SironFranco.jpeg',
  Authors: [authors.SironFranco],
  Titulo: `Homenagem a Ulysses Guimarães`,
  Material: 'destroços de eicóptero e resina',
  DataInauguracao: '2003',
  DatasImportantes: [new Data(`2003`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Cidade Nova',
  Endereco: 'Rua Afonso Cavalcanti, 455',
  Latitude: `-22.9113568`,
  Longitude: `-43.2079669`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Para sua confecção, Siron Franco utilizou destroços do helicóptero que caiu no mar de Angra dos Reis, matando Ulysses e sua mulher, dona Mora, o ex-senador Severo Gomes e sua mulher, Henriquetta, e o piloto Jorge Comeratto. Também integram a escultura uma garrafa de poire, bebida à base de pera preferida pelo ex-deputado, um exemplar da Bíblia e fotos e poemas de Ulysses. A inauguração será feita pelo prefeito César Maia e contará com a presença de Tito Henrique, enteado de Ulysses e filho de dona Mora.',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ChafarizdasTresGracas_RemoBernucci: Obra = {
  ID: 136,
  Imagem: 'ChafarizdasTresGracas_RemoBernucci.jpeg',
  Authors: [authors.RemoBernucci],
  Titulo: `Chafariz da Tres Graças`,
  Material: 'bronze e concreto',
  DataInauguracao: `09/1964`,
  DatasImportantes: [
    new Data(`09/1964`, 'data de inauguração'),
    new Data(`2002`, 'transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Cidade Nova',
  Endereco: 'Rua Afonso Cavalcanti',
  Latitude: `-22.911202`,
  Longitude: `-43.2101102`,
  Status: 'Transferida',
  Descricao: `O Chafariz Danças das Águas conhecido tambem como "Chafariz das Ninfas" foi inspirado
    na Grécia antiga e apresenta três figuras femininas como ninfas: Oceania (representa as águas
   do oceano), a Nereida(simboliza as águas dos mares) e Náiades (remete às águas das fontes).
   Segundo a mitologia grega, essas figuras, que representam a juventude eterna, protegiam quem se
   banhasse em suas águas, dotando de fertilidade quem bebia de suas fontes. As esculturas em bronze
   tem 3 metros de altura. Foi inaugurado na Glória em 1964 e transferido para a frente do
   CASS (Centro Administrativo São sebastião) em 2002, devido aos constantes danos para o furto do
   bronze das esculturas. Esta desativado desde maio de 2017.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Chafariz_AugusteMartin: Obra = {
  ID: 137,
  Imagem: 'Chafariz_AugusteMartin.jpeg',
  Authors: [authors.AugusteMartin],
  Titulo: `Chafariz`,
  Material: 'ferro fundido',
  DataInauguracao: `01/1911`,
  DatasImportantes: [new Data(`01/1911`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Rio Comprido',
  Endereco: 'Praça Paulo de Frontin',
  Latitude: `-22.9248202`,
  Longitude: `-43.2092378`,
  Status: 'Permanece em espaço público',
  Descricao: `Inicialmente foi instalado na Praça Barão de Drummond em 1911 e retirado em 1963.
    Foi instalado na atual localização em 1964. Sobre uma bacia circular em gnaisse, duas sereias
    e uma de Netuno sustentam uma bacia menor, da qual se eleva a figura de um menino sentado um
    golfinho estilizado. Existe um similar no deposito do Parque Noronha Santos que foi retirado
    do Campo de São Cristovão. O Chafariz esta desativado desde maio de 2017.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Indio_JulesJeanJulesSalmson: Obra = {
  ID: 138,
  Imagem: 'Indio_JulesJeanJulesSalmson.jpeg',
  Authors: [authors.JulesJeanJulesSalmson],
  Titulo: `Índio`,
  Material: 'ferro fundido',
  DataInauguracao: `1967`,
  DatasImportantes: [new Data(`1967`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Estrada do Açude',
  Latitude: `-22.9626135`,
  Longitude: `-43.2890959`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento está incluído nessa relação devido o convênio de Gestão Compartilhada
    do ParNacional da Tijuca entre a Prefeitura do Rio de Janeiro e oIBAMA desde 22 de março de
    2001. Escultura representando um índio. Situava-se em outro local do parque, sendo transferida
    para o sítio atual quando da construção do portão.`,
  Fatos: [
    new Fact('', '', 'Fundição/Atelier', "Val d'Osne", [], 'Rio de Janeiro'),
    new Fact(
      '05/10/2000',
      '',
      'Tombamento',
      'Tombamento Federal pelo processo 0762-T-65, inscrito no livro Arqueológico, Etnográfico e Paisagístico inscrição 042 e pelo Município pelo Decreto 19.011/2000 de 5 de outubro de 2000.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Leao_HippolyteFrancoisAugusteHippolytePeyrol: Obra = {
  ID: 139,
  Imagem: 'Leao_HippolyteFrancoisAugusteHippolytePeyrol.jpeg',
  Authors: [authors.HippolyteFrancoisAugusteHippolytePeyrol],
  Titulo: `Leão`,
  Material: 'marmore de carrara',
  DataInauguracao: `1906`,
  DatasImportantes: [
    new Data(`01/1929`, 'data de inauguração -inventario dos monumentos'),
    new Data(`1906`, 'data de inauguração -arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Avenida Edson Passos',
  Latitude: `-22.9163366`,
  Longitude: `-43.1758382`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando um felino em posição de ataque, como um guardião do Chafariz.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const India_JulesJeanJulesSalmson: Obra = {
  ID: 140,
  Imagem: 'India_JulesJeanJulesSalmson.jpeg',
  Authors: [authors.JulesJeanJulesSalmson],
  Titulo: `Índia`,
  Material: 'ferro fundido',
  DataInauguracao: `1967`,
  DatasImportantes: [new Data(`1967`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Estrada do Açude',
  Latitude: `-22.9626135`,
  Longitude: `-43.2890959`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'Fundição/Atelier', "Val d'Osne", [], 'Rio de Janeiro'),
    new Fact('', '', '', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const CristoRedentor_PaulLandowski: Obra = {
  ID: 141,
  Imagem: 'CristoRedentor_PaulLandowski.jpeg',
  Authors: [authors.PaulLandowski],
  Titulo: `Cristo Redentor`,
  Material: 'concreto e pedra sabão',
  DataInauguracao: `12/10/1931`,
  DatasImportantes: [new Data(`12/10/1931`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Parque Nacional da Tijuca',
  Latitude: `-22.9519878`,
  Longitude: `-43.2104148`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento está iincluído nessa relação devido o convênio de Gestão Compartilhada
    do Parque Nacional da Tijuca entre a Prefeitura do Rio de Janeiro e oIBAMA desde 22 de março de
    2001. O Cristo Redentor foi a realização de uma idéia, oriunda de meados do século XIX, de se
    construir uma estátua monumental do Cristo no pináculo do Corcovado. Por iniciativa do Círculo
    Católico do Rio de Janeiro, em 1920, teve início o movimento de coleta de assinaturas e doações
    que possibilitaram a obra. Entre vários projetos, foi escolhido o da estátua com a figura do
    Cristo de braços abertos e preconizada por Heitor da Silva Costa. A escultura foi então
    encomendada ao grande escultor francês Paul Landowski, que recebeu a ajuda, na elaboração do
    rosto, do seu colaborador romeno Gheorghe Leonida, sendo os cálculos realizados por Albert
    Caquot, o maior engenheiro francês de sua geração. Um molde da estátua em escala reduzida e os
    moldes em tamanho natural da cabeça e das mãos foram então enviados para o Rio de Janeiro. Em
    estilo Art Déco, a estátua com estrutura de perfis metálicos e argamassa armada revestida por
    um mosaico de pedra sabão, tem 30 metros de altura, 28 metros de largura, alem dos oito metros
    do pedestal, pesando toneladas. Durante a cerimônia de inauguração, os holofotes que iluminavam
    a estátua foram acionados, de Roma, por Guglielmo Marconi. Em 2007, em votação internacional, o
    Cristo Redentor foi eleito uma das novas Sete Maravilhas do Mundo.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Escravo_MarliMazeredo: Obra = {
  ID: 142,
  Imagem: 'Escravo_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Escravo`,
  Material: 'Bronze',
  DataInauguracao: `07/2001`,
  DatasImportantes: [new Data(`07/2001`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Parque Nacional da Tijuca- Centro de visitação',
  Latitude: `-22.9560201`,
  Longitude: `-43.2801111`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento está incluído nessa relação devido o convênio de Gestão Compartilhada do
    ParNacional da Tijuca entre a Prefeitura do Rio de Janeiro e o IBAMA desde 22 de março de 2001.
    A escultura, homenageia os escravos que trabalharam na Floresta da Tijuca.`,
  Fatos: [new Fact('', '', "Fundação Val D'Osne", '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FonteWallaceI_CharlesAugusteLebourg: Obra = {
  ID: 143,
  Imagem: 'FonteWallaceI_CharlesAugusteLebourg.jpeg',
  Authors: [authors.CharlesAugusteLebourg],
  Titulo: `Fonte Wallace`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Parque Nacional da Tijuca- Jardim dos Manacás',
  Latitude: `-22.9553992`,
  Longitude: `-43.2852251`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento está incluído nessa relação devido o convênio de Gestão Compartilhada do ParNacional da Tijuca entre a Prefeitura do Rio de Janeiro e oIBAMA desde 22 de março de 2001.
    Fonte introduzida no parque, provavelmente, pelo paisagista Auguste AugusteFrancoisMarieGlaziou, durante a gestão do Barão d&rsquo;Escragnolle. Suas cariátides, sustentando a cúpula, representam a Bondade, a Caridade, a Sobriedade e a Simplicidade.`,
  Fatos: [
    new Fact('', '', 'bem tombado municipal', '', [], ''),
    new Fact('', '', "Fundação Val D'Osne", '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const AlmiranteCochrane_Lana: Obra = {
  ID: 144,
  Imagem: 'AlmiranteCochrane_Lana.jpeg',
  Authors: [authors.Lana],
  Titulo: `Almirante Cochrane`,
  Material: 'Bronze',
  DataInauguracao: `01/1930`,
  DatasImportantes: [new Data(`01/1930`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Largo Almirante Cochrane',
  Latitude: `-22.9223314`,
  Longitude: `-43.2311171`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem em um bloco monolitico de granito de grande dimensão com a efigie do Almirante.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const ACleydePradoMaia_EdgarDuvivier: Obra = {
  ID: 145,
  Imagem: 'ACleydePradoMaia_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `A Cleyde Prado Maia`,
  Material: 'Bronze',
  DataInauguracao: `12/2008`,
  DatasImportantes: [new Data(`12/2008`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Rua são Francisco Xavier',
  Latitude: `-22.9218073`,
  Longitude: `-43.2214041`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Uma pomba inspirada na "Pomba da Paz" de Picasso, é a homenagem Cleyde Prado Maia.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'FEB- Fundição e Esculturas em Bronze',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PaulaFreitas_BenevenutoBerna: Obra = {
  ID: 146,
  Imagem: 'PaulaFreitas_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Paula Freitas`,
  Material: 'Bronze',
  DataInauguracao: `10/1935`,
  DatasImportantes: [new Data(`10/1935`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Afonso Pena',
  Latitude: `-22.907748`,
  Longitude: `-43.194313`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem está fixada num bloco de granito em formato de livro, com a figura circundado por uma coroa de louros.
    Em 1935, por iniciativa de um grupo de ex-discípulos e Authors:idades públicas da cidade, foi erguida nos jardins do colégio o monumento a sua memória. Em 3 de outubro de 1946, foi solenemente reinaugurado na praça.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const AfraniodeMeloFranco_BrunoGiorgi: Obra = {
  ID: 146,
  Imagem: 'AfraniodeMeloFranco_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: `Afrânio de Mello Franco`,
  Material: 'Bronze',
  DatasImportantes: [new Data(`10/2010`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Shopping Leblon',
  Latitude: `-22.9827089`,
  Longitude: `-43.21996`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const FranciscoPaulaMayrink_PinedoParis: Obra = {
  ID: 147,
  Imagem: 'FranciscoPaulaMayrink_PinedoParis.jpeg',
  Authors: [authors.PinedoParis],
  Titulo: `Francisco Paula Mayrink`,
  Material: 'Bronze',
  DataInauguracao: `01/1959`,
  DatasImportantes: [new Data(`01/1959`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Afonso Pena',
  Latitude: `-22.9173049`,
  Longitude: `-43.2178181`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é uma reverência cívica promovida pelo Instituto Histórico e Geográfico da Cidade ao seu conselheiro. Esta nesta praça porque ele morava ali proximo.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Fidelidade_BenevenutoBerna: Obra = {
  ID: 148,
  Imagem: 'Fidelidade_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Fidelidade`,
  Material: 'Bronze',
  DataInauguracao: `01/1957`,
  DatasImportantes: [new Data(`01/1957`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Afonso Pena',
  Latitude: `-22.9184591`,
  Longitude: `-43.2178791`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura, representando dois cães em tamanho natural, erigida por iniciativa do vereador Frederico Trotta.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const XavierdeBrito_ElzaCansancaoMedeiros: Obra = {
  ID: 149,
  Imagem: 'XavierdeBrito_ElzaCansancaoMedeiros.jpeg',
  Authors: [authors.ElzaCasancaoDeMedeiros],
  Titulo: `Coronel Xavier de Brito`,
  Material: 'Bronze',
  DataInauguracao: `11/1972`,
  DatasImportantes: [new Data(`11/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Comandante Xavier de Brito',
  Latitude: `-22.9323644`,
  Longitude: `-43.2431001`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem com o busto, foi uma iniciativa dos Cadetes daEscola Militar de Realengo, em comemoração dos 50 anos da Revolta de 05 de julho de 1922, Revolta dos Tenentes, ao comandante da Escola Militar de Realengo, que liberou insurreição dos cadetes e vários oficiais em solidariedade às tropas amotinadas.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const LamartineBabo_Desconhecida: Obra = {
  ID: 150,
  Imagem: 'LamartineBabo_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Lamartine Babo`,
  Material: 'Bronze',
  DataInauguracao: `09/1966`,
  DatasImportantes: [new Data(`09/1966`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Lamartine Babo',
  Latitude: `-22.92408`,
  Longitude: `-43.2363155`,
  Base: 'Com pedestal',
  MaterialBase: 'mármore',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem a Lamartine Babo foi erigido por iniciativa de seus amigos do América Futebol Clube.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const LuisGastaoDEscragnolleDoria_CurzioZani: Obra = {
  ID: 151,
  Imagem: 'LuisGastaoDEscragnolleDoria_CurzioZani.jpeg',
  Authors: [authors.CurzioZani],
  Titulo: `Luis Gastão D'Escragnolle Doria`,
  Material: 'Bronze',
  DataInauguracao: `08/1928`,
  DatasImportantes: [
    new Data(`08/1928`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1955`, 'data de inauguração - arte ambiente'),
  ],
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Professor Pinheiro Guimarães',
  Latitude: `-22.9551364`,
  Longitude: `-43.2865952`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const OswaldoDinizMagalhaes_MateusFernandes: Obra = {
  ID: 152,
  Imagem: 'OswaldoDinizMagalhaes_MateusFernandes.jpeg',
  Authors: [authors.MateusFernandes],
  Titulo: `Oswaldo Diniz Magalhães`,
  Material: 'Bronze',
  DataInauguracao: `05/1957`,
  DatasImportantes: [new Data(`05/1957`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Saens Pena',
  Latitude: `-22.9237269`,
  Longitude: `-43.2319407`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua em bronze de um atleta com um bastão na mão, simbolizando o rádio-ginasta, com o seu principal instrumento de ginastica. No pedestal aparece a efígie do rosto do professor. O monumento foi inaugurado na presença do homenageado que o inaugurou.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BarletJames_RuffoFanucchi: Obra = {
  ID: 153,
  Imagem: 'BarletJames_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: `Bartlet James`,
  Material: 'Bronze',
  DataInauguracao: `10/1952`,
  DatasImportantes: [
    new Data(`10/1952`, 'data de inauguração'),
    new Data(`1976`, 'removido para Depósito'),
    new Data(`1996`, 'transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua de Santana',
  Latitude: `-22.9248074`,
  Longitude: `-43.2333804`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi retirado para as obras do Metrô em 1976 e guardado no depósito da Fundação Parques e Jardins. Somente foi recolocado em 1996 em um novo pedestal.
    O monumento ao deputado foi uma iniciativa do Movimento Libertador da Terra Carioca. que patrocinou a obra.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ThomasDelfinoDosSantos_RuffoFanucchi: Obra = {
  ID: 154,
  Imagem: 'ThomasDelfinoDosSantos_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: `Thomas Delfino dos Santos`,
  Material: 'Bronze',
  DataInauguracao: `11/1954`,
  DatasImportantes: [new Data(`11/1954`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Saens Pena',
  Latitude: `-22.9239159`,
  Longitude: `-43.232214`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento teve como na comissão para a sua construção os srs. Heitor Beltrão e Frederico Trotta. A iniciativa
    foi da União Autonomista Carioca do Movimento Libertador da Terra Carioca.
A homenagem foi retirada da praça Saens Pena para as obras do Metrô em 1976, guardado no depósito da Fundação Parques e Jardins
e recolocado em 1996.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const DomPedroI_Desconhecida: Obra = {
  ID: 155,
  Imagem: 'DomPedroI_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Dom Pedro I`,
  Material: 'ferro fundido',
  DataInauguracao: `01/2004`,
  DatasImportantes: [new Data(`01/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Av. Pedro II',
  Latitude: `-22.9030866`,
  Longitude: `-43.2141042`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi instalada nesse local em função da implantação do quartel da Guarda Municipal da cidade, que tem
    D. Pedro I como seu patrono. O modelo dessa peça em gesso é uma obra de Marc Ferrez do Museu personalidades Nacional de 1826.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra - AGR',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SaoCristovao_Desconhecida: Obra = {
  ID: 156,
  Imagem: 'SaoCristovao_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `São Cristovão`,
  Material: 'Bronze',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Praça Seve',
  Latitude: `-22.8965944`,
  Longitude: `-43.2165602`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Tradicional imagem de São Cristóvão, carregando o Menino Jesus, em bronze, doada à cidade pelos jornaisO DiaeA notícia, e pelo então Deputado Chagas Freitas, em homenagem à classe dos motoristas. A peça foi furtada e refeita pela Fundição Zani e reinaugurada em 16 de junho de 2002.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JoseBonifacio_MarcosAndreSales: Obra = {
  ID: 157,
  Imagem: 'JoseBonifacio_MarcosAndreSales.jpeg',
  Authors: [authors.MarcosAndreSales],
  Titulo: `José Bonifácio `,
  Material: 'Bronze',
  DataInauguracao: `1938`,
  DatasImportantes: [new Data(`1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.90536`,
  Longitude: `-43.2243186`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao Patriarca da Independência, tutor do Imperador do Brasil, D. Pedro, que nasceu no Palácio da Quinta da Boa Vista. A obra foi paga pelo Ministério da Viação e executada na Italia. Veio para o Brasil, partindo de Genova, em setembro de 1910.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const NiloPecanha_NicolinaVazDeAssisPintodoCouto: Obra = {
  ID: 158,
  Imagem: 'NiloPecanha_NicolinaVazDeAssisPintodoCouto.jpeg',
  Authors: [authors.NicolinaVazDeAssisPintodoCouto],
  Titulo: `Nilo Peçanha`,
  Material: 'Bronze',
  DataInauguracao: `01/1911`,
  DatasImportantes: [new Data(`01/1911`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.9053655`,
  Longitude: `-43.2243448`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem ao Presidente da República, que reformulou o parque, mantendo o projeto do paisagista AugusteFrancoisMarieGlaziou que o realizou no periodo imperial. A obra foi paga pelo Ministério da Viação.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SerpentedoLago_NicolinaVazDeAssisPintodoCouto: Obra = {
  ID: 159,
  Imagem: 'SerpentedoLago_NicolinaVazDeAssisPintodoCouto.jpeg',
  Authors: [authors.NicolinaVazDeAssisPintodoCouto],
  Titulo: `Serpente`,
  Material: 'Bronze',
  DataInauguracao: `10/1910`,
  DatasImportantes: [new Data(`10/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.9060389`,
  Longitude: `-43.2225536`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em bronze de uma serpente sobre uma pedra, de onde jorrava água por sua boca, para movimentação da agua do lago criado.A obra foi paga pelo Ministério da Viação.
    Desativado em data desconhecida.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CantodasSereias_NicolinaVazDeAssisPintodoCouto: Obra = {
  ID: 160,
  Imagem: 'CantodasSereias_NicolinaVazDeAssisPintodoCouto.jpeg',
  Authors: [authors.NicolinaVazDeAssisPintodoCouto],
  Titulo: `Canto das Sereias`,
  Material: 'mármore de carrara',
  DataInauguracao: `09/1910`,
  DatasImportantes: [new Data(`09/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.9056738`,
  Longitude: `-43.2215889`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Grupo escultórico executada em um único bloco de pedra, que representa um grupo de sereias, com um homem atraído pelo seu canto das sereias.A obra foi paga pelo Ministério da Viação e executada na Italia. Veio para o Brasil, partindo de Genova, em setembro de 1910.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const LuizGonzaga_JoasPereiraDosPassos: Obra = {
  ID: 162,
  Imagem: 'LuizGonzaga_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Luiz Gonzaga`,
  Material: 'Bronze',
  DataInauguracao: `10/2003`,
  DatasImportantes: [new Data(`10/2003`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Feira de São Cristóvão',
  Latitude: `-22.8983825`,
  Longitude: `-43.2196584`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A estátua representa o Rei do Baião em seus típicos trajes nordestinos, sobre um mapa do Brasil.// A frente da clássica Feira de Tradições Nordestinas, no pavilhão do bairro de São Cristóvão, está a estátua de Luiz Gonzaga, o Rei do Baião. Com seu figurino de shows inspirado nos vaqueiros nordestinos e sua sanfona, ele se encontra sobre o mapa do Brasil.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Artística RioArte',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PedroI_Desconhecida: Obra = {
  ID: 163,
  Imagem: 'PedroI_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Dom Pedro I`,
  Material: 'Bronze',
  DataInauguracao: `06/1972`,
  DatasImportantes: [new Data(`06/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Benfica',
  Endereco: 'Rua São Luiz Gonzaga',
  Latitude: `-22.8965033`,
  Longitude: `-43.2394225`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Foi erigido em comemoração aos 150 anos da Independência do Brasil, marcando o local de onde D. Pedro I se encaminhava para a Fazenda Imperial de Santa Cruz, Foi uma iniciativa do Rotary Ckub e na inauguração esteve presente o Secretario de Obras, Emilio Ibrahim, representando o governador Chagas Freitas.
    O modelo dessa peça em gesso é uma obra de Marc Ferrez do Museu personalidades Nacional de 1826.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Cartola_OttoDumovich: Obra = {
  ID: 164,
  Imagem: 'Cartola_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Cartola`,
  Material: 'Bronze',
  DataInauguracao: `11/2006`,
  DatasImportantes: [new Data(`11/2006`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Mangueira',
  Endereco: 'Rua Visconde de Niteroi',
  Latitude: `-22.9016384`,
  Longitude: `-43.2410216`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi instalada em frente ao Centro Cultural Cartola, na Mangueira e doada à Cidade do Rio de Janeiro pela empresa Unysse// `,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const RenatoRusso_Ique: Obra = {
  ID: 164,
  Imagem: 'RenatoRusso_Ique.jpeg',
  Authors: [authors.Ique],
  Titulo: `Renato Russo`,
  Material: 'Bronze',
  DataInauguracao: `2012`,
  DatasImportantes: [new Data(`2012`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Rua República Árabe da Síria',
  Latitude: `-22.8041966`,
  Longitude: `-43.2043507`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Definidos por muitos como a voz da geração dos anos 1980, Renato Russo é o nome artístico de Renato Manfredini Júnior, cantor e compositor brasileiro, fundador da Legião Urbana, uma das maiores bandas de rock brasileiro. Ele morreu em 1996 em decorrência de complicações da AIDS. O monumento em bronze assinado pelo cartunista Ique reproduz o tamanho natural do cantor e foi inaugurado em 2012 na Ilha do Governador, onde Renato viveu com sua família durante a infância.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EscravaAnastacia_JaimeSampaio: Obra = {
  ID: 165,
  Imagem: 'EscravaAnastacia_JaimeSampaio.jpeg',
  Authors: [authors.JaimeSampaio],
  Titulo: `Escrava Anastácia`,
  Material: 'Bronze',
  DataInauguracao: `07/1981`,
  DatasImportantes: [new Data(`07/1981`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Benfica',
  Endereco: 'Praça Padre Souza',
  Latitude: `-22.8897097`,
  Longitude: `-43.235025`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido representando o sacrifício de mulher escrava. Uma comissão solicitou a Prefeitura o espaço
    que custeou e obra e doou a Cidade do Rio de Janeiro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FonteMedusa_Desconhecida: Obra = {
  ID: 166,
  Imagem: 'FonteMedusa_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Fonte Medusa`,
  Material: 'ferro fundido',
  DataInauguracao: `10/1910`,
  DatasImportantes: [new Data(`10/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Benfica',
  Endereco: 'Largo do Pedregulho',
  Latitude: `-22.8990219`,
  Longitude: `-43.2359633`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Fonte tipo estela com a figura de uma Medusa. Essa fonte foi instalada por D.Pedro II com o objetivo de abastecer a população que foi residir nas proximidades da Quinta da Boa vista, e até hoje exerce esta função.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const HelenoDaCostaBrandao_HonorioPecanha: Obra = {
  ID: 167,
  Imagem: 'HelenoDaCostaBrandao_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Heleno da Costa Brandão`,
  Material: 'Bronze',
  DataInauguracao: `08/1948`,
  DatasImportantes: [new Data(`08/1948`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Praça Tobias Barreto',
  Latitude: `-22.9129879`,
  Longitude: `-43.2421185`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento erguido pelos moradores do bairro em homenagem ao médico.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AosHahnemannianos_Desconhecida: Obra = {
  ID: 168,
  Imagem: 'AosHahnemannianos_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Aos Hahnemannianos- Rodrigues Galhardo, Joaquim Murtinho, Licínio Cardoso, Braga e Costa e Dias da Cruz`,
  Material: 'Bronze',
  DataInauguracao: `04/1972`,
  DatasImportantes: [new Data(`04/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Praça Tobias Barreto',
  Latitude: `-22.9130602`,
  Longitude: `-43.2425489`,
  Base: 'Com pedestal',
  MaterialBase: 'pedra são tome',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido pelo Dr. David Castro numa iniciativa do Instituto Hahnemanniano do Brasil, em homenagem aos pioneiros.
    Apoiada em uma base única ergue-se 5 pedestais. Em cada uma esta fixada a efigie do médico. Abaixo de cada placa tem-se a data do nascimento e falecimento de cada homenageado.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const NoelRosa_AlfredoHerculanoFreixo: Obra = {
  ID: 170,
  Imagem: 'NoelRosa_AlfredoHerculanoFreixo.jpeg',
  Authors: [authors.AlfredoHerculanoFreixo],
  Titulo: `Noel Rosa`,
  Material: 'granito',
  DataInauguracao: `08/1938`,
  DatasImportantes: [new Data(`08/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Praça Barão de Drummond',
  Latitude: `-22.9164223`,
  Longitude: `-43.250327`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Foi inaugurado na Praça Tobias Barreto e transferido em 1946 para a atual localização.
    Coluna de granito tendo na parte superior uma efígie de Noel Rosa, e na parte inferior um ramo de louros e um violão.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const NoelRosa_JoasPereiraDosPassos: Obra = {
  ID: 171,
  Imagem: 'NoelRosa_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Noel Rosa`,
  Material: 'Bronze',
  ValorObra: 'R$ 32000,00',
  DataInauguracao: `08/1996`,
  DatasImportantes: [new Data(`08/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Largo do Maracanã',
  Latitude: `-22.9133266`,
  Longitude: `-43.2353111`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Grupo escultórico em bronze, representando Noel Rosa sentado numa mesa de bar,
    sendo atendido pelo garçom. O grupo escultórico fica no centro, emoldurado por quatro portas
    estilizadas em granito e ferro. Executado pelo projeto Rio-Cidade no Largo do Maracanã,
    trata-se do primeiro monumento interativo da cidade.// Um dos maiores compositores de samba no Brasil, Noel Rosa foi fundamental para a legitimação do samba, estilo de música da periferia, entre a classe média. Conhecido como “Poeta da Vila”, o monumento em sua homenagem encontra-se no bairro de Vila Isabel. Instalado em 1996 e com autoria de Joás Pereira Passos, esse foi o primeiro monumento interativo do Rio ao permitir que as persons sentassem a mesa com Noel. (Foto: Alexandre Macieira.)`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Artística RioArte',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const NoelRosa_GilbertoMandarino: Obra = {
  ID: 172,
  Imagem: 'NoelRosa_GilbertoMandarino.jpeg',
  Authors: [authors.GilbertoMandarino],
  Titulo: `Noel Rosa`,
  Material: 'Bronze',
  DataInauguracao: `05/1987`,
  DatasImportantes: [new Data(`05/1987`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Av. 28 de Setembro',
  Latitude: `-22.9158194`,
  Longitude: `-43.2462725`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Pequeno busto próximo aos bares frequentados pelo compositor.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PedroErnesto_BenevenutoBerna: Obra = {
  ID: 173,
  Imagem: 'PedroErnesto_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Pedro Ernesto`,
  Material: 'Bronze',
  DataInauguracao: `11/1953`,
  DatasImportantes: [new Data(`11/1953`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Boulevard 28 de setembro',
  Latitude: `-22.9140258`,
  Longitude: `-43.2370577`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem em bronze foi umainiciativa da Sociedade Beneficente dos Engenheiros Municipais.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AJuventudeACulturaEAoEsporte_HaroldoBarroso: Obra = {
  ID: 174,
  Imagem: 'AJuventudeACulturaEAoEsporte_HaroldoBarroso.jpeg',
  Authors: [authors.HaroldoBarroso],
  Titulo: `A  Juventude, a Cultura e ao Esporte`,
  Material: 'concreto',
  DataInauguracao: `02/03/1974`,
  DatasImportantes: [new Data(`02/03/1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `O Monumento tem vinte e um  metros de altura, é formado por elementos de concreto
    pré-moldado, num total de dez, pesando cada um dez toneladas, unidas e justapostas em linhas
    geométricas. O Monumento foi inaugurado pelo ex-presidente Médici e o Governador Chagas Freitas.
    O local escolhido é perfeito a frente da UERJ, símbolo da educação cultural e tecnológica, e de
    outro o Estádio do Maracanã, palco de grandes manifestações esportivas. A obra de construção do
    monumento começaram no dia 15 de janeiro e foram concluídas em 2 de março de 1974. A obras sofreu
    um pequeno atraso por causa dos fortes ventos, que não permitiram a fixação perfeita dos módulos. 
    Foi desmontado em dezembro de 2010 para as obras de remanejamento do rio Joana, sendo transferido
    para o deposito, no parque Noronha Santos, onde permanece até a presente data. É a unica obra
    publica na cidade do Rio de Janeiro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AosCampeoesdoMundoBellini_MateusFernandes: Obra = {
  ID: 175,
  Imagem: 'AosCampeoesdoMundoBellini_MateusFernandes.jpeg',
  Authors: [authors.MateusFernandes],
  Titulo: `Aos Campeões do Mundo -  Bellini e outros`,
  Material: 'Bronze',
  DataInauguracao: `11/1960`,
  DatasImportantes: [new Data(`11/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Maracanã',
  Endereco: 'Avenida Maracanã',
  Latitude: `-22.9135268`,
  Longitude: `-43.2276378`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é uma homenagem aos jogadores que se consagraram campeães do mundo, com destaque a estátua de um jogador conhecido como o Bellini. No pedestal tem as placas com o nome dos jogadores que participaram de várias Copa do Mundo, quando o Brasil foi o campeão. O monumento foi uma iniciativa do Rio da Voz e Jornal dos Esportes.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EdsonPassos_HildegardoLeaoVelloso: Obra = {
  ID: 176,
  Imagem: 'EdsonPassos_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Edson Passos`,
  Material: 'Bronze',
  DataInauguracao: `06/1955`,
  DatasImportantes: [new Data(`06/1955`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Usina',
  Endereco: 'Av. Edson Passos',
  Latitude: `-22.9462468`,
  Longitude: `-43.2578849`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma iniciativa dos amigos e do Clube de Engenharia, e esta situado no acesso a Avenida Edson Passos, aberta pelo engenheiro. Foi inuauguurado pelo prefeito Alim Pedro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const IrmaoGoncalvesXavier_Desconhecida: Obra = {
  ID: 177,
  Imagem: 'IrmaoGoncalvesXavier_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Irmão Gonçalves Xavier`,
  Material: 'Bronze',
  DataInauguracao: `09/1999`,
  DatasImportantes: [new Data(`09/1999`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Rua Dr. Sartamini',
  Latitude: `-22.9208109`,
  Longitude: `-43.2231799`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao Irmão foi erigido por iniciativa da Diretoria do Colégio e pela Associação dos Antigos Alunos
    Maristas do Rio de Janeiro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SaoMaron_RomeuAlves: Obra = {
  ID: 179,
  Imagem: 'SaoMaron_RomeuAlves.jpeg',
  Authors: [authors.RomeuAlves],
  Titulo: `São Maron`,
  Material: 'Bronze',
  DataInauguracao: `07/1997`,
  DatasImportantes: [new Data(`07/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça São Maron',
  Latitude: `-22.9206706`,
  Longitude: `-43.2226506`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi oferecido ao povo carioca pela comunidade maronita, por iniciativa da Paróquia de Nossa Senhora
    do Líbano.Foi executado na Fundição Zani.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SaoCharbel_RomeuAlves: Obra = {
  ID: 180,
  Imagem: 'SaoCharbel_RomeuAlves.jpeg',
  Authors: [authors.RomeuAlves],
  Titulo: `São Charbel`,
  Material: 'Bronze',
  DataInauguracao: `10/1994`,
  DatasImportantes: [new Data(`10/1994`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Av. Maracanã',
  Latitude: `-22.928664`,
  Longitude: `-43.239848`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A estátua foi oferecida ao povo do Rio de Janeiro pela missão libanesa maronita.A obra foi produzida no Líbano e
    fundida pela Fundição Zani. A homenagem foi inaugurada pelo cardeal Eugenio Sales, por ter sido canonizado pelo Papa Paulo
    VI em 9 de outubro de 1977.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AugusteFrancoisMarieGlaziou_NicolinaVazDeAssisPintodoCouto: Obra =
  {
    ID: 181,
    Imagem: 'AugusteFrancoisMarieGlaziou_NicolinaVazDeAssisPintodoCouto.jpeg',
    Authors: [authors.NicolinaVazDeAssisPintodoCouto],
    Titulo: `Auguste François Marie Glaziou`,
    Material: 'marmore',
    DataInauguracao: `10/1910`,
    DatasImportantes: [new Data(`10/1910`, 'data de inauguração')],
    Propriedade: 'Pública',
    Zona: 'Norte',
    Bairro: 'São Cristóvão',
    Endereco: 'Quinta da Boa Vista',
    Latitude: `-22.9048496`,
    Longitude: `-43.2217892`,
    Base: 'Com pedestal',
    MaterialBase: 'Granito',
    Status: 'Permanece em espaço público',
    Descricao: `Busto em homenagem ao paisagista criador do parque.A peça é esculpida em um bloco maciço de mármore.A obra foi paga pelo Ministério da Viação e executada na Italia. Veio para o Brasil, partindo de Genova, em setembro de 1910.`,
    Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
    Temporalidade: 'Permanente',
    Categoria: 'Narratividade',
    Funcao: 'Referencial',
    Natureza: 'Cívica',
    Tipologia: 'Busto',
    Execucao: true,
  };

export const DomPedroII_JeanMagrou_HeitorDaSilvaCosta: Obra = {
  ID: 182,
  Imagem: 'DomPedroII_JeanMagrou_HeitorDaSilvaCosta.jpeg',
  Authors: [authors.JeanMagrou, authors.HeitorDaSilvaCosta],
  Titulo: `Dom Pedro II`,
  Material: 'Bronze',
  DataInauguracao: `12/1925`,
  DatasImportantes: [new Data(`12/1925`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.9054389`,
  Longitude: `-43.2247523`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua sobre um pedestal de cantaria de Petrópolis, erigida no dia do seu centenário
    de nascimento, de acordo com projeto do engenheiro Heitor da Silva Costa. A peça em bronze foi
    executada na França por Jean Magrou que assinou a peça. Estatua do Imperador esta de pe, tendo
    na mão esquerda um livro e a direita sobre o peito, perpetuando uma imagem de um imperador cultor,
    projetor da ciência, artes e letras, de acortdo com o projeto de Coube a iniciativa do Instituto
    Histórico e Geográfico Brasileiro de homenagear o Imperador e foi financiado por subscrição
    pública.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ImperatrizLeopoldina_EdgarDuvivierFilho: Obra = {
  ID: 183,
  Imagem: 'ImperatrizLeopoldina_EdgarDuvivierFilho.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Imperatriz Leopoldina`,
  Material: 'Bronze',
  DataInauguracao: `1996`,
  DatasImportantes: [
    new Data(`1996`, 'data de inauguração - arte ambiente'),
    new Data(`03/1997`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Quinta da Boa Vista',
  Latitude: `-22.9055816`,
  Longitude: `-43.2253663`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em bronze em tamanho natural, reproduzindo a figura de D. Leopoldina acompanhada de seus filhos,
    primogênita D. Maria da Gloria, futura rainha de Portugal e o caçula, de colo,que viria a suceder a seu pai, como imperador
    a D. Pedro II. O monumento foi uma iniciativa do Instituto Histórico e geográfico Brasileiro, compos uma comissão, na linha
    de reverenciar vultos expressivos da trajetória do pais, no ano de comemoração dos 200 anos do nascimento da primeira
    imperatriz, esposa do príncipe D. Pedro.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FranciscoDeMeloPalheta_Desconhecida: Obra = {
  ID: 184,
  Authors: [authors.Desconhecida],
  Titulo: `Francisco de Melo Palheta`,
  Material: 'mármore',
  DataInauguracao: `1974`,
  DatasImportantes: [new Data(`1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'São Cristóvão',
  Endereco: 'Rua Francisco Palheta',
  Latitude: `-22.8905307`,
  Longitude: `-43.22940946`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Brasil_DanielBellionGomesPinto: Obra = {
  ID: 161,
  Imagem: 'Brasil_DanielBellionGomesPinto.jpeg',
  Authors: [authors.DanielBellionGomesPinto],
  Titulo: `Brasil`,
  Renomeacao: false,
  Classificacao: 'perene',
  Categoria: 'sublimidade',
  Tipologia: 'Escultura',
  Material: `Aço Corten`,
  Descricao: `Escultura em aço cortada apresentando o mapa do Brasil.`,
  Execucao: true,
  DataInauguracao: `10/2004`,
  Endereco: 'Avenida Chile',
  Latitude: `-22.9089781`,
  Longitude: `-43.179683`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Eexecutado pelo estaleiro Aker Promar',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'Doado pelo estaleiro Aker Promar',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Vencedora',
      'Obras vencedora do concurso nacional ao MOnumento a Getúlio VArgas pela criação do Banco NAcional de Desenvolvimento Economico - BNDES',
      [],
      'Rio de Janeiro',
    ),
  ],
};

export const AgripinoGrieco_DanielCrow_CurzioZani: Obra = {
  ID: 185,
  Imagem: 'AgripinoGrieco_DanielCrow_CurzioZani.jpeg',
  Authors: [authors.DanielCrow, authors.CurzioZani],
  Titulo: `Agripino Grieco`,
  Material: 'Bronze',
  DataInauguracao: `10/1968`,
  DatasImportantes: [
    new Data(`10/1968`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1976`, 'data de inauguração - arte ambiente'),
    new Data(`1978`, 'transferido para Praça Agripino Grieco'),
    new Data(`1996`, 'transferido pelo Rio-Cidade'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Méier',
  Endereco: 'Rua Dias Cruz',
  Latitude: `-22.9018844`,
  Longitude: `-43.2797528`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao: `Instalado inicialmente instalado no jardim do Meier e reinaugurado na praça criada
    em sua homenagem em 10/11/1978. Com as alterações na praça, foi remanejado pelo Rio-Cidade, em
    1996, na atual localização. Foi uma doação do Emabaixador Vicente Paulo Gatti. Foi morador
    ilustre do bairro, da Rua Aristides Caire. Na inauguração esteve presente, na comemoração dos
    seus 80 anos de vida. A inauguração foi organizada pela Administração Regional.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlegoriaAoComercio_Desconhecida: Obra = {
  ID: 186,
  Authors: [authors.Desconhecida],
  Titulo: `Alegoria ao Comércio`,
  Material: 'mármore',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Méier',
  Endereco: 'Jardim do Méier',
  Latitude: `-22.9001765`,
  Longitude: `-43.2807281`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AristidesCaire_TitoBernucci: Obra = {
  ID: 187,
  Imagem: 'AristidesCaire_TitoBernucci.jpeg',
  Authors: [authors.TitoBernucci],
  Titulo: `Aristides Caire`,
  Material: 'Bronze',
  DataInauguracao: `1968`,
  DatasImportantes: [new Data(`1968`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Méier',
  Endereco: 'Jardim do Méier',
  Latitude: `-22.9001765`,
  Longitude: `-43.2807281`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BaraodoRioBranco_JoaoTurin: Obra = {
  ID: 188,
  Imagem: 'BaraodoRioBranco_JoaoTurin.jpeg',
  Authors: [authors.JoaoTurin],
  Titulo: `Barão do Rio Branco`,
  Material: 'Bronze',
  DataInauguracao: `04/1926`,
  DatasImportantes: [
    new Data(`04/1926`, 'data de inauguração'),
    new Data(`2015`, 'furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Méier',
  Endereco: 'Jardim do Méier',
  Latitude: `-22.9001765`,
  Longitude: `-43.2807281`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `Busto acadêmico do homenageado. O monumento foi inaugurao 14 anos após a sua morte reverenciando o chanceler. Esse busto foi apresentado por Turin no Salon des Artites François em 1913.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const NiltonSantos_EdgarDuvivierFilho: Obra = {
  ID: 189,
  Imagem: 'NiltonSantos_EdgarDuvivierFilho.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Nilton Santos`,
  Material: 'Bronze',
  DataInauguracao: `09/2009`,
  DatasImportantes: [new Data(`09/2009`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Engenho de Dentro',
  Endereco: 'R. José dos Reis',
  Latitude: `-22.8921404`,
  Longitude: `-43.294776`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem numa de suas posturas típicas. Doação à cidade pelo Clube de Regatas Botafogo em frente ao seu estadio
    do Engenhão.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'FEB- Fundição e Esculturas em Bronze',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const OrlandoSilva_MiguelPastor: Obra = {
  ID: 190,
  Imagem: 'OrlandoSilva_MiguelPastor.jpeg',
  Authors: [authors.MiguelPastor],
  Titulo: `Orlando Silva`,
  Material: 'Bronze',
  DataInauguracao: `01/1959`,
  DatasImportantes: [new Data(`01/1959`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cachambi',
  Endereco: 'Praça Orlando Silva',
  Latitude: `-22.8909848`,
  Longitude: `-43.2739674`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem executada em vida por iniciativa da Prefeitura Municipal.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const ChafarizDaExposicaoNacional1908_Desconhecida: Obra = {
  ID: 191,
  Imagem: 'ChafarizDaExposicaoNacional1908_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Chafariz da Exposição Nacional de 1908`,
  Material: 'ferro fundido',
  DataInauguracao: `1908`,
  DatasImportantes: [new Data(`1908`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Bonsucesso',
  Endereco: 'Praça das Nações',
  Latitude: `-22.8668416`,
  Longitude: `-43.257319`,
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', 'Cia Nacional de Fundição', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const MarechalMascarenhasdeMoraes_IrmaosBernardini: Obra = {
  ID: 192,
  Imagem: 'MarechalMascarenhasdeMoraes_IrmaosBernardini.jpeg',
  Authors: [authors.IrmãosBernardini],
  Titulo: `Marechal Mascarenhas de Moraes`,
  Material: 'Bronze',
  DataInauguracao: `1946`,
  DatasImportantes: [new Data(`1946`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Bonsucesso',
  Endereco: 'Praça das Nações',
  Latitude: `-22.8668416`,
  Longitude: `-43.257319`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao Patriarca da Independência, tutor do Imperador do Brasil, D. Pedro, que nasceu no Palácio da Quinta da Boa Vista. A obra foi paga pelo Ministério da Viação e executada na Italia. Veio para o Brasil, partindo de Genova, em setembro de 1910.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PaulHarris_Desconhecida: Obra = {
  ID: 193,
  Imagem: 'PaulHarris_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Paul Harris`,
  Material: 'Bronze',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Bonsucesso',
  Endereco: 'Praça Paul Harris',
  Latitude: `-22.8680923`,
  Longitude: `-43.25622776`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Em 19 de julho de 1961, o Governador carlos Lacerda, decretou como logradouro a Praça Paul Harris em Bonsucesso. Em 1963 inaugurou o ajardinamento da praça, que teve a iniciativa do rotariano o Professor Augusto Medeiros da Motta.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CarlosAlbertodaCruzWenceslau_Desconhecida: Obra = {
  ID: 952,
  Imagem: 'CarlosAlbertodaCruzWenceslau_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Carlos Alberto da Cruz Wenceslau`,
  Renomeacao: false,
  Classificacao: 'perene',
  Categoria: 'personalidades',
  Tipologia: `Busto`,
  Material: `Peça em argamassa e pedestal revestido com granito`,
  Descricao: `Homenagem da Universidade Castelo Branco e do Lions Clube de Realengo.`,
  Execucao: true,
  DataInauguracao: `11/2009`,
  Zona: 'este',
  Bairro: 'Bangú',
  Endereco: 'Praça Padre Miguel',
  Latitude: `-22.8780552`,
  Longitude: `-43.4347683`,
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
};

export const MaedAgua_NewtonSa: Obra = {
  ID: 196,
  Imagem: 'MaedAgua_NewtonSa.jpeg',
  Authors: [authors.NewtonSa],
  Titulo: `Mãe d’Água`,
  Material: 'Bronze',
  DataInauguracao: `01/1968`,
  DatasImportantes: [new Data(`01/1968`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praça Jerusalém',
  Latitude: `-22.8188248`,
  Longitude: `-43.2033869`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em bronze inspirada na lenda amazônica, composta por figura aquática, meio
    índia meio peixe, sobre vitória régia também em bronze. A comissão de Finanças da Camara de
    Vereadores propos que o ultimo trabalho de Newton Sá fosse adquirida e colocada em praça publica.
    A obra ficou exposta na camara e foi adquirida pelo Sr. Governador Carlos Lacerda, e doado para
    o Estado e posteriormente transferida para o Municipio. Foi comprada em 16/06/1964 do irmão do
    artista Edson Sá . Esta desativado desde maio de 2017.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ExudosVentos_MarioCravoNeto: Obra = {
  ID: 197,
  Imagem: 'ExudosVentos_MarioCravoNeto.jpeg',
  Authors: [authors.MarioCravoNeto],
  Titulo: `Exú dos Ventos`,
  Material: 'aço pintado',
  Altura: '12 m',
  DataInauguracao: `12/2000`,
  DatasImportantes: [
    new Data(`12/2000`, 'data de inauguração - inventário dos monumentos'),
    new Data(`2002`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cidade Universitária',
  Endereco: 'final da Linha Amarela',
  Latitude: `-22.859908`,
  Longitude: `-43.2340757`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AlexanderFleming_LeonardoLima: Obra = {
  ID: 198,
  Authors: [authors.LeonardoLima],
  Titulo: `Alexander Fleming`,
  Material: 'Bronze',
  DataInauguracao: `1956`,
  DatasImportantes: [new Data(`1956`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cidade Universitária',
  Endereco: 'Centro de Ciencias da Saúde',
  Latitude: `-22.8427903`,
  Longitude: `-43.236221`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const LimaBarreto_FranciscoAndrade: Obra = {
  ID: 199,
  Imagem: 'LimaBarreto_FranciscoAndrade.jpeg',
  Authors: [authors.FranciscoAndrade],
  Titulo: `Lima Barreto`,
  Material: 'Bronze',
  DataInauguracao: `1931`,
  DatasImportantes: [
    new Data(`1931`, 'data de inauguração - arte ambiente'),
    new Data(`01/1935`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praça Calcutá',
  Latitude: `-22.7935416`,
  Longitude: `-43.1726375`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem registra o local onde viveu. Para homenagear o romancista carioca, um grupo
    de artistas e intelectuais, encabeçados por Agripino Grieco, Guttaman Bicho, Phocion Serpa e Jordão
    de Oliveira, resolveram erguer um busto. A praça da Freguesia foi o local escolhido, Definindo o
    local, discutiu  se a posição em que ficaria. Sugeriu-se de frente para a Igreja Nossa Senhora da
    Ajuda. Possibilidade que foi rapidamente descartada: o escritor não era religioso. Ironicamente,
    talvez de fronte dos botequins . Enfim, optou-se pelo mar. pois a escolha agradaria em cheio a
    Lima Barreto, pois numa de suas frases afirma: Pouco olho o céu, quase nunca lua, mas sempre o marco.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CarmenMiranda_MateusFernandes: Obra = {
  ID: 200,
  Imagem: 'CarmenMiranda_MateusFernandes.jpeg',
  Authors: [authors.MateusFernandes],
  Titulo: `Carmen Miranda`,
  Material: 'Bronze',
  DataInauguracao: `11/1960`,
  DatasImportantes: [new Data(`11/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praça Carmen Miranda',
  Latitude: `-22.8187729`,
  Longitude: `-43.1915205`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado no Largo da Carioca, e em 1979 na localização atual. O monumento foi obtido atraves de subscrição, promovido pela Revista do Radio atraves do diretor Sr. Anselmo Domingos. Foi uma iniciativa da Revista e da Loja Reida Voz. Na inauguração estiveram presentes varios artistas como Orlando Silva, Silvio Caldas, João Dias e Celiz Zenatti.// Estrela no cinema americano, a atriz e cantora Carmen Miranda se tornou a terceira personalidade mais popular nos EUA abrindo portas para a entrada da música brasileira no mundo. Entre 1930 e 1950 ela emplacou hits como a “Ta-hí (Pra Você Gostar de Mim)” e estrelou filmes como “Banana da Terra” (1939), em que aparece pela primeira vez caracterizada como a personagem que a lançou ao mundo enquanto interpreta a música “O que é que a baiana tem?”, de Dorival Caymmi. O busto em bronze da Pequena Notável foi instalado em 1960 no Largo da Carioca, mas desde 1970 se encontra na Ilha do Governador. `,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CastroAlves_ModestinoKanto: Obra = {
  ID: 201,
  Imagem: 'CastroAlves_ModestinoKanto.jpeg',
  Authors: [authors.ModestinoKanto],
  Titulo: `Castro Alves`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praça Manuel Madruga',
  Latitude: `-22.7932557`,
  Longitude: `-43.184517`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem de seus admiradores.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Leao_Desconhecida: Obra = {
  ID: 202,
  Imagem: 'Leao_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Leão`,
  Material: 'ferro fundido',
  DataInauguracao: `05/01/1967`,
  DatasImportantes: [new Data(`05/01/1967`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Ilha do Governador',
  Endereco: 'Praia do Zumbi',
  Latitude: `-22.8194259`,
  Longitude: `-43.1756813`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Removida do espaço público',
  Descricao: `Originalmente se tratava de uma escultura em argamassa armada representando um leão
    em movimento. Destruída por um acidente, foi substituída por outra, em ferro, de um leão sentado,
    observando a paisagem. Foi inaugurado o primeiro Leão de argamassa, instalado pelo Lions Ilha.
    Douze horas depois da inauguração seis homens depredaram a peça que foi refeita e reinstalada
    até julho de 2013.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const IndiaI_JulesJeanJulesSalmson: Obra = {
  ID: 203,
  Imagem: 'IndiaI_JulesJeanJulesSalmson.jpeg',
  Authors: [authors.JulesJeanJulesSalmson],
  Titulo: `Índia`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Penha',
  Endereco: 'Largo da Penha',
  Latitude: `-22.84243`,
  Longitude: `-43.2781373`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando uma índia com um peixe em uma das nmãos sobre pedestal de mesmo material.`,
  Fatos: [
    new Fact('', '', 'Fundição/Atelier', "Val d'Osne", [], 'Rio de Janeiro'),
    new Fact(
      '2000',
      '2000',
      'Tombamento',
      'Tombado pelo Município pelo Decreto 19.011/2000',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Mae_RuiLeal: Obra = {
  ID: 204,
  Imagem: 'Mae_RuiLeal.jpeg',
  Authors: [authors.RuiLeal],
  Titulo: `Mãe`,
  Material: 'argamassa',
  DataInauguracao: `05/1966`,
  DatasImportantes: [new Data(`05/1966`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Madureira',
  Endereco: 'Praça Vice Almirante Nelson Fernandes',
  Latitude: `-22.9135203`,
  Longitude: `-43.5975711`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em argamassa de argila em tamanho natural sentada em um pequeno banco de madeira, com uma criança em seu colo.
    A homenageada conforme descrito no Diario de Noticias, é a Mãe Senhora da Bahia. A insuguração foi muito concorrida com a programação a cargo de D. Elza Osborne. Compareceram varias Authors:idades cinclusive os embaixadores da Nigeria e do senegal e o compositor Caymmi.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const PedroFernandes_EdgarDuvivierFilho: Obra = {
  ID: 205,
  Imagem: 'PedroFernandes_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Pedro Fernandes`,
  Material: 'Bronze',
  DataInauguracao: `08/2007`,
  DatasImportantes: [new Data(`08/2007`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Irajá',
  Endereco: 'Av. Monsenhor Felix, 380',
  Latitude: `-22.8494607`,
  Longitude: `-43.3254336`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi uma iniciativa de sua filha Rosa Fernandes, ao patriarca da família e o reconhecimento moradores
    do bairro.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'FEB- Fundição e Esculturas em Bronze',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SaoCristovao_SergioBertoni: Obra = {
  ID: 206,
  Imagem: 'SaoCristovao_SergioBertoni.jpeg',
  Authors: [authors.SergioBertoni],
  Titulo: `São Cristóvão`,
  Material: 'Bronze',
  DataInauguracao: `09/1966`,
  DatasImportantes: [new Data(`09/1966`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cordovil',
  Endereco: 'Km 0 da via Dutra',
  Latitude: `-22.8189257`,
  Longitude: `-43.3201278`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A estátua em bronze do Santo está inserido em um construção formando uma capela que possui 20 metros de altura e com laminas de 4 lâminas de concreto. A homenagem foi realizada pela Mercedes Benz do Brasil<, ao protetor dos motoristas.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const VicenteDeCarvalho_RuffoFanucchi: Obra = {
  ID: 207,
  Imagem: 'VicenteDeCarvalho_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: `Vicente de Carvalho`,
  Material: 'Bronze',
  DataInauguracao: `01/1960`,
  DatasImportantes: [new Data(`01/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vicente de Carvalho',
  Endereco: 'Largo Vicente de Carvalho',
  Latitude: `-22.8534197`,
  Longitude: `-43.3149425`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi uma iniciativa do vereador Milton de Castro Menezes com apoio do Instituto Histórico e Geográfico da Cidade.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FonteWallaceII_CharlesAugusteLebourg: Obra = {
  ID: 208,
  Imagem: 'FonteWallaceII_CharlesAugusteLebourg.jpeg',
  Authors: [authors.CharlesAugusteLebourg],
  Titulo: `Fonte Wallace`,
  Material: 'ferro fundido',
  DataInauguracao: `11/1960`,
  DatasImportantes: [new Data(`11/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9199976`,
  Longitude: `-43.684932`,
  Status: 'Permanece em espaço público',
  Descricao: `Fonte criada a pedido do filantropo inglês Richard Wallace.Trata-se de uma fonte em ferro fundido ornada por quatro cariatides que representam a bondade, a simplicidade, a caridade e a sobriedade e sustentam uma cúpula ornamentada com escamas de dragão, terminando numa ponteira. A água que sai do interior da cúpula e cai numa pequena bacia colocada aos pés das cariatides.
    Desativado desde de maio de 2017.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Bastoes_CeleidaTostes: Obra = {
  ID: 209,
  Imagem: 'Bastoes_CeleidaTostes.jpeg',
  Authors: [authors.CeleidaTostes],
  Titulo: `Bastões`,
  Material: 'argamassa',
  DataInauguracao: `1986`,
  DatasImportantes: [new Data(`1986`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9195845`,
  Longitude: `-43.3093173`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Roda_CeleidaTostes: Obra = {
  ID: 210,
  Imagem: 'Roda_CeleidaTostes.jpeg',
  Authors: [authors.CeleidaTostes],
  Titulo: `Roda`,
  Material: 'argamassa',
  DataInauguracao: `1986`,
  DatasImportantes: [
    new Data(`1986`, 'data de inauguração - arte ambiente'),
    new Data(`01/1995`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9799141`,
  Longitude: `-43.2418108`,
  Base: 'Com pedestal',
  MaterialBase: 'argamassa',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de peça redonda com furo ao centro.`,
  Fatos: [
    new Fact(
      '1965',
      '1965',
      'Tombamento',
      'Tombado pelo INEPAC pelo Decreto 927.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Mo_CeleidaTostes: Obra = {
  ID: 211,
  Imagem: 'Mo_CeleidaTostes.jpeg',
  Authors: [authors.CeleidaTostes],
  Titulo: `Mó`,
  Material: 'argamassa',
  DataInauguracao: `1986`,
  DatasImportantes: [
    new Data(`1986`, 'data de inauguração - arte ambiente'),
    new Data(`01/1995`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9799141`,
  Longitude: `-43.2418108`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de peça formada por uma pequena roda com um cone ao centro.`,
  Fatos: [
    new Fact(
      '1965',
      '1965',
      'Tombamento',
      'Tombado pelo INEPAC pelo Decreto 927.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Contemplacao_JesperNeewgaard: Obra = {
  ID: 212,
  Imagem: 'Contemplacao_JesperNeewgaard.jpeg',
  Authors: [authors.JesperNeewgaard],
  Titulo: `Contemplação`,
  Material: 'mármore de Carrara',
  DataInauguracao: `11/1995`,
  DatasImportantes: [new Data(`11/1995`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9799141`,
  Longitude: `-43.2418108`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Peça abstrata. È a primeira obra do artista na America do Sul.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doada pelo próprio escultor à cidade',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1965',
      '1965',
      'Tombamento',
      'Tombado pelo INEPAC pelo Decreto 927.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Indio_CharlesAugusteLebourg: Obra = {
  ID: 213,
  Authors: [authors.CharlesAugusteLebourg],
  Titulo: `Indio`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9799141`,
  Longitude: `-43.2418108`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SantosDumont_Desconhecida: Obra = {
  ID: 214,
  Imagem: 'SantosDumont_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Santos Dumont`,
  Material: 'Bronze',
  DataInauguracao: `07/2001`,
  DatasImportantes: [new Data(`07/2001`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Praça Santos Dumont',
  Latitude: `-22.9741195`,
  Longitude: `-43.2261039`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento doado à cidade pelo Presidente do Lions Clube do Rio de Janeiro da Gávea,
    Sr. Patrice Max Jacques Grellier. Essa homenagem visava enaltecer o nome daquele que tão longe
    levou o nome do Brasil. Esse monumento foi custeado pela entidade. A figura de Santos Dumont
    nessa homenagem esta sem o tradicional chapéu.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra - AGR',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BaraodeItamarati_PauloMazzucchelli: Obra = {
  ID: 215,
  Imagem: 'BaraodeItamarati_PauloMazzucchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: `Rocha Leão, o Barão de Itamarati`,
  Material: 'Bronze',
  DataInauguracao: `01/1938`,
  DatasImportantes: [new Data(`01/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Praça Santos Dumont',
  Latitude: `-22.9732459`,
  Longitude: `-43.2253913`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inicialmente inaugurado em 1938 na praça Rocha Leão. Foi removida para o depósito em 05 de novembro de 1969 ficando guardado ate 1998, quando numa reforrma da praça foi instalado.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const LineudePaulaMachado_HildegardoLeaoVelloso: Obra = {
  ID: 216,
  Imagem: 'LineudePaulaMachado_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Lineu de Paula Machado`,
  Material: 'Bronze',
  DataInauguracao: `07/1948`,
  DatasImportantes: [new Data(`07/1948`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Rua Jardim Botanico',
  Latitude: `-22.9736912`,
  Longitude: `-43.2256268`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua em bronze do tamanho natural com o homenageado trajando um terno em elegante postura. Éuma homenagem aoBenemérito promovida peloJockey Club Brasileiro. O monumento foi inaugurado durante um parea a ele dedicado.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Arvore_HelioPelegrino: Obra = {
  ID: 217,
  Imagem: 'Arvore_HelioPelegrino.jpeg',
  Authors: [authors.HelioPelegrino],
  Titulo: `Arvore`,
  Material: 'ferro e plástico',
  DataInauguracao: `2001`,
  DatasImportantes: [new Data(`2001`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Rua Padre Leonel Franca - Galpão das Artes Urbanas',
  Latitude: `-22.9793281`,
  Longitude: `-43.2348017`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ProtecaooCaoeaCrianca_HippolyteFrancoisAugusteHippolytePeyrol: Obra =
  {
    ID: 219,
    Imagem:
      'ProtecaooCaoeaCrianca_HippolyteFrancoisAugusteHippolytePeyrol.jpeg',
    Authors: [authors.MateusFernandes],
    Titulo: `Proteção, o Cão e a Criança`,
    Material: 'marmore carrara',
    DataInauguracao: `01/1906`,
    DatasImportantes: [new Data(`01/1906`, 'data de inauguração')],
    Propriedade: 'Pública',
    Zona: 'Sul',
    Bairro: 'Ipanema',
    Endereco: 'Jardim de Alah',
    Latitude: `-22.9821703`,
    Longitude: `-43.217167`,
    Base: 'Com pedestal',
    MaterialBase: 'Granito',
    Status: 'Permanece em espaço público',
    Descricao: `Escultura inicialmente instalada na Praia de Botafogo em 1906. Em 1937, quando
    Azevedo Neto projetou o Jardim de Alá, esta escultura foi transferida para aquele espaço.
    Escultura em mármore, representando um Cão em posição de vigia, próximo a uma figura de uma
    criança dormindo sobre um cesto. A escultura esta sobre um pedestal de granito, onde esta
    gravada o nome Proteção`,
    Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
    Temporalidade: 'Permanente',
    Categoria: 'Sublimidade',
    Funcao: 'Decorativa',
    Natureza: 'Artística',
    Tipologia: 'Escultura',
    Execucao: true,
  };

export const SemTitulo_MariaLuciaPivetti: Obra = {
  ID: 220,
  Authors: [authors.MariaLuciaPivetti],
  Titulo: `Sem título`,
  Material: 'Aço',
  DataInauguracao: `1993`,
  DatasImportantes: [new Data(`1993`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Avenida Vieira Souto, 176',
  Latitude: `-22.9864563`,
  Longitude: `-43.2013803`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const QuadradoemTorcaonoEspaco_FranzWeissmann: Obra = {
  ID: 221,
  Imagem: 'QuadradoemTorcaonoEspaco_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Quadrado em Torção no Espaço`,
  Material: 'aço pintado',
  Altura: '200 cm',
  Largura: '160 cm',
  Profundidade: '220 cm',
  DataInauguracao: `01/1985`,
  DatasImportantes: [new Data(`01/1985`, 'data de inauguração')],
  Propriedade: 'privada',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Avenida Vieira Souto, 176',
  Latitude: `-22.9867849`,
  Longitude: `-43.1992605`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Escultura abstrata em cessão de uso em área pública da Prefeitura.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AlbertoIReidaBelgica_LaurindoRamos: Obra = {
  ID: 222,
  Imagem: 'AlbertoIReidaBelgica_LaurindoRamos.jpeg',
  Authors: [authors.LaurindoRamos],
  Titulo: `Alberto I Rei da Belgica`,
  Material: 'Bronze',
  DataInauguracao: `09/1922`,
  DatasImportantes: [new Data(`09/1922`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Avenida Rainha Elizabeth',
  Latitude: `-22.9853739`,
  Longitude: `-43.1924953`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido em homenagem ao Rei quando visitou o Brasil, num reconhecimento
    dos moradores de Copacabana, que encantava com a simplicidade com que se direigia ao mar para o
    banho matinal O discurso de inauguração coube ao Dr. Luiz Carpenter que inauteceu sua presença
    no bairro. O busto apresenta o rei com seu trage de gala.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Mercurio_Desconhecida: Obra = {
  ID: 223,
  Imagem: 'Mercurio_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Mercúrio`,
  Material: 'mármore de carrara',
  DataInauguracao: `11/1960`,
  DatasImportantes: [new Data(`11/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Rua Padre Leonel França',
  Latitude: `-22.9790285`,
  Longitude: `-43.2302088`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em estilo neoclássico, representando o deus com o caduceu na mão direita,
    do mesmo conjunto das Atenas que exitem no Bosque da Freguesia, na Praça dos Estudantes e no
    Jardim do Méier.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ClementinoFraga_GiulioStarace: Obra = {
  ID: 224,
  Imagem: 'ClementinoFraga_GiulioStarace.jpeg',
  Authors: [authors.GiulioStarace],
  Titulo: `Clementino Fraga`,
  Material: 'Bronze',
  DataInauguracao: `03/1983`,
  DatasImportantes: [new Data(`03/1983`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Praça Santos Dumont',
  Latitude: '-22.974484',
  Longitude: '-43.2263524',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado em 1983 na Avenida Vice-Governador, permanecendo até
    1998 quando para a obra de ampliação do Planetário da Gávea foi transferido para o depósito da
    Prefeitura. No ano de 11 de setembro de 2000 foi reinstalado no outro lado da pista, na mesma
    rua. Em março de 2012 foi novamente tranferido para o depósito devido a uma  obra urbana. Em
    15 de setembro de 2012 foi reinaugurado na Praça Santos Dumont atendendo uma solicitação da
    familia.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MulherComCornucopia_PierreLoison: Obra = {
  ID: 225,
  Imagem: 'MulherComCornucopia_PierreLoison.jpeg',
  Authors: [authors.PierreLoison],
  Titulo: `Mulher Com Cornucopia`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FonteWallaceIII_CharlesAugusteLebourg: Obra = {
  ID: 226,
  Imagem: 'FonteWallaceIII_CharlesAugusteLebourg.jpeg',
  Authors: [authors.CharlesAugusteLebourg],
  Titulo: `Fonte Wallace`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado municipal', '', [], ''),
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', "Fundação Val D'Osne", '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const TomJobim_ChristinaMotta: Obra = {
  ID: 227,
  Imagem: 'TomJobim_ChristinaMotta.jpeg',
  Authors: [authors.ChristinaMotta],
  Titulo: `Tom Jobim`,
  DataInauguracao: `2014`,
  DatasImportantes: [new Data(`2014`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Arpoador',
  Latitude: `-22.988574`,
  Longitude: `-43.1930468`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: '',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Memorial',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const IsmaelSilva_OttoDumovich: Obra = {
  ID: 227,
  Imagem: 'IsmaelSilva_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Ismael Silva`,
  DataInauguracao: `2010`,
  DatasImportantes: [new Data(`2010`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cidade Nova',
  Endereco: 'Praça Compositor Ismael Silva',
  Latitude: `-22.9130434`,
  Longitude: `-43.2026903`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Monumento em bronze solicitado pela Escola de Samba Estácio de Sá à Prefeitura do Rio para homenagear o sambista Ismael Silva, fundador do bloco que se tornou o precursor da primeira escola de samba que se tem notícia, a Deixa Falar.',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Memorial',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PioCorreia_AlicePittaluga: Obra = {
  ID: 228,
  Imagem: 'PioCorreia_AlicePittaluga.jpeg',
  Authors: [authors.AlicePittaluga],
  Titulo: `Pio Correia`,
  Material: 'Bronze',
  DataInauguracao: `1985`,
  DatasImportantes: [new Data(`1985`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CamposPorto_PauloMazzuchelli: Obra = {
  ID: 229,
  Imagem: 'CamposPorto_PauloMazzuchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: `Campos Porto`,
  Material: 'Bronze',
  DataInauguracao: `1954`,
  DatasImportantes: [new Data(`1954`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const XochipilliDeusdasFlores_Desconhecida: Obra = {
  ID: 230,
  Imagem: 'XochipilliDeusdasFlores_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Xochipilli Deus das Flores`,
  Material: 'pedra',
  DataInauguracao: `1935`,
  DatasImportantes: [new Data(`1935`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', 'doação do governo do méxico', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Ceres_Merzio: Obra = {
  ID: 231,
  Imagem: 'Ceres_Merzio.jpeg',
  Authors: [authors.Merzio],
  Titulo: `Ceres`,
  Material: 'ferro fundido',
  DataInauguracao: `1888`,
  DatasImportantes: [new Data(`1888`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Diana_Merzio: Obra = {
  ID: 232,
  Imagem: 'Diana_Merzio.jpeg',
  Authors: [authors.Merzio],
  Titulo: `Diana`,
  Material: 'ferro fundido',
  DataInauguracao: `1888`,
  DatasImportantes: [new Data(`1888`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', 'bem tomado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ChafarizDasMusas_HerbertWHogg: Obra = {
  ID: 233,
  Imagem: 'ChafarizDasMusas_HerbertWHogg.jpeg',
  Authors: [authors.HerbertWHogg],
  Titulo: `Chafariz Das Musas`,
  Material: 'ferro fundido',
  DataInauguracao: `1895`,
  DatasImportantes: [new Data(`1895`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Status: 'Permanece em espaço público',
  Descricao: `Fundição inglesa`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const FreiLeandroDoSacramento_RibeiroDaCosta: Obra = {
  ID: 234,
  Imagem: 'FreiLeandroDoSacramento_RibeiroDaCosta.jpeg',
  Authors: [authors.RibeiroDaCosta],
  Titulo: `Frei Leandro Do Sacramento`,
  Material: 'Bronze',
  DataInauguracao: `1895`,
  DatasImportantes: [new Data(`1895`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Busto',
  Execucao: true,
};

export const DomJoaoVI_RodolfoBernardelli: Obra = {
  ID: 235,
  Imagem: 'DomJoaoVI_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Dom Joao VI`,
  Material: 'Bronze',
  DataInauguracao: `1908`,
  DatasImportantes: [new Data(`1908`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JoaoBarbosaRodrigues_AntonioPintodeMattos: Obra = {
  ID: 236,
  Imagem: 'JoaoBarbosaRodrigues_AntonioPintodeMattos.jpeg',
  Authors: [authors.AntonioPintodeMattos],
  Titulo: `João Barbosa Rodrgiues`,
  Material: 'Bronze',
  DataInauguracao: `1933`,
  DatasImportantes: [new Data(`1933`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const VonMartius_AntonioPintodeMattos: Obra = {
  ID: 237,
  Imagem: 'VonMartius_AntonioPintodeMattos.jpeg',
  Authors: [authors.AntonioPintodeMattos],
  Titulo: `Von Martius`,
  Material: 'Bronze',
  DataInauguracao: `1934`,
  DatasImportantes: [new Data(`1934`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AFonteDeusaThetis_LouisSauvageau: Obra = {
  ID: 238,
  Imagem: 'AFonteDeusaThetis_LouisSauvageau.jpeg',
  Authors: [authors.LouisSauvageau],
  Titulo: `A Fonte Deusa Thetis`,
  Material: 'ferro fundido',
  DataInauguracao: `01/1903`,
  DatasImportantes: [new Data(`01/1903`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A escultura sobre as bacias chamada "A Fonte" de 1878, parece ter sido a única
    estatua deste escultor destinada a arte. No Chafariz a figura da fonte esta no alto de uma
    grande bacia de ferro bastante ornamentada, sob a qual crianças estão montadas em golfinhos.
    Com a serenidade estampada na semblante, esta figura reina nomeio das águas, coroada de plantas
    aquáticas e adornada por um colar de conchas e deixa escorrer lentamente o conteúdo da bilha
    que graciosamente segura. Desativado desde o ano de 2001. Fundição Val d'Osne`,
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Eco_MestreValentim: Obra = {
  ID: 239,
  Authors: [authors.MestreValentim],
  Titulo: `Eco`,
  Material: 'chumbo e estanho',
  DataInauguracao: `1783`,
  DatasImportantes: [new Data(`1783`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado no Largo da Carioca, e em 1979 na localização atual. O monumento foi obtido atraves de subscrição, promovido pela Revista do Radio atraves do diretor Sr. Anselmo Domingos. Foi uma iniciativa da Revista e da Loja Reida Voz. Na inauguração estiveram presentes varios artistas como Orlando Silva, Silvio Caldas, João Dias e Celiz Zenatti.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const OCorneteiro_Ique: Obra = {
  ID: 240,
  Imagem: 'OCorneteiro_Ique.jpeg',
  Authors: [authors.Ique],
  Titulo: `O Corneteiro`,
  Material: 'Bronze',
  DataInauguracao: `04/2004`,
  DatasImportantes: [new Data(`04/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Rua Visconde de Pirajá',
  Latitude: `-22.9840218`,
  Longitude: `-43.2092142`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `grupo escultórico composto por um corneteiro, um pedaço de roda de carruagem e um canhão. A obra representa o
    corneteiro Luis Lopes. Este ficou conhecido pela Batalha dos Pirajá, na Bahia em 1822, quando a ordem do Exército Brasileiro
    era recuar, o corneteiro avançou e com o som da sua corneta fez as tropas portuguesas recuarem com medo de uma nova investida
    dos inimigos.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'FEB- Fundição e Esculturas em Bronze',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const LouisPasteur_HeitorDaSilvaCosta: Obra = {
  ID: 241,
  Imagem: 'LouisPasteur_HeitorDaSilvaCosta.jpeg',
  Authors: [authors.HeitorDaSilvaCosta],
  Titulo: 'Louis Pasteur',
  Material: 'Bronze',
  DataInauguracao: `06/1925`,
  DatasImportantes: [new Data(`06/1925`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Avenida Pasteur',
  Latitude: `-22.9538729`,
  Longitude: `-43.1693478`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem realizada pelo engenheiro arquiteto Heitor da Silva Costa, com a colaboração do Professor Gronchi. Foi inaugurado em 1925 no começo da averinda de mesmo nome e transferida para essa praça ao lado da antiga Escola de Medicina em agosto de 1947. Foi realizado poriniciativa da Sociedade de Medicina e Cirurgia. Na inauguração estavam presentes o Ministro das Relações Exteriores e o Emabixador da Franaça.
    A herma de Pasteur, o representa pensativoem busca da solução do problema da raiva, no simbolismo de um cão, que emerge da base do pedestal.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Tartaruga_JoseResende: Obra = {
  ID: 242,
  Imagem: 'Tartaruga_JoseResende.jpeg',
  Authors: [authors.JoseResende],
  Titulo: `Tartaruga`,
  Material: 'aço corten',
  DataInauguracao: `2002`,
  DatasImportantes: [new Data('2002', 'data de inauguração')],
  Propriedade: 'privada',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Rosário',
  Latitude: `-22.9013634`,
  Longitude: `-43.1758651`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Escultura de chapas de aço corten sobrepostas.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarioPedrosa_RomeuAlves: Obra = {
  ID: 243,
  Imagem: 'MarioPedrosa_RomeuAlves.jpeg',
  Authors: [authors.MateusFernandes],
  Titulo: `Mario Pedrosa`,
  Material: 'Bronze',
  DataInauguracao: `01/1984`,
  DatasImportantes: [new Data(`01/1984`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça Nossa Senhora da Paz',
  Latitude: `-22.9842363`,
  Longitude: `-22.9842363`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento homenagea o pensador e critico de arte.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2000',
      '2000',
      'Tombamento',
      'Tombado pelo Municipio pela Lei nº 3.089 de 05/09/00',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AMeninadosBaloesEncantados_SandraGuinle: Obra = {
  ID: 244,
  Imagem: 'AMeninadosBaloesEncantados_SandraGuinle.jpeg',
  Authors: [authors.SandraGuinle],
  Titulo: `A Menina dos Balões Encantados`,
  Material: 'Bronze',
  DataInauguracao: `10/2007`,
  DatasImportantes: [new Data(`10/2007`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça Nossa Senhora da Paz',
  Latitude: `-22.9842363`,
  Longitude: `-43.2053417`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça doada à cidade pela escultora para adornar a praça. Símbolo do projeto social
    "Crianças em Paz" . Escultura figurativa de uma criança segurando balões de gás. O braço com o
    balões da escultura foram furtados em março de 2018.`,
  Fatos: [
    new Fact('', '2018', 'Furto', 'Braços e balões', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Interacao_MarliMazeredo: Obra = {
  ID: 245,
  Imagem: 'Interacao_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Interação`,
  Material: 'resina',
  DataInauguracao: `1992`,
  DatasImportantes: [new Data(`1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça Nossa Senhora da Paz',
  Latitude: `-22.9842363`,
  Longitude: `-43.2053417`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Primavera_LuciaGuerreiro: Obra = {
  ID: 246,
  Imagem: 'Primavera_LuciaGuerreiro.jpeg',
  Authors: [authors.LuciaGuerreiro],
  Titulo: `Primavera`,
  Material: 'Bronze',
  DataInauguracao: `05/1989`,
  DatasImportantes: [new Data(`05/1989`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça Nossa Senhora da Paz',
  Latitude: `-22.9837398`,
  Longitude: `-43.2049451`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando a brincadeira de roda entre duas crianças,doação do escritor Paulo Coelho à cidade.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const PinheiroMachado_HildegardoLeaoVelloso: Obra = {
  ID: 247,
  Imagem: 'PinheiroMachado_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Pinheiro Machado`,
  Material: 'Bronze',
  DataInauguracao: `01/1931`,
  DatasImportantes: [new Data(`01/1931`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça Nossa Senhora da Paz',
  Latitude: `-22.9841828`,
  Longitude: `-43.2059628`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento erquido por uma iniciativa da Camara dos Deputados, Deputado Francisco Valladares.
    O monumento art-decor, tem a figura de Pinehiro Machado em tamanho natural, sobre uma tribuna tendo nas laterais do pedestal as figuras feminicas representando a Gloria e a Apoteose.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MulhereoFelino_Desconhecida: Obra = {
  ID: 248,
  Imagem: 'MulhereoFelino_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Mulher e o Felino`,
  Material: 'mármore de carrara',
  DatasImportantes: [new Data(`séc XX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Jardim de Alá',
  Latitude: `-22.982413`,
  Longitude: `-43.2140066`,
  Base: 'Com pedestal',
  MaterialBase: 'ganisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina apoiada em um cântaro sobre um fenino deitado ao chão.`,
  Fatos: [
    new Fact(
      '2011',
      '2011',
      'Tombamento',
      'Tombado pelo Município pelo Decreto 34 982',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AlmiranteSaldanhadaGama_AntonioCaringi: Obra = {
  ID: 249,
  Imagem: 'AlmiranteSaldanhadaGama_AntonioCaringi.jpeg',
  Authors: [authors.AntonioCaringi],
  Titulo: `Almirante Saldanha da Gama`,
  Material: 'Bronze',
  DataInauguracao: `04/1946`,
  DatasImportantes: [new Data(`04/1946`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Jardim de Alá',
  Latitude: `-22.9852252`,
  Longitude: `-43.2150875`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Composto por pedestal de granito, com escadarias, de frente para o mar, há um golfinho e a proa de um navio na parte de trás. Possui um obelisco, tendo ao lado direito um baixo-relevo em bronze figurando o navio Riachuelo, e em cima uma figura de mulher que representa a sabedoria. Do lado esquerdo outro baixo relevo o navio Paraíba e em cima outra figura feminina simboliza a Força.
    Na parte traseira do obelisco, foi esculpida uma frase de Rui Barbosa, uma roda de leme e o nome de Riachuelo. Entre o obelisco e a proa do navio, situa-se a estatua de Saldanha da Gama, em uniforme de gala, apoiando suas mãos na espada, em cima no obelisco o seu brazao.
    Omonumento foi erguido por ocasião do encerramento das solenidades comemorativas dos 127 anos da Batalha do Riachuelo, pelo Ministério da Marinha. Foi inaugurado por dois guardas marinhas. Na base tem inscito as seguintes palavras de Rui Barbosa:" Heoir ddos herois, o organizador possivel da nossa Marinha, homem mais completa e o carater mais extraordinario que ja conheci".`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const MarechalEuricoGasparDutra_DSantosLeal: Obra = {
  ID: 250,
  Authors: [authors.DSantosLeal],
  Titulo: `Marechal Eurico Gaspar Dutra`,
  Material: 'Bronze',
  DataInauguracao: `04/1972`,
  DatasImportantes: [new Data(`04/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Removida do espaço público',
  Descricao: `Inicialmente foi instalado na Rua Dr.Garnier em frente ao Regimento Mallet. A homenagem em busto de bronze ao ex-presidente partiu de antigos vizinhos seus da Rua Redentor. Foi inaugurado em 1972 no Jardim de Alá e transferido para o depósito devido as obras do Metrô em 2013.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ChafarizdasSaracuras_MestreValentim: Obra = {
  ID: 251,
  Imagem: 'ChafarizdasSaracuras_MestreValentim.jpeg',
  Authors: [authors.MestreValentim],
  Titulo: `Chafariz das Saracuras`,
  Material: 'bronze, granito e mármore',
  DataInauguracao: `01/1795`,
  DatasImportantes: [new Data(`01/1795`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça General Osório',
  Latitude: `-22.9846733`,
  Longitude: `-43.1972236`,
  Status: 'Permanece em espaço público',
  Descricao: `Esse Chafariz foi a última obra de civil de Mestre Valentim e uma encomenda das
    feiras carmelitas, para o abastecimento de água do Convento da Ajuda (no seu pátio interno).
    O Chafariz é lavrado em gnaisse, ornamentado com 4 saracuras e 4 tartarugas. Na piramide
    central uma cartela em mármore de lioz, apresenta as armas do vice-rei que ordenou a
    construção do Chafariz. Acima da piramide tem um cruz em bronze. Quando foi demolido o
    convento da Ajuda, o Chafariz foi desmontado e oferecido a Municipalidade, pelo cardeal
    arcebispo D Joaquim Arcoverde. Foi inaugurado em 13 de maio de 1912, na presença do general
    Bento Ribeiro, prefeito da Cidade, do Sr. Jeronino Coelho, diretor de Obras e de Julio Furtado,
    diretor de Matas e Jardins.mEsta desativado desde maio de 2017.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const GeneralOsorio_Desconhecida: Obra = {
  ID: 252,
  Imagem: 'GeneralOsorio_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `General Osório`,
  Material: 'Bronze',
  DataInauguracao: `05/2002`,
  DatasImportantes: [new Data(`05/2002`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça General Osório',
  Latitude: `-22.9846383`,
  Longitude: `-43.1979334`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Inaugurado no dia do nascimento de Osório, esse foi doado à Cidade pelo Exército
    Brasileiro, pelos generais da reserva do exército. Trata-se de uma réplica de um modelo do
    Arsenal de Guerra.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra - AGR',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1200',
      '1200',
      'Tombamento',
      'Tombado pelo Município pela Lei n 3089 de 05/09/00',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CardealJoaquimArcoverde_Desconhecida: Obra = {
  ID: 252,
  Imagem: 'CardealJoaquimArcoverde_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Cardeal Joaquim Arcoverde`,
  Material: 'Bronze',
  DataInauguracao: `1939`,
  DatasImportantes: [new Data(`1939`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça General Osório',
  Latitude: `-22.9210789`,
  Longitude: `-43.1788887`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};
export const Jaguar_JoaoTurin: Obra = {
  ID: 253,
  Imagem: 'Jaguar_JoaoTurin.jpeg',
  Authors: [authors.JoaoTurin],
  Titulo: `Jaguar`,
  Material: 'Bronze',
  DataInauguracao: `12/1948`,
  DatasImportantes: [new Data(`12/1948`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Praça General Osório',
  Latitude: `-22.9846733`,
  Longitude: `-43.1972236`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Em 1944, João Turin recebeu no Salão Nacional do Rio de Janeiro com a escultura
    " Tigre Esmagando a Cobra" uma Medalha de Prata. O artista vendeu as duas obras à Prefeitura
    do Rio de Janeiro. Ambas foram colocadas, em espaços públicos da cidade. Instalado
    primeiramente na Praça General Osório foi transferido para a Praça Nossa Senhora da Paz em
    1987, quando da realização das obras do Metrô. Retornando ao local de origem em 1994.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Acoplamento_MarioCravo: Obra = {
  ID: 254,
  Imagem: 'Acoplamento_MarioCravo.jpeg',
  Authors: [authors.MarioCravo],
  Titulo: `Acoplamento`,
  Material: 'ferro',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça doada à Prefeitura do Rio de Janeiro pela companhia Cerveja Brahma.
    Escultura de grande dimensão, composta por duas esferas, unidas em seu centro. Ambas são
    pintadas na cor amarela queimada na área externa e de preto na interna. Esta fixada
    diretamente no piso, provocando a sensação que esta prestes a rolar.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Evolucao_MelindaGarcia: Obra = {
  ID: 255,
  Imagem: 'Evolucao_MelindaGarcia.jpeg',
  Authors: [authors.MelindaGarcia],
  Titulo: `Evolução`,
  Material: 'ferro',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça composta por peças semelhantes na forma. Os espeços vazados entre as menores e as maiores demonstram formas diferentes.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Essa peça foi doada à Prefeitura pela Bolsa de Valores do Rio de Janeiro.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Maternidade_RomeoAlves: Obra = {
  ID: 256,
  Imagem: 'Maternidade_RomeoAlves.jpeg',
  Authors: [authors.RomeoAlves],
  Titulo: `Maternidade`,
  Material: 'granito',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9722059`,
  Longitude: `-43.2034528`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça formando um conjunto de duas figuras de diferentes tamanhos, entrelaçadas como uma mãe e o filho.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'A peça foi doada à Cidade do Rio de Janeiro por Emillio Gianelli da empresa EGESA.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const PrincipedosBosques_RemoBernucci: Obra = {
  ID: 257,
  Imagem: 'PrincipedosBosques_RemoBernucci.jpeg',
  Authors: [authors.RemoBernucci],
  Titulo: `Príncipe dos Bosques`,
  Material: 'argamassa',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Figura abstrata num desenvolvimento retilíneo com cerca de três metros de altura.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      ' A peça foi doado à prefeitura da Cidade do Rio de Janeiro pela empresa Carioca de Engenharia S.A./ ou Sociedade Amigos da LAgoa',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Iansa_TatiMoreno: Obra = {
  ID: 258,
  Imagem: 'Iansa_TatiMoreno.jpeg',
  Authors: [authors.TatiMoreno],
  Titulo: `Iansã`,
  Material: 'latão',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Foto: Guito Moreto// Escultura em latão na cor tipica do Orixá, representada com sua vestimenta tipica,
    adereços e postura na incorporação. A localização entre pedras e densa vegetação hamoniza a
    peça ao ambiente.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'A peça doada à Prefeitura pela Ecisa Engenharia, Comércio e Indústria Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarcosTamoio_FloryGama: Obra = {
  ID: 259,
  Imagem: 'MarcosTamoio_FloryGama.jpeg',
  Authors: [authors.FloryGama],
  Titulo: `Marcos Tamoio`,
  Material: 'Bronze',
  DataInauguracao: `09/1981`,
  DatasImportantes: [new Data(`09/1981`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.972386`,
  Longitude: `-43.2029359`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem na entrada do parque, foi realizada pela Prefeitura da Cidade, para imortalizar aquele que criou o
    parque das esculturas carioca. Foi inaugurado com a presença do ex-governador Faria Lima, e do vice presidente da camara dos
    veadores Moacir Bastos, que coordenpu a campanha para erguer essa homenadem`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const EstruturaEmDiagonal_FranzWeissmann: Obra = {
  ID: 260,
  Imagem: 'EstruturaEmDiagonal_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Estrutura em diagonal`,
  Material: 'aço pintado',
  Altura: '300 cm',
  Largura: '300 cm',
  Profundidade: '300 cm',
  Peso: '2000 kg',
  DataInauguracao: `03/1978`,
  DatasImportantes: [new Data(`03/1978`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Essa peça na cor vermelho foi doada à Prefeitura pela Construtora Andrade Gutiérrez.
    Escultura formada por diversas chapas, todas em cortes diagonais, formando no espaço linhas cortas em vários angulos quando da visão do admirador.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doador',
      'Construtora Andrade Gutierrez',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Compressao_FranciscoStockinger: Obra = {
  ID: 261,
  Imagem: 'Compressao_FranciscoStockinger.jpeg',
  Authors: [authors.FranciscoStockinger],
  Titulo: `Compressão`,
  Material: 'granito',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura composta por dois elementos, um formado por dois blocos maciços de pedra bruta, e
    outro que surge entre os blocos formado tubos de aço que ultrapassam os blocos. A obra de materiais brutos,
    sem tratamento, que expõe uma ação ou ato da compreensão.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Esta peça foi doada à Prefeitura da Cidade do Rio de Janeiro pela Sul-América Seguros',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Oxossi_Caribe: Obra = {
  ID: 262,
  Imagem: 'Oxossi_Caribe.jpeg',
  Authors: [authors.Caribe],
  Titulo: `Oxossi`,
  Material: 'concreto',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Confeccionada em um mural, apresenta em baixo relevo a figura de Oxossi com toda a vestimenta, armas e a simbologia do Orixa. Com cerca de três metros a peça induz a grandiosidade.',
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Esta foi doada a Prefeitura da Cidade pela Coca-Cola Indústria S.A.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Construcao_BrunoGiorgi: Obra = {
  ID: 263,
  Imagem: 'Construcao_BrunoGiorgi.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: `Construção`,
  Material: 'Bronze',
  Peso: '800 kg',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura formando um só conjunto de vazios e curvos. Escultura composta por quatro peças,
    todas em formas retangulares e retorcidas, delgadas e verticiais, diferentes entre si, que colocadas umas 
    sobre as outras formando no espaço vazios e curvas. A fixação de uma sobre a outra, elabora uma montagem,
    onde todos unidos formam o conjunto`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Foi doada à Prefeitura da Cidade pela Companhia Atlântica-Boavista de Seguros',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const CosmoI_MariaGuilherminaFernandes: Obra = {
  ID: 264,
  Imagem: 'CosmoI_MariaGuilherminaFernandes.jpeg',
  Authors: [authors.MariaGuilherminaFernandes],
  Titulo: `Cosmo I`,
  Material: 'pedra sabão',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A peça foi doada pelo Sindicato Nacional da Indústria de Construção Naval.
    Escultura que trabalha a partir de duas formas básicas unidas. A peça alonga-se com cerca de três metros, permitindo diferentes angulos a visão das formas em diferentes dimensões.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'A peça foi doada pelo Sindicato Nacional da Indústria de Construção Naval',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const HomemSentado_MarioAgostinelli: Obra = {
  ID: 265,
  Imagem: 'HomemSentado_MarioAgostinelli.jpeg',
  Authors: [authors.MarioAgostinelli],
  Titulo: `Homem Sentado`,
  Material: 'Bronze',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura figurativa em tamanho natural, sentada.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doado pela Empresa Carioca de Engenharia Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Sofia_SoniaEbling: Obra = {
  ID: 266,
  Imagem: 'Sofia_SoniaEbling.jpeg',
  Authors: [authors.SoniaEbling],
  Titulo: `Sofia`,
  Material: 'Bronze',
  DataInauguracao: `1951`,
  DatasImportantes: [
    new Data(`1951`, 'data de inauguração - arte ambiente'),
    new Data(`03/1979`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando uma figura feminina de formas tipicamente brasileira.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'A peça foi doada à Cidade do Rio de Janeiro pelo Jockey Club Brasileiro.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '1957',
      'Criaçãoe',
      'Data de criação da obra',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Incerteza_Marmura: Obra = {
  ID: 267,
  Imagem: 'Incerteza_Marmura.jpeg',
  Authors: [authors.Marmura],
  Titulo: `Incerteza`,
  Material: 'Bronze',
  DataInauguracao: `1954`,
  DatasImportantes: [
    new Data(`1954`, 'data de inauguração - arte ambiente'),
    new Data(`03/1979`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura apresentando m figura humana sentada numa postura de recolhimento, com o olhar baixo e encolhida.`,
  Fatos: [
    new Fact(
      '1954',
      '1954',
      'Criação',
      'Obra datada de 1954',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'Doada à Cidade do Rio de Janeiro pela Empresa João Fortes Engenharia.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Prometeu_EvandroCarneiro: Obra = {
  ID: 268,
  Imagem: 'Prometeu_EvandroCarneiro.jpeg',
  Authors: [authors.EvandroCarneiro],
  Titulo: `Prometeu`,
  Material: 'Bronze',
  Altura: '105 cm',
  Largura: '125 cm',
  Profundidade: '42 cm',
  DataInauguracao: `1958`,
  DatasImportantes: [
    new Data(`1958`, 'data de inauguração - arte ambiente'),
    new Data(`01/1988`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura formada por três elementos com contornos de braço, perna e asa, todas orientadas na mesma direção.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'Essa peça foi doada à Cidade pela construtora Gávea Ltda e pela Bolsa de Arte do Rio de Janeiro.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Encontro_EdgarDuvivier: Obra = {
  ID: 269,
  Imagem: 'Encontro_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `Encontro`,
  Material: 'ferro',
  DataInauguracao: `01/1979`,
  DatasImportantes: [new Data(`04/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Está peça foi doada à Prefeitura pelo próprio escultor. Escultura em chapa
    recortada em diversas linhas composta por três elementos unidos. A peça forma uma estrutura
    leve e esbelta revelando um delicadeza.`,
  Fatos: [
    new Fact('', '1960', 'Criação', 'Data da obra', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Doação',
      'Doada pelo Edgar Duvivier',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const StuartAngel_Desconhecida: Obra = {
  ID: 269,
  Imagem: 'StuartAngel_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Stuart Angel`,
  Material: 'ferro',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Avenida Pasteur, 220',
  Latitude: `-22.9513578`,
  Longitude: `-43.1750967`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '1960', 'Criação', 'Data da obra', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Busto',
  Execucao: true,
};

export const EvangelistaMateus_AlfredoCeschiatti: Obra = {
  ID: 270,
  Imagem: 'EvangelistaMateus_AlfredoCeschiatti.jpeg',
  Authors: [authors.AlfredoCeschiatti],
  Titulo: `Evangelista Mateus`,
  Material: 'argamassa',
  Altura: '300 cm ',
  DataInauguracao: `1968`,
  DatasImportantes: [
    new Data(`1968`, 'data de inauguração - arte ambiente'),
    new Data(`03/1979`, 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura pode ser o estudo do Authors: para a obra instalada em entrada da Catedral
    de Brasilia em bronze. Escultura de uma figura humana de corpo inteiro em posição ereta,
    personalizando a grandeza, a correção e a firmeza.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doado pela Cotepa Engenharia Ltda.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1968',
      '',
      'Criação',
      'Obra criada em 1968',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Estrutura_SergioCamargo: Obra = {
  ID: 271,
  Imagem: 'Estrutura_SergioCamargo.jpeg',
  Authors: [authors.SergioCamargo],
  Titulo: `Estrutura`,
  Material: 'mármore',
  DataInauguracao: `1973`,
  DatasImportantes: [
    new Data(`1973`, 'data de inauguração - arte ambiente'),
    new Data(`03/1979`, 'data de inauguração - inventario dos ambientes'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça composta por 10 blocos maciços de mármore, que sofreram cotes diferentes no seu topo. O encaixe forma uma estrutura alongada em movimento.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'A obra foi doada à Prefeitura pela Companhia de Comércio e Navegação',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '1973',
      'Criação',
      'A obra datada de 1973',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const IndioLutandocomoFelino_HippolyteFrancoisAugusteHippolytePeyrol: Obra =
  {
    ID: 272,
    Imagem:
      'IndioLutandocomoFelino_HippolyteFrancoisAugusteHippolytePeyrol.jpeg',
    Authors: [authors.HippolyteFrancoisAugusteHippolytePeyrol],
    Titulo: `Índio Lutando com o Felino`,
    Material: 'mármore de carrara',
    DataInauguracao: `01/1906`,
    DatasImportantes: [new Data(`01/1906`, 'data de inauguração')],
    Propriedade: 'Pública',
    Zona: 'Sul',
    Bairro: 'Lagoa',
    Endereco: 'Parque da Catacumba',
    Latitude: `-22.9725107`,
    Longitude: `-43.2028513`,
    Base: 'Sem pedestal',
    Status: 'Permanece em espaço público',
    Descricao: `Inaugurada em 1906 na Rua da Glória e em 1979 na atual localização no parque da Catacumba.
    Esta obra em mármore de Carrara foi adquirida durante a administração do Prefeito Pereira Passos.
    A escultura representa a figura do homem em luta com um tigre. Peça de padrão neoclássico com intensa expressão de força e movimento.`,
    Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
    Temporalidade: 'Permanente',
    Categoria: 'Sublimidade',
    Funcao: 'Decorativa',
    Natureza: 'Artística',
    Tipologia: 'Escultura',
    Execucao: true,
  };

export const AoPolicialMilitar_Desconhecida: Obra = {
  ID: 903,
  Imagem: 'AoPolicialMilitar_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Ao Policial Militar`,
  Renomeacao: false,
  Classificacao: 'perene',
  Categoria: 'personalidades',
  Tipologia: `Estátua`,
  Material: `Peça em bronze com pedestal revestido de granito`,
  Descricao: `Inaugurado na década de 1990 e reinaugurado em 2006 no calçadão da Penha.
A homenagem foi uma iniciativa dos moradores da Leopoldina aos soldados do 16º Batalhão da Policia Militar que morreram em combate.`,
  Execucao: true,
  DataInauguracao: `1990`,
  Latitude: `-22.8422422`,
  Longitude: `-43.2778349`,
  Remocao: true,
  DataRealocacao: '2006',
  Realocacao: true,
  ZonaRealocacao: 'Norte',
  BairroRealocacao: 'Penha',
  EnderecoRealocacao: 'Av. Bras de Pina',
  LatitudeRealocacao: '-22.8421743',
  LongitudeRealocacao: '-43.2778422',
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
};

export const SanMartin_LouisJosephDaumas: Obra = {
  ID: 273,
  Imagem: 'SanMartin_LouisJosephDaumas.jpeg',
  Authors: [authors.LouisJosephDaumas],
  Titulo: `San Martin`,
  Material: 'Bronze',
  DataInauguracao: `03/1972`,
  DatasImportantes: [new Data(`03/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9752392`,
  Longitude: `-43.200364`,
  Base: 'Com pedestal',
  MaterialBase: 'mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Foi uma oferta do Governo da Argentina ao Governo do Estado da Guanabara, doada por um grupo de Oficiais da Armada Argentina.
    Monumento equestre de grandes proporções, com o Authors: da independência da Argentina, Chile e Peru, sobre um cavalo empinado e apontado para o oeste, direção tomada pelo "libertador" após o início da marcha sobre os espanhóis.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JoaoPessoa_SamuelMartinsRibeiro: Obra = {
  ID: 274,
  Imagem: 'JoaoPessoa_SamuelMartinsRibeiro.jpeg',
  Authors: [authors.SamuelMartinsRibeiro],
  Titulo: `João Person`,
  Material: 'Bronze',
  DataInauguracao: `07/1937`,
  DatasImportantes: [new Data(`07/1937`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9767349`,
  Longitude: `-43.1993368`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Inicialmente o Monumento foi instalado na Praia de Botafogo, entre as Ruas Voluntários da Patria e São Clemente. Quando foi construído o viaduto, ligando a praia de Botafogo a Rua da Passagem, a estátua foi transferida para o Largo Indio do Brasil. Foi novamente transferido para a Avenida Epitácio Person a pedido da família, devido a constante vandalismo, no ano de 2000. A homenagem foi uma iniciativa de uma Comissão de Antigos Membros da Aliança Liberal.
    A estátua apresenta a figura de João Person de pé, ereto, olhar firme, com as mãos para trás, atitude que lhe era peculiar.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const LagoaSolidaria_MarliMazeredo: Obra = {
  ID: 275,
  Imagem: 'LagoaSolidaria_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Lagoa Solidária`,
  Material: 'resina',
  Altura: '300 cm',
  DataInauguracao: `09/2000`,
  DatasImportantes: [new Data(`09/2000`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Lagoa Rodrigo de Freitas',
  Latitude: `-22.9767754`,
  Longitude: `-43.2191189`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando o mapa da Lagoa, em forma de coração, com uma figura feminina junto a uma criança, em
    gesto de proteção. A escultura é uma menção a campanha da Lago Solidária criada e coordenada pela Sra. Ângela Fragoso Pires
    e teve o apoio e a colaboração dos clubes da orla da Lagoa. No decorrer de três anos, a campanha arrecadou doações, que foram
    repassadas para instituições de caridade. Para o ano de 2000, a escultora instalou na Av. Mario Ribeiro, o símbolo da campanha,
    peça ampliada do troféu, marcando no espaço público a campanha de solidariedade.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Curumim_PedroCorreiaDeAraujo: Obra = {
  ID: 276,
  Imagem: 'Curumim_PedroCorreiaDeAraujo.jpeg',
  Authors: [authors.PedroCorreiaDeAraujo],
  Titulo: `Curumim`,
  Material: 'Bronze',
  DataInauguracao: `03/1979`,
  DatasImportantes: [new Data(`03/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Avenida Borges de Medeiros',
  Latitude: `-22.9632571`,
  Longitude: `-43.2097131`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representa a figura de um índio segurando um arco na mão esquerda e com a direita na posição de quem
    atirou a flecha num peixe da Lagoa. A escultura foi inicialmente instalada numa pedra na borda da Lagoa. Em 2 de setembro de 2011, foi
    reinstalada numa pedra dentro da lagoa, a pedido do Authors:, devido aos constantes vandalismo e danos que a escultura sofreu
    ao longo dos anos.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'A escultura foi doada á cidade pela ESUSA, empresa que executou as obras de urbanização e saneamento da Lagoa.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ATomJobimOEstudante_OttoDumovich_OttoDumovich: Obra = {
  ID: 277,
  Imagem: 'ATomJobimOEstudante_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `A Tom Jobim O Estudante`,
  Material: 'Bronze',
  DataInauguracao: `02/2002`,
  DatasImportantes: [new Data(`02/2002`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Parque Lage',
  Latitude: `-22.9602718`,
  Longitude: `-43.2120361`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça representando um jovem estudante sentado no banco do parque, com uma prancha na mão, desenhando a cena que Tom Jobim
    ao lado de seu filho planta uma palmeira no parque. Em 2002 após a atuação da Prefeitura no parque Lage, para a recuperação espaço com
    base do convênio entre a Prefeitura da Cidade e o Parque Nacional da Tijuca. Foi instalada essa homeagem à Tom Jobim, lembrando sua campanha
    na defesa do parque. O monumento permanece no parque por uma cessão de uso.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const OttoLaraResende_JoasPereiraDosPassos: Obra = {
  ID: 278,
  Imagem: 'OttoLaraResende_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Otto Lara Resende`,
  Material: 'Bronze',
  DataInauguracao: `12/2002`,
  DatasImportantes: [new Data(`12/2002`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Espaço Otto Lara Resende',
  Latitude: `-22.9662835`,
  Longitude: `-43.2193151`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A iniciativa partiu da família e da Secretária Municipal de Comunicação, a jornalista Agata Messina.
    O grupo escultórico é composto por quatro elementos: aestátua de Otto Lara Resende, no segundo plano uma mesa de escritor, com
    uma folha em bronze com texto e frases do jornalista e um aparelho de telefone. Ao lado da mesa tem-se uma cadeira e ao fundo
    tem-se um painel em laminado vazado para se ter ao fundo uma paisagem as alamedas de palmeiras do Jardim Botânico.// Figura notável no meio intelectual, o jornalista e escritor Otto Lara Resende teve grande atividade jornalistica em diversos periódicos como O Globo, Folha de São Paulo, Jornal do Brasil, Manchete, Diário Carioca, Correio da Manhã. Amigo de diversos escritores foi eleito membro da Academia Brasileira de Letras. `,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Artística RioArte',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo Escultórico',
  Execucao: true,
};

export const AnjoComPeixe_AndreaVerrocchio: Obra = {
  ID: 279,
  Authors: [authors.AndreaVerrocchio],
  Titulo: `Anjo com Peixe`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ADanca_AlicePittaluga: Obra = {
  ID: 280,
  Imagem: 'ADanca_AlicePittaluga.jpeg',
  Authors: [authors.AlicePittaluga],
  Titulo: `A Dança`,
  Material: 'argamassa',
  DatasImportantes: [new Data(`sec XX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Ossanha_TatiMoreno: Obra = {
  ID: 281,
  Imagem: 'Ossanha_TatiMoreno.jpeg',
  Authors: [authors.TatiMoreno],
  Titulo: `Ossanha`,
  Material: 'resina',
  DatasImportantes: [new Data(`séc XX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Narciso_MestreValentim: Obra = {
  ID: 282,
  Authors: [authors.MestreValentim],
  Titulo: `Narciso`,
  Material: 'chumbo e estanho',
  DataInauguracao: `1783`,
  DatasImportantes: [new Data(`1783`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: ``,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SaintHilaire_HumbertoCozzo: Obra = {
  ID: 283,
  Imagem: 'SaintHilaire_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: `Saint Hilaire`,
  Material: 'Bronze',
  DataInauguracao: `1900`,
  DatasImportantes: [new Data(`1900`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Jardim Botânico',
  Latitude: `-22.968731`,
  Longitude: `-43.2223012`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MedidadoTempo_FransKrajcberg: Obra = {
  ID: 284,
  Imagem: 'MedidadoTempo_FransKrajcberg.jpeg',
  Authors: [authors.FransKrajcberg],
  Titulo: `Medida do Tempo`,
  Material: 'cipó de madeira',
  DataInauguracao: `08/1986`,
  DatasImportantes: [new Data(`08/1986`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A obra foi adquirida pela Prefeitura da Cidade do Rio de Janeiro.
    Escultura realizada com tramas de cipós da Amazonia. O trabalho desenvolvido teve como
    inspiração o antigo instrumento que medir o tempo, a ampulheta.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Guardiao_CeleidaTostes: Obra = {
  ID: 285,
  Imagem: 'Guardiao_CeleidaTostes.jpeg',
  Authors: [authors.CeleidaTostes],
  Titulo: `Guardião`,
  Material: 'argamassa',
  DataInauguracao: `04/1986`,
  DatasImportantes: [
    new Data(`04/1986`, 'data de inauguração - arte ambiente'),
    new Data(`04/1988`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Peça doada à Cidade do Rio de Janeiro pela Empresa Senge Serviços de Engenharia S.A.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FrutodoEspaco_AntonioManuelOliveira: Obra = {
  ID: 286,
  Imagem: 'FrutodoEspaco_AntonioManuelOliveira.jpeg',
  Authors: [authors.AntonioManuelOliveira],
  Titulo: `Fruto do Espaço`,
  Material: 'ferro',
  DataInauguracao: `01/1980`,
  DatasImportantes: [
    new Data(`01/1980`, 'data de inauguração - inventario dos monumentos'),
    new Data(`1983`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura na qual figura as letras H.O iniciais do homenageado Hélio Oiticica. O monumento foi exposto em 1980 no Parque da Catacumba,
    e adquirido pela Prefeitura no ano seguinte.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Jacare2000_MariaHelenaVasco: Obra = {
  ID: 287,
  Imagem: 'Jacare2000_MariaHelenaVasco.jpeg',
  Authors: [authors.MariaHelenaVasco],
  Titulo: `Jacare 2000`,
  Material: 'ferro',
  DataInauguracao: `1997`,
  DatasImportantes: [new Data(`1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const RaizesAmazonas_FernandoCasas: Obra = {
  ID: 283,
  Imagem: 'RaizesAmazonas_FernandoCasas.jpeg',
  Authors: [authors.FernandoCasas],
  Titulo: `Raízes - Amazonas`,
  Material: 'mármore',
  DataInauguracao: `01/1990`,
  DatasImportantes: [new Data(`01/1990`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de duas peças em mármore esculpida, sem forma geométrica definida e sem superficie regular, criando
    uma peça bruta natural. A peça sem pedestal integra ao solo como se dele tivesse sido retirada.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Nijinsky_MarliMazeredo: Obra = {
  ID: 289,
  Imagem: 'Nijinsky_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Nijinsky`,
  Material: 'Bronze',
  Altura: '400 cm',
  DataInauguracao: `11/1992`,
  DatasImportantes: [new Data(`11/1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9723448`,
  Longitude: `-43.2028562`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua estilizada do homenageado, doada à cidade pela empresa Prismart. A peça de corpo inteiro tem as pernas
    unidas em uma.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Doada à cidade pela empresa Prismart',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const DianaaCacadora_HeloisaDolabella: Obra = {
  ID: 290,
  Imagem: 'DianaaCacadora_HeloisaDolabella.jpeg',
  Authors: [authors.HeloisaDolabella],
  Titulo: `Diana a Caçadora`,
  Material: 'Bronze',
  DataInauguracao: `01/1987`,
  DatasImportantes: [new Data(`01/1987`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Figura humana abstrata com movimento.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Foi doada à Prefeitura pela empresa P. Marchette Engenharia.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Lua_JoseRaulAllegretti: Obra = {
  ID: 291,
  Imagem: 'Lua_JoseRaulAllegretti.jpeg',
  Authors: [authors.JoseRaulAllegretti],
  Titulo: `Lua`,
  Material: 'aço e alumínio',
  DataInauguracao: `01/1985`,
  DatasImportantes: [new Data(`01/1985`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Em aço inoxidavel de forma esférica apoiada em um poste de aço pintado. Funciona como um mobile.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'A obra doada à Prefeitura pela Embratel.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Flora_MarioCravo: Obra = {
  ID: 292,
  Imagem: 'Flora_MarioCravo.jpeg',
  Authors: [authors.MarioCravo],
  Titulo: `Flora`,
  Material: 'ferro pintado',
  DataInauguracao: `04/1988`,
  DatasImportantes: [
    new Data(`04/1988`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de peças finas com cortes ora curvos ora retos, criando dobras e vazados. É pintada na cor amarelo ouro na
    faixa externa e marrom conhaque nos trechos internos.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Peça doada pela L. O. Graca Engenharia Ltda. à Cidade do Rio de Janeiro.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Atleta_RemoBernucci: Obra = {
  ID: 293,
  Imagem: 'Atleta_RemoBernucci.jpeg',
  Authors: [authors.RemoBernucci],
  Titulo: `Atleta`,
  Material: 'Bronze',
  DataInauguracao: `01/1987`,
  DatasImportantes: [new Data(`01/1987`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque da Catacumba',
  Latitude: `-22.9725107`,
  Longitude: `-43.2028513`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura adquirida ao Authors: pelo Sr. Emilio Alfredo Giannelli e doada à Prefeitura.
    Figura humana abstrata com movimento leve.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AVictorAssisBrasil_Desconhecida: Obra = {
  ID: 294,
  Imagem: 'AVictorAssisBrasil_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Victor Assis Brasil`,
  Material: 'Bronze',
  DataInauguracao: `2003`,
  DatasImportantes: [new Data(`2003`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Parque dos Patins',
  Latitude: '-22.9722713',
  Longitude: '-43.2186377',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const RitualdoVento_GabrielFonseca: Obra = {
  ID: 295,
  Imagem: 'RitualdoVento_GabrielFonseca.jpeg',
  Authors: [authors.GabrielFonseca],
  Titulo: `Ritual do Vento`,
  Material: 'aço e alumínio',
  DataInauguracao: '1999',
  DatasImportantes: [new Data('1999', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Praça José Mariano Filho',
  Latitude: '-22.9626826',
  Longitude: '-43.2045609',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Cinco peças nas cores amarelo, verde, azul, vermelho e laranja que giram com a força do vento.  As esculturas  apresentam a leveza como em vôo. Esta obra foi exposta inicialmente na Av. Rio Branco 1, conhecida como RB1, em 1999, e posteriormente doada à Cidade do Rio de Janeiro pelo próprio Authors:.',
  Fatos: [
    new Fact('', '', 'transferida da avenida rio branco, n 1', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AVieiraSoutoePauloDeFrontin_RodolfoBernardelli: Obra = {
  ID: 296,
  Imagem: 'AVieiraSoutoePauloDeFrontin_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `À Vieira Souto e Paulo de Frontin`,
  Material: 'Bronze',
  DataInauguracao: `08/1949`,
  DatasImportantes: [new Data(`08/1949`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Lagoa',
  Endereco: 'Avenida Delfim Moreira',
  Latitude: '-22.9844815',
  Longitude: '-43.218036',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `No centenário do nascimento de Vieira Souto, o Senado Federal, a Escola de
    Engenharia, o Clube de Engenharia, a Associação dos Antigos Alunos da Politécnica e a
    Prefeitura do Distrito Federal, inaugurou o monumento nas proximidades do nº 46 da avenida
    que leva o nome. Em 1992, na reforma da urbanização da Av. Vieira Souto, o marco foi deslocado,
    para o canteiro central próximo ao Jardim de Ala.`,
  Fatos: [
    new Fact(
      '',
      '1992',
      'removido para o canteiro central do Jardim de Ala',
      '',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const JozefPilsudski_Farynkiewicz: Obra = {
  ID: 297,
  Imagem: 'JozefPilsudski_Farynkiewicz.jpeg',
  Authors: [authors.FArynkiewicz],
  Titulo: `Jozef Pilsudski`,
  Material: 'Bronze',
  DataInauguracao: `1937`,
  DatasImportantes: [
    new Data(`1937`, 'data de inauguração - inventário dos monumentos'),
    new Data(`12/1967`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Avenida Vieira Souto',
  Latitude: `-22.9869301`,
  Longitude: `-43.1988272`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem ao herói nacional e fundador da Polônia, doado à Cidade do Rio de Janeiro, pela Associação dos Ex-Comabatentes Polonesas, representado pelo Sr. Janus Pawiewski, como um simbolo eterno de liberdade.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const RubenDario_DanielCrow_CurzioZani: Obra = {
  ID: 298,
  Imagem: 'RubenDario_DanielCrow_CurzioZani.jpeg',
  Authors: [authors.DanielCrow, authors.CurzioZani],
  Titulo: `Rubén Dario`,
  Material: 'Bronze',
  DataInauguracao: `01/1956`,
  DatasImportantes: [new Data(`01/1956`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Avenida Vieira Souto',
  Latitude: `-22.9885742`,
  Longitude: `-43.2280441`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento foi doado pelo Emabaixador da Nicarágua no Brasil, Sr. Sanson Balladares erealizado pela Prefeitura da Cidade do Rio de Janeiro para homengear o poeta ibero - americano. O monumento tem ainscrição: A.L.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const Trio_JoseCesarBranquinho: Obra = {
  ID: 299,
  Imagem: 'Trio_JoseCesarBranquinho.jpeg',
  Authors: [authors.JoseCesarBranquinho],
  Titulo: `Trio`,
  Material: 'Aço',
  DataInauguracao: `10/2000`,
  DatasImportantes: [new Data(`10/2000`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Parque Dois Irmãos',
  Latitude: `-22.989442`,
  Longitude: `-43.2287399`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em ferro (vergalhão) pintada, composto por três peças figurando a silhueta humana.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FormasnoEspacoI_OscarNiemeyer: Obra = {
  ID: 301,
  Imagem: 'FormasnoEspacoI_OscarNiemeyer.jpeg',
  Authors: [authors.OscarNiemeyer],
  Titulo: `Formas no Espaço I`,
  Material: 'ferro pintado',
  DataInauguracao: `1998`,
  DatasImportantes: [
    new Data(`1998`, 'data de inauguração - arte cidade'),
    new Data(`03/2002`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `No ano de 2000 a escultura de metal, foi exposta temporariamente na praia do Leme,
    numa iniciativa da Secretaria Municipal de Meio Ambiente. Em 2002 foi doada a Prefeitura e,
    inaugurada em caráter definitivo as esculturas no Parque Dois Irmãos. Em junho de 2011 foi
    transferido do parque devido a estado de degradação. Em ferro recortado recebe uma pintura
    vermelha para destacar na paisagem.`,
  Fatos: [
    new Fact(
      '2000',
      '2000',
      'Exposição',
      'A obra foi exposta temporariamente na praia do Leme, por iniciativa da Secretaria Municipal de Meio Ambiente',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2002',
      '2002',
      'Doação',
      'Obra foi doada à cidade',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FormasnoEspacoII_OscarNiemeyer: Obra = {
  ID: 302,
  Imagem: 'FormasnoEspacoII_OscarNiemeyer.jpeg',
  Authors: [authors.OscarNiemeyer],
  Titulo: `Formas no Espaço II`,
  Material: 'ferro pintado',
  DataInauguracao: `2000`,
  DatasImportantes: [
    new Data(`2000`, 'data de inauguração - arte cidade'),
    new Data(`03/2002`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `No ano de 2000 a escultura de metal, foi exposta temporariamente na praia do Leme,
    numa iniciativa da Secretaria Municipal de Meio Ambiente. Em 2002 foi doada a Prefeitura e,
    inaugurada em caráter definitivo as esculturas no Parque Dois Irmãos. Em junho de 2011 foi
    transferido do parque devido a estado de degradação.
    Em ferro recortado recebe uma pintura vermelha para destacar na paisagem.`,
  Fatos: [
    new Fact(
      '2000',
      '2000',
      'Exposição',
      'A obra foi exposta temporariamente na praia do Leme, por iniciativa da Secretaria Municipal de Meio Ambiente',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2002',
      '2002',
      'Doação',
      'Obra foi doada à cidade',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Violencia_OscarNiemeyer: Obra = {
  ID: 303,
  Authors: [authors.OscarNiemeyer],
  Titulo: `Violência`,
  Material: 'ferro pintado',
  DataInauguracao: `1999`,
  DatasImportantes: [
    new Data(`1999`, 'data de inauguração - arte cidade'),
    new Data(`03/2002`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `No ano de 2000 a escultura de metal, foi exposta temporariamente na praia do Leme,
    numa iniciativa da Secretaria Municipal de Meio Ambiente. Em 2002 foi doada a Prefeitura e,
    inaugurada em caráter definitivo as esculturas no Parque Dois Irmãos. Em junho de 2011 foi
    transferido do parque devido a estado de degradação.
    Em ferro recortado recebe uma pintura vermelha para destacar na paisagem.`,
  Fatos: [
    new Fact(
      '2000',
      '2000',
      'Exposição',
      'A obra foi exposta temporariamente na praia do Leme, por iniciativa da Secretaria Municipal de Meio Ambiente',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2002',
      '2002',
      'Doação',
      'Obra foi doada à cidade',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Mulher_OscarNiemeyer: Obra = {
  ID: 304,
  Authors: [authors.OscarNiemeyer],
  Titulo: `Mulher`,
  Material: 'ferro pintado',
  DataInauguracao: `2000`,
  DatasImportantes: [
    new Data(`2000`, 'data de inauguração - arte cidade'),
    new Data(`03/2002`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Sem pedestal',
  Status: 'Removida do espaço público',
  Descricao: `No ano de 2000 a escultura de metal, foi exposta temporariamente na praia do Leme,
    numa iniciativa da Secretaria Municipal de Meio Ambiente. Em 2002 foi doada a Prefeitura e,
    inaugurada em caráter definitivo as esculturas no Parque Dois Irmãos. Em junho de 2011 foi
    transferido do parque devido a estado de degradação.
    Em ferro recortado recebe uma pintura vermelha para destacar na paisagem.`,
  Fatos: [
    new Fact(
      '2000',
      '2000',
      'Exposição',
      'A obra foi exposta temporariamente na praia do Leme, por iniciativa da Secretaria Municipal de Meio Ambiente',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2002',
      '2002',
      'Doação',
      'Obra foi doada à cidade',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const OthonXavierdeBrito_RuffoFanucchi: Obra = {
  ID: 305,
  Imagem: 'OthonXavierdeBrito_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: `Othon Xavier de Brito`,
  Material: 'Bronze',
  DataInauguracao: `03/1957`,
  DatasImportantes: [new Data(`03/1957`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Praça Atahulpa',
  Latitude: `-22.9882698`,
  Longitude: `-43.2279353`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem partiu de seua amigos, discípulos e admiradores, através de uma
    comissão de honra e de uma comissão executiva. O Marechal Rondon foi o presidente da comissão
    que ergueu o monumento. O monumento e um monolitoem granito com uma placa grande vertical em
    alto relevo representando o Dr.º Othon Xavier.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const MiguelCouto_HeitorUsai: Obra = {
  ID: 306,
  Imagem: 'MiguelCouto_HeitorUsai.jpeg',
  Authors: [authors.HeitorUsai],
  Titulo: `Miguel Couto`,
  Material: 'Bronze',
  DataInauguracao: `09/1944`,
  DatasImportantes: [new Data(`09/1944`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Praça Nossa Senhora Auxiliadora',
  Latitude: `-22.9778508`,
  Longitude: `-43.2229555`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento, foi erguido no jardim da Praia de Botafogo, em frente a Rua Marques
    de Olinda, onde viveu por muitos anos.A iniciativa da homenagem pertence aos colegas,
    discípulos e amigos. Constituída uma comissão para levar a efetivo a construção de um
    monumento por subscrição popular, chefiada pelos professores David Speroni e Gregório Araoz
    Afaro. A comissão executiva da homenagem estava assim constituída: professor Aloísio de Castro,
    presidente da Academia Nacional de Medicina; professor J. Moreira da Fonseca, da Faculdade
    Nacional, sr. Levi Carneiro e Mario de Andrade Ramos e dr. Carlotta Pereira de Queiroz. Ainda 
    faziam parte da comissão os professores Carlos Chagas, Fernando Magalhães e Helion Povoa, 
    ministro Rodrigo Otávio, Barão de Ramiz Galvão, srs. Felix Pacheco, Otávio Pinto e Domingos 
    Niobel.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ZozimoBarrosoDoAmaral_RobertoSa: Obra = {
  ID: 307,
  Imagem: 'ZozimoBarrosoDoAmaral_RobertoSa.jpeg',
  Authors: [authors.RobertoSa],
  Titulo: `Zózimo Barroso do Amaral`,
  Material: 'Bronze',
  DataInauguracao: `11/2001`,
  DatasImportantes: [new Data(`11/2001`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Avenida Delfim Moreira, posto 12',
  Latitude: `-22.9884362`,
  Longitude: `-43.2273224`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua em tamanho natural, que retrata o homenageado contemplando a Cidade, de forma descontraída, onde o paletó
    está sobre o ombro. Para caracterizar a sua profissão e completar um conjunto existe uma parte da murada do Leblon em bronze
    onde está fixada uma máquina de escrever, uma agenda e uma página do Jornal do Brasil com o nome estampado em sua coluna.
O monumento foi erguido pela Prefeitura da Cidade por solicitação da família.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JeanSibelius_WainoAaltonem: Obra = {
  ID: 308,
  Imagem: 'JeanSibelius_WainoAaltonem.jpeg',
  Authors: [authors.WainoAaltonem],
  Titulo: `Jean Sibelius`,
  Material: 'Bronze',
  DataInauguracao: `1928`,
  DatasImportantes: [
    new Data(`1928`, 'data de inauguração - arte cidade'),
    new Data(`12/1965`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leblon',
  Endereco: 'Praça Sibelius',
  Latitude: `-22.9774598`,
  Longitude: `-43.226212`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao músico filandes Jean Sibelius, ocorreu em comemoração pela passagem
     do 1º centenário de nascimento. Foi encomendado pelo Ministerio da Educação Nacional da 
     Filandia. A cabeça com a fisionomia do homenageado esta sobre um grande pedestal de 2,80
      revestido de granito polido negro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const ZuzuAngel_MarliMazeredo: Obra = {
  ID: 310,
  Imagem: 'ZuzuAngel_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Zuzu Angel`,
  Material: 'resina',
  Altura: '500 cm',
  DataInauguracao: `04/1998`,
  DatasImportantes: [new Data(`04/1998`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'São Conrado',
  Endereco: 'Auto Estrada Lagoa Barra',
  Latitude: `-22.998539`,
  Longitude: `-43.2672116`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de uma figura feminina esbelta, de formas estilizadas em resina. No pedestal um medalhão com a
    efígie identifica a homenageada.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Ceres_ThomasRegnaudin: Obra = {
  ID: 313,
  Imagem: 'Ceres_ThomasRegnaudin.jpeg',
  Authors: [authors.ThomasRegnaudin],
  Titulo: `Ceres`,
  Material: 'mármore',
  DataInauguracao: `1843`,
  DatasImportantes: [
    new Data(`1843`, 'data de inauguração - arte ambiente'),
    new Data(`12/2000`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Rua São Clemente, 360- Palácio da Cidade',
  Latitude: `-22.9510716`,
  Longitude: `-43.1938886`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura realizada para ser instalada no Jardim do Valongo, preservando as originais em
    local seguro e as réplicas em exposição ao público. Com o retorno das originais para o Jardim do Valongo,essas peças foram
    transferidas para esse jardim. A escultura está no Palácio da Cidade por cessão de uso.
    A escultura foi instalada no Cais onde desembarcaria a Imperatriz Tereza Crisitna. De estilo neoclassico, em mármore de Carrara,
    é uma do conjunto de quatro esculturas que são as primeiras peças em área pública da Cidade. Foi transferida para o Jardim do Valongo
    quando de sua construção por Pereira Passos. A escultura é esculpida com grandes e vaporosas vestes e cabelos presos e traças e pés descalços,
    apresenta em uma das mãos um ramalhete indicando o plantio como sua missão de proteção.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Réplica',
      'A obra é uma reprodução da peça Faustine de Thomas Regnaudin, existente nos Jardins de Versailles',
      [],
      'Versailles',
    ),
    new Fact(
      '2017',
      '2017',
      'Remoção',
      'A obra foi retirada do jardim do Palácio da Cidade pelo prefeito da cidade',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2000',
      '2000',
      'Restauro',
      'A obra foi restaurada pelo projeto Arte Cidade Restauro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1938',
      '1938',
      'Bem tombado municipal',
      'Tombado pelo IPHAN pelo processo 0099-T-38 em 30 de junho de 1938 no Livro Belas Artes Nº inscr.: 157 ;Vol. 1 ;F. 028 Livro Histórico Nº inscr.: 065 ;Vol. 1 ;F. 012',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Marte_Desconhecida: Obra = {
  ID: 314,
  Imagem: 'Marte_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Marte`,
  Material: 'Bronze',
  DataInauguracao: `1843`,
  DatasImportantes: [
    new Data(`1843`, 'data de inauguração - arte ambiente'),
    new Data(`12/2000`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Rua São Clemente, 360- Palácio da Cidade',
  Latitude: `-22.9510716`,
  Longitude: `-43.1938886`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Arte Cidade Restauro - Escultura realizada para ser instalada no Jardim do Valongo, preservando as originais em local seguro e as réplicas em exposição ao público. Com o retorno das originais para o Jardim do Valongo,essas peças foram transferidas para esse jardim.Foi retirada do jardim do Palacio em 2017, pelo prefeito da cidade.
    A escultura esta no Palácio da Cidade por cessão de uso.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MercurioPalacioCidade_Desconhecida: Obra = {
  ID: 315,
  Imagem: 'MercurioPalacioCidade_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Mercúrio`,
  Material: 'mármore',
  DataInauguracao: `1843`,
  DatasImportantes: [
    new Data(`1843`, 'data de inauguração - arte ambiente'),
    new Data(`12/2000`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Rua São Clemente, 360- Palácio da Cidade',
  Latitude: `-22.9510716`,
  Longitude: `-43.1938886`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Arte Cidade Restauro - Escultura realizada para ser instalada no Jardim do Valongo, preservando as originais em local seguro e as réplicas em exposição ao público. Com o retorno das originais para o Jardim do Valongo,essas peças foram transferidas para esse jardim. Essa peça reproduz a figura de Hermes, clássica greca.
    A escultura esta no Palácio da Cidade por cessão de uso.Foi retirada do jardim do Palacio em 2017, pelo prefeito da cidade.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Minerva_Giustiniani: Obra = {
  ID: 316,
  Imagem: 'Minerva_Giustiniani.jpeg',
  Authors: [authors.Giustiniani],
  Titulo: `Minerva`,
  Material: 'mármore',
  DataInauguracao: `1843`,
  DatasImportantes: [
    new Data(`1843`, 'data de inauguração - arte ambiente'),
    new Data(`12/2000`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Rua São Clemente, 360- Palácio da Cidade',
  Latitude: `-22.9510716`,
  Longitude: `-43.1938886`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Arte Cidade Restauro -Escultura realizada para ser instalada no Jardim do Valongo, preservando as originais em
    local seguro e as réplicas em exposição ao público. Com o retorno das originais para o Jardim do Valongo essas peças foram
    transferidas para esse jardim. A escultura esta no Palácio da Cidade por cessão de uso.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Réplica',
      'A obra é uma reprodução da peça Athenas de Giustiniani, do Museu do Vaticano',
      [],
      'Roma',
    ),
    new Fact(
      '2017',
      '2017',
      'Remoção',
      'A obra foi retirada do jardim do Palácio da Cidade pelo prefeito da cidade',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1938',
      '1938',
      'Bem tombado municipal',
      'Tombado pelo IPHAN pelo processo 0099-T-38 em 30 de junho de 1938 no Livro Belas Artes Nº inscr.: 157 ;Vol. 1 ;F. 028 Livro Histórico Nº inscr.: 065 ;Vol. 1 ;F. 012',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Restauro',
      'A obra foi restaurada pelo projeto Arte Cidade Restauro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Chafariz_AndrewHandyside: Obra = {
  ID: 317,
  Imagem: 'Chafariz_AndrewHandyside.jpeg',
  Authors: [authors.AndrewHandyside],
  Titulo: `Chafariz`,
  Material: 'ferro fundido',
  DataInauguracao: `01/1906`,
  DatasImportantes: [new Data(`01/1906`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praça Nicaragua',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de uma obra de Authors:ia de Andrew Handyside & Co, com sede em Derby, Inglaterra. Foi fundida por Brittania Foundry, correspondeente ao modelo 38 do catalago chamado An Illustrated book of Designs for Fountains and Vases, de 1879.
    É um pequeno Chafariz, sobre um tanque circular de granito eleva-se um coluna central octogonal, decorada com volutas e folhas de acanto, sustentando duas taças, sobre a maior. Três pequenos tritões servem de apoio a uma taça menor que sustenta um fuste terminado em coroa, do qual caem as águas Desativado desde o ano de 2000.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Maternidade_CelsoAntonio: Obra = {
  ID: 318,
  Imagem: 'Maternidade_CelsoAntonio.jpeg',
  Authors: [authors.CelsoAntonio],
  Titulo: `Maternidade`,
  Material: 'mármore',
  DataInauguracao: `1952`,
  DatasImportantes: [new Data(`1952`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.9432666`,
  Longitude: `-43.1815827`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Esculpida em 1943 originalmente para ornar o prédio do Ministério da Educação e Saúde, hoje Palácio Gustavo Capanema, ao lado de outra obra do mesmo material e Authors:ia a, Moça Reclinada. Esta em area publica desde 1952, onde recebeu o tombamento em 2008.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FredericoOzanam_DanteCroce: Obra = {
  ID: 319,
  Imagem: 'FredericoOzanam_DanteCroce.jpeg',
  Authors: [authors.DanteCroce],
  Titulo: `Frederico Ozanam`,
  Material: 'Bronze',
  DataInauguracao: `08/1972`,
  DatasImportantes: [new Data(`08/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praça Ozanom',
  Latitude: `-22.9528505`,
  Longitude: `-43.176184`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi doado à Cidade peloConselho Superior do Brasil da Fundação da Sociedade de São Vicente de Paula,para comemorar o centenário da Fundação da Confederação Vicentina do Brasil. A imagem do homenageado refere-se ao seu periodo universitario, quando caminhava com um livro na mão.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PedroJuanCaballero_LeonardoLima: Obra = {
  ID: 320,
  Imagem: 'PedroJuanCaballero_LeonardoLima.jpeg',
  Authors: [authors.LeonardoLima],
  Titulo: `Pedro Juan Caballero`,
  Material: 'Bronze',
  DataInauguracao: `04/1959`,
  DatasImportantes: [new Data(`04/1959`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praia de Botafogo',
  Latitude: `-22.94978`,
  Longitude: `-43.1804029`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Peça com a figura do homeanageado com o traje tipico de sua época.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JuruenadeMatos_JoaquimMoreiraJunior: Obra = {
  ID: 321,
  Imagem: 'JuruenadeMatos_JoaquimMoreiraJunior.jpeg',
  Authors: [authors.JoaquimMoreiraJunior],
  Titulo: `Juruena de Matos`,
  Material: 'Bronze',
  DataInauguracao: `06/1967`,
  DatasImportantes: [new Data(`06/1967`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praia de Botafogo',
  Latitude: `-22.9482754`,
  Longitude: `-43.182365`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma homenagem do Gremio Ciltural Catulo da Paixão Cearense, ao
    colegio Juruema de Matos seu fundador em 1927,  uma das escolas tradicionais unidade
    educacionais da cidade, cujo endereço era na Praia de Botafogo nº 188.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FranciscoMoranzan_RomeuAlves: Obra = {
  ID: 322,
  Imagem: 'FranciscoMoranzan_RomeuAlves.jpeg',
  Authors: [authors.RomeuAlves],
  Titulo: `Francisco Moranzan`,
  Material: 'Bronze',
  DataInauguracao: `11/1956`,
  DatasImportantes: [new Data(`11/1956`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praia de Botafogo',
  Latitude: `-22.9496267`,
  Longitude: `-43.1809877`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado originariamente na Praia de Botafogo, na antiga Praça Honduras, posteriormente foi retirado, retornando em 17/11/2004, atendendo a solicitação do consulado de Honduras.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CarvalhodeBrito_RuffoFanucchi: Obra = {
  ID: 323,
  Imagem: 'CarvalhodeBrito_RuffoFanucchi.jpeg',
  Authors: [authors.RuffoFanucchi],
  Titulo: `Carvalho de Brito`,
  Material: 'Bronze',
  DataInauguracao: `01/1955`,
  DatasImportantes: [new Data(`01/1955`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praia de Botafogo',
  Latitude: `-22.9479693`,
  Longitude: `-43.1824868`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento acadêmico com a fisionomia marcante do homenageado. Inaugurado pelo prefeito Alim Pedro, com discurso do Ministro da Justiça e membro da Academia de Letras, Dr. Afonso Pena. O agradecimento em nome da família foi realizada pelo o deputado  Ramiro Berbert de Castro.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SimonBolivar_Desconhecida: Obra = {
  ID: 324,
  Imagem: 'SimonBolivar_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Simón Bolívar`,
  Material: 'Bronze',
  DataInauguracao: `10/1978`,
  DatasImportantes: [
    new Data(`10/1978`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1981`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praça Juliano Moreira',
  Latitude: `-22.9557549`,
  Longitude: `-43.1775749`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua equestre de Símom Bolivar foi uma doação do Governo de Venezuela ao Brasil. Destaca a beleza da obra com a seguinte inscrição no pedestal:SIMOM BOLIVAR,LIBERTADORDAVENEZUELA, COLOMBIA, EQUADOR,PANAMA, PERU E BOLIVIA.AVENEZUELA AOBRASIL1978`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const GeneralJoseArtigas_VHabegger: Obra = {
  ID: 325,
  Imagem: 'GeneralJoseArtigas_VHabegger.jpeg',
  Authors: [authors.VHabegger],
  Titulo: `General Artigas`,
  Material: 'Bronze',
  DataInauguracao: `09/1979`,
  DatasImportantes: [
    new Data(`09/1979`, 'data de inauguração - inventário dos monumentos'),
    new Data(`01992`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praça  Chaim Weizmann',
  Latitude: `-22.9418039`,
  Longitude: `-43.182243`,
  Base: 'Com pedestal',
  MaterialBase: 'mármore',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma doação do Governo do Uruguai à Cidade do Rio. Foi inaugurado
    em 1º de setembro de 1979 na Praça Artigas, na Cidade Nova. Devido às obras do Teleporto, foi
    removido e instalado nessa praça em 19 de setembro de 1997.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Bocage_JoaquimCorrea: Obra = {
  ID: 326,
  Imagem: 'Bocage_JoaquimCorrea.jpeg',
  Authors: [authors.JoaquimCorrea],
  Titulo: `Bocage`,
  Material: 'mármore',
  DataInauguracao: `12/1965`,
  DatasImportantes: [new Data(`12/1965`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Praça Bariloche',
  Latitude: `-22.9421948`,
  Longitude: `-43.1823222`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi doado pelo governo português, durante as comemorações do IV Centenário da fundação da Cidade do Rio de Janeiro, no mesmo ano do bicentenário do poeta. O busto foi uma retribuição a doação a Portugal pelo Brasil, o busto de Olavo Bilac para Setubal. Foi levado por uma comissão de membros da Academia Brasileira de Letras. O busto de Bocage veio para o Brasil pelo professor Hernani Cidade`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const YhitzakRabin_EdgarDuvivierFilho: Obra = {
  ID: 327,
  Imagem: 'YhitzakRabin_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Yhitzak Rabin`,
  Material: 'Bronze',
  DataInauguracao: `09/2000`,
  DatasImportantes: [new Data(`09/2000`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Parque Yitzhak Rabin',
  Latitude: `-22.9514848`,
  Longitude: `-43.1787674`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O pequeno monumento foi uma iniciativa do Presidente da Câmara Municipal do Rio de Janeiro, o Vereador
    Gérson Bergher`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BartolomeuMitre_LourillandSan: Obra = {
  ID: 328,
  Imagem: 'BartolomeuMitre_LourillandSan.jpeg',
  Authors: [authors.LourillandSan],
  Titulo: `Bartolomeu Mitre`,
  Material: 'Bronze',
  DataInauguracao: `08/1961`,
  DatasImportantes: [
    new Data(`08/1961`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1996`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.9428531`,
  Longitude: `-43.1807615`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma doação do governo argentino à cidade como um sinal de amizade. Na inauguração esteve presente o diretor geral de La Nacion, de Buenos Aires, jornalista Bartolomeu Mitre, bisneto do heroi nacioanl.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CarlosChagas_HumbertoCozzo: Obra = {
  ID: 329,
  Imagem: 'CarlosChagas_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: `Carlos Chagas`,
  Material: 'Bronze',
  DataInauguracao: `12/1974`,
  DatasImportantes: [
    new Data(`12/1974`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1979`, 'data de inauguração - arte ambientes'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.906624`,
  Longitude: `-43.2007664`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma iniciativa do Governo do Estado e do Conselho Federal de
    Cultura, no ano do centenário de nascimento do grande médico e cientista brasileiro. O
    monumento foi retirado e transferido para o deposito em 15 de junho de 2018, devido a atos
    de vandalismos.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MarechalCandidoRandon_AntonioPintodeMattos: Obra = {
  ID: 330,
  Imagem: 'MarechalCandidoRandon_AntonioPintodeMattos.jpeg',
  Authors: [authors.AntonioPintodeMattos],
  Titulo: `Marechal Candido Rondon`,
  Material: 'Bronze',
  DataInauguracao: `04/1972`,
  DatasImportantes: [new Data(`04/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Avenida General Polidoro',
  Latitude: '-22.9564863',
  Longitude: '-43.1867384',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const FontedoManequinho_BelmiroDeAlmeida: Obra = {
  ID: 331,
  Imagem: 'FontedoManequinho_BelmiroDeAlmeida.jpeg',
  Authors: [authors.BelmiroDeAlmeida],
  Titulo: `Fonte do Manequinho`,
  Material: 'Bronze',
  DataInauguracao: `1906`,
  DatasImportantes: [
    new Data(`1906`, 'data de inauguração - arte ambiente'),
    new Data(`01/1914`, 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Av. Venceslau Braz ',
  Latitude: `-22.9549784`,
  Longitude: `-43.1775234`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Inspirada na célebre estátua do Manekken Piss, de Bruxelas, a estátua original foi
    roubada em 1990, sendo logo fundida uma réplica sua. O Manequinho tem sua imagem associada ao
    time do Botafogo desde 1957, quando o jogador Didi vestiu a escultura com a camisa alvinegra,
    repetindo o gesto de um torcedor anônimo em 1948. Inaugurado na Praça Floriano, transferido em
    1922 para a Praia de Botafogo e em 1990 para atual localização. Zani Fundição Artística E Metalúrgica Ltda.`,
  Fatos: [
    new Fact(
      'data',
      '2008',
      'Bem tombado municipal',
      'Tombado pelo Município pelo Decreto 29 690',
      [],
      'cidade',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AEcadeQueiroz_RodolfoPintoCouto: Obra = {
  ID: 332,
  Imagem: 'AEcadeQueiroz_RodolfoPintoCouto.jpeg',
  Authors: [authors.RodolfoPintoCouto],
  Titulo: `À Eça de Queiroz`,
  Material: 'mármore',
  DataInauguracao: `02/1923`,
  DatasImportantes: [new Data(`02/1923`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Avenida Lauro Sodré',
  Latitude: `-22.9580386`,
  Longitude: `-43.1774031`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Inaugurado em 1923 na Avenida Santos Dumont, foi posteriormente transferido para a
    Avenida Rui Barbosa, próximo à Praia de Botafogo . Finalmente, em 1982 passou para a localização
    atual, no canteiro central da Avenida Lauro Sodré. O monumento nasceu da iniciativa de
    intelectuais e homens de letras do Brasil, com Coelho Neto à frente.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Referencial',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const FonteIrmaosRamosPinto_AntonioTeixeiraLopes: Obra = {
  ID: 333,
  Imagem: 'FonteIrmaosRamosPinto_AntonioTeixeiraLopes.jpeg',
  Authors: [authors.AntonioTeixeiraLopes],
  Titulo: `Fonte Irmaos Ramos Pinto`,
  Material: 'mármore',
  Altura: '7 m',
  Peso: '37 toneladas',
  DataInauguracao: `02/1906`,
  DatasImportantes: [new Data(`02/1906`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Centro',
  Endereco: 'Botafogo',
  Latitude: `-22.9585127`,
  Longitude: `-43.1772141`,
  Base: 'semm pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Compõe por uma figura de cupido, no cume de um rochedo e na base de um grupo de ninfas em
    graciosas atitudes. A água jorrava por três belas cabeças caindo em forma de cascata na grande
    bacia do monumento. A fonte foi inaugurada em 1906 no Largo da Glória, e em 29/10/1950 foi
    inaugurada para a embocadura do Tunel Novo na presença do prefeito e de familiares da doadora
    da obra,. Foi doada pelos irmãos Adriano Ramos Pinto, donos de célebre vinícola, à cidade do
    Rio de Janeiro. A cabeça que foi retirada do monumento com a obra de restauração foi guardada
    no deposito da Prefeitura. Desativado em data desconhecida.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const OswaldoCruz_HumbertoCozzo: Obra = {
  ID: 334,
  Imagem: 'OswaldoCruz_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: `Oswaldo Cruz`,
  Material: 'Bronze',
  DataInauguracao: `12/1972`,
  DatasImportantes: [new Data(`12/1972`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.941589`,
  Longitude: `-43.1770629`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é uma homenagem por sua dedicação a medicina experimental no Brasil.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AlmiranteTamandare_HildegardoLeaoVelloso: Obra = {
  ID: 335,
  Imagem: 'AlmiranteTamandare_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Almirante Tamandaré`,
  Material: 'Bronze',
  DataInauguracao: `12/1937`,
  DatasImportantes: [new Data(`12/1937`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.9414676`,
  Longitude: `-43.1774591`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O Monumento foi uma iniciativa da Marinha, que escolheu o local da praia de Botafogo, onde desde 1916 existia uma herma do Marquês. No alto, está o Almirante, Patrono da Marinha do Brasil, de pé, em atitude de comando ladeado por figuras que simbolizam a Glória. A linha do pedestal representa uma quilha de navio, um belo exemplar do periodo artístico do Art-Decor.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Crepusculo_HenriWeigele: Obra = {
  ID: 336,
  Imagem: 'Crepusculo_HenriWeigele.jpeg',
  Authors: [authors.HenriWeigele],
  Titulo: `Crepúsculo`,
  Material: 'mármore',
  DataInauguracao: `01/1906`,
  DatasImportantes: [new Data(`01/1906`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.94237`,
  Longitude: `-43.1803717`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina figurativa, adquirida na França para adornar o jardim criado por Pereira Passos.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PoesiaemRuinas_JeanMagrou: Obra = {
  ID: 337,
  Imagem: 'PoesiaemRuinas_JeanMagrou.jpeg',
  Authors: [authors.JeanMagrou],
  Titulo: `Poesia em Ruínas`,
  Material: 'mármore',
  DataInauguracao: `01/1904`,
  DatasImportantes: [
    new Data(`01/1904`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1910`, 'data de inauguração - arte ambiente'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Botafogo',
  Endereco: 'Jardim da Praia de Botafogo',
  Latitude: `-22.94237`,
  Longitude: `-43.1803717`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura neoclássica feminina com elegante movimento, com os braço apoiado em um pedestal, adquirida na França para o espaço público carioca.Estátua em tamanho natural, com os braços elevados apoiando a cabeça`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const NossaSenhoradaImaculadaConceicao_GiuseppeNavone: Obra = {
  ID: 338,
  Imagem: 'NossaSenhoradaImaculadaConceicao_GiuseppeNavone.jpeg',
  Authors: [authors.GiuseppeNavone],
  Titulo: `Nossa Senhora da Imaculada Conceição`,
  Material: 'mármore',
  DataInauguracao: `1906`,
  DatasImportantes: [
    new Data(`1906`, 'data de inauguração - arte ambiente'),
    new Data(`12/1954`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Largo do Machado',
  Latitude: `-22.9304186`,
  Longitude: `-43.1785621`,
  Status: 'Permanece em espaço público',
  Descricao: `Peça executada em 1907 foi doada à cidade pela Arquidiocese do Rio de Janeiro, nas
    comemorações do centenário da promulgação do dogma da Imaculada Conceição de Nossa Senhora, e
    se ergue sobre um alto pedestal revestido em mármore branco.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const LorenzoFernandes_AristocherBenjamimMeschessi: Obra = {
  ID: 339,
  Imagem: 'LorenzoFernandes_AristocherBenjamimMeschessi.jpeg',
  Authors: [authors.AristocherBenjamimMeschessi],
  Titulo: `Lorenzo Fernandes`,
  Material: 'Bronze',
  DataInauguracao: `08/1949`,
  DatasImportantes: [new Data(`08/1949`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Largo do Machado',
  Latitude: `-22.9309478`,
  Longitude: `-43.1794647`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem em um busto acadêmico, foi uma iniciativa do Conservatório Brasileiro de Música.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const EmilioDeMenezes_JoaoTurin: Obra = {
  ID: 340,
  Imagem: 'EmilioDeMenezes_JoaoTurin.jpeg',
  Authors: [authors.JoaoTurin],
  Titulo: `Emílio de Menezes`,
  Material: 'Bronze',
  DataInauguracao: `08/1950`,
  DatasImportantes: [new Data(`08/1950`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Largo do Machado',
  Latitude: `-22.9303875`,
  Longitude: `-43.1784601`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto do homenageado esta apresentado de forma academica, com vestimenta da época e expressão forte. Essa peça foi apresentado no Salon dos Artites François em 1921.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlfredoGomes_PauloMazzucchelli: Obra = {
  ID: 341,
  Imagem: 'AlfredoGomes_PauloMazzucchelli.jpeg',
  Authors: [authors.PauloMazzucchelli],
  Titulo: `Alfredo Gomes`,
  Material: 'Bronze',
  DataInauguracao: `09/1937`,
  DatasImportantes: [new Data(`09/1937`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Largo do Machado',
  Latitude: `-22.9312377`,
  Longitude: `-43.1794`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto em homenagem ao mestre, foi uma iniciativa de seus ex-alunos.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ONascimentodeVenus_MathurinMoreau: Obra = {
  ID: 342,
  Imagem: 'ONascimentodeVenus_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `O Nascimento de Venus`,
  Material: 'ferro fundido',
  DataInauguracao: `1854`,
  DatasImportantes: [new Data(`1854`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const CristovaoColombo_MathurinMoreau: Obra = {
  ID: 343,
  Imagem: 'CristovaoColombo_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Cristovao Colombo`,
  Material: 'ferro fundido',
  DataInauguracao: `1892`,
  DatasImportantes: [new Data(`1892`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const America_MathurinMoreau: Obra = {
  ID: 344,
  Imagem: 'America_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `America`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Oceania_MathurinMoreau: Obra = {
  ID: 345,
  Imagem: 'Oceania_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Oceania`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Africa_MathurinMoreau: Obra = {
  ID: 346,
  Imagem: 'Africa_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Africa`,
  Material: 'ferro fundido',
  DataInauguracao: '1896',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Europa_MathurinMoreau: Obra = {
  ID: 347,
  Imagem: 'Europa_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Europa`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Asia_MathurinMoreau: Obra = {
  ID: 348,
  Imagem: 'Asia_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Asia`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MeninoComPassaro_MathurinMoreau: Obra = {
  ID: 349,
  Imagem: 'MeninoComPassaro_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Menino com pássaro`,
  Material: 'ferro fundido',
  DatasImportantes: [new Data(`séc XIX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FiguraFemininaI_Desconhecida: Obra = {
  ID: 350,
  Imagem: 'FiguraFemininaI_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Figura Feminina I`,
  Material: 'terracota',
  DataInauguracao: `1986`,
  DatasImportantes: [new Data(`1986`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FiguraFemininaII_Desconhecida: Obra = {
  ID: 351,
  Imagem: 'FiguraFemininaII_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Figura Feminina II`,
  Material: 'terracota',
  DataInauguracao: `1986`,
  DatasImportantes: [new Data(`1986`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Museu da República',
  Latitude: '-22.9258592',
  Longitude: '-43.1784094',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const TeixeiraSoares_JoseOtavioCorreaLima: Obra = {
  ID: 352,
  Imagem: 'TeixeiraSoares_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: `Teixeira Soares`,
  Material: 'Bronze',
  DataInauguracao: `07/1930`,
  DatasImportantes: [new Data(`07/1930`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Cosme Velho',
  Endereco: 'Praça S. Judas Tadeu',
  Latitude: `-22.9404358`,
  Longitude: `-43.1981981`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é composto pela figura do homenageado com um mapa na mão direita, sobre
    um pedestal de granito vermelho de São Paulo. Abaixo existem duas alegorias de figuras femininas
    em bronze representando a Viação e a Agricultura.Tem um painel em baixo relevo reproduzindo
    cena de dois trecho da estrada de ferro São Paulo - Paraná. O Monumento foi erigido na Praça
    Mauá, junto a sede do Touring clube e defronte de um outro grande brasileiro o Barão de Mauá,
    por iniciativa da Revista das Estradas de Ferro. Em nome da Comissão executiva falou Paulo de
    Frontim. Foi transferido para a Rua Cosme Velho, ao lado da Estação de Ferro Corcovado por ter
    sido este o idealizador da ferrovia`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ArcosdasPalmeiras_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 517,
  Imagem: 'ArcosdasPalmeiras_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Arcos das Palmeiras`,
  Material: 'aço',
  ValorObra: 'R$ 49500,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [
    new Data(`07/1996`, 'data de inauguração'),
    new Data(`2017`, 'furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Latitude: `-22.907364`,
  Longitude: `-43.5658462`,
  Status: 'Furtada',
  Descricao: `4 estruturas com a parte superior fixadas chapas no sentido horizontal finalizada
    por uma esfera pintada na cor da laranja. O marco é uma referência as antigas palmeiras das
    fazendas que existiram na região. As peças (3 arcos) desapareceram em 2017, constatado em vistoria em novembro de 2017`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Ornamental',
  Natureza: 'Urbanística',
  Tipologia: 'Marco',
  Execucao: true,
};

export const MonumentoAosMortosdaIIGuerraMundial_MarcosKonderNeto_HelioRibasMarinho: Obra =
  {
    ID: 354,
    Imagem:
      'MonumentoAosMortosdaIIGuerraMundial_MarcosKonderNeto_HelioRibasMarinho.jpeg',
    Authors: [authors.MarcosKonderNeto, authors.HelioRibasMarinho],
    Titulo: `Monumento Aos Mortos da II Guerra Mundial`,
    Material: 'granito',
    DataInauguracao: `08/1959`,
    DatasImportantes: [new Data(`08/1959`, 'data de inauguração')],
    Propriedade: 'Pública',
    Zona: 'Sul',
    Bairro: 'Glória',
    Endereco: 'Parque do Flamengo',
    Latitude: `-22.9162731`,
    Longitude: `-43.1741866`,
    Base: 'Sem pedestal',
    Status: 'Permanece em espaço público',
    Descricao: `Foi inaugurado em 1959 mas somente em 1960, a cripta recebeu as urnas com os restos mortais dos 467 brasileiros mortos no conflito, vindos do Cemitério de Pistoia. O monumento tem três planos, plataforma, patamar e subsolo. Entre os pilones do pórtico, com 31 metros de altura, está o túmulo do soldado desconhecido, centro de todas as solenidades cívicas, local também da chama eterna. À direita vê-se o grupo escultórico de Júlio Castelli Filho, feito em módulos de ferro metalizado.e a escultura de  Alfredo Ceschiatti em homenagem às três forças armadas. No patamar, ao nível da praça, encontra-se o museu, um jardim interno, um lago e painéis de Cerâmica de Anísio Medeiros.No subsolo, na entrada do qual se acham gravados os nomes dos dois mil brasileiros, militares e civis, mortos pelos torpedeamentos alemães, umagrande cripta contém 468 jazigos de mármore nacional com tampos em mármore de Carrara preto, nos quais estão gravados o nome. Quinze jazigos não possuem nomes gravados, e dois são cenotáfios dedicados a desaparecidos.
    O Monumento teve sua construção iniciada a 24 de junho de 1957 e, embora inaugurado a 07 de abril do mesmo ano, a sua obra só foi concluída em 24 de junho de 1960.`,
    Fatos: [
      new Fact('', '', 'bem tombado federal', '', [], ''),
      new Fact('', '', 'bem tombado municipal', '', [], ''),
    ],
    Temporalidade: 'Permanente',
    Categoria: 'Narratividade',
    Funcao: 'Referencial',
    Natureza: 'Cívica',
    Tipologia: 'Grupo escultórico',
    Execucao: true,
  };

export const SemTituloI_AmilcarDeCastro: Obra = {
  ID: 355,
  Imagem: 'SemTituloI_AmilcarDeCastro.jpeg',
  Authors: [authors.AmilcarDeCastro],
  Titulo: `Sem Titulo`,
  Material: 'aço corten',
  DataInauguracao: `01/2000`,
  DatasImportantes: [new Data(`01/2000`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Aterro do Flamengo',
  Latitude: `-22.9144254`,
  Longitude: `-43.1717319`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura abstrata em chapa de aço corten, com uma dobra e duas faces. A peça esta em cessão de uso em área da
    Prefeitura.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const EspacoCircularEmCuboVirtual_FranzWeissmann: Obra = {
  ID: 356,
  Imagem: 'EspacoCircularEmCuboVirtual_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Espaço Circular em Cubo Virtual`,
  Material: 'aço corten',
  Altura: '300 cm',
  Largura: '300 cm',
  Profundidade: '300 cm',
  DataInauguracao: `01/1978`,
  DatasImportantes: [new Data(`01/1978`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Avenida Infante Dom Henrique, 85',
  Latitude: `-22.9144254`,
  Longitude: `-43.1717319`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça única com quatro faces retangulares, formando um semicírculo vazado. A obra foi criada em 1958. A peça esta em cessão de uso em área da Prefeitura.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AsTresForcasArmadas_AlfredoCeschiatti: Obra = {
  ID: 359,
  Imagem: 'AsTresForcasArmadas_AlfredoCeschiatti.jpeg',
  Authors: [authors.AlfredoCeschiatti],
  Titulo: `As Três Forças Armadas- Monumento aos Mortos da Segunda Guerra`,
  Material: 'franito',
  DataInauguracao: `1960`,
  DatasImportantes: [new Data(`1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Parque do Flamengo',
  Latitude: '-22.9160527',
  Longitude: '-43.173734',
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'bem tombado federal', '', [], ''),
    new Fact('', '', 'bem tombado municipal', '', [], ''),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Cuauhtemoc_CarlosObregonSantacilia: Obra = {
  ID: 360,
  Imagem: 'Cuauhtemoc_CarlosObregonSantacilia.jpeg',
  Authors: [authors.CarlosObregonSantacilia],
  Titulo: `Cuauhtémoc`,
  Material: 'Bronze',
  DataInauguracao: `09/1922`,
  DatasImportantes: [new Data(`09/1922`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Flamengo',
  Endereco: 'Praça Cauhtemoc',
  Latitude: `-22.9372812`,
  Longitude: `-43.1730611`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento doado pelo governo do México, como parte das comemorações do Centenário da Independência (1922), à cidade do Rio de Janeiro. Foi inaugurado na data nacional mexicana.
    O conjunto com a escultura e pedestal com serpentes estilizadas e baixos relevos, que invocam a mitologia Asteca é talvez um dos primeiros elementos Art-Déco a ser instalado em nossa cidade, numa época onde ainda predominava o eclético e o estilo neo-colonial. Trata-se de uma réplica da original deCarlos Obregon Santacília.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'comemorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JosedeAlencar_RodolfoBernardelli: Obra = {
  ID: 361,
  Imagem: 'JosedeAlencar_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `José de Alencar`,
  Material: 'Bronze',
  DataInauguracao: `05/1897`,
  DatasImportantes: [new Data(`05/1897`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Catete',
  Endereco: 'Praça José de Alencar',
  Latitude: `-22.9325975`,
  Longitude: `-43.1775974`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento apresenta José de Alencar sentado e em tamanho natural. Em seu pedestal foram embutidos medalhões em baixos relevos representando cenas extraídas dos romances: O Guarany, o Gaúcho, o Sertanejo e Iracema. A obra foi uma iniciativa do jornalista Ferreira de Araujo.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BernardoOHiggins_CaroccabaThor: Obra = {
  ID: 362,
  Imagem: 'BernardoOHiggins_CaroccabaThor.jpeg',
  Authors: [authors.CaroccabaThor],
  Titulo: `Bernardo O'Higgins`,
  Material: 'Bronze',
  DataInauguracao: `1965`,
  DatasImportantes: [
    new Data(`1965`, 'data de inauguração -arte ambiente'),
    new Data(`03/1967`, 'data de inauguraçã- inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Flamengo',
  Endereco: 'Praia do Flamengo',
  Latitude: `-22.9369399`,
  Longitude: `-43.1734655`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O presente a Cidade do Rio de Janeiro pelo Chile pelos 400 anos da cidade, foi inaugurado dois anos depois, em março de 1967, A cerimônia foi presidida pelo governador Negrão de Lima,Foi inaugurado na
    Avenida Chile, no canteiro central, posteriormente foi transferida para um canteiro laterial em 23 de maio de 1996 e finalmente em 17 de janeiro de 2007 foi inaugurado nesse local.
Doado a cidade do Rio de Janeiro pelo governo do Chile por ocasião do seu IV Centenário da fundação da Cidade.
Estátua com a figura do homenageado com uma farda chilena tendo na direita uma espera e na esquerda uma folha de papel. A estátua esta sobre um pedestal de concreto revertido em granito cinza.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Primavera_JeanBaptistePoultier: Obra = {
  ID: 366,
  Imagem: 'Primavera_JeanBaptistePoultier.jpeg',
  Authors: [authors.JeanBaptistePoultier],
  Titulo: `Primavera`,
  Material: 'mármore',
  DataInauguracao: `01/1929`,
  DatasImportantes: [new Data(`01/1929`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9151454`,
  Longitude: `-43.1755404`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Uma das peças de um grupo escultórico das quatro estações do ano, reprodução de Ceres de Jean Baptiste Poultier,exposta no jardim do Palácio de Versailles.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Verao_PierreLeGros: Obra = {
  ID: 367,
  Imagem: 'Verao_PierreLeGros.jpeg',
  Authors: [authors.PierreLeGros],
  Titulo: `Verão `,
  Material: 'mármore',
  DataInauguracao: `01/1929`,
  DatasImportantes: [new Data(`01/1929`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9166947`,
  Longitude: `-43.175759`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça do grupo escultórico representando o Verão, reprodução do Pandora de Pierre Le Gros existente nos Jardins de Versalles.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AlfredoAgache_HeitorUsai: Obra = {
  ID: 368,
  Imagem: 'AlfredoAgache_HeitorUsai.jpeg',
  Authors: [authors.HeitorUsai],
  Titulo: `Alfredo Agache`,
  Material: 'Bronze',
  DataInauguracao: `07/1969`,
  DatasImportantes: [
    new Data(`07/1969`, 'data de inauguração'),
    new Data(`1992`, 'data de transferência para localização atual'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: '-22.916358',
  Longitude: '-43.177946,17',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao: `É uma homenagem oferecido pelo Clube de Engenharia, pela Associação dos Artistas Brasileiros e o Comite Nacional de Urbanismo, como parte das comemorações para assinalar a passagem do Dia da França.
    Apos a reforma da praça depois da obra do Metro, em 1992, o monumento foi reinaugurado.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AfonsoCelso_LaurindoRamos: Obra = {
  ID: 369,
  Imagem: 'AfonsoCelso_LaurindoRamos.jpeg',
  Authors: [authors.LaurindoRamos],
  Titulo: `Afonso Celso`,
  Material: 'Bronze',
  DataInauguracao: `03/1960`,
  DatasImportantes: [new Data(`03/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9149362`,
  Longitude: `-43.1757546`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem do academico traz o fardão e fisionomia tranquila. Foi erguido por iniciativa da Sociedade dos Amigos de Afonso Celso.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CarmemGomes_JoseBarreto: Obra = {
  ID: 370,
  Imagem: 'CarmemGomes_JoseBarreto.jpeg',
  Authors: [authors.JoseBarreto],
  Titulo: `Carmem Gomes`,
  Material: 'Bronze',
  DataInauguracao: `03/1958`,
  DatasImportantes: [new Data(`03/1958`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.916683`,
  Longitude: `-43.1758761`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi uma iniciativa da Sociedade dos Artistas Líricos Brasileiros, na época era presidida pelo filho da cantora, Jim Barboza. O monumeto teve o patrocinio de uma comissão composta por Carlos Magno, Negrão de Lima e Clovis Slagado.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const Outono_JacquesHouzeau: Obra = {
  ID: 372,
  Imagem: 'Outono_JacquesHouzeau.jpeg',
  Authors: [authors.JacquesHouzeau],
  Titulo: `Outono`,
  Material: 'mármore',
  DataInauguracao: `01/1929`,
  DatasImportantes: [new Data(`01/1929`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9151205`,
  Longitude: `-43.1757731`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Representação do Outono reprodução da escultura Faune de Jacques Houzeau,<existente no jardim do Palácio de Versailles.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Inverno_SimonHurtvelle: Obra = {
  ID: 373,
  Imagem: 'Inverno_SimonHurtvelle.jpeg',
  Authors: [authors.SimonHurtvelle],
  Titulo: `Inverno`,
  Material: 'mármore',
  DataInauguracao: `01/1929`,
  DatasImportantes: [new Data(`01/1929`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9166769`,
  Longitude: `-43.1759531`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Elegante escultura representando o inverno com vestimenta típica para a estação do ano. É uma reprodução de Théophraste, de Simon Hurtvelle.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const ChafarizdosGolfinhos_AndreRedont: Obra = {
  ID: 374,
  Imagem: 'ChafarizdosGolfinhos_AndreRedont.jpeg',
  Authors: [authors.AndreRedont],
  Titulo: `Chafariz dos Golfinhos`,
  Material: 'argamassa',
  DataInauguracao: `01/1929`,
  DatasImportantes: [new Data(`01/1929`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9160074`,
  Longitude: `-43.1759671`,
  Status: 'Permanece em espaço público',
  Descricao: `O Chafariz é composto por quatro golfinhos laterais, um jorro névoa e outro central, com cerca de doze metos de altura. São inspirados aos existentes no jardim do Palácio de Versailles, projetado pelo arquiteto paisagista da equipe de Alfred Agache.
    Esta desativado desde maio de 2017.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CandidoMendes_BenevenutoBerna: Obra = {
  ID: 375,
  Imagem: 'CandidoMendes_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Cândido Mendes`,
  Material: 'Bronze',
  DataInauguracao: `24/03/1982`,
  DatasImportantes: [
    new Data(`24/03/1982`, 'data de inauguração'),
    new Data('24/03/2001', 'data de transferência para localização atual'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: '-22.916358',
  Longitude: '-43.177946,17',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao: `A homenagem visa registrar a importância de sua atuação em instituições de ensino e pesquisa nas áreas de sociologia e direito.`,
  Fatos: [
    new Fact(
      '1995',
      '1995',
      'Tombamento',
      'Tombado pelo Município pelo Decreto nº 13898',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const VeraJanacopulos_AdrianaJanacopolus: Obra = {
  ID: 376,
  Imagem: 'VeraJanacopulos_AdrianaJanacopolus.jpeg',
  Authors: [authors.AdrianaJanacopolus],
  Titulo: `Vera Janacópulos`,
  Material: 'Bronze',
  DataInauguracao: `12/1958`,
  DatasImportantes: [new Data(`12/1958`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.916683`,
  Longitude: `-43.1758761`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem partiu do Circulo de Arte Vera Janacópulos, presidia pelo deputado Gustavo Capanema, fundada em 1955 com o objetivo de propagar e ensinar o método do canto de camara. A obra foi executada pela sua irmã Adriana, com os custos por conta da prefeitura.
    A inauguração contou com a presença do Prefeito Sá Freire, que fez erigir o monumento por conta dos cofres publicos. Esteve presente o diretor do Departamento Parques e Jardins Sr. Mauro Viegas, Pascoal carlos Magno, do representante do Ministro da Educação, Sr. Poessel.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ClovisBevilaqua_HonorioPecanha: Obra = {
  ID: 377,
  Imagem: 'ClovisBevilaqua_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Clóvis Beviláqua`,
  Material: 'Bronze',
  DataInauguracao: `12/1943`,
  DatasImportantes: [new Data(`12/1943`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9149128`,
  Longitude: `-43.1756288`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `É uma homenagem a quem organizou o projeto do Código Civil Brasileiros e representou o Brasil no Tribunal de Haia. A inauguração fez parte das comemorações do Dia do Jurista.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const TeixeiraMendes_BrunoGiorgi: Obra = {
  ID: 378,
  Imagem: 'TeixeiraSoares_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.BrunoGiorgi],
  Titulo: `Teixeira Mendes`,
  Material: 'Bronze',
  DataInauguracao: `11/1977`,
  DatasImportantes: [new Data(`11/1977`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9206872`,
  Longitude: `-43.1743128`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi inaugurada nas comemorações do dia da bandeira, por ter sido um dos idealizadores da Bandera Nacional. Foi erguido por iiniciativa do Clube Positivista e do Almirante Henrique Batista, presidente da Associação dos Ex-Combatentes do Brasil, seção do Rio de Janeiro.
    O ato teve a presença do prefeito Marcos Tamoio e do Embaixador Paulo Carneiro.
    Passados sete anos de sua inauguração. o busto foi roubado, e recolocado em 16 de dezembro de 1986, se recorreu ao Authors: da obra, Bruno Giorgi, que tinha o modelo original e Authors:ização a confecção da réplica.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PereiraCarneiro_EgistoBertozzi: Obra = {
  ID: 379,
  Imagem: 'PereiraCarneiro_EgistoBertozzi.jpeg',
  Authors: [authors.EgistoBertozzi],
  Titulo: `Pereira Carneiro`,
  Material: 'Bronze',
  DataInauguracao: `01/1962`,
  DatasImportantes: [new Data(`01/1962`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9206872`,
  Longitude: `-43.1743128`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado na praça Marechal Âncora, devido a sua ligação com a Panair do Brasil. Foi posteriormente transferido para essa praça.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GetulioVargas_JoasPereiraDosPassos: Obra = {
  ID: 380,
  Imagem: 'GetulioVargas_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Getulio Vargas`,
  Material: 'Bronze',
  DataInauguracao: `08/2004`,
  DatasImportantes: [new Data(`08/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9214288`,
  Longitude: `-43.1734074`,
  Base: 'Com pedestal',
  MaterialBase: 'mármore',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi idealizado e erguido por Henock de Almeida e executado pelo artista plástico Joás,para compor o
    Memorial a Getulio Vargas,criado para as comemorações do cinquentenário de sua morte.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SaoSebastiao_DanteCroce_CurzioZani_ArnaldoValilo: Obra = {
  ID: 382,
  Imagem: 'SaoSebastiao_DanteCroce_CurzioZani_ArnaldoValilo.jpeg',
  Authors: [authors.DanteCroce, authors.CurzioZani, authors.ArnaldoValilo],
  Titulo: `São Sebastião`,
  Material: 'granito',
  DataInauguracao: `08/1965`,
  DatasImportantes: [new Data(`08/1965`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9206872`,
  Longitude: `-43.1743128`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento erguido pelo Instituto Histórico e Geográfico Brasileiro, por subscrição popular, paraos festejos relativos do Quarto Centanário da Cidade.
    Na escultura foi utilizando o granito da pedreira do Alto da Boa Vista, o bloco pesava 20 toneladas e foi executado na marmoaria Nossa Senhora de Fátima tendo a imagem 7 metros e 30 centimetros.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BadenPowell_VincenzoLarocca: Obra = {
  ID: 383,
  Imagem: 'BadenPowell_VincenzoLarocca.jpeg',
  Authors: [authors.VincenzoLarocca],
  Titulo: `Baden Powell`,
  Material: 'Bronze',
  DataInauguracao: `07/1960`,
  DatasImportantes: [new Data(`07/1960`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: '-22.9214443',
  Longitude: '-43.1760816',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi uma iniciativa da União dos Escoteiros Do Brasil ao Escoteiro Chefe Mundial. O monumento foi inaugurado pelo escoteiro Jose de Araujo Filho, que descerou a bandeira. Compareceram a cerimonia 2 jovens venezianos e 1 (um) mexicano.
    Esta representado em meio corpo com os braços cruzados.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const RodolfoAmoedo_JoseOtavioCorreaLima: Obra = {
  ID: 384,
  Imagem: 'RodolfoAmoedo_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: `Rodolfo Amoêdo`,
  Material: 'Bronze',
  DataInauguracao: `1938`,
  DatasImportantes: [
    new Data(`1938`, 'data de inauguração - arte ambiente'),
    new Data(`01/1940`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9208183`,
  Longitude: `-43.1745739`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento em homenagem ao professor e diretor da Escola de Belas Artes, que fez discípulos entre outros: Eliseu Visconti, Lucílio de Albuquerque e Chamberland`,
  Fatos: [new Fact('', '', 'l', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SaoFranciscoDeAssiseSantaClara_EduardoDeSa: Obra = {
  ID: 385,
  Imagem: 'SaoFranciscoDeAssiseSantaClara_EduardoDeSa.jpeg',
  Authors: [authors.EduardoDeSa],
  Titulo: `São Francisco de Assis e Santa Clara`,
  Material: 'Bronze',
  DataInauguracao: `10/1927`,
  DatasImportantes: [new Data(`10/1927`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Luis de Camões',
  Latitude: `-22.9213639`,
  Longitude: `-43.1734884`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi erguido por uma iniciativa do Sr Amaro da Silveira, que doou a
    peça à Cidade, por ocasião do 7º centenário da morte do Santo. É a representação de Santa
    Clara e São Francisco, em tamanho natural. Representa São Francisco de Assis, sobre um pedestal
    de granito, de joelhos recitando o “Cântico do Sol”, tendo ao seu lado Santa Clara, orando. O
    conjunto é completado por quatro baixos-relevos, assinalando os principais episódios da vida
    do grande santo” As placas no pedestal não são originais. O Jornal do Brasil descreve o furto
    das placas na edição de 10 de abril de 1963.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Escoteiro_FernandoThauby: Obra = {
  ID: 386,
  Imagem: 'Escoteiro_FernandoThauby.jpeg',
  Authors: [authors.FernandoThauby],
  Titulo: `Escoteiro`,
  Material: 'Bronze',
  DataInauguracao: `12/1923`,
  DatasImportantes: [
    new Data(`12/1923`, 'data de inauguração'),
    new Data(`2019`, 'furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `A estátua representa um menino com o uniforme dos boys-scouts, segurando com sua mão direita, braço erguido a bandeira da Pátria Chilena e com a esquerda, o chapéu dos legionários do general Baden Powell. O boy-scout - segundo a concepção do Authors: - realizara uma longa caminhada para subir ao cume da serra, levando a bandeira de sua Pátria, e, ao chegar ao cimo descobre-se radiante.
    O monumento foi uma doação des crianças do Chile, em agradecimento a colaboração do Brasil em socorro das vítimas do terremoto de Coquimbo e Atacama.
    A estátua foifundida em 1915. Do molde original, foram tiradas duas cópias: uma delas estáno Rio, e asegunda encontra-se no Parque Metropolitano de Santiago.
    Essa estátua serviu de modelo para o selo comemorativo do AcampamentoInternacional de Patrulhas, realizado em 1954, em São Paulo. Em março de 2009 teve a bandeira furtada.
    Em maio de 2019, a escultura foi furtada.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SemTitulo_HaroldoBarroso: Obra = {
  ID: 387,
  Imagem: 'SemTitulo_HaroldoBarroso.jpeg',
  Authors: [authors.HaroldoBarroso],
  Titulo: `Sem Titulo`,
  Material: 'granito',
  DataInauguracao: `01/1997`,
  DatasImportantes: [new Data(`01/1997`, 'data de inauguração')],
  Propriedade: 'Privada',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Parque do Flamengo',
  Latitude: `-22.9144254`,
  Longitude: `-43.1717319`,
  Base: 'Sem pedestal',
  Status: 'Em cessão de uso em espaço público',
  Descricao: `Escultura contemporânea de granito, composta de três peças num único bloco.
    A peça esta em cessão de uso em área da Prefeitura.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Harmonia_MarliMazeredo: Obra = {
  ID: 388,
  Imagem: 'Harmonia_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Harmonia`,
  Material: 'resina e bronze',
  DataInauguracao: `07/1992`,
  DatasImportantes: [new Data(`07/1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Parque Carlos Lacerda',
  Latitude: '-22.9220198',
  Longitude: '-43.1706281',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça abstrata comemorativa da Eco-Rio 1992. Originalmente era composto por duas peças. Uma esta no parque e a outra no deposito da Prefeitura desde 2005 devido ao dano na sua base.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'comemorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const FelinoDeitado_HippolyteFrancoisAugusteHippolytePeyrol: Obra = {
  ID: 389,
  Imagem: 'FelinoDeitado_HippolyteFrancoisAugusteHippolytePeyrol.jpeg',
  Authors: [authors.HippolyteFrancoisAugusteHippolytePeyrol],
  Titulo: `Felino`,
  Material: 'mármore',
  DataInauguracao: `01/1906`,
  DatasImportantes: [
    new Data(`01/1906`, 'data de inauguração -arte ambiente'),
    new Data(`01/1910`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.9154769`,
  Longitude: `-43.1756772`,
  Base: 'Com pedestal',
  Status: 'Transferida',
  Descricao: `Inicialmente foram instalados nos jardins da Praia de Botafogo e transferido para a Praça Paris.Escultura do felino em posição de ataque, como guardião do Chafariz.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AlmiranteBarroso_JoseOtavioCorreaLima: Obra = {
  ID: 390,
  Imagem: 'AlmiranteBarroso_JoseOtavioCorreaLima.jpeg',
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: `Almirante Barroso`,
  Material: 'Bronze',
  DataInauguracao: `06/1908`,
  DatasImportantes: [new Data(`06/1908`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: '-22.918184',
  Longitude: '-43.1767396',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Transferida',
  Descricao:
    'Esta escultura de José Otávio Correa de Lima foi inaugurada na Praça Luís de Camões em 19 de novembro de 1909. Com as obras do Metrô, o monumento foi transferido para a Praça Paris, onde permaneceu desde então. Possui as seguintes legendas: “Riachuelo XI de junho de MDCCCLXV” e “Ao Almirante Barroso a nação”. Na base no monumento estão seus restos mortais. Essa pedra foi encontrada quando o monumento foi transferido da Praça Juares Tavola para a Praça Paris devido as obras do Metro. A pedra tem em baixo-relevo a seguinte inscrição: No dia 11 de junho de 1908, sendo presidente da República o Sr. Dr. Afonso Augusto Moreira Pena e ministro da Marinha o Exmo. Sr. Alexandrino de Alencar foi lançada esta pedra sobre a qual será erigido, por ordem do Exmo. Sr. Augusto Tavares de Lira, Ministro da Justiça e Negócios Interiores, de acordo com o Decreto nº 1697, de 10 de junho de 1907, o monumento ao Almirante Barroso e aos heróis da Batalha Naval de Riachuelo./ Este monumento foi inaugurado em 1909, na Praça Juarez Tavola, próxima ao Hotel Gloria. Com as obras do Metrô o monumento foi transferido para a Praça Paris onde se encontra desde então.O monumento descrito pelo Authors: Sobre um pedestal em granito de Petrópolis assenta a estátua de Barroso, em bronze, com quatro metros de altura, representada em atitude triunfal, no passadiço de comando. No sopé da coluna, em cada face lateral, assentam duas figuras aladas simbolizando, respectivamente, na gênios da Pátria e da Vitória, sobre duas proas, tudo em bronze. À frente do pedestal, um grande baixo-relevo, com dois metros e quarenta de comprimento, por um metro de alto, representa a Batalha Naval de Riachuelo. Quatro medalhões, nos ângulos, perpetuam as efígies de Oliveira Pimentel, Pedro Afonso, Andrade Maia e Lima Barros, encadeando esses medalhões nove placas, na base, com os nomes dos vapores e dos intrépidos comandantes que formaram parte numa batalha. Ainda dois medalhões abaixo contém as efígies dos heróis Greenhalgh e Marcílio Dias',
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const FranciscoAdolfoVarnhagen_JoseOtavioCorreaLima: Obra = {
  ID: 391,
  Authors: [authors.JoseOtavioCorreaLima],
  Titulo: `Francisco Adolfo Varnhagen`,
  Material: 'Bronze',
  DataInauguracao: `21/10/1938`,
  DatasImportantes: [new Data(`21/10/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Paris',
  Latitude: `-22.917001`,
  Longitude: `-43.177775`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem realizada pelo Instituto Histórico e Geográfico Brasileiro em comemoração ao centenário de sua fundação, no qual Adolfo Varnhagem foi primeiro-secretário.O busto a Varhagem esta num pedestal de granito, na parte inferior do pedestal se encontra a figura de umafeminina tendo um livro aberto nas mãos simbolizando a história.`,
  Fatos: [
    new Fact(
      '21/10/1938',
      '21/10/1938',
      'Inauguração',
      'O busto de Adolfo Varnhagem foi colocado celebração ao centenário da fundação do Instituto Histórico e Geográfico Brasileiro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '21/10/1938',
      '21/10/1938',
      'Homenageado',
      'Francisco Adolfo Varnhagem, primeiro secretário do Instituto Histórico e Geográfico Brasileiro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BrigadeiroEduardoGomes_EdgarDuvivier: Obra = {
  ID: 392,
  Imagem: 'BrigadeiroEduardoGomes_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `Brigadeiro Eduardo Gomes`,
  Material: 'Bronze',
  DataInauguracao: `26/10/1988`,
  DatasImportantes: [new Data(`26/10/1988`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Pistoia, Aterro do Flamengo',
  Latitude: `-22.9152736`,
  Longitude: `-43.174109`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao brigadeiro em trajes civis, foi uma doação de alunos da Escola Superior de Guerra.`,
  Fatos: [
    new Fact(
      `26/10/1988`,
      `26/10/1988`,
      'Homenagem',
      'Homenagem de uma turma da Escola Superior de Guerra',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      `01/02/1998`,
      `01/02/1998`,
      'Vandalismo',
      'A peça foi pichada por grafiteros.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      `25/08/1998`,
      `25/08/1998`,
      'Vandalismo',
      'A peça foi pichada por grafiteros.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      ``,
      ``,
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CardealArcoverde_HeitorUsai: Obra = {
  ID: 393,
  Imagem: 'CardealArcoverde_HeitorUsai.jpeg',
  Authors: [authors.HeitorUsai],
  Titulo: `Cardeal Arcoverde`,
  Material: 'Bronze',
  DataInauguracao: `07/1939`,
  DatasImportantes: [new Data(`07/1939`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Jardim da Glória',
  Latitude: `-22.9210361`,
  Longitude: `-43.1768286`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `É uma homenagem da Cidade do Rio de Janeiro, ao primeiro Cardeal do Brasil, no 1º Conciliato Brasileiro, permanecendo em frente ao Palácio Episcopal.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PedroAlvaresCabral_RodolfoBernardelli: Obra = {
  ID: 394,
  Imagem: 'PedroAlvaresCabral_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Pedro Àlvares Cabral`,
  Material: 'Bronze',
  DataInauguracao: `05/1900`,
  DatasImportantes: [new Data(`05/1900`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Largo da Glória',
  Latitude: `-22.9202612`,
  Longitude: `-43.1761148`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O Monumento foi incluído na programação das festas comemorativas do quarto
    centenário do Descobrimento do Brasil, celebradas no ano de 1900.
    O Monumento mede dez metros, com o pedestal hexagonal que é de granito com as esculturas
    representnado o navegadorr Pedro Alvares Cabral, o escrivão da frota Pedro Vaz Caminha e o
    Frei Henrique do Coimbra, capelão e celebrante da primeira missa. Numa das face do pedestal
    está gravado o seguinte: A Associação do Quarto Centenário do Descobrimento do Brasil mandou 
    erigir este Monumento `,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EduardoTapajos_GilbertoMandarino: Obra = {
  ID: 395,
  Imagem: 'EduardoTapajos_GilbertoMandarino.jpeg',
  Authors: [authors.GilbertoMandarino],
  Titulo: `Eduardo Tapajós`,
  Material: 'Bronze',
  DataInauguracao: `05/2000`,
  DatasImportantes: [
    new Data(`05/2000`, 'data de inauguração'),
    new Data(`11/2017`, 'furto'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Praça Juarez Tavola',
  Latitude: `-22.9230746`,
  Longitude: `-43.1723698`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Furtada',
  Descricao: `Peça academica com a fisionomia tranquila e traje elegante.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AberturadosPortos_EugeneBenet: Obra = {
  ID: 396,
  Imagem: 'AberturadosPortos_EugeneBenet.jpeg',
  Authors: [authors.EugeneBenet],
  Titulo: `Abertura dos Portos`,
  Material: 'Bronze',
  DataInauguracao: `01/1908`,
  DatasImportantes: [new Data(`01/1908`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Rua do Russel',
  Latitude: `-22.9222642`,
  Longitude: `-43.1726345`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Esse monumento foi uma iniciativa da Associção Comercial do Rio de Janeiro para as comemorações da abertura dos portos para as nações amigas promovida por D. João VI. O grupo escultórico é composto por duas figuras femininas em bronze de 3 metros de altura, uma representando o Comércio e a Navegação. Estas estátuas são inspiradas nas existentes na Praça da Concórdia em Paris. A estátua que simboliza o comércio tem mão direita apoiada em um escudo, e a da navegada esta apoiada em uma âncora.`,
  Fatos: [new Fact('', '', 'bem tombado municipal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AlbertoDeOliveira_PetrusVerdier: Obra = {
  ID: 397,
  Imagem: 'AlbertoDeOliveira_PetrusVerdier.jpeg',
  Authors: [authors.PetrusVerdier],
  Titulo: `Alberto de Oliveira`,
  Material: 'Bronze',
  DataInauguracao: `04/1928`,
  DatasImportantes: [new Data(`04/1928`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Rua do Russel',
  Latitude: `-22.920732`,
  Longitude: `-43.1746418`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao poeta tem a figura academica. O monumento foi inaugurado pelas poetisas Maria Eugenia e Ana Amelia de Queiroz Carneiro de Mendonça, que desceram a bandeira nacional que o cobria. O discurso de inauguração foi realizado pelo Ministro de Justiça Dr. Vianna de Castello.O monumento foi inaugurado no dia que se completo 30 anos da publicação de seu primeiro livro. `,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const LasarSegall_LiubaWolf: Obra = {
  ID: 398,
  Imagem: 'LasarSegall_LiubaWolf.jpeg',
  Authors: [authors.LiubaWolf],
  Titulo: `Lasar Segall`,
  Material: 'Bronze',
  DataInauguracao: `09/1964`,
  DatasImportantes: [new Data(`09/1964`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Glória',
  Endereco: 'Rua do Russel',
  Latitude: `-22.9220889`,
  Longitude: `-43.1727406`,
  Base: 'Com pedestal',
  MaterialBase: 'mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Em 1960, a artista Liuba , fez o busto em bronze emhomenagem ao pintor, mas somente instalado em 1964.
    A homenagem foi uma iniciativa do Departamento de Cultura do Estado, em comemoração do 30º aniversario da primeira exposição de arte moderna.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const ClarisseIndiodoBrasil_HCunhaMelo: Obra = {
  ID: 399,
  Imagem: 'ClarisseIndiodoBrasil_HCunhaMelo.jpeg',
  Authors: [authors.HCunhaMelo],
  Titulo: `Clarisse Índio do Brasil`,
  Material: 'Bronze',
  DataInauguracao: `01/1992`,
  DatasImportantes: [new Data(`01/1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Largo dos Leões',
  Latitude: `-22.9535532`,
  Longitude: `-43.1968612`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Na homenagem Clarisse está representada com a fisionomia tranquila, em volta em três figuras anjo na altura do
    ombro.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ClariceLispector_EdgarDuvivier: Obra = {
  ID: 399,
  Imagem: 'ClariceLispector_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `Clarisse Lispector e seu caompanheiro, o cão Ulisses`,
  Material: 'Bronze',
  DataInauguracao: `2016`,
  DatasImportantes: [new Data(`2016`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Largo dos Leões',
  Latitude: `-22.9535532`,
  Longitude: `-43.1968612`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Nascida na Ucrânia e naturalizada brasileira, a premiada Clarice Lispector é considerada uma das mais importantes escritoras brasileiras do século XX tendo sido traduzida para mais de 10 idiomas, do tcheco ao japonês. Com autoria de Edgar Duvivier, a escultura que traz Clarice com seu companheiro, o cão UIisses, está no bairro do Leme, onde ela viveu por 12 anos.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Chacrinha_Ique: Obra = {
  ID: 399,
  Imagem: 'Chacrinha_Ique.jpeg',
  Authors: [authors.Ique],
  Titulo: `Chacrinha`,
  Material: 'Bronze',
  DataInauguracao: `2010`,
  DatasImportantes: [new Data(`2010`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Jardim Botânico',
  Endereco: 'Rua General Garzon',
  Latitude: `-22.9672036`,
  Longitude: `-43.218367`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Caminhando pelas ruas do bairro Jardim Botânico podemos encontrar Abelardo Barbosa, o Chacrinha. Considerado um dos melhores apresentadores de televisão, O “Velho Guerreiro” desfila com uma gravata borboleta, um bacalhau na mão e a tradicional buzina pendurada no pescoço. A Estátua assinada pelo cartunista Ique Woitschach foi instalada, em 2010, no caminho que o apresentador fazia para o antigo Teatro Fênix. `,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Harmonia_JeanJacquesPradier: Obra = {
  ID: 400,
  Imagem: 'Harmonia_JeanJacquesPradier.jpeg',
  Authors: [authors.JeanJacquesPradier],
  Titulo: `Harmonia`,
  Material: 'ferro fundido',
  DataInauguracao: `11/1970`,
  DatasImportantes: [new Data(`11/1970`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Largo dos Leões',
  Latitude: `-22.9536396`,
  Longitude: `-43.1969072`,
  Status: 'Permanece em espaço público',
  Descricao: `Escultura de uma figura feminina segurando em uma das mãos uma harpa e a outra se
    apoiando em um tonel. Mão se sabe quando chegou ao Brasil, porém foi instalado após ter sido
    doada à cidade do Rio de Janeiro, pela antiga CEDAG (atualmente CEDAE  Companhia Estadual de
    Água e Esgoto) em 1970. De acordo com o Jornal do Brasil de 03 de novembro de 1970, a peça
    iria ser instalada no Largo da Carioca, cuja ideia foi rejeita. Posteriormente foi sugerida
    a instalação no Largo do Boticário tambem não aceita, sendo então alocada no Largo dos
    Leões, onde permanece até hoje. Desativado em data desconhecida.`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Utilitária',
  Natureza: 'Urbanística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AbrahamMedina_Desconhecida: Obra = {
  ID: 500,
  Imagem: 'AbrahamMedina_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Abraham Medina`,
  Material: 'Bronze',
  DataInauguracao: `01/1996`,
  DatasImportantes: [new Data(`01/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Av. Atlântica',
  Latitude: `-22.986188`,
  Longitude: `-43.188913`,
  SuporteEspacial: 'Calçadão',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem para lembrar o carioca entusiasta que promoveu e valorizou a cidade.`,
  Fatos: [
    new Fact(
      '1999',
      '1999',
      'Furto',
      'O óculos da obra foi roubado',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Fundição/Atelier', "Porão d'Arte", [], 'Rio de Janeiro'),
    new Fact('', '', 'Doação', 'Obra doada à cidade', [], 'Rio de Janeiro'),
    new Fact('', '', 'Homenagem', 'Homenagem dos amigos', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GetulioVargasBenfica_Desconhecida: Obra = {
  ID: 501,
  Imagem: 'GetulioVargasBenfica_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Getulio Vargas`,
  Material: 'Bronze',
  DataInauguracao: `01/1996`,
  DatasImportantes: [new Data(`01/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Benfica',
  Endereco: 'R. Dom Hélder Câmara',
  Latitude: `-22.8856285`,
  Longitude: `-43.2521434`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao ex-presidente foi erguida pelo reconhecimento e esta localizada no interior do conjunto dos
    ex-combatentes.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra - AGR',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PadreGuilhermeDecaminada_Desconhecida: Obra = {
  ID: 502,
  Imagem: 'PadreGuilhermeDecaminada_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Padre Guilherme Decaminada`,
  Material: 'Bronze',
  DataInauguracao: `12/1997`,
  DatasImportantes: [new Data(`12/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Santa Cruz',
  Endereco: 'Praça Dom Romualdo',
  Latitude: `-22.9196488`,
  Longitude: `-43.683583`,
  SuporteEspacial: 'Praça',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao padre missionário foi erguido por gratidão ao seu trabalho incansavelmente na região.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlvaroCostaMelo_Desconhecida: Obra = {
  ID: 503,
  Imagem: 'AlvaroCostaMelo_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Álvaro Costa Melo`,
  Material: 'Bronze',
  DataInauguracao: `10/1996`,
  DatasImportantes: [new Data(`10/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Bonsucesso',
  Endereco: 'R. Cardoso de Moraes',
  Latitude: `-22.862519`,
  Longitude: `-43.254069`,
  SuporteEspacial: 'Trevo',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto é uma homenagem da Força Empresarial de Bonsucesso ao empresário local. Foi uma iniciativa do
    gerente do Unibanco Wilson Xavier que angariou fundos e instalou um busto.`,
  Fatos: [
    new Fact('', '', 'Fundição/Atelier', 'Laborate', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '01/05/1993',
      'Homenageado',
      'Alvaro da Costa Mello',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '07/05/1993',
      '07/05/1993',
      'Missa de 7 dia',
      'Missa Realizada as 10h na Igreja da Candelaria',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenagem',
      'Força Empresarial de Bonsucesso, iniciativa do gerente do Unibanco Wilson Xavier que angariou fundos e instalou o busto',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AlmiranteMiguelGrau_PedroMMartinezCorada: Obra = {
  ID: 504,
  Imagem: 'AlmiranteMiguelGrau_PedroMMartinezCorada.jpeg',
  Authors: [authors.PedroMMartinezCorada],
  Titulo: `Almirante Miguel Grau`,
  Material: 'Bronze',
  DataInauguracao: `01/1994`,
  DatasImportantes: [new Data(`01/1994`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Av. Ayrton Senna',
  Latitude: `-23.010372`,
  Longitude: `-43.366192`,
  SuporteEspacial: 'Canteiro',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Busto foi doado à cidade do Rio de Janeiro pela Marinha de Guerra Peruana.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Marinha de Guerra Peruana',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ChicoXavier_Desconhecida: Obra = {
  ID: 505,
  Imagem: 'ChicoXavier_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Chico Xavier`,
  Material: 'gesso',
  DataInauguracao: `01/1997`,
  DatasImportantes: [new Data(`01/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Depósito Fundação Parques e Jardins',
  Latitude: '-22.906689',
  Longitude: '-43.197172',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Removida do espaço público',
  Descricao:
    'Busto encontrado nos jardins do Parque Lage e doada a Fundação Parques e Jardins, depósito',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JoaoPauloII_MarioAgostinelli: Obra = {
  ID: 506,
  Imagem: 'JoaoPauloII_MarioAgostinelli.jpeg',
  Authors: [authors.MarioAgostinelli],
  Titulo: `João Paulo II`,
  Material: 'Bronze',
  Altura: '250 cm',
  Peso: '450 kg',
  ValorObra: 'R$ 150.000,00',
  DataInauguracao: `04/10/1997`,
  DatasImportantes: [new Data(`04/10/1997`, 'data de inauguração')],
  Promotor:
    'Secretaria de Projetos Especiais da Prefeitura da Cidade do Rio de Janeiro',
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida República do Chile, 245',
  Latitude: `-22.909576`,
  Longitude: `-43.1811314`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A peça foi confeccionada durante dois meses e meio. A iniciativa da Prefeitura da Cidade do Rio de Janeiro fez parte da organização do II Encontro Mundial das famílias com o Papa João Paulo II. O pontifície é representado de corpo inteiro numa postura típica de acolhimento. Junto a ele, encontra-se uma placa com as inscrições: Papa João Paulo II. E conhecereis a verdade e a verdade vos libertará.Jardim da Catedral Metropolitana de São Sebastião,`,
  Fatos: [
    new Fact(
      '1997',
      '1997',
      'Encomenda oficial',
      'Secretaria de Projetos Especiais da Prefeitura da Cidade do Rio de Janeiro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1997',
      '1997',
      'Patrocínio',
      '150 empresários de origens árabe e judaica.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '04/10/1997',
      '04/10/1997',
      'Inauguração',
      'A estátua foi inaugurada logo após a celebração da missa para 5 mil convidados, realizada pelo Papa João Paulo II',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '06/11/1997',
      '06/11/1997',
      'Vandalismo',
      'Grafitaram a estátua do Papa João Paulo II',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '11/11/1997',
      '11/11/1997',
      'Limpeza',
      'Assim que foi informada da pichação, a Secretaria Especial de Monumentos Públicos mandou verificar a obra. A limpeza foi feita com solventes especiais para remoção de tinta.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const MacacoTiao_GianPietroZanni: Obra = {
  ID: 507,
  Imagem: 'MacacoTiao_GianPietroZanni.jpeg',
  Authors: [authors.GianPietroZanni],
  Titulo: `Macaco Tião`,
  Material: 'Bronze',
  DataInauguracao: `08/1997`,
  DatasImportantes: [new Data(`08/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'São Cristóvão',
  Endereco: 'Jardim Zoologico',
  Latitude: `-22.904452`,
  Longitude: `-43.2295205`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua homenageando o mais popular animal na história do Zoológico do Rio de Janeiro, morto em 1996, patrocinada
    pela rede de lanchonetes Mc Donalds, que o adotara dois anos antes.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'celebratória',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const OMascateRiachuelo_HonorioPecanha: Obra = {
  ID: 508,
  Imagem: 'OMascateRiachuelo_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `O Mascate `,
  Material: 'Bronze',
  DataInauguracao: `03/1996`,
  DatasImportantes: [new Data(`01/1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Riachuelo',
  Endereco: 'Rua Vinte e Quatro de Maio',
  Latitude: `-22.9024367`,
  Longitude: `-43.2586028`,
  SuporteEspacial: 'Calçada',
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Réplica da escultura de um homem com alegorias, uma amostra de tecido num braço, e com a outra mão abrindo um baú de
    mercadorias. É uma homenagem do SENAC ao pioneiro do comercio no Brasil. É uma reprodução do original realizado em 1964.`,
  Fatos: [
    new Fact(
      '1964',
      '1964',
      'Original',
      'A escultura é uma cópia ampliada do troféu O Mascate, esculpido por Honório Peçanha',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const DuploTotem_CeleidaTostes: Obra = {
  ID: 509,
  Imagem: 'DuploTotem_CeleidaTostes.jpeg',
  Authors: [authors.CeleidaTostes],
  Titulo: `Duplo Totem`,
  Material: 'argamassa',
  DataInauguracao: `01/1995`,
  DatasImportantes: [new Data(`01/1995`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Gávea',
  Endereco: 'Parque da Cidade',
  Latitude: `-22.9799141`,
  Longitude: `-43.2418108`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de duas peças formando conjunto.`,
  Fatos: [
    new Fact(
      '1965',
      '1965',
      'Tombamento',
      'Tombado pelo INEPAC pelo Decreto 927.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const EliasGoraybe_Desconhecida: Obra = {
  ID: 510,
  Imagem: 'EliasGoraybe_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Elias Goraybe`,
  Material: 'Bronze',
  DataInauguracao: `11/1997`,
  DatasImportantes: [new Data(`11/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'R. Padre Elias Goraybe',
  Latitude: `-22.925928`,
  Longitude: `-43.233995`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem ao primeiro pároco da Matriz Maronita Nossa Senhora do Líbano, erigido por uma iniciativa da comunidade
    libanesa do Rio de Janeiro.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística e Metalúrgica Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Laranja_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 511,
  Imagem: 'Laranja_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Laranja`,
  Material: 'Aço',
  ValorObra: 'R$ 36050,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'Av. Cesário de Melo',
  Latitude: `-22.9067898`,
  Longitude: `-43.5609534`,
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de uma escultura em aço pintado na cor laranja vazada criando uma forma de uma laranja. A obra custou 4 vezes o valor da escultura Prometeu, de Bruno Giorgi, avaliada em R$ 8mil`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Broca_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 512,
  Imagem: 'Broca_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Broca`,
  Material: 'ferro pintado',
  ValorObra: 'R$ 4070,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'R. Coronel Agostinho',
  Latitude: `-22.9028566`,
  Longitude: `-43.5590348`,
  Status: 'Permanece em espaço público',
  Descricao: `Escultura representando uma broca como saindo do solo.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ArcoIris_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 513,
  Imagem: 'ArcoIris_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Arco Ìris`,
  Material: 'ferro pintado',
  ValorObra: 'R$ 9.870,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'R. Coronel Agostinho',
  Latitude: `-22.9028566`,
  Longitude: `-43.5590348`,
  Status: 'Permanece em espaço público',
  Descricao: `É uma escultura composta por colunas feitas em chapa de aço retangular unidas por peças do mesmo material
    pintada em diversas cores para representar um arco-íris.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Laranjais_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 514,
  Imagem: 'Laranjais_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Laranjais`,
  Material: 'argamassa',
  ValorObra: 'R$ 42.280,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'R. Coronel Agostinho',
  Latitude: `-22.9025031`,
  Longitude: `-43.5600507`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Colunas de concreto com uma semiesfera na extremidade, representando os laranjais
    existentes na região durante o período de ocupação rural. Cada colunata custou a época foi
    R42.280,00.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Colunatas_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 515,
  Imagem: 'Colunatas_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Colunatas`,
  Material: 'aço e concreto',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'Praça Dr. Raul Boaventura e Viúva Dantas',
  Latitude: `-22.9025031`,
  Longitude: `-43.5600507`,
  Status: 'Permanece em espaço público',
  Descricao: `Colunas de concreto com uma semiesfera na extremidade, representando os laranjais existentes na região durante o
    período de ocupação rural. Cada colunata custou a época foi R$2.280,00.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Piramide_GeorgeLopes_GeorgeGuerra: Obra = {
  ID: 516,
  Imagem: 'Piramide_GeorgeLopes_GeorgeGuerra.jpeg',
  Authors: [authors.GeorgeLopes, authors.GeorgeGuerra],
  Titulo: `Pirâmide`,
  Material: 'Aço',
  ValorObra: 'R$ 17.520,00',
  DataInauguracao: `07/1996`,
  DatasImportantes: [new Data(`07/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'Praça Dr. Raul Boaventura',
  Latitude: `-22.9024629`,
  Longitude: `-43.5596437`,
  Status: 'Permanece em espaço público',
  Descricao: `Escultura formando uma pirâmide representando o desenvolvimento da região.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Urbanística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const SaoFranciscodePaula_GiovanniBattistaMaini: Obra = {
  ID: 518,
  Imagem: 'SaoFranciscodePaula_GiovanniBattistaMaini.jpeg',
  Authors: [authors.GiovanniBattistaMaini],
  Titulo: `São Francisco de Paula`,
  Material: 'argamassa',
  DataInauguracao: `11/1995`,
  DatasImportantes: [new Data(`11/1995`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Praça Euvaldo Lodi',
  Latitude: `-23.008364`,
  Longitude: `-43.3046528`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A escultura foi inaugurada na data comemorativa dos 40 anos da Ordem dos Mínimos do Brasil na Barra da Tijuca.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Réplica',
      'Réplica da obra do escultor setecentista Giovanni Battista Maini, na Basílica de São Pedro, no Vaticano.',
      [],
      'Roma',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Carnica_AnaluNabuco: Obra = {
  ID: 519,
  Imagem: 'Carnica_AnaluNabuco.jpeg',
  Authors: [authors.AnaluNabuco],
  Titulo: `Carniça`,
  Material: 'Bronze',
  DataInauguracao: `01/1997`,
  DatasImportantes: [new Data(`01/1997`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Praça Professor Souza Araújo',
  Latitude: `-23.0069404`,
  Longitude: `-23.0069404`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura patrocinada pelo Apoio Espaço 41 Oficina de Arte e doada à Cidade do Rio de Janeiro.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Doação',
      'Patrocínio Espaço 41 Oficina de Arte',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Atena_Desconhecida: Obra = {
  ID: 520,
  Imagem: 'Atena_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Atena`,
  Material: 'mármore',
  DataInauguracao: `01/1992`,
  DatasImportantes: [
    new Data(`01/1992`, 'data de inauguração'),
    new Data(`1994`, 'reinstação'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Jacarepaguá',
  Endereco: 'Bosque da Freguesia',
  Latitude: `-22.9473113`,
  Longitude: `-43.3468612`,
  Base: 'Sem pedestal',
  Status: 'Transferida',
  Descricao: `A escultura que representa a Deusa das Artes. Foi retirada em 1988 da Praça das Nações e transferida para o depósito da Prefeitura da época. Em 1994 foi instalada para o parque a fim de embelezar o espaço.`,
  Fatos: [
    new Fact(
      '1988',
      '1988',
      'Remoção',
      'Praça das Nações',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1994',
      '1994',
      'Realocação',
      'A obra foi reinstalada no Bosque da Freguesia a fim de embelezar o espaço',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Menora_Desconhecida: Obra = {
  ID: 521,
  Imagem: 'Menora_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Menorá`,
  Material: 'Concreto',
  DataInauguracao: `01/1995`,
  DatasImportantes: [new Data(`01/1995`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça Cardeal Arco Verde',
  Latitude: `-22.9649633`,
  Longitude: `-43.1801527`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi idealizado pelo Vereador Gerson Berge, devido a estar próximo a
    primeira sinagoga do Rio de Janeiro, e ser Copacabana o bairro que acolhe uma grande parte da
    comunidade Judaica no Rio de Janeiro. O monumento retirado em 1988, a fim de possibilitar as
    obras de construção, da estação do metro de Copacabana, foi reinstalado em 12 de dezembro de
    2004, por iniciativa da organização judaica Beit Lubavitch pela CHL Incorporações.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Celebratória',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Animais_JorgeMarioJauregui: Obra = {
  ID: 523,
  Imagem: 'Animais_JorgeMarioJauregui.jpeg',
  Authors: [authors.JorgeMarioJauregui],
  Titulo: `Animais`,
  Material: 'Aço',
  DataInauguracao: `03/1996`,
  DatasImportantes: [new Data(`03/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Flamengo',
  Endereco: 'Praça José de Alencar',
  Latitude: `-22.9335555`,
  Longitude: `-43.1775038`,
  Status: 'Permanece em espaço público',
  Descricao: `Peças representando animais recortados fixadas ao longo de um mural. Foi instalado no Projeto Rio-Cidade na área de lazer infantil.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Lâmina escultórica',
  Execucao: true,
};

export const ADiasdaCruz_VicenteDelRio_LuizCarlosToledo: Obra = {
  ID: 524,
  Imagem: 'ADiasdaCruz_VicenteDelRio_LuizCarlosToledo.jpeg',
  Authors: [authors.VicenteDelRio, authors.LuizCarlosToledo],
  Titulo: `À Dias da Cruz`,
  Material: 'Bronze',
  DataInauguracao: `09/1996`,
  DatasImportantes: [new Data(`09/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Méier',
  Endereco: 'R. Dias da Cruz ',
  Latitude: `-22.9021836`,
  Longitude: `-43.2802868`,
  Base: 'Com pedestal',
  MaterialBase: 'Pedra',
  Status: 'Furtada',
  Descricao: `O monumento, um monolito com blocos de pedra de mão, tem a efígie do médico. A peça em bronze é cópia da
    existente na Praça Tobias Barreto, em Vila Isabel. A efígie foi furtada em 2009.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const AAugustoMotta_Desconhecida: Obra = {
  ID: 525,
  Imagem: 'AAugustoMotta_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `À Augusto Motta`,
  Material: 'Bronze',
  DataInauguracao: `09/1996`,
  DatasImportantes: [new Data(`09/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Bonsucesso',
  Endereco: 'Largo Augusto Motta',
  Latitude: `-22.8655207`,
  Longitude: `-43.2550024`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao professor e educador, foi doada pelo Rotary Club Leopoldinense e
    inaugurado na reforma da praça promovida pelo Rio-Cidade Bonsucesso.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const FernandoBaptistaGoncalves_EdgarDuvivierFilho: Obra = {
  ID: 526,
  Imagem: 'FernandoBaptistaGoncalves_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Fernando Baptista Gonçalves`,
  Material: 'Bronze',
  DataInauguracao: `02007`,
  DatasImportantes: [
    new Data(`2007`, 'data de inauguração - arte-cidade'),
    new Data(`01/2009`, 'data de inauguração inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Rua Humaitá',
  Latitude: `-22.9558313`,
  Longitude: `-43.1981995`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de uma homenagem dos antigos alunos do Colégio Padre Antonio Vieira ao mestre. Foi doada à Cidade pela
    associação dos alunos.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'FEB- Fundição e Esculturas em Bronze',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SagradoCoracaodeJesus_Desconhecida: Obra = {
  ID: 527,
  Imagem: 'SagradoCoracaodeJesus_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Sagrado Coração de Jesus`,
  Material: 'Ferro fundido',
  DataInauguracao: `1955`,
  DatasImportantes: [new Data(`1955`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Rua Frei Solano',
  Latitude: `-22.9620994`,
  Longitude: `-43.2038849`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A imagem do Sagrado Coração de Jesus está com a presença conhecida na praça desde 1971.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JCarlos_HumbertoCozzo: Obra = {
  ID: 528,
  Imagem: 'JCarlos_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: `J.Carlos`,
  Material: 'Bronze',
  DataInauguracao: `06/1961`,
  DatasImportantes: [new Data(`06/1961`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Praça Henrique Brito e Cunha',
  Latitude: `-22.9604587`,
  Longitude: `-43.203308`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi instalada nessa praça pela proximidade a residência de J Carlos, por iniciativa de seus amigos. A cabeça em bronze se sustenta sobre uma coluna circular de granito preto, onde existe uma gravação em baixo relevo com o nome do cartunista e diversos rostos femininos.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const ManuelMadruga_FloryGama: Obra = {
  ID: 529,
  Imagem: 'ManuelMadruga_FloryGama.jpeg',
  Authors: [authors.FloryGama],
  Titulo: `Manuel Madruga`,
  Material: 'Bronze',
  DataInauguracao: `07/1958`,
  DatasImportantes: [new Data(`07/1958`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Praça Henrique Brito e Cunha',
  Latitude: `-22.9605414`,
  Longitude: `-43.2037342`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Foi uma homenagem da Sociedade dos Artistas Nacionais e a Sociedade Brasileira de Belas Artes no 1º aniversário da morte do grande artista.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const QuintinoBocaiuva_HildegardoLeaoVelloso: Obra = {
  ID: 530,
  Imagem: 'QuintinoBocaiuva_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Quintino Bocaiúva`,
  Material: 'Bronze',
  DataInauguracao: `11/1944`,
  DatasImportantes: [new Data(`11/1944`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Humaitá',
  Endereco: 'Praça General Alcio Souto',
  Latitude: `-22.9610274`,
  Longitude: `-43.2031116`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi o projeto vitorioso de um consurso publico. O Authors: resolveu adotar um projeto simples, com o homenageado em gruande dimessão ( cerca de onze metros de altura), com a descrição biografica na parte posterior.
    Foi inicialmente instalado na antiga Praça Piaçava, atual José Mariano, no Jardim Botânico. Por ocasião da construção do viaduto pelo DER, o monumento não foi desmontado. Em 1982, foi restaurado e assentado sobre um pedestal de concreto armado.`,
  Fatos: [new Fact('', '', 'bem tombado estadual', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const EusebioDeOliveira_CesarDoria: Obra = {
  ID: 531,
  Imagem: 'EusebioDeOliveira_CesarDoria.jpeg',
  Authors: [authors.CesarDoria],
  Titulo: `Eusébio de Oliveira`,
  Material: 'Bronze',
  DataInauguracao: `12/1941`,
  DatasImportantes: [new Data(`12/1941`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Avenida Pasteur',
  Latitude: `-22.9539283`,
  Longitude: `-43.169591`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi erguida por iniciativa do Departamento Nacional de Produção Mineral e do Ministério da Agricultura, em frente à sede do antigo Serviço Geológico e Mineralógico do Brasil,que por longos anos foi diretor dessa tradicional instituição científica.
    Na obra, foram empregados blocos de pedras de cores azul, verde e rosa, simbolizando a especialização em que tanto se aprofundou o espírito pesquisador do geólogo homenageado.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const HeroisdeLagunaeDourados_AntonioPintodeMattos: Obra = {
  ID: 532,
  Imagem: 'HeroisdeLagunaeDourados_AntonioPintodeMattos.jpeg',
  Authors: [authors.AntonioPintodeMattos],
  Titulo: `Heróis de Laguna e Dourados`,
  Material: 'Bronze',
  DataInauguracao: `12/1938`,
  DatasImportantes: [new Data(`12/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.9546327`,
  Longitude: `-43.1666687`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A idéia da elevação de um monumento aos heróis do drama épico da Retirada da Laguna e de Dourados, capitaneados pelo Tenente Antônio João, data de 1920. O primeiro projeto proposto foi de Authors:ia do General Maciel de Miranda, e deveria ser erguido no pátio interno do Quartel-general do Rio de Janeiro, depois Ministério da Guerra. O atual monumento, em bronze e granito foi uma iniciativa do Coronel Pedro Cordolino Ferreira de Azevedo.
    Sobre uma base circular, esta uma cripta onde se encontram os retos mortais do Coronel Camisão, do Guia Lopes e o cenotáfio do Tenente Antônio João, desde 15 de novembro de 1941 quando foi inaugurada. Desta ergue-se a coluna onde esta a alegoria da Vitória. Ao centro da coluna estão as estátuas dos três principais heróis: o Tenente Antônio João, no momento em que, baleado, cambaleando; o Guia Lopes, recurvado, apoiando o queixo no dorso da mão esquerda, e o Coronel Camisão, com a fisionomia de quem tem uma grave decisão a tomar, tendo numa das mãos a espada e na outra o mapa de campanha. Mais acima, do ponto onde parte a coluna, erguem-se três grandes alegorias em bronze, representando a Pátria, a Espada e a História
    Diversos artistas participaram na execução monumento: a grade à entrada da cripta é de Curzio Zani; a porta de entrada do recinto de Calmon Barreto; a imagem de Cristo sobre a lápide do mesmo artista; o lanceiro guardando o sarcófago de Leão Veloso, e os sete medalhões com efígies e nomes dos heróis de Adalberto de Matos.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const FredericChopin_AugustZamoiski: Obra = {
  ID: 533,
  Imagem: 'FredericChopin_AugustZamoiski.jpeg',
  Authors: [authors.AugustZamoiski],
  Titulo: `Frédéric Chopin`,
  Material: 'Bronze',
  DataInauguracao: `1939`,
  DatasImportantes: [
    new Data(`1939`, 'data de inauguração - arte ambiente'),
    new Data(`09/1944`, 'data de inauguração - inventário dos monumentos'),
    new Data(`1950`, 'data transferência para a Praça Floriano Peixoto'),
    new Data(`30/06/1959`, 'data transferência para o local de origem'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praia Vermelha',
  Latitude: `-22.9552611`,
  Longitude: `-43.1660723`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado na Praia Vermelha em 1944.
    O monumento foi uma iniciativa do professor Aloísio de Castro e da
    Associação dos Poloneses do Rio de Janeiro, em desagravo à destruição da estátua de Chopin, em
    Varsóvia, pelas tropas alemãs invasoras , no quinto aniversário da deflagração da Segunda
    Guerra Mundial com a invasão da Polonia. Foi doado pela colônia polonesa ao Brasil. Trata-se de
    uma estatua em bronze, sobre pedestal cúbico, representado o compositor de pé com expressão
    pensativa, chamado pelos polonose "Estatua da Saudade"`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AdelinoMoreira_MestreSaul: Obra = {
  ID: 1166,
  Imagem: 'AdelinoMoreira_MestreSaul.jpeg',
  Authors: [authors.MestreSaul],
  Titulo: `Adelino Moreira`,
  Renomeacao: false,
  Classificacao: 'perene',
  Categoria: 'plasticidade',
  Tipologia: `Estátua`,
  Material: `Peça em bronze`,
  Descricao: `Estátua ao compositor morador do bairro. Sentado em uma cadeira segurando um violão e apoiando a outra mão
    sobre uma mesa, como se estivesse escrevendo uma composição.`,
  Execucao: true,
  DataInauguracao: `01/2008`,
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'R. Campo Grande',
  Latitude: `-22.9018191`,
  Longitude: `-43.5589983`,
  Fatos: [new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro')],
};

export const MarechalCandidoRondon_NewtonSa: Obra = {
  ID: 534,
  Imagem: 'MarechalCandidoRondon_NewtonSa.jpeg',
  Authors: [authors.NewtonSa],
  Titulo: `Marechal Rondon`,
  Material: 'Bronze',
  DataInauguracao: `09/1980`,
  DatasImportantes: [new Data(`09/1980`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praia Vermelha',
  Latitude: `-22.9544749`,
  Longitude: `-43.1654532`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi erigido em comemoração ao 15 º aniversário da Embratel (Empresa Brasileira de Telecomunicações),
    ao seu patrono. O Marechal esta representando com sua farda. Foi inaugurado pelo General Frederico Rondon, primo do homenageaso .`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const IntentonaComunista_HumbertoCozzo: Obra = {
  ID: 535,
  Imagem: 'IntentonaComunista_HumbertoCozzo.jpeg',
  Authors: [authors.HumbertoCozzo],
  Titulo: `Intentona Comunista`,
  Material: 'Bronze',
  DataInauguracao: `01/1974`,
  DatasImportantes: [new Data(`01/1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.9551823`,
  Longitude: `-43.1665068`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Em 1935, ocorreu a tentativa de golpe de estado no Rio de Janeiro, com a sublevação
    do 3º Regimento de Infantaria, na Praia Vermelha. Enquanto os outros focos de insurreição foram
    debelados no 3º Regimento de Infantaria, sob a chefia de Agildo Barata, provocou a morte de
    31 militares legalistas. No dia seguinte, após a pronta ação do General Dutra, esse derradeiro
    e mais sanguinário foco do levante nacional capitaneado por Luiz Carlos Prestes estava
    completamente vencido. O resultado, a longo prazo serviu para a instalação da ditadura do
    Estado Novo, em 1937. As vítimas, originalmente sepultadas no Cemitério de São João Batista,
    foram para esse monumento estilo art decor.Ao centro, numa coluna ladeada por duas urnas,
    vê-se a figura da República, com uma coroa de louros nas mãos. A seus pés, mais à frente,
    ergue-se a estátua em bronze de um soldado ferido. O Monumento em 1974 foir transferido do Cemitério São João Batista para a praça.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Vestal_Desconhecida: Obra = {
  ID: 536,
  Imagem: 'Vestal_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Vestal`,
  Material: 'Mármore',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.954543`,
  Longitude: `-43.1653888`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina com uma chama sagrada nas mãos, uma representação neoclássica.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Ceres_Desconhecida: Obra = {
  ID: 537,
  Imagem: 'Ceres_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Ceres`,
  Material: 'Mármore',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.9555294`,
  Longitude: `-43.1655727`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina com espigas de trigo em suas mãos, numa representação neoclássica`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const Flora_Desconhecida: Obra = {
  ID: 538,
  Imagem: 'Flora_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Flora na Urca`,
  Material: 'Mármore',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.954543`,
  Longitude: `-43.1653888`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina neoclássica representando Flora com a cabeça coroada de flores.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Pomona_Desconhecida: Obra = {
  ID: 539,
  Imagem: 'Pomona_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Pomona`,
  Material: 'Mármore',
  DataInauguracao: `01/1963`,
  DatasImportantes: [new Data(`01/1963`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Praça General Tibúrcio',
  Latitude: `-22.9555294`,
  Longitude: `-43.1655727`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura feminina com frutos nas mãos de linhas neoclássicas. `,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};
export const SaoPedrodoMar_EdgarDuvivier: Obra = {
  ID: 540,
  Imagem: 'SaoPedrodoMar_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `São Pedro do Mar`,
  Material: 'Argamassa',
  DataInauguracao: `07/1959`,
  DatasImportantes: [new Data(`07/1959`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Urca',
  Endereco: 'Rua Portugal',
  Latitude: `-22.9483318`,
  Longitude: `-43.165671`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Singela escultura de corpo de cilindrico, representando o Apóstolo sobre uma pedra da baía.
    Segundo o Sr. José Carlos Audiface de Brito " Indo a Itália após Torneio Mundial de Caça Submarino, conheci em Rapallo o Cristo submerso. Tive então a ideia da imagem de São Pedro. Regressando ao Brasil, conseguiu o apoio do caçador e jornalista João Carlos Vogt e do escultor Edgard Duvivier Com subscrição levantou os fundos necessários à fundição. A ideia inicial de colocá-la no fundo do mar nas Cagaras, foi abandonada para possibilitar o acesso a mesma de maior número de persons, opinando-se pela colocação na Urca em frente a igreja cujo o pároco padre Barbosa, que colaborou com o projeto. A imagem de São Pedro executada para ser olhada de cima para baixo, na previsão inicial de colocá-la no fundo do mar nas ilhas Cagaras, Na cerimônia de colocação, em 1959, participaram os membros da equipe brasileira ao mencionado I Torneio Mundial de Caça Submarina em Portugual, e os desportistas Vitório Berredo, Victor Wellisch, Américo Santarelle e outros."`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const DelPrete_RobertoSa: Obra = {
  ID: 581,
  Imagem: 'DelPrete_RobertoSa.jpeg',
  Authors: [authors.RobertoSa],
  Titulo: `Del Prete`,
  Material: 'Bronze',
  DataInauguracao: `1997`,
  DatasImportantes: [
    new Data(`1997`, 'data de inauguração - arte ambiente'),
    new Data(`11/1999`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Laranjeiras',
  Endereco: 'Praça Del Prete',
  Latitude: `-22.9330908`,
  Longitude: `-43.1845779`,
  Status: 'Permanece em espaço público',
  Descricao: `Monumento composto pela estatua do piloto em tamanho real vestindo um uniforme de piloto, uma réplica de um avião,
    suspensa a cerca de sete metros do chão e uma grande placa esta contada a historia do piloto. Inicialmente Del Prete foi
    homenageado com um busto na Rua das Laranjeiras , atualmente no deposito da Prefeitura. Em 1997, o Prefeito Luís Paulo Conde
    mandou refazer a homenagem escala gigante.`,
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Celebratória',
  Natureza: 'Cívica',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const EdmundoBittencourt_HildegardoLeaoVelloso: Obra = {
  ID: 582,
  Imagem: 'EdmundoBittencourt_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Edmundo Bittencourt`,
  Material: 'Bronze',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  DataInauguracao: `05/1951`,
  DatasImportantes: [new Data(`05/1951`, 'data de inauguração')],
  Zona: 'Sul',
  Bairro: 'Bairro Peixoto',
  Endereco: 'Praça Edmundo Bittencourt',
  Latitude: `-22.9665292`,
  Longitude: `-43.1903542`,
  Descricao: `O monumento foi erguido por Iniciativa de um grupo de admiradores`,
  Fatos: [
    new Fact(
      '',
      '',
      'Comissão de homenagem',
      'Conferenciaram com o Prefeito Carlos Vital os membros da Comissão que dá apoio a homenagem, composto pelo ministro Horácio Laffer, jornalistas Herbert Moses, João da Silveira, Octávio Gualberto, Pedro Brandão e Chermont de Brito. O prefeito colocou todos os recursos da municipalidade ao dispor da comissão visitante.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Propriedade: 'Pública',
  Status: 'Permanece em espaço público',
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ComendadorPauloFPeixotoDaFonseca_HonorioPecanha: Obra = {
  ID: 582,
  Imagem: 'ComendadorPauloFPeixotoDaFonseca_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Comendador Paulo F. Peixoto da Fonseca`,
  Material: `Bronze`,
  DataInauguracao: `12/1944`,
  DatasImportantes: [new Data(`12/1944`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Bairro Peixoto',
  Endereco: 'Praça Edmundo Bittencourt',
  Latitude: `-22.9674093`,
  Longitude: `-43.1896705`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `No singelo monumento de granito bruto tem uma placa fixada a efígiedo Comendador e uma outra alusivaàssuas doações à Cidade.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const AlbertEinstein_HonorioPecanha: Obra = {
  ID: 582,
  Imagem: 'AlbertEinstein_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Albert Einstein`,
  Material: 'Bronze',
  DataInauguracao: `07/1979`,
  DatasImportantes: [new Data(`07/1979`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Bairro Peixoto',
  Endereco: 'Praça Edmundo Bittencourt',
  Latitude: `-22.9670196`,
  Longitude: `-43.1895441`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado nocentenário do nascimentode Albert Einstein, por iniciativa daLoja Albert Einstein  de b&rsquo;Nai  de B&rsquo;Ricth do Brasil. O ato foi prescedido de uma benção ecunemica com os religiosos Dom Estevão Bittencourt, Partos Nehemias Marien e o Rabino Nersod Benesay. O descerraemnto foi feito por representantes diplomático de Israel e Alemanha.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Cabeça',
  Execucao: true,
};

export const IbrahimSued_MarcosAndreSales: Obra = {
  ID: 582,
  Imagem: 'IbrahimSued_MarcosAndreSales.jpeg',
  Authors: [authors.MarcosAndreSales],
  Titulo: `Ibrahim Sued`,
  Material: 'Bronze',
  DataInauguracao: `09/2004`,
  DatasImportantes: [new Data(`09/2004`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Atlantica',
  Latitude: `-22.967595`,
  Longitude: `-43.1786041`,
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi iniciativa de sua filha e realizada por subscrição de seus amigos jornalistas. Foi uma doação à
    Cidade. A inauguração contou com a presença do Prefeito Cesar Maia e o neto de Ibrahim agradeceu a homenagem.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const SerenaDamadoMar_LuizFigueiredo: Obra = {
  ID: 582,
  Imagem: 'SerenaDamadoMar_LuizFigueiredo.jpeg',
  Authors: [authors.LuizFigueiredo],
  Titulo: `Serena, Dama do Mar`,
  Material: 'Bronze',
  DataInauguracao: `12/1988`,
  DatasImportantes: [new Data(`12/1988`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Atlantica com Rainha Elizabeth',
  Latitude: `-22.9854498`,
  Longitude: `-43.188948`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em bronze doada à Cidade pela Companhia de Navegação Viamar com apoio da Fundação Rio Arte. Foi
    inaugurada pera os festejos de Iemanjá no ano de 1988.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Liboredo Fundição Artística Ind E Com Ltda',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const CarlosDrummondDeAndrade_LeoSantana: Obra = {
  ID: 582,
  Imagem: 'CarlosDrummondDeAndrade_LeoSantana.jpeg',
  Authors: [authors.LeoSantana],
  Titulo: `Carlos Drummond de Andrade`,
  Material: 'Bronze',
  DataInauguracao: `10/2002`,
  DatasImportantes: [new Data(`10/2002`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Atlantica, em frente a Avenida Rainha Elizabeth',
  Latitude: `-22.9844361`,
  Longitude: `-43.1891361`,
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi instalado em comemoração ao centenário do nascimento do poeta.
A criação foi inspirada numa foto tirada pelo fotógrafo Rogério Reis. O poeta está sentado no banco em frente à Avenida Rainha
Elizabet, voltado para a cidade e de costas para o mar.//Localizada próximo ao posto 6, em Copacabana, o monumento a Carlos Drummond de Andrade é campeão em interação por quem passeia pelo calçadão mais famoso da cidade. A obra foi instalada em 2002 para comemoração do centenário do poeta mineiro e mais influente escritor brasileiro no século XX.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: `Estátua`,
  Execucao: true,
};

export const DorivalCaymmi_OttoDumovich: Obra = {
  ID: 582,
  Imagem: 'DorivalCaymmi_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Dorival Caymmi`,
  Material: 'Bronze',
  DataInauguracao: `12/2008`,
  DatasImportantes: [new Data(`12/2008`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Atlantica, posto 6',
  Latitude: `-22.9858274`,
  Longitude: `-43.1888194`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem à Dorival Caymmi foi uma solicitação da família à Prefeitura da Cidade, inspirada numa fotografia de
    Evandro Teixeira, em Ipanema.// No final da praia de Copacabana, o cantor e compositor baiano Dorival Caymmi saúda os visitantes. Inspirada no registro das lentes de Evandro Teixeira, a estátua de autoria de Otto Dumovich foio inaugurada em 2008 e está próxima da vila dos pescadores, um tema recorrente nas suas canções.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const TimMaia_ChristinaMotta: Obra = {
  ID: 582,
  Imagem: 'TimMaia_ChristinaMotta.jpeg',
  Authors: [authors.ChristinaMotta],
  Titulo: `Tim Maia`,
  Material: 'Bronze',
  DataInauguracao: `2015`,
  DatasImportantes: [new Data(`2015`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Tijuca',
  Endereco: 'Praça Afonso Pena',
  Latitude: `-22.9180065`,
  Longitude: `-43.2186448`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Nascido e eternizado na Tijuca, na Praça Afonso Pena, Tim Maia é responsável pela introdução do soul na música popular brasileira e reconhecido mundialmente como um dos maiores ícones da música no Brasil. Com autoria de Cristina Motta, a estátua do cantor é uma das mais recentes no Rio e possui 150 quilos, apenas 10 a menos do que o cantor.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const AnaCarolina_MarliMazeredo: Obra = {
  ID: 582,
  Imagem: 'AnaCarolina_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Ana Carolina`,
  Material: 'Bronze',
  DatasImportantes: [new Data(`sec XX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Laranjeiras',
  Endereco: 'Rua das Laranjeiras, 180',
  Latitude: `-22.9327728`,
  Longitude: `-43.1873852`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const TeneteSiqueiraCamposAoLevantedos18doForte_JoseRangel: Obra = {
  ID: 582,
  Imagem: 'TeneteSiqueiraCamposAoLevantedos18doForte_JoseRangel.jpeg',
  Authors: [authors.JoseRangel],
  Titulo: `Tenete Siqueira Campos- Ao Levante dos 18 do Forte`,
  Material: 'Bronze',
  DataInauguracao: `07/1974`,
  DatasImportantes: [new Data(`07/1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Atlântica',
  Latitude: `-22.9709691`,
  Longitude: `-43.1833224`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é o reconhecimento "Aos 18 combatentes" , sairam em marcha pela Avenida Atlântica. A estátua em bronze representa Siqueira Campos no momento que foi alvejado por um tiro.
    Foi inaugurado em 20 de setembro de 1959 na Praça Eugênio Jardim e reinaugurado na Avenida Atlântica em 05 de julho de 1974.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const PrincesaIsabel_EdgarDuvivierFilho: Obra = {
  ID: 582,
  Imagem: 'PrincesaIsabel_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivierFilho],
  Titulo: `Princesa Isabel`,
  Material: 'Bronze',
  Altura: '250 cm',
  DataInauguracao: `2002`,
  DatasImportantes: [
    new Data(`2002`, 'data de inauguração - arte ambiente'),
    new Data(`13/05/2003`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Princesa Isabel',
  Latitude: `-22.9651773`,
  Longitude: `-43.1733377`,
  Base: 'Com pedestal',
  MaterialBase: 'Mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua em bronze inspirada numa fotografia, onde a princesa tem na mão uma pena como se estivesse acabando de
    assinar a Lei Áurea a na outra um gesto solto. A confecção da escultura foi solicitada ao prefeito`,
  Fatos: [
    new Fact(
      '',
      '13/05/2003',
      'Inauguração',
      'Estátua foi inaugurada em comemoração aos 115 anos da Abolição da Escravatura',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Solicitação',
      'A solicitação do monumento partiu de Marilda de Sá, da Associação das Mulheres Empresárias que achavam injusto a princesa não ter sido homenageada naquela avenida. A confecção da escultura foi solicitada ao prefeito',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/ Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro'),
    new Fact('', '01/08/2018', 'Furto', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CoelhoCintra_LaurindoRamos: Obra = {
  ID: 582,
  Imagem: 'CoelhoCintra_LaurindoRamos.jpeg',
  Authors: [authors.LaurindoRamos],
  Titulo: `Coelho Cintra`,
  Material: 'Bronze',
  DataInauguracao: `01/1958`,
  DatasImportantes: [new Data(`01/1958`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Avenida Princesa Isabel',
  Latitude: `-22.9618369`,
  Longitude: `-43.1750678`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento é uma homenagem ao Pai de Copacabana, dos moradores do bairro,por
    ocasião do centenário da urbanização do bairro, em 1892. Inicialmente foi instalado nesse
    local, em 1996 foi transferido para a Avenida Lauro Sodre, onde permaneceu até junho de
    2006, quando retornou para a Avenida Princesa Isabel, por solicitação da família.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const ViscondedoRioBranco_FelixMauriceCharpentier: Obra = {
  ID: 582,
  Imagem: 'ViscondedoRioBranco_FelixMauriceCharpentier.jpeg',
  Authors: [authors.FelixMauriceCharpentier],
  Titulo: `Visconde do Rio Branco`,
  Material: 'Bronze',
  DataInauguracao: `05/1902`,
  DatasImportantes: [new Data(`05/1902`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça Demétrio Ribeiro',
  Latitude: `-22.9620366`,
  Longitude: `-43.1780575`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `No monumento, o visconde se encontra sentado e veste o uniforme de senador do
    Império. O braço direito descansa em dois livros - A convenção de 20 de fevereiro, obra sua
    de 1871, da Coleção de Leis do Brasil.As peças em bronze foram modeladas e fundidas em
    Paris. Foi inaugurada na praça da Glória, nas proximidades da base do Outeiro e nessa praça
    em 1996. O monumento foi erguido por subscrição publica com a iniciativa a cargo da Viscondessa.
    Tem sete metros de altura, sendo de dois metros de altura da estátua em bronze. O pedestal mede 3,35 metros. O Visconde está sentado e veste o uniforme de Senador do Império. O braço direito descansa em dois livros  A Convenção de 20 de fevereiro, obra sua, de diplomata e de estadista, e o volume de 1871, da  Coleção de Leis do Brasil. No chão, encostada à cadeira, vê-se uma pasta de papéis, com o dístico: _ Presidente do Conselho de Ministros.O pedestal é de Pedra da Boloye e está assentado sobre uma base de granito brasileiro.Na frente há em bronze uma figura de mulher representando a história. Ela traz escrita, numa tábua, de bronze, as palavras latinas, dos Anais de Tácito e está em atitude de quem as lê. A mão direita sustenta um estilete.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};
export const IrmaosBernardelli_HildegardoLeaoVelloso: Obra = {
  ID: 582,
  Imagem: 'IrmaosBernardelli_HildegardoLeaoVelloso.jpeg',
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Rodolfo e Henrique, os Irmãos Bernardelli`,
  Material: 'Bronze',
  DataInauguracao: `09/1952`,
  DatasImportantes: [new Data(`09/1952`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça do Lido',
  Latitude: `-22.965493`,
  Longitude: `-43.1763076`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `É uma dupla homenagem aos irmãos Rodolfo e Henrique Bernardelli no centenario de Rodolfo. A praça foi escolhida porque nesse local viveram e trabaharam os authors.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Vitoria_MarliMazeredo: Obra = {
  ID: 582,
  Imagem: 'Vitoria_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Vitória`,
  Material: 'Bronze',
  DataInauguracao: `04/1992`,
  DatasImportantes: [new Data(`04/1992`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça do Lido',
  Latitude: `-22.9660791`,
  Longitude: `-43.1757018`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura abstrata representando a "Vitória".`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Atelier Mazeredo',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const SarahKubitschek_MarliMazeredo: Obra = {
  ID: 582,
  Imagem: 'SarahKubitschek_MarliMazeredo.jpeg',
  Authors: [authors.MarliMazeredo],
  Titulo: `Sarah Kubitschek`,
  Material: 'Bronze e resina',
  DataInauguracao: `1996`,
  DatasImportantes: [
    new Data(`1996`, 'data de inauguração - arte ambiente'),
    new Data(`06/1999`, 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Praça Sara Kubitschek',
  Latitude: `-22.9796065`,
  Longitude: `-43.1910441`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento tem o busto de Sara envolto num semicírculo em resina e mármore. Foi uma iniciativa da Associação
    Cultural Sara Kubitschek em homenagem a sua patrona.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Atelier Mazeredo',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SerzedeloCorreia_RodolfoBernardelli: Obra = {
  ID: 582,
  Imagem: 'SerzedeloCorreia_RodolfoBernardelli.jpeg',
  Authors: [authors.RodolfoBernardelli],
  Titulo: `Serzedelo Correia`,
  Material: 'Bronze',
  DataInauguracao: `11/1910`,
  DatasImportantes: [new Data(`11/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Laranjeiras',
  Endereco: 'Rua das Laranjeiras, 180',
  Latitude: `-22.9687313`,
  Longitude: `-43.1839672`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi em reconhecimento ao engenheiro do governo do Prefeito Pereira Passos. Foi uma iniciativa dos dos moradores de Copacabana.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Baleia_AngeloVenosa: Obra = {
  ID: 582,
  Imagem: 'Baleia_AngeloVenosa.jpeg',
  Authors: [authors.AngeloVenosa],
  Titulo: `Baleia`,
  Material: 'Aço',
  Altura: '370 cm',
  Largura: '1 000cm',
  Profundidade: '220 cm',
  Peso: '16000 kg',
  DataInauguracao: `01/1988`,
  DatasImportantes: [
    new Data(`01/1988`, 'data de inauguração'),
    new Data('1998', 'data de transferência'),
  ],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Praça Alm. Júlio de Noronha, 86',
  Latitude: `-22.962494`,
  Longitude: `-43.1655975`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Peça de 16 toneladas composta por dois elementos em aço de 2 polegadas de espessua, com 17 aneis de 4 metros de altura,; 2,50m de
    largura e 6 metros de comprimento e outra peça menos com 1 metro de altura 2 de largura e 2,50 de comprimento.`,
  Fatos: [
    new Fact('', '', 'Título', 'Sem título', [], 'Rio de Janeiro'),
    new Fact('', '', 'Renomeação', 'A baleia', [], 'Rio de Janeiro'),
    new Fact(
      '',
      '',
      'Doação',
      'João Fortes Engenharia para a reurbanização da Praça Mauá',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1998',
      '1998',
      'Transferencia',
      'A obra foi removida para a instalação do monumento ao Almirante Pedro Marx',
      [],
      'Rio de Janeiro',
    ),
    new Fact('', '', 'Propriedade', 'Pública', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarechalFlorianoPeixoto_EduardoDeSa: Obra = {
  ID: 582,
  Imagem: 'MarechalFlorianoPeixoto_EduardoDeSa.jpeg',
  Authors: [authors.EduardoDeSa],
  Titulo: `Marechal Floriano Peixoto`,
  Material: 'Bronze',
  DataInauguracao: `21/04/1910`,
  DatasImportantes: [new Data(`21/04/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Floriano',
  Latitude: `-22.9101486`,
  Longitude: `-43.176075`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O Monumento foi executado para exaltar a nacionalidade brasileira e sua formação, com grupos alegoricos lembrando os indígenas, os portugueses, os negros e a presença católica no Brasil.
    Nas laterias do grande pedestal vê-se um grupo representando os silvícolas (quadro Inspirado no canto VI do Y-Juca-Pirama, poema dos Timbiras, de Gonçalves Dias). Em outra face está esculpida uma alegoria 
    da conquista portuguesa, com um grupo em que domina o lendário Caramuru. Depois, o período da catequese, simbolizado pelo padre Anchieta. E, finalmente, um conjunto representando a colaboração étnica da raça
    africana, inspirado em uma passagem do poema Cachoeira de Paulo Afonso, de Castro Alves. No topo do monumento encontra-se uma figura de mulher, exprimindo a Paz e o Amor e um grupo principal constituído pelo
    Marechal Floriano Peixoto, com espada em punho. As figuras de Tiradentes,Jose Bonifácioe Benjamim Constant, que surgem da bandeira da República. Na base as grandes datas da nossa história  1500, 1822, 1888 e 1889
    estão gravadas e no plano inferior, as seguintes legendas  A sã política é filha da moral e da razão  O amor por principio e a ordem por base; o progresso por fim  Libertas quae sera tamen  A bem amada Pátria, a
    gratidão de seus filhos.`,
  Fatos: [new Fact('', '', 'Subscrição Popular', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Militar',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const PauloSampaio_Desconhecida: Obra = {
  ID: 582,
  Imagem: 'PauloSampaio_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Paulo Sampaio`,
  Material: 'Bronze',
  DataInauguracao: '09/2002',
  DatasImportantes: [
    new Data('09/2002', 'data de inauguração - inventário dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Ancora',
  Latitude: `-22.9057922`,
  Longitude: `-43.1685091`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A primeira homenagem foi retirada da Praça em frente ao prédio da finada companhia aereo em 1967. Esse busto
    foi um pedido dos ex-funcionários da Panair ao prefeitura`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const CasteloBranco_DanteCroce: Obra = {
  ID: 582,
  Imagem: 'CasteloBranco_DanteCroce.jpeg',
  Authors: [authors.DanteCroce],
  Titulo: `Castelo Branco`,
  Material: 'Bronze',
  DataInauguracao: `05/1974`,
  DatasImportantes: [new Data(`05/1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Praça Júlio de Noronha',
  Latitude: `-22.9623683`,
  Longitude: `-43.1653128`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi uma iniciativa do governo do Estado. A saudação foi feita pelo governador Chagas Freitas, que destacou a personalidade do ex-presidente e disse que “o povo e o governo cariocas resgatam uma divida.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const UniaodosPovosAmericanos_MarioAgostinelli: Obra = {
  ID: 582,
  Imagem: 'UniaodosPovosAmericanos_MarioAgostinelli.jpeg',
  Authors: [authors.MarioAgostinelli],
  Titulo: `União dos Povos Americanos`,
  Material: 'Bronze',
  DataInauguracao: `01/1955`,
  DatasImportantes: [new Data(`01/1955`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Canteiro Central da Avenida Armando Lombardi',
  Latitude: `-23.0071074`,
  Longitude: `-43.3046769`,
  Base: 'Com pedestal',
  MaterialBase: 'Mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Monumento composto por três figuras femininas em bronze com uma bandeira, entre elas,
    simbolizando a união das Três Américas sobre um pedestal. O monumento foi retirado do canteiro
    central para as obras do metro retornando para o espaço publico em 2016.`,
  Fatos: [
    new Fact(
      '01/1955',
      '01/1955',
      'Inauguração',
      'O monumento foi criado em homenagem aos 10 anos de criação da Associação Comercial e Industrial da Barra',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Remoção',
      'O monumento foi retirado do canteiro central para as obras do metro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '2016',
      '2016',
      'Realocação',
      'O monumento foi retirado do canteiro central para as obras do metro',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const BichosHibridos_VaniaVilela: Obra = {
  ID: 582,
  Imagem: 'BichosHibridos_VaniaVilela.jpeg',
  Authors: [authors.VaniaVilela],
  Titulo: `Bichos Hibridos`,
  Material: 'Aço pintado',
  DataInauguracao: `06/2000`,
  DatasImportantes: [new Data(`06/2000`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Avenida Armando Lombardi',
  Latitude: `-23.0068329`,
  Longitude: `-43.3149617`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `A escultura representa a flora brasileira e foi doada à Cidade pelo Projeto Esculturas Monumentais do RB1.
    Escultura abstrata em ferro pintada nas cores preto e vermelho com recortes, com cerca de 3 metros a peça maior e 2,50 a menor,
    cria a peça uma dimensão lúdica. A peça esta desaparecida desde a obra da duplicação do Viaduto do Joá, que retirou para a
    execução da obra`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AoPAN2007_RobertoKenjiFukuda: Obra = {
  ID: 582,
  Imagem: 'AoPAN2007_RobertoKenjiFukuda.jpeg',
  Authors: [authors.RobertoKenjiFukuda],
  Titulo: `Ao  PAN 2007`,
  Material: 'Aço',
  Altura: '1500 cm',
  Peso: '5000 kg',
  DataInauguracao: `07/2007`,
  DatasImportantes: [new Data(`07/2007`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Avenida Abelardo Bueno com Ayrton Senna',
  Latitude: `-22.9734851`,
  Longitude: `-43.3639193`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura composta por um globo terrestre com o mapa do continente americano dos dois lados, demarcando a parte
    do planeta onde se realiza a competição. Atletas representantes do basquete, do velocismo e do tênis sustentam o globo. A obra
    foi doada à Cidade pelas construtoras Carvalho Hosken e a RJZ Cyrela. Foram necessários seis meses para que a escultura ficasse pronta.
    Precisou ser dividida em duas partes devido ao tamanho, foram necessárias mais duas semanas até que a montagem pudesse ser
    concluída no local definitivo.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const Surf_MauricioBentes: Obra = {
  ID: 582,
  Imagem: 'Surf_MauricioBentes.jpeg',
  Authors: [authors.MauricioBentes],
  Titulo: `Surf`,
  Material: 'Ferro',
  DatasImportantes: [new Data(`Século XX`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Avenida Ayrton Senna',
  Latitude: `-22.9829359`,
  Longitude: `-43.3675667`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const AyrtonSenna_MarioPitanguy: Obra = {
  ID: 582,
  Imagem: 'AyrtonSenna_MarioPitanguy.jpeg',
  Authors: [authors.MarioPitanguy],
  Titulo: `Ayrton Senna`,
  Material: 'Bronze',
  DataInauguracao: `27/12/2019`,
  DatasImportantes: [new Data(`27/12/2019`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Copacabana',
  Endereco: 'Em frente ao Hotel Copacabana Palace',
  Latitude: `-22.865393`,
  Longitude: `-43.254853`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Estátua em tamanho real do piloto Ayrton Senna. O piloto foi retratado trajando um macacão e está posicionado no deugrau mais alto de um pódio e celebrando uma vitória com a bandeira do brasil nas mãos.`,
  Fatos: [
    new Fact(
      '27/12/2019',
      '27/12/2019',
      'Inauguração',
      '',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Doação',
      'A estátua foi doada pela agencia de viagens virtual Hurb-Hotel Urbano.',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Homenageado',
      'Ayrton Senna, piloto de Fórmula-1',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const JoaquimNabuco_OttoDumovich: Obra = {
  ID: 582,
  Imagem: 'JoaquimNabuco_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Joaquim Nabuco`,
  Material: 'Bronze',
  DataInauguracao: `03/2011`,
  DatasImportantes: [new Data(`03/2011`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Presidente Wilson',
  Latitude: `-22.9100063`,
  Longitude: `-43.1721709`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact('', '', 'Homenageado', 'Joaquim Nabuco', [], ''),
    new Fact(
      '',
      '',
      'Homenagem',
      'A homenagem fez parte das festividades do centenário da morte do Patrono da Academia Brasileira de Letras',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Solicitação',
      'A obra foi solicitada pelo Presidente da Academia Brasileira de Letras, Marcos Villaça à Prefeitura da cidade.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CandidoDeOliveira_LaurindoRamos: Obra = {
  ID: 582,
  Imagem: 'CandidoDeOliveira_LaurindoRamos.jpeg',
  Authors: [authors.LaurindoRamos],
  Titulo: `Candido de Oliveira`,
  Material: 'Bronze',
  DataInauguracao: `06/1991`,
  DatasImportantes: [new Data(`06/1991`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Largo do Caco',
  Latitude: `-22.9073659`,
  Longitude: `-43.1904433`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Fatos: [
    new Fact(
      '',
      '',
      'Solicitação',
      'Réplica solicitada a pedido da Prefeitura para reconhecimento publico.',
      [],
      'Rio de Janeiro',
    ),
    new Fact('06/1991', '06/1991', 'Inauguração', '', [], 'Rio de Janeiro'),
    new Fact(
      '1919',
      '1919',
      'Réplica',
      'A homenagem é uma réplica da peça existente na Faculdade de Direito da Universidade Federal do Rio de Janeiro',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Zani Fundição Artística E Metalúrgica Ltda.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SebastiaoLeme_MariaDaGloriaVianaPatrasso: Obra = {
  ID: 582,
  Imagem: 'SebastiaoLeme_MariaDaGloriaVianaPatrasso.jpeg',
  Authors: [authors.MariaDaGloriaVianaPatrasso],
  Titulo: `Sebastião Leme`,
  Material: 'Bronze',
  DataInauguracao: `10/1951`,
  DatasImportantes: [new Data(`10/1951`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Cardeal Leme',
  Latitude: `-22.9077046`,
  Longitude: `-43.1941086`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem em agradecimento ao segundo cardeal brasileiro.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Homenageado',
      'Cardeal Sebastião Leme',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '10/1951',
      '10/1951',
      'Inauguração',
      'Foi inaugurado nas comemorações do nono aniversario do falecimento, pelo prefeito João Carlos Vidal e pelo Cardeal D. Jaime Camara.',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Busto',
  Execucao: true,
};

export const PierreRichiereGuillaumeChartier_JoasPereiraDosPassos: Obra = {
  ID: 582,
  Imagem: 'PierreRichiereGuillaumeChartier_JoasPereiraDosPassos.jpeg',
  Authors: [authors.JoasPereiraDosPassos],
  Titulo: `Pierre Richier e Guillaume Chartier`,
  Material: 'Bronze',
  DataInauguracao: `03/2007`,
  DatasImportantes: [new Data(`03/2007`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Silva Jardim, 87',
  Latitude: `-22.90751`,
  Longitude: `-43.181356`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Os homenageados estão atras de uma grande mesa em bronze,onde ser lê-se esta inscrição Em memória de mim. Trata-se
    de monumento comemorativo à primeira celebração eucarística protestante no Brasil e nas Américas, realizada no Rio de Janeiro
    em 21 de março de 1557.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Fundição e Atelier Messias',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Religiosa',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CondeCunha_LuisPaesLeme: Obra = {
  ID: 582,
  Imagem: 'CondeCunha_LuisPaesLeme.jpeg',
  Authors: [authors.LuisPaesLeme],
  Titulo: `Conde Cunha`,
  Material: 'Bronze',
  DataInauguracao: `06/1949`,
  DatasImportantes: [new Data(`06/1949`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Caju',
  Endereco: 'Rua Arsenal de Guerra',
  Latitude: '-22.8807209',
  Longitude: '-43.2176044',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem ao fundador do Arsenal do Rio de Janeiro. O erguido graçaas a iniciativa di Almirante Renato Guilhobel, firetor do AMRJ. O busto esta sob a guarda do Arsenal devido a obra da praça Barão de Ladário.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AntonioJanuzzi_Desconhecida: Obra = {
  ID: 582,
  Imagem: 'AntonioJanuzzi_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Antonio Januzzi`,
  Material: 'Bronze',
  DataInauguracao: `06/2012`,
  DatasImportantes: [new Data(`06/2012`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Noronha Santos',
  Latitude: `-22.9067826`,
  Longitude: `-43.1973166`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `O busto foi uma doação da ONG Ital Rio para à cidade do Rio. Foi retirado em 11 de janeiro de 2016, devido as obras do VLT no centro do Rio e transferido para o depósito da Prefeitura.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JosedoPatrocinio_RodolfoAmoedo_RuffoFanucchi: Obra = {
  ID: 582,
  Imagem: 'JosedoPatrocinio_RodolfoAmoedo_RuffoFanucchi.jpeg',
  Authors: [authors.RodolfoAmoedo, authors.RuffoFanucchi],
  Titulo: `José do Patrocínio`,
  Material: 'Bronze',
  DataInauguracao: `10/1953`,
  DatasImportantes: [new Data(`10/1953`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Graça Aranha com Evaristo da Veiga',
  Latitude: '-22.9090705',
  Longitude: '-43.1752651',
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Trata-se de um monolito com uma placa em bronze com a figura do homenageado.<Temporariamente sem visitação no jardim da Biblioteca Nacional. O monumento foi erguido por iniciativa dos jornalistas Heitor Beltrão e Ariosto Berna, com apoio do Movimento Libertador da Terra Carioca e da Casa do Jornalista. Foi inaugurado na data do centenário do Nascimento do homenageado.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const LimaBarreto_EdgarDuvivier: Obra = {
  ID: 582,
  Imagem: 'LimaBarreto_EdgarDuvivier.jpeg',
  Authors: [authors.EdgarDuvivier],
  Titulo: `Lima Barreto do Centro`,
  Material: 'Bronze',
  DataInauguracao: `03/2012`,
  DatasImportantes: [new Data(`03/2012`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua do Lavradio',
  Latitude: `-22.9111016`,
  Longitude: `-43.1826368`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Esse monumento foi erguido a pedido da Casa Lima Barreto, pela Prefeitura nas comemorações do nascimento do escritor brasileiro.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AMelvinJoneseArmandoFarjado_Ariel: Obra = {
  ID: 582,
  Authors: [authors.Ariel],
  Titulo: `À Melvin Jones e Armando Farjado`,
  Material: 'Bronze',
  DataInauguracao: `04/1977`,
  DatasImportantes: [new Data(`04/1977`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua São José',
  Latitude: `-22.9062264`,
  Longitude: `-43.1767306`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Monolito em homenagem ao primeiro Leão Brasileiro Armando Farjado, e de Melvin Jones. Na frente o marco tem duas mãos unidas esculpida na pedra. O Monumento foi inaugurado na passagem do 25º aniversatio da fundação do clube, durante XXV Convenção Nacional do Lios Club, com a participação de oito mil associados.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Relevo',
  Execucao: true,
};

export const GabrielHabib_HildegardoLeaoVelloso: Obra = {
  ID: 582,
  Authors: [authors.HildegardoLeaoVelloso],
  Titulo: `Gabriel Habib`,
  Material: 'Bronze',
  DataInauguracao: `11/2014`,
  DatasImportantes: [new Data(`11/2014`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Buenos Aires',
  Latitude: `-22.9052872`,
  Longitude: `-43.185118`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O busto foi realizado nos anos de 1960 e instalada na loja da rede no Centro e
    posteriormente para uma outra no bairro de Inhaúma. Somente em 2014 foi doado pela familia para
    o reconhecimento público.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GrandjeandeMontiny_NicolinaVazDeAssisPintodoCouto: Obra = {
  ID: 582,
  Authors: [authors.NicolinaVazDeAssisPintodoCouto],
  Titulo: `Grandjean de Montiny`,
  Material: 'Bronze',
  DataInauguracao: `10/1910`,
  DatasImportantes: [new Data(`10/1910`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Rua Primeiro de Março',
  Latitude: `-22.900622`,
  Longitude: `-43.176356`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Esse monumento foi inicialmente instalado na Quinta da Boa Vista junto de Auguste Francois Marie Glaziou. Em 1996,
    por solicitação da Casa França Brasil a Fundação Parques e Jardins concedeu a cessão de uso para aquela instituição,
    permanecendo até hoje. A obra foi paga pelo Ministério da Viação.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const JuliodeCastilhos_Desconhecida: Obra = {
  ID: 582,
  Authors: [authors.Desconhecida],
  Titulo: `Julio de Castilhos`,
  Material: 'Bronze',
  DataInauguracao: `11/2011`,
  DatasImportantes: [new Data(`11/2011`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Avenida Atlântica',
  Latitude: `-22.9055225`,
  Longitude: `-43.1907273`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `O busto de Julio de Castilhos estava no depósito da Prefeitura desde os anos de 1980. Por iniciativa da Casa de Deodoro, a Prefeitura realizou a cessão do uso do busto, a fim de garantir sua integridade bem como sua exposição em espaço público.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AryBarroso_LeoSantana: Obra = {
  ID: 582,
  Imagem: 'AryBarroso_LeoSantana.jpeg',
  Authors: [authors.LeoSantana],
  Titulo: `Ary Barroso`,
  Material: 'Bronze',
  DataInauguracao: '2003',
  DatasImportantes: [new Data(`2003`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Avenida Atlântica',
  Latitude: `-22.9627074`,
  Longitude: `-43.1701671`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Consagrado compositor de "Aquarela do Brasil", uma das músicas que mais produz direitos autorais no exterior, Ary Barroso ganhou uma estátua de bronze no ano de seu centenário de nascimento em Copacabana.',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const AryBarroso_OttoDumovich: Obra = {
  ID: 582,
  Imagem: 'AryBarroso_OttoDumovich.jpeg',
  Authors: [authors.OttoDumovich],
  Titulo: `Ary Barroso`,
  Material: 'Bronze',
  DataInauguracao: '13/04/1974',
  DatasImportantes: [new Data(`13/04/1974`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Rua Antônio Viêira, 1',
  Latitude: `-22.9641083`,
  Longitude: `-43.1746567`,
  Base: 'Com pedestal',
  Status: 'Permanece em espaço público',
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Narratividade',
  Funcao: 'Referencial',
  Natureza: 'Cívica',
  Tipologia: 'Busto',
  Execucao: true,
};
export const AoNuncaMais_CristinaPozzobon: Obra = {
  ID: 582,
  Imagem: 'AoNuncaMais_CristinaPozzobon.jpeg',
  Authors: [authors.CristinaPozzobon],
  Titulo: `Ao Nunca Mais`,
  Material: 'Aço',
  DataInauguracao: `04/2014`,
  DatasImportantes: [new Data(`04/2014`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Floriano',
  Latitude: `-22.9112566`,
  Longitude: `-43.1758078`,
  Status: 'Permanece em espaço público',
  Descricao: `Monumento em homenagem a resistência e a luta pela anistia, doado à Cidade pela Comissão de Anistia do Ministério da Justiça, por ocasião dos 50 anos do Golpe de 1964. As estrelas cravadas no piso são em homenagem aos militares cassados após o golpe de 1964.Escultura com cerca de 3 m de altura, ele tem uma bandeira "desconstruída" com estrelas afixadas no chão.// O monumento “Ao nunca mais” é uma homenagem à resistência brasileira e à luta pela anistia e foi instalado em 2014 patrocinado pela Comissão de Anistia do Ministério da Justiça e doado à Prefeitura em ocasião dos 50 anos do Golpe de 1964. Feito pela artista plástica Cristina Pozzobon, a escultura tem 2,5 metros de altura, em aço córten, representa a bandeira brasileira cortada ao meio com estrelas no chão.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Pax_MarioMatos: Obra = {
  ID: 582,
  Imagem: 'Pax_MarioMatos.jpeg',
  Authors: [authors.MarioMatos],
  Titulo: `Pax (Paz)`,
  Material: 'Bronze',
  DatasImportantes: [new Data('', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Marechal Floriano',
  Latitude: `-22.9099245`,
  Longitude: `-43.1763903`,
  Base: 'Com pedestal',
  MaterialBase: 'Mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Figura feminina em bronze representando a Paz, sobre uma coluna em mármore de Verona com florões e detalhe corinto
    no topo. Esse monumento faz par com outro, que representa o trabalho, emoldurando o prédio da Camara dos Vereadores da Cidade,
    o Palácio Pedro Ernesto.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Trabalho_MarioMatos: Obra = {
  ID: 582,
  Authors: [authors.MarioMatos],
  Titulo: `Trabalho`,
  Material: 'Bronze',
  DatasImportantes: [new Data(``, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Floriano Peixoto',
  Latitude: `-22.9099245`,
  Longitude: `-43.1763903`,
  Base: 'Com pedestal',
  MaterialBase: 'Mármore',
  Status: 'Permanece em espaço público',
  Descricao: `Figura feminina em bronze representando o Trabalho, sobre uma coluna em mármore de Verona com florões e detalhe
    corinto no topo. Esse monumento faz par com outro, que representa a Paz, emoldurando o prédio da Câmara dos Vereadores.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const CriancasComAConcha_FrançoisLespingola: Obra = {
  ID: 582,
  Imagem: 'CriancasComAConcha_FrancoisLespingola.jpeg',
  Authors: [authors.FrançoisLespingola],
  Titulo: `Crianças com a Concha`,
  Material: 'Mármore',
  DataInauguracao: `01/1928`,
  DatasImportantes: [new Data(`01/1928`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: `-22.9119106`,
  Longitude: `-43.1750856`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Esse conjunto de crianças, são reproduções das peças do Jardim de Versalhes, de
    François Lespingola, doada à cidade pelafamília Guinle, para a decoração do Chafariz.`,
  Fatos: [new Fact('', '', '', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const CriancasComoPassaro_SimonMazieredas: Obra = {
  ID: 582,
  Authors: [authors.SimonMazieredas],
  Titulo: `Crianças com o Pássaro`,
  Material: 'Mármore',
  DataInauguracao: `01/1928`,
  DatasImportantes: [new Data(`01/1928`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: `-22.9119106`,
  Longitude: `-43.1750856`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Esse conjunto de crianças são reproduções de Simon Mazièredas peças Jardim de Versalhes, doada à cidade pela família Guinle.`,
  Fatos: [
    new Fact(
      '',
      '1990',
      'Tombado pelo IPHAN pelo processo 1132-T-84 e inscrito no Livro de Belas Artes Nº inscr.: 597 ;Vol. 2 ;F. 018',
      '',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const CriancasComATocha_PhilippeGarnier: Obra = {
  ID: 582,
  Imagem: 'CriancasComATocha_PhilippeGarnier.jpeg',
  Authors: [authors.PhilippeGarnier],
  Titulo: `Crianças com a Tocha`,
  Material: 'Mármore',
  DataInauguracao: `01/1928`,
  DatasImportantes: [new Data(`01/1928`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: `-22.9119106`,
  Longitude: `-43.1750856`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `Esse conjunto de crianças são reproduções, das peças existentes nos Jardim de
    Versalhes, de Philippe Granier, doada à cidade pela família Guinle para a decoração do Chafariz.`,
  Fatos: [
    new Fact(
      '',
      '1990',
      'Tombado pelo IPHAN pelo processo 1132-T-84 e inscrito no Livro de Belas Artes Nº inscr.: 597 ;Vol. 2 ;F. 018',
      '',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const CriancasComflores_PierreLaviron: Obra = {
  ID: 474,
  Imagem: 'CriancasComflores_PierreLaviron.jpeg',
  Authors: [authors.PierreLaviron],
  Titulo: `Crianças com flores`,
  Material: 'Mármore',
  DataInauguracao: `01/1928`,
  DatasImportantes: [new Data(`01/1928`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Praça Mahatma Gandhi',
  Latitude: `-22.9119106`,
  Longitude: `-43.1750856`,
  Base: 'Com pedestal',
  MaterialBase: 'Gnaisse',
  Status: 'Permanece em espaço público',
  Descricao: `O conjunto de crianças são reproduções das peças do Jardim de Versalhes, de
    Pierre Laviron,doada à cidade pelafamília Guinle.`,
  Fatos: [
    new Fact(
      '',
      '1990',
      'Tombado pelo IPHAN pelo processo 1132-T-84 e inscrito no Livro de Belas Artes Nº inscr.: 597 ;Vol. 2 ;F. 018',
      '',
      [],
      '',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Sublimidade',
  Funcao: 'Decorativa',
  Natureza: 'Artística',
  Tipologia: 'Grupo escultórico',
  Execucao: true,
};

export const APepeLopes_VeraTorres: Obra = {
  ID: 474,
  Imagem: 'APepeLopes_VeraTorres.jpeg',
  Authors: [authors.VeraTorres],
  Titulo: `À Pepê Lopes`,
  Material: `Bronze`,
  DataInauguracao: `10/1991`,
  DatasImportantes: [new Data(`10/1991`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Av. Pepe Lopes',
  Latitude: `-23.0143024`,
  Longitude: `-43.3120514`,
  Status: 'Permanece em espaço público',
  Descricao: `Escultura em bronze composta por uma prancha e uma asa delta, sobre um pedestal um granito. Foi inaugurado pela
    esposa Ana Carolina e seus filhos Bianca e João Pedro. A homenagem foi contratado por sufistas e patrocinadores.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const MarechalJoaquimIgnacioBaptistaCardoso_SMartinsRibeiro: Obra = {
  ID: 474,
  Imagem: 'MarechalJoaquimIgnacioBaptistaCardoso_SMartinsRibeiro.jpeg',
  Authors: [authors.SMartinsRibeiro],
  Titulo: `Marechal Joaquim Ignácio Baptista Cardoso`,
  Material: `Bronze`,
  DataInauguracao: `01/1938`,
  DatasImportantes: [new Data(`01/1938`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Av. Gal. Felicíssimo Cardoso',
  Latitude: `-23.0003431`,
  Longitude: `-43.3536991`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O monumento foi inaugurado em 1938 na Rua General Canabarro. Posteriormente foi transferido para o Campo de
    São Cristóvão, em 1967, perto do quartel que comandou. Em 1988, o busto foi transferido para a Barra da Tijuca, na Rua
    General Felicíssimo Cardoso, local que homenagea o irmão do General Ignácio.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const AmericoVespucio_ArmandoAmaya: Obra = {
  ID: 474,
  Imagem: 'AmericoVespucio_ArmandoAmaya.jpeg',
  Authors: [authors.ArmandoAmaya],
  Titulo: `Américo Vespúcio`,
  Material: `Bronze`,
  DataInauguracao: `1985`,
  DatasImportantes: [
    new Data('1985', 'data de inauguração - arte ambiente'),
    new Data('04/1987', 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Barra da Tijuca',
  Endereco: 'Praça São Perpétuo (do Ó)',
  Latitude: `-23.0130247`,
  Longitude: `-43.3176068`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem foi executada em 1985, e doado à cidade do Rio de Janeiro pelo Kiwanis Club, de Glendade, Califórnia,
    EUA.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const SemTítulo_RobertoBurleMarx: Obra = {
  ID: 474,
  Imagem: 'SemTítulo_RobertoBurleMarx.jpeg',
  Authors: [authors.RobertoBurleMarx, authors.HaruyoshiOno],
  Titulo: `Sem Título`,
  Material: `Concreto`,
  DataInauguracao: `1985`,
  DatasImportantes: [
    new Data('1985', 'data de inauguração - arte ambiente'),
    new Data('01/1989', 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Recreio dos Bandeirantes',
  Endereco: 'Praça Heitor Bastos Tigre',
  Latitude: `-23.0164763`,
  Longitude: `-43.4724718`,
  Status: 'Permanece em espaço público',
  Descricao: `Execução de Gian Carlo Patuzzi - A homenagem foi prestada uma homenagem ao diplomata Heitor Bastos Tigre,
    dedicando-lhe uma praça no Recreio dos Bandeirantes, em retribuição ao preito de sua família, com o intuito de reverenciar
    a memória de seu ente querido, encomendou uma escultura do renomado artista Roberto Burle Marx, o que caracterizou também
    um presente ao Rio de Janeiro.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const ChafarizOutono_MathurinMoreau: Obra = {
  ID: 474,
  Imagem: 'ChafarizOutono_MathurinMoreau.jpeg',
  Authors: [authors.MathurinMoreau],
  Titulo: `Chafariz Outono`,
  Material: `Ferro fundido`,
  DataInauguracao: `08/1923`,
  DatasImportantes: [
    new Data('Séc XIX', 'data da obra - arte ambiente'),
    new Data('08/1923', 'data de inauguração - inventario dos monumentos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Jacarepaguá',
  Endereco: 'Largo do Rio Grande',
  Latitude: `-22.9166383`,
  Longitude: `-43.4146136`,
  Base: 'Com pedestal',
  MaterialBase: 'Ferro fundido',
  Status: 'Permanece em espaço público',
  Descricao: `O Chafariz em ferro fundido foi doado pelo Barão da Taquara para o embelezamento da localidade e o abastecimento dos animais que por ali passavam. Um pedestal quadrangular, tendo em cada face uma carranca que verte água numa bacia em semicírculo, serve de pedestal a uma graciosa estátua masculina, representando a estação.`,
  Fatos: [new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const HenriqueDodsworth_HonorioPecanha: Obra = {
  ID: 474,
  Imagem: 'HenriqueDodsworth_HonorioPecanha.jpeg',
  Authors: [authors.HonorioPecanha],
  Titulo: `Henrique Dodsworth`,
  Material: `Bronze`,
  DataInauguracao: `10/1945`,
  DatasImportantes: [new Data('10/1945', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Jacarepaguá',
  Endereco: 'Praça Professora Camisão',
  Latitude: `-22.9404063`,
  Longitude: `-43.3437331`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem ao antigo prefeito da cidade, por iniciativa do moradores do bairro de Jacarepaguá.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const DianadeGabies_Desconhecida: Obra = {
  ID: 474,
  Imagem: 'DianadeGabies_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Diana de Gabies`,
  Material: `Ferro fundido`,
  DataInauguracao: `sec. XIX`,
  DatasImportantes: [new Data('', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Jacarepaguá',
  Endereco: 'Praça Barão da Taquara',
  Latitude: `-22.8976865`,
  Longitude: `-43.3521423`,
  Status: 'Permanece em espaço público',
  Descricao: `Escultura inicialmente instalada no Passeio Público da Cidade. Na praça existia um pequeno Chafariz conhecido
    como As crianças e o guarda-chuva, inaugurado em 1936. Com o seu desaparecimento foi substituído por esta escultura.
    É uma reprodução em ferro fundida da existente do Museu do Louvre.`,
  Fatos: [
    new Fact('', '', 'Fundição/Atelier', "Val d'Osne", [], 'Rio de Janeiro'),
    new Fact('', '', 'Bem tombado municipal', '', [], 'Rio de Janeiro'),
  ],
  Temporalidade: 'Permanente',
  Tipologia: 'Estátua',
  Execucao: true,
};

export const BaraodeTaquara_BenevenutoBerna: Obra = {
  ID: 474,
  Imagem: 'BaraodeTaquara_BenevenutoBerna.jpeg',
  Authors: [authors.BenevenutoBerna],
  Titulo: `Barão de Taquara`,
  Material: `Bronze`,
  DataInauguracao: `10/1939`,
  DatasImportantes: [new Data('10/1939', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Jacarepaguá',
  //Endereco: 'Praça Barão da Taquara', TODO
  Endereco: 'Praça Seca',
  Latitude: `-22.8973527`,
  Longitude: `-43.3521058`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `O Monumento foi reguido numa homenagem de gratidão dos moradores de Jacarepaguá e por interferência da Irmandade de Nossa Senhora de Paula e pelo Centro Carioca.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const BrigadeiroAntônioDeSampaio_VictorBrecheret: Obra = {
  ID: 474,
  Imagem: 'BrigadeiroAntonioDeSampaio_VictorBrecheret.jpeg',
  Authors: [authors.VictorBrecheret],
  Titulo: `Brigadeiro Antônio de Sampaio`,
  Material: `Bronze`,
  DataInauguracao: `12/1952`,
  DatasImportantes: [new Data('12/1952', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Vila Militar',
  Endereco: 'Av. Duque de Caxias',
  Latitude: `-22.8641808`,
  Longitude: `-43.3988234`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem oferecida pelo Regimento Sampaio e pelo Liceu de Artes de São Paulo.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const LuisManuelOsorio_Desconhecida: Obra = {
  ID: 474,
  Imagem: 'LuisManuelOsorio_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Luis Manuel Osorio (Duque de Caxias)`,
  Material: `Bronze`,
  DataInauguracao: `05/1978`,
  DatasImportantes: [new Data('05/1978', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Vila Militar',
  Endereco: 'Av. Duque de Caxias',
  Latitude: `-22.8680986`,
  Longitude: `-43.406908`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `A homenagem ao Patrono da Cavalaria Brasileira, esta situadaem frente ao Batalhão da Vila Militar.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const MarechalHermesdaFonseca_DaviLeal: Obra = {
  ID: 474,
  Imagem: 'MarechalHermesdaFonseca_DaviLeal.jpeg',
  Authors: [authors.DaviLeal],
  Titulo: `Marechal Hermes da Fonseca`,
  Material: `Bronze`,
  DataInauguracao: `09/1941`,
  DatasImportantes: [new Data('09/1941', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Vila Militar',
  Endereco: 'Praça Marechal Hermes',
  Latitude: `-22.8658028`,
  Longitude: `-43.4022003`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Permanece em espaço público',
  Descricao: `Homenagem erguida atraves da oferta do filho primogenio do Marechal, o Sr. Mario Hermes. Na inauguração estiveram diversaas autoridades e o discurso inaugural foi do General Gois Monteiro.`,
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const GeneralJoaoSeverianodaFonseca_Desconhecida: Obra = {
  ID: 474,
  Imagem: 'GeneralJoaoSeverianodaFonseca_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `General João Severiano da Fonseca`,
  Material: `Bronze`,
  DataInauguracao: `05/1978`,
  DatasImportantes: [new Data('05/1978', 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Vila Militar',
  Endereco: 'Praça General João Severiano da Fonseca',
  Latitude: `-22.8697124`,
  Longitude: `-43.4125117`,
  Base: 'Com pedestal',
  MaterialBase: 'Concreto',
  Status: 'Removido do espaço público',
  Descricao: `Inicialmente instalado na Praça Marechal Hermes, o busto em bronze foi depois transferido ao logradouro que
    leva seu nome. O busto está desaparecido, em decorrência da obra da via Transolimpica.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra do Rio - AGR',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Tipologia: 'Busto',
  Execucao: true,
};

export const Aviadores_RebecaMatteBello: Obra = {
  ID: 474,
  Imagem: 'Aviadores_RebecaMatteBello.jpeg',
  Authors: [authors.RebecaMatteBello],
  Titulo: `Aviadores`,
  Material: `Bronze`,
  DataInauguracao: `1922`,
  DatasImportantes: [
    new Data('1922', 'data de inauguração - arte ambiente'),
    new Data('11/1923', 'data de inauguração - inventario dos monumentos'),
    new Data('1937', 'data de tranferência para Campo dos Afonsos'),
  ],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Vila Militar',
  Endereco: 'Campo dos Afonsos',
  Latitude: `-22.8828759`,
  Longitude: `-43.3836072`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Escultura simbolizando a luta pela conquista do ar através das figuras Dédalo contemplando Ícaro, seu filho, morto. Foi uma doação do povo chileno à Cidade, durante as comemorações do Centenário da Independência, como reconhecimento à Santos Dumont na descoberta do mais pesado que o ar.
Inaugurado na Praça Mauá, foi transferido em 1937 para o Campo dos Afonsos.`,
  Fatos: [
    new Fact(
      '',
      '',
      'Fundição/Atelier',
      'Arsenal de Guerra do Rio - AGR',
      [],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Tipologia: 'Escultura',
  Execucao: true,
};

export const Piramide_MestreValentim: Obra = {
  ID: 97,
  Imagem: 'Piramide_MestreValentim.jpeg',
  Authors: [authors.MestreValentim],
  Titulo: `Pirâmide`,
  Material: 'bloco gnaisse',
  DataInauguracao: `01/1806`,
  DatasImportantes: [new Data(`01/1806`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Passeio Público',
  Latitude: `-22.9132919`,
  Longitude: `-43.1763128`,
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao: `Foi construída no período de 1794 a 1806 e são as últimas obras públicas do Mestre Valentim. A pirâmide com cerca de quinze metros de altura e dois metros em cada face da seção. Há uma inscrição cartela oval em mármore de Ào Amor ao Publico"`,
  Fatos: [new Fact('', '', 'bem tombado federal', '', [], '')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const Obelisco_Archi5: Obra = {
  ID: 169,
  Imagem: 'Obelisco_Archi5.jpeg',
  Authors: [authors.Archi5],
  Titulo: `Obelisco`,
  Material: 'aço corten',
  Altura: '12 m',
  DataInauguracao: `08/1996`,
  DatasImportantes: [new Data(`08/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Vila Isabel',
  Endereco: 'Praça Barão de Drummond',
  Latitude: `-22.9166566`,
  Longitude: `-43.2502656`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Obelisco de doze metros de altura, composto por três elementos. O primeiro, um bloco quadrado confeccionado por blocos de granito. O segundo elemento, um cilindro de dois metros de altura. O terceiro elemento é a piramide com duas hastes de aço com dez metros de altura. Cada haste é formada por duas barras, formando um triangulo na base que diminui até a extremidade.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const Obelisco_EduardoDeSa: Obra = {
  ID: 17,
  Imagem: 'Obelisco_EduardoDeSa.jpeg',
  Authors: [authors.EduardoDeSa],
  Titulo: 'Obelisco',
  Material: 'granito',
  DataInauguracao: '1906',
  DatasImportantes: [new Data('1906', 'inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Avenida Rio Branco',
  Latitude: '-22.912687',
  Longitude: '-43.174773',
  SuporteEspacial: 'trevo',
  Base: 'Sem pedestal',
  Status: 'Permanece em espaço público',
  Descricao:
    'Com 18 metros de altura, o obelisco é o marco comemorativo da construção da Avenida Central, em 1906.  Reza a lenda que Getúlio amarrou seu cavalo aqui, quando tomou a Capital Federal na revolução de 30.',
  Fatos: [new Fact('', '', 'Bem tombado estadual', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodoCaisdaImperatriz_Desconhecido: Obra = {
  ID: 3,
  Imagem: 'ObeliscodoCaisdaImperatriz_Desconhecido.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: 'Obelisco do Cais da Imperatriz',
  Material: 'granito',
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Gamboa',
  Endereco: 'Avenida Barão de Tefé',
  Latitude: '-22.896938',
  Longitude: '-43.187266',
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: '',
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const Obelisco_PauloCase: Obra = {
  ID: 241,
  Imagem: 'Obelisco_PauloCase.jpeg',
  Authors: [authors.PauloCase],
  Titulo: `Obelisco de Ipanema`,
  Material: 'concreto',
  Altura: '22m',
  DataInauguracao: `08/1996`,
  DatasImportantes: [new Data(`08/1996`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Ipanema',
  Endereco: 'Rua Visconde de Pirajá',
  Latitude: `-22.9837493`,
  Longitude: `-43.2139005`,
  Base: 'Com pedestal',
  MaterialBase: 'Granito',
  Status: 'Permanece em espaço público',
  Descricao: `Projetado para ser o marco de Ipanema. Foi criado pelo Rio-Cidade e se situa no local onde os bondes faziam o
    retorno para circular no bairro. O Obelisco tem forma circular com uma bola no seu topo.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodoLargodoBomRetiro_Desconhecida: Obra = {
  ID: 1234,
  Imagem: 'ObeliscodoLargodoBomRetiro_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Obelisco do Largo do Bom Retiro`,
  Material: `Bloco de gnaisse esculpido`,
  DataInauguracao: `08/1928`,
  DatasImportantes: [new Data(`08/1928`, 'data inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Alto da Boa Vista',
  Endereco: 'Parque Nacional da Tijuca',
  Latitude: `-22.9473613`,
  Longitude: `-43.2912779`,
  Status: 'Permanece em espaço público',
  Descricao: `O monumento está incluído nessa relação devido o convênio de Gestão Compartilhada do ParNacional da Tijuca entre a Prefeitura do Rio de Janeiro e oIBAMA desde 22 de março de 2001.
    Peça maciça esculpida com base quadrada e outra piramidal de cerca de 6 metros de altura`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodaAvenidaRioBranco_AZanuzziCia: Obra = {
  ID: 132,
  Imagem: 'ObeliscodaAvenidaRioBranco_AZanuzziCia.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Obelisco da Avenida Rio Branco`,
  Material: `Peça em granito`,
  DataInauguracao: `11/1906`,
  DatasImportantes: [new Data(`11/1906`, 'data inauguração')],
  Propriedade: 'Pública',
  Zona: 'Central',
  Bairro: 'Centro',
  Endereco: 'Av. Rio Branco',
  Latitude: `-22.9130043`,
  Longitude: `-43.1747473`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `A Zanuzzi Cia - Obelisco circular de granito extraído do Morro da Viúva, com 28 toneladas paracomemorar da abertura da Avenida Rio Branco, iniciada em 8 de março de 1904 e concluído na data de inauguração do marco, dia 14 de novembro de 1906.
    O projeto foi organizado no escritório tecnico da Comissão Construtora da Avenida Central, cujo chefe era o Dr. Paulo de Frontim e executado pelos Srs A. Januzzi & Irmãos, tendo como engenheiro de acompanhamento o Dr. Le Cocq.
    O Monumento tem cinco blocos, com dimensões e decoração distintas, todos em pedra, coincidentes com o principal material utilizado nas construções novas da Avenida. O primeiro forma os degraus que o elevam. O segundo a base, o terceiro com cerca de 2m foi destinado à colocação de placas de bronze de identificação do Monumento. O quarto possui elementos decorativos típicos das construções ecléticas O quinto bloco com 10 metros finalizado por uma pirâmide.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const MonumentoALiberdadeDeExpressao_FranzWeissmann: Obra = {
  ID: 145,
  Imagem: 'MonumentoALiberdadeDeExpressao_FranzWeissmann.jpeg',
  Authors: [authors.FranzWeissmann],
  Titulo: `Monumento A Liberdade De Expressao`,
  Material: 'concreto',
  Altura: '1600 cm',
  DataInauguracao: `12/10/1954`,
  DatasImportantes: [new Data(`12/10/1954`, 'data de inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Sao Cristovao',
  Endereco: 'Entrada da Quinta da Boa Vista',
  Latitude: `-22.9051942`,
  Longitude: `-43.2239741`,
  Status: 'Removida do espaço público',
  Descricao: `Considerado o primeiro monumento de Arte Concreta da América Latina. Foi o primeiro monumento à liberdade de expressão a ser erigido nas Américas, além de ter sido a primeira obra pública de Franz Weissmam, encomendada pela Associação das Emissoras de São Paulo.`,
  Fatos: [
    new Fact(
      '1954',
      '1954',
      'Doação',
      'Doação da Sociedade Interamericana de Imprensa',
      [],
      'Rio de Janeiro',
    ),
    new Fact(
      '1962',
      '1962',
      'Demolição',
      'Demolido para as obras de alargamento de rua promovida pela prefeitura e pelo departamento de urbanização da SURSAN',
      [persons.CarlosLacerda],
      'Rio de Janeiro',
    ),
  ],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodoAlargamentodeCopacabana_Desconhecida: Obra = {
  ID: 349,
  Imagem: 'ObeliscodoAlargamentodeCopacabana_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Obelisco  do Alargamento de Copacabana`,
  Material: `Bloco de granito`,
  DataInauguracao: `07/1919`,
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Sul',
  Bairro: 'Leme',
  Endereco: 'Praça Júlio de Noronha',
  Latitude: `-22.9623683`,
  Longitude: `-43.1653128`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `Marco oferecido pelos moradores de Copacabana à Cidade, emcomemoração da reconstrução
    e alargamento da Avenida Atlântica. Foi inaugurado na Avenida Atlantica próximo a Rua Almirante
    Gonçalves. Foi transferido, na década de 1930, quando os canteiros centrais foram eliminadoso,
    para a Praça Júlio de Noronha.Possui forma de coluna circular adornado com dois medalhões de bronze com a efígie do Prefeito Paulo de Frontin e outro com a do Presidente da República Delfim Moreira.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodaLinhaAmarela_Desconhecida: Obra = {
  ID: 885,
  Imagem: 'ObeliscodaLinhaAmarela_Desconhecida.jpeg',
  Authors: [authors.Desconhecida],
  Titulo: `Obelisco da Linha Amarela`,
  Material: `Peça em aço`,
  DataInauguracao: `10/1997`,
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Cachambi',
  Endereco: 'Rua Dom Helder Camara',
  Latitude: `-22.8862914`,
  Longitude: `-43.2885982`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `Peça circular com uma estrutura central e outra cobrindo a primeira através de uma tela.`,
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const AAlimPedro_MiguelPastor: Obra = {
  ID: 964,
  Imagem: 'AAlimPedro_MiguelPastor.jpeg',
  Authors: [authors.MiguelPastor],
  Titulo: `À Alim Pedro`,
  Material: `Peça em concreto revestida de pedra São Tomé`,
  DataInauguracao: `01/1958`,
  DatasImportantes: [new Data(`01/1958`, 'data inauguração')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'Praça Alim Pedro',
  Latitude: `-22.9067826`,
  Longitude: `-43.5655838`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `Alim foi um ilustre morador do bairro, que recebeu a homenagem em um obelisco com sua efÍgie. Foi idealizada pela Engenheira Elza Osnorne, para ser um marco dos festejos dos 25 anos da formatura da turma da qual saíram inúmeros engenheiros da prefeitura, inclusive o ex prefeito. Apesar de ser homenagem em vida, os motivos que deram origem as homenagens, a engenheira Elza Osborne, procurou honrar no 14 DOE, da Secretaria de Obras. O viaduto  recebeu o nome do homenageado em 14 de novembro de 1958 e não teve uma inauguração solene. 
    A efigie do homenageado em bronze foi furtada em outubro de 2017.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const AAugustodeVasconcelos_MiguelPastor: Obra = {
  ID: 967,
  Imagem: 'AAugustodeVasconcelos_MiguelPastor.jpeg',
  Authors: [authors.MiguelPastor],
  Titulo: `À Augusto de Vasconcelos`,
  Material: `Peça em bronze e pedestal de granito`,
  DatasImportantes: [new Data('', 'século XIX')],
  Propriedade: 'Pública',
  Zona: 'Oeste',
  Bairro: 'Campo Grande',
  Endereco: 'Av. Santa Cruz',
  Latitude: `-22.8941433`,
  Longitude: `-43.5361555`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `Na homenagem ao senador originalmente existia a sua efigie numa placa em bronze fixada no pedestal que tem
    caracteristicas de um obelisco.`,
  Fatos: [new Fact('data', 'data', 'fato', 'descrição', [], 'cidade')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};

export const ObeliscodeRochaMiranda_ErnaniFreire: Obra = {
  ID: 1037,
  Authors: [authors.ErnaniFreire],
  Titulo: `Obelisco de Rocha Miranda`,
  Material: `Peça em ferro pintado`,
  DataInauguracao: `01/2002`,
  DatasImportantes: [new Data(`01/2002`, 'data inauguração')],
  Propriedade: 'Pública',
  Zona: 'Norte',
  Bairro: 'Rocha Miranda',
  Endereco: 'Praça Oito de Maio',
  Latitude: `-22.8543015`,
  Longitude: `-43.3517691`,
  SuporteEspacial: 'praça',
  Status: 'Permanece em espaço público',
  Descricao: `Obelisco de quinze metros de altura de ferro pintado de branco, c<om uma grande esfera no topo<. Construído para ser a referência do bairro a distância, tem na sua base piramidal,placas comemorativas das principais batalhas da FEB na II Guerra Mundial.
    Em 2018 a esfera superior foi retirada devido a corrosão. O destino da peça é desconhecido.`,
  Fatos: [new Fact('', '', '', '', [], 'Rio de Janeiro')],
  Temporalidade: 'Permanente',
  Categoria: 'Plasticidade',
  Funcao: 'Referencial',
  Natureza: 'Urbanística',
  Tipologia: 'Obelisco',
  Execucao: true,
};
